...
start_cnssdaemon() {
	...
	# In this case, daemon is started without any args,
	# which will make it connect to the first available QMI server.
	...
	for mod in $(cat /lib/wifi/qca-wifi-modules); do
		case ${mod} in
			...
			wifi_3_0_sim1) [ -d /sys/module/${mod} ] || { \
				insmod ${mod} netlink_ath_ssid_event=30 netlink_band_steering_event=22 netlink_acfg=27|| { \
					lock -u /var/run/wifilock
					unload_qcawificfg80211
					return 1
				}
