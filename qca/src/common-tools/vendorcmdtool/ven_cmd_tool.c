                              ...
                              char *user_command)
{
    ...
    node_t *node = NULL, *attr = NULL, *child = NULL;
    int idx = 0, ret = 0, cmd_num = 0, size = 0;
    struct cmds_params *cmd;
    ...
    for (idx = 0; idx < cmd_num; idx++) {
        ...
        if (strncmp(name, "VendorCmd", 9) == 0) {
            if (user_command) {
                ...
                if (strncmp(&buf[0], user_command, strlen(user_command)) == 0) {
                    // Match found for given command.
                    // So read and store all attributes.
                             ...
                             struct cmd_params *cmds[], char *user_command)
{
    ...
    if (!user_command) {
        ...
        for (table_idx = 0; table_idx <= max_table_idx; table_idx++) {
            if (strncmp(user_command, cmd_ptr->cmd_name, strlen(user_command)) == 0) {
                         ...
                         char *user_command)
{
    ...
    int size = 0, fd = -1, max_table_idx = 0;
    size_t filesize = 0;
                       ...
                       struct option *long_options)
{
    ...
    if (strcasecmp(xml_filename, "qcacommands_vap.xml") == 0) {
