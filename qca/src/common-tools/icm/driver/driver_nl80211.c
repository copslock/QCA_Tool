...
static int qca_nl80211_start_acs_event(ICM_INFO_T *picm, u8 *data, size_t len)
{
    ...
    struct nlattr *nl, *nl_pcl[5], *cinfo[QCA_WLAN_VENDOR_EXTERNAL_ACS_EVENT_CHAN_INFO_ATTR_MAX + 1];
    int rem, i = 0;
    u32 *freq_list = NULL;
    ...
    nla_for_each_nested(nl, tb[QCA_WLAN_VENDOR_ATTR_EXTERNAL_ACS_EVENT_CHAN_INFO], rem) {
        ...
        if (cinfo[QCA_WLAN_VENDOR_EXTERNAL_ACS_EVENT_CHAN_INFO_ATTR_VHT_SEG_0])
            acs_data->chan_info[i].ic_vhtop_ch_num_seg1 = nla_get_u8(cinfo[QCA_WLAN_VENDOR_EXTERNAL_ACS_EVENT_CHAN_INFO_ATTR_VHT_SEG_0]);

        if (cinfo[QCA_WLAN_VENDOR_EXTERNAL_ACS_EVENT_CHAN_INFO_ATTR_VHT_SEG_1])
            acs_data->chan_info[i].ic_vhtop_ch_num_seg2 = nla_get_u8(cinfo[QCA_WLAN_VENDOR_EXTERNAL_ACS_EVENT_CHAN_INFO_ATTR_VHT_SEG_1]);
