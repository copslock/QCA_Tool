							    ...
							    struct ovsmgr_dp_flow *flow, struct sk_buff *skb)
{
	...
	ether_addr_copy(key.eth.src, flow->dmac);
	ether_addr_copy(key.eth.dst, flow->smac);

	if (flow->tuple.ip_version == 4) {
		key.ipv4.addr.src = flow->tuple.ipv4.dst;
		key.ipv4.addr.dst = flow->tuple.ipv4.src;
		key.eth.type = htons(ETH_P_IP);
	} else {
		memcpy(&key.ipv6.addr.src, &flow->tuple.ipv6.dst, sizeof(key.ipv6.addr.dst));
		memcpy(&key.ipv6.addr.dst, &flow->tuple.ipv6.src, sizeof(key.ipv6.addr.src));
		key.eth.type = htons(ETH_P_IPV6);
	}

	key.tp.src = htons(flow->tuple.dst_port);
	key.tp.dst = htons(flow->tuple.src_port);
	...
	list_for_each_entry(p, &nod->port_list, node) {
		...
		if (dev) {
			...
			ovsmgr_info("%p: Found the egress interface for this flow, returning ingress port: %s\n",
					nod, p->dev->name);
			return p->dev;
		}
	}
					 ...
					 struct sw_flow_actions *sfa, struct sk_buff *skb)
{
	...
	case IPPROTO_UDP:
		break;
	default:
		ovsmgr_warn("%p: IP protocol is not supported, proto=%d\n", sf, key->ip.proto);
		return false;
	}
	...
	if (!nod) {
		read_unlock(&ovsmgr_ctx.lock);
		ovsmgr_warn("%p: datapath instance is not found, ingress port_num = %d\n", sf, port_num);
		return false;
	}
	...
	if (!nodp) {
		read_unlock(&ovsmgr_ctx.lock);
		ovsmgr_warn("%p: datapath port instance is not found, ingress port_num = %d\n", sf, port_num);
		return false;
	}
	...
	if (nodp->vport_type == OVS_VPORT_TYPE_INTERNAL) {
		read_unlock(&ovsmgr_ctx.lock);
		ovsmgr_warn("%p: ingress port(%d:%s): incoming routed flows not forwarded through hook.\n",
	...
	for (a = sfa->actions, rem = sfa->actions_len; rem > 0;
		...
		case OVS_ACTION_ATTR_OUTPUT: {
			...
			if (!nodp) {
				read_unlock(&ovsmgr_ctx.lock);
				ovsmgr_warn("%p: datapath port instance is not found, port_num = %d\n", sf, port_num);
				return false;
			}

			if (nodp->vport_type == OVS_VPORT_TYPE_INTERNAL) {
				read_unlock(&ovsmgr_ctx.lock);
				ovsmgr_warn("%p: output port(%d:%s): egress routed flows not forwarded through hook.\n",
						sf, port_num, nodp->dev->name);
				return false;
			}
		...
		case OVS_ACTION_ATTR_CT:
		     break;
		default:
		     ovsmgr_warn("%p: Action is not supported, action=%x\n", sf, nla_type(a));
		     return false;
		}
	}
	...
	if ((egress_cnt > 1) && (skb->pkt_type != PACKET_MULTICAST)) {
		ovsmgr_warn("%p: Not multicast packet :egress > 1, type = %d, egress_cnt = %d\n",
...
static int ovsmgr_dp_flow_first_egress_find(struct sw_flow *sf)
{
	...
	if (!sfa) {
		ovsmgr_warn("%p: flow actions is null\n", sf);
		return -EINVAL;
	}
...
static int ovsmgr_dp_flow_get(void *dp, struct sw_flow *sf, struct ovsmgr_dp_flow *flow)
{
	...
	if (!flow->indev) {
		ovsmgr_warn("%p: Input device (indev) is NULL\n", sf);
		return -EINVAL;
	}

	flow->outdev = ovsmgr_dp_flow_egress_find(dp, sf);
	if (!flow->outdev) {
		ovsmgr_warn("%p: Output device (outdev) is NULL\n", sf);
		return -EINVAL;
	}
	...
	if (key->eth.type == htons(ETH_P_IP)) {
		...
		ovsmgr_warn("%p: Eth type is not accelerated: %x\n", sf, htons(key->eth.type));
		return -EINVAL;
	}
	 ...
	 */
	if (key->eth.tci) {
		flow->ingress_vlan.h_vlan_TCI = ntohs(key->eth.tci) & ~VLAN_TAG_PRESENT;
		ovsmgr_info("%p: ingress VLAN tag is %X\n", sf, flow->ingress_vlan.h_vlan_TCI);
	}
		...
		struct sw_flow *sf, struct sw_flow_actions *sfa)
{
	...
	if (!sf) {
		...
		if (!hook_called) {
			ovsmgr_warn("%p: Pre flow hook is not registered.\n", dp);
		} else {
	...
	if (!ovsmgr_dp_flow_can_accel(sf, key, sfa, skb)) {
		ovsmgr_warn("%p: flow cannot be accelerated\n", skb);
		return;
	}

	out = ovsmgr_dp_flow_egress_find(dp, sf);
	if (unlikely(!out)) {
		ovsmgr_warn("%p: egress interface is not found in flow rule - %p\n", skb, sf);
		return;
	}
	...
	if (!hook_called) {
		ovsmgr_warn("%p: Post flow hook is not registered.\n", dp);
	} else {
...
static void ovsmgr_dp_port_vlan_notify(void *dp, struct ovsmgr_dp_flow *flow, int event)
{
	...
	if (!nod) {
		...
		ovsmgr_warn("%p: datapath instance is not found\n", dp);
		return;
	}
	...
	if (!nodp) {
		read_unlock_bh(&ovsmgr_ctx.lock);
		ovsmgr_warn("%p: Port is not found, dev = %s\n", dp, flow->indev->name);
		return;
	}
...
static void ovsmgr_dp_port_del(void *dp, void *vp, struct net_device *dev)
{
	...
	ovsmgr_info("%p:%p port deleting - dev = %s\n", dp, vp, dev->name);
				   ...
				   const char *master, struct net_device *dev)
{
	...
	if (!master) {
		ovsmgr_info("%p: There is no master\n", dp);
	} else {
		ovsmgr_info("%p: Master device name set: %s\n", dp, master);
		master_dev = __dev_get_by_name(&init_net, master);
		if (!master_dev) {
			ovsmgr_warn("%p: Master netdev not found\n", dp);
		}
	}

	ovsmgr_info("%p:%p new port - dev = %s\n", dp, vp, dev->name);

	nodp = kmalloc(sizeof(*nodp), GFP_KERNEL);
	if (!nodp) {
		ovsmgr_warn("%p: Memory allocation failed\n", dp);
		return;
	}
...
static void ovsmgr_dp_del(void *dp, struct net_device *dev)
{
	...
	ovsmgr_info("%p: datapath deleting - dev = %s\n", dp, dev->name);
...
static void ovsmgr_dp_add(void *dp, struct net_device *dev)
{
	...
	ovsmgr_info("%p: new datapath - dev = %s\n", dp, dev->name);

	new_dp = kmalloc(sizeof(*new_dp), GFP_KERNEL);
	if (!new_dp) {
		ovsmgr_warn("%p: failed to allocation memory\n", dp);
		return;
	}
...
static void ovsmgr_dp_flow_key_fill(struct ovsmgr_dp_flow *flow, int in_port, struct sw_flow_key *key)
{
	...
	if (flow->ingress_vlan.h_vlan_TCI) {
		key->eth.tci = htons(flow->ingress_vlan.h_vlan_TCI | VLAN_TAG_PRESENT);
	}
	...
	key->tp.src = htons(flow->tuple.src_port);
	key->tp.dst = htons(flow->tuple.dst_port);
	key->ip.proto = flow->tuple.protocol;
						     ...
						     struct sk_buff *skb, struct ovsmgr_vlan_info *ovi)
{
	...
	if (unlikely(!nod)) {
		read_unlock_bh(&ovsmgr_ctx.lock);
		ovsmgr_warn("%p: Couldn't find datapath instance for dev = %s\n", flow, flow->indev->name);
	...
	if (!in_port) {
		read_unlock_bh(&ovsmgr_ctx.lock);
		ovsmgr_warn("%p: input datapath port instance is not found for dev = %s\n",
				nod, flow->indev->name);
		return OVSMGR_FLOW_STATUS_DENY_ACCEL;
	}
	...
	if (!out_port) {
		read_unlock_bh(&ovsmgr_ctx.lock);
		ovsmgr_warn("%p: output datapath port instance is not found for dev = %s\n",
	...
	if (!sf) {
		ovsmgr_warn("%p:Couldn't find flow rule\n", flow);
		return OVSMGR_FLOW_STATUS_DENY_ACCEL;
	}
	...
	for (a = acts->actions, rem = acts->actions_len; rem > 0;
		...
		case OVS_ACTION_ATTR_OUTPUT:
			if (out_port_num == nla_get_u32(a)) {
				ovsmgr_info("%p: Found egress port in flow rule: %d\n", flow, out_port_num);
				found_egress = true;
		...
		case OVS_ACTION_ATTR_PUSH_VLAN: {
			 ...
			 */
			if (vlan_index == 2) {
				ovsmgr_info("%p: More than 2 VLAN headers, don't accelerate\n", flow);
				return OVSMGR_FLOW_STATUS_DENY_ACCEL;
			}
					  ...
					  uint32_t tx_packets, uint32_t tx_bytes)
{
	...
	if (!ovsmgr_dp_dev_is_master(dev)) {
		ovsmgr_warn("%p: %s is not an OVS bridge device\n", dev, dev->name);
		return;
	}
...
void ovsmgr_dp_flow_stats_update(struct ovsmgr_dp_flow *flow, struct ovsmgr_dp_flow_stats *stats)
{
	...
	if (ovs_accel_flow_stats_update(nod->dp, out_vport, &key, stats->pkts, stats->bytes)) {
		ovsmgr_warn("%p: Couldnt update statistics\n", nod->dp);
	}
}
					       ...
					       struct net_device *dev, struct ovsmgr_dp_flow *flow)
{
	...
	if (!nod) {
		read_unlock_bh(&ovsmgr_ctx.lock);
		ovsmgr_warn("%p: Couldn't find datapath instance, dev = %s\n", skb, dev->name);
		return NULL;
	}
	...
	if (!nodp) {
		read_unlock_bh(&ovsmgr_ctx.lock);
		ovsmgr_warn("%p: Couldn't find datapath port instance, dev = %s\n", skb, dev->name);
		return NULL;
	}

	if (nodp->vport_type != OVS_VPORT_TYPE_INTERNAL) {
		read_unlock_bh(&ovsmgr_ctx.lock);
		ovsmgr_warn("%p: Interface type is not internal, dev = %s\n", skb, dev->name);
		return NULL;
	}
