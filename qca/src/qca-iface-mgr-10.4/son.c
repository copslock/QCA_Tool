...
char ap_ifname[MAX_NUM_RADIOS][MAX_IFNAME_SIZE];
char sta_ifname[MAX_NUM_RADIOS][MAX_IFNAME_SIZE];
char ap_radio[MAX_NUM_RADIOS][MAX_IFNAME_SIZE];
char sta_radio[MAX_NUM_RADIOS][MAX_IFNAME_SIZE];
char ap_macaddr[MAX_NUM_RADIOS][MAX_MAC_ADDR_LEN];;
char sta_macaddr[MAX_NUM_RADIOS][MAX_MAC_ADDR_LEN];
unsigned char ap_macaddr_hex[MAX_NUM_RADIOS][MAX_MAC_ADDR_LEN];
unsigned char bh_sta_macaddr_hex[MAX_NUM_RADIOS][MAX_MAC_ADDR_LEN];
unsigned char action_sta_macaddr_hex[MAX_MAC_ADDR_LEN];
char * tmp_ap_macaddr_hex = NULL;
char * tmp_bh_sta_macaddr_hex= NULL;
char * tmp_action_sta_macaddr_hex=NULL;

char nbh_apiface[MAX_NUM_RADIOS][MAX_IFNAME_SIZE];
char nbh_radio[MAX_NUM_RADIOS][MAX_IFNAME_SIZE];
char nbh_macaddr[MAX_NUM_RADIOS][MAX_MAC_ADDR_LEN];
int num_nbh_vaps = 0;
int num_bh_vaps = 0;
...
int copy_macaddr_hex(char *macaddr_hex_copy, const char *macaddr)
{
    ...
    size = strlen(macaddr)+1; /* Size is 18 */
    /* If the size of macaddr (including the NULL terminator) is not 18,
     * consider the macaddr to be invalid and do not process it.
     */
    if (size != MAX_MAC_ADDR_LEN) {
        printf("Macaddr is of incorrect format!\n");
        return -1;
    }
    /* Copy "macaddr" to "macaddr_hex_copy"  excluding the colons */
    strlcpy(macaddr_hex_copy, macaddr, 3);
    strlcpy(macaddr_hex_copy+2, macaddr+3, 3);
    strlcpy(macaddr_hex_copy+4, macaddr+6, 3);
    strlcpy(macaddr_hex_copy+6, macaddr+9, 3);
    strlcpy(macaddr_hex_copy+8, macaddr+12, 3);
    strlcpy(macaddr_hex_copy+10, macaddr+15, 3);
...
int get_stamac_addr()
{
    ...
    {
        int ret=0;
        ret = fscanf(file,"%ms",&macaddr);
        if (ret == 1 && macaddr)
        {
            if (strlen(macaddr) + 1 > MAX_MAC_ADDR_LEN) {
                printf("Invalid sta macaddr len %zu in the config file\n", strlen(macaddr));
                free(macaddr);
                macaddr = NULL;
                return -1;
            }
            memset(action_sta_macaddr_hex, 0, (strlen(macaddr)+1));
...
static int re_macaddr_match(unsigned char *macaddr)
{
   int j;
...
static int sta_macaddr_match(unsigned char *macaddr)
{
    int j;
...
set80211priv(struct son_driver_data *drv, int op, void *data, int len, char *ifname )
{
 ...
 //   printf("%s:called with ifname %s and op %x \n", __func__,iwr.ifr_name, op);
    ...
    /*
     * If argument data fits inline; put it there.
     * Else, argument data too big for inline transfer; setup a
        ...
        switch (iwe->cmd) {
            ...
            case IWEVCUSTOM:
                if (custom + iwe->u.data.length <= end) {
                    if ((buf = malloc(iwe->u.data.length + 1)) != NULL) {
                        memset(buf, 0, iwe->u.data.length);
...
static void handle_error_in_config()
{
    if (errno != 0) {
        perror("scanf");
    } else {
...
static void free_allocated_mem_for_config(char *mode, char *ifname,
                                          char *radio, char *macaddr)
{
    if (mode) {
        free(mode);
        mode = NULL;
...
void son_main_function(void *conf_file, enum driver_mode drv_mode)
{
    ...
    int error=0;
    char *mode = NULL, *ifname = NULL, *radio = NULL,*macaddr = NULL;
...

    {
        ...
        {
            retval = fscanf(file, "%ms", &mode);
            if (retval == 1) {
                if (strlen(mode) > MAX_MODE_SIZE) {
                    printf("Invalid mode len %zu in the config file\n", strlen(mode));
                    free(mode);
                    mode = NULL;
                    goto out;
                }
                ...
                handle_error_in_config();
                break;
            }
            retval = fscanf(file, "%ms", &ifname);
            if (retval == 1) {
                if (strlen(ifname) + 1 > MAX_IFNAME_SIZE) {
                    printf("Invalid ifname size %zu in the config file\n", strlen(ifname));
                    free(ifname);
                    ifname = NULL;
                    goto out;
                }
            ...
            retval = fscanf(file, "%ms", &radio);
            if (retval == 1) {
                if (strlen(radio) + 1 > MAX_IFNAME_SIZE) {
                    printf("Invalid radio ifname size %zu in the config file\n", strlen(radio));
                    free(radio);
                    radio = NULL;
                    goto out;
                }
            ...
            retval = fscanf(file, "%ms", &macaddr);
            if (retval == 1) {
                if (strlen(macaddr) + 1 > MAX_MAC_ADDR_LEN) {
                    printf("Invalid macaddr len %zu in the config file\n", strlen(macaddr));
                    free(macaddr);
                    macaddr = NULL;
                    goto out;
                }
            ...
            {
                memcpy(&sta_ifname[num_bh_sta_vaps],ifname,(strlen(ifname)+1));
                memcpy(&sta_radio[num_bh_sta_vaps],radio,(strlen(radio)+1));
                memset(&sta_macaddr[num_bh_sta_vaps],0,(strlen(macaddr)+1));
                memcpy(&sta_macaddr[num_bh_sta_vaps],macaddr,(strlen(macaddr)+1));
                memset(&bh_sta_macaddr_hex[num_bh_sta_vaps], 0, (strlen(macaddr)+1));
                ...
                num_bh_sta_vaps++;
                free_allocated_mem_for_config(mode, ifname, radio, macaddr);
            }
            if(strcmp(mode,"ap") == 0)
            {
                memset(&ap_ifname[num_bh_vaps],0, (strlen(ifname)+1));
                memcpy(&ap_ifname[num_bh_vaps], ifname, (strlen(ifname)+1));
                memcpy(&ap_radio[num_bh_vaps], radio, (strlen(radio)+1));
                memset(&ap_macaddr[num_bh_vaps], 0, (strlen(macaddr)+1));
                memcpy(&ap_macaddr[num_bh_vaps], macaddr, (strlen(macaddr)+1));
            ...
            {
                memset(&nbh_apiface[num_nbh_vaps],0, (strlen(ifname)+1));
                memcpy(&nbh_apiface[num_nbh_vaps], ifname, (strlen(ifname)+1));
                memcpy(nbh_radio[num_nbh_vaps], radio, (strlen(radio)+1));
                memset(nbh_macaddr[num_nbh_vaps], 0, (strlen(macaddr)+1));
                memcpy(nbh_macaddr[num_nbh_vaps], macaddr, (strlen(macaddr)+1));
                num_nbh_vaps ++;
                free_allocated_mem_for_config(mode, ifname, radio, macaddr);
            }
        ...
        if (num_bh_sta_vaps!= 0)
            is_re = 1;
        else if (num_nbh_vaps != 0 || num_bh_vaps != 0)
            is_root = 1;
    }
