...
void wlanManagerMenuGetFreqHandler(struct cmdContext *Context, const char *Cmd)
{
	...
	type = interface_getType(arg);
	if (type == interfaceType_WLAN2G || type == interfaceType_WLAN5G) {
		cmdf(Context, "%s uses %dG frequency.\n", arg,
			((type == interfaceType_WLAN2G) ? 2 : 5));
	} else {
		cmdf(Context, "Not QCA WLAN interface! \n", arg);
	}
}
...
/* -F- wlanManager_GetSock
 *  Sock is not ready, make it ready
 */
...
/* -F- wlanManager_ReleaseSock
 *  Unregister the Sock if no one use it to avoid wifi module unable to unload.
 */
void wlanManager_ReleaseSock(void)
{
	if (!wlanManagerS.IsInit)
		return;
