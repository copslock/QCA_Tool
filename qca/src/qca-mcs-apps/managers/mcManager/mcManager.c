...
{
	struct __mc_iftbl_entry *ifEntry = mcManagerFindIFEntryByGroup(&mdbEntry->group, &ifNode->table);

	if (!ifEntry) {
		if (mcManagerAddIFEntry(mdbEntry, &ifNode->table) == MCS_NOK) {
			mcManagerDebug(DBGERR, "%s: Add IF Entry failed\n", __func__);
			return MCS_NOK;
		}
		ifNode->sync = interfaceSync_NEW;
	} else {
		if (mcManagerUpdateIFEntry(ifEntry, mdbEntry, &ifNode->table) == MCS_NOK) {
...
void mcManagerMCProcess(struct mcManagerSnooper *snooper, struct __mc_mdb_entry *mdbEntry, int32_t num_entries)
{
	...
	HMapForEachSafe(node, n, &snooper->ifTableMap) {
		ifTemp = HMapEntry(node, struct mcManagerIFNode, node);
		if (ifTemp->sync) {
			if (mcManagerUpdateForwardTable(&ifTemp->interface, &ifTemp->table, sizeof(struct mcManagerIFTable)) < 0) {
			...
			mcManagerDebug(DBGDEBUG,"%s:step 4.2", __func__);
			hmapRemove(&snooper->ifTableMap, node);
			mcManagerIFTableCLR(&ifTemp, &ifTemp->interface);
			mcManagerDebug(DBGDEBUG,"%s:step 4.3", __func__);
		}
	}
