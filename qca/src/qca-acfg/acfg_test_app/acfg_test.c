        ...
        acfg_radar_t *radar)
{
     ...
     */
    acfg_set_chanswitch(ifname, 36, 2); /* 5 GHz channel 36 */
        ...
        acfg_diag_event_t *diag)
{
    ...
    snprintf((char *)buf, sizeof(buf), "Channel band: %d", diag->channel_band);
    acfg_logger(buf);
...
cb_hw_mode_change_status(uint8_t *ifname, acfg_hw_mode_change_status_t *hw_mode_change)
{
    ...
    snprintf((char *)buf, sizeof(buf), "HW mode change result: %s",
