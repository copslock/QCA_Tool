...
int wrap_set_channel(char *params[])
{
    ...
    if (params[2] && ((compare_string(params[2], "-band") == 0) ||
            (compare_string(params[2], "--band") == 0))) {
        if (!params[3]) {
            printf("%s: Please specify chan_band, should be [0, 3] inclusive\n", __func__);
            return QDF_STATUS_E_INVAL;
        }
        get_uint32(params[3], (uint32_t *) &chan_band);
        if (chan_band > 3) {
            printf("%s: Invalid chan_band: %d, should be [0, 3] inclusive\n",
                    __func__, chan_band);
            return QDF_STATUS_E_INVAL;
        }
    }

    dbg_print_params("vap - %s; channel - %d; band - %d",params[0],chan, chan_band);

    status = acfg_set_channel((uint8_t *)params[0], chan, chan_band);
...
int wrap_get_channel(char *params[])
{
    ...
    status = acfg_get_channel((uint8_t *)params[0], &chan, &chan_band);
    msg("Channel - %d, chan_band - %d",chan, chan_band);
...
param_info_t wrap_set_txpow_params[] = {
    ...
    {"fstype", PARAM_STRING, "frame subtype"},
    {"fixed", PARAM_STRING, "fixed?"},
};
...
int wrap_set_txpow(char *params[])
{
    ...
    get_hex(params[2], &fstype);

    dbg_print_params("vap - %s; txpow - %d fstype - %d",params[0],
            txpow, fstype);

    status = acfg_set_txpow((uint8_t *)params[0], &txpow, fstype);
...
int wrap_acl_delmac(char *params[])
{
    ...
    status = acfg_acl_delmac((uint8_t *)params[0], mac);
...
int wrap_acl_delmac_secondary(char *params[])
{
    ...
    status = acfg_acl_delmac_secondary((uint8_t *)params[0], mac);
...
int wrap_set_ctl_table(char *params[])
{
    ...
    wmi_tgt_type = *req.data;
    if (wmi_tgt_type == (uint8_t)ACFG_WMI_TGT_TYPE_INVAILD) {
    ...
    if (wmi_tgt_type == (uint8_t)ACFG_WMI_TGT_TYPE_NON_TLV) {
        len = fread(clt_buf, 1, MAX_CTL_SIZE, fp);
        status = acfg_set_ctl_table((uint8_t *)params[0], mode, len, clt_buf);
    } else if (wmi_tgt_type == (uint8_t)ACFG_WMI_TGT_TYPE_TLV) {
        fseek(fp, 0, SEEK_END);
        total_len = ftell(fp);
        fseek(fp , 0, SEEK_SET);
        seq = 0;
...
int wrap_send_raw_pkt(char *params[])
{
    ...
    uint32_t type, chan, frmlen, chan_band = 0;
    ...
    if (params[13] && ((compare_string(params[13], "-band") == 0) ||
                (compare_string(params[13], "--band") == 0))) {
        if (!params[14]) {
            printf("%s: Please specify chan_band, should be [0, 3] inclusive\n", __func__);
            return QDF_STATUS_E_INVAL;
        }
        get_uint32(params[14], (uint32_t *) &chan_band);
        if (chan_band > 3) {
            printf("%s: Invalid chan_band: %d, should be [0, 3] inclusive\n",
                    __func__, chan_band);
            return QDF_STATUS_E_INVAL;
	...
	*(pi_dur) = 0x0;
    ...
    status = acfg_send_raw_pkt((uint8_t *)params[0], pkt, frmlen, type, chan, chan_band, nss, preamble, mcs, retry, power, scan_dur);
    if (status != QDF_STATUS_SUCCESS){
...
int wrap_send_raw_multi(char *params[])
{
	...
	*(pi_dur) = 0x0;
    ...
    status = acfg_send_raw_multi((uint8_t *)params[0], pkt, frmlen, type, chan, chan_band, nss, preamble, mcs, retry, power, scan_dur);
    if (status != QDF_STATUS_SUCCESS){
        printf("%s: send raw pkt failed\n", __func__);
    }
    free(pkt);
...
#define PHYMODE_NUM  29
    char *ieee80211_phymode[PHYMODE_NUM]= {
        ...
        "IEEE80211_MODE__11AXA_HE20      = 22,    5GHz, HE20 ",
        ...
        "IEEE80211_11AXA_HE160           = 31,    5GHz, HE160 ",
        "IEEE80211_11AXA_HE80_80         = 32,    5GHz, HE80_80 ",
};
    uint8_t i;

    printf("Invalid phymode!! Supported phymode codes are: \n");
...
int wrap_get_radio_supported_rates(char *params[])
{
    uint32_t status = QDF_STATUS_E_FAILURE ;
    acfg_rateset_t  rs;
    enum acfg_phymode  phymode = ACFG_PHYMODE_INVALID;
    uint8_t  i, rate_code;
    char print_str[8];

    get_uint32(params[1], &phymode);

    if (phymode >= ACFG_PHYMODE_INVALID){
        print_phymodes();
        return acfg_to_os_status(status);
    }
...
int wrap_offchan_rx(char *params[])
{
    ...
    status = acfg_offchan_rx((uint8_t *)params[0], chan, scan_dur, params);
    if (status != QDF_STATUS_SUCCESS){
