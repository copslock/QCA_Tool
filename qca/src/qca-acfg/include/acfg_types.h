...
#define HWMODE_SIZE 4
#define HTCAP_SIZE 100
#define VHTCAP_SIZE 250
...
typedef struct {
    ...
    uint8_t chan_band; /* read-write, mandatory */
    ...
    uint8_t dbdc_enable; /*Enable Repeater DBDC process, 0 or 1; 1 if unspecified*/
...
typedef struct {
    ...
    char acs_freq_list[MAX_LIST_LEN];
    uint32_t acs_6g_only_psc;
    ...
    uint8_t wnm;
    uint8_t rrm;
    acfg_wlan_profile_radio_params_t *radio_params; /*radio back pointer*/
    bool default_params_set;
    uint32_t he_rx_mcsmap;
    uint32_t he_tx_mcsmap;
...
typedef struct {
    ...
    bool default_params_set;
    uint8_t acfg_standard;
} acfg_wlan_profile_t;

typedef struct {
    uint8_t  wps_state;
    char hw_mode[HWMODE_SIZE];
    int ieee80211ac;
    int ieee80211n;
    char ht_capabs[HTCAP_SIZE];
    char vht_capabs[VHTCAP_SIZE];
