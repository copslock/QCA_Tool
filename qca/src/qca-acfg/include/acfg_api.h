...
uint32_t
acfg_set_txpow(uint8_t *vapname, acfg_txpow_t *txpow, uint32_t fstype);
...
uint32_t
acfg_send_raw_pkt(uint8_t  *vap_name, uint8_t *pkt_buf, uint32_t len, uint8_t type, uint16_t chan, uint8_t chan_band, uint8_t nss, uint8_t preamble, uint8_t mcs, uint8_t retry, uint8_t power, uint16_t scan_dur);
