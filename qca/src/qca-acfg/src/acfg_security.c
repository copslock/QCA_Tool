        ...
        char *conffile)
{
    ...
    strlcat(conffile, ".conf", MAX_CONF_FILE_NAME_LEN);

    /*Remove old conf file before creating new one*/
    if(0 > snprintf(command, sizeof(command),"rm -rf %s", conffile)) {
        return QDF_STATUS_E_FAILURE;
    }
    err = system(command);
    if(err != 0)
...
#endif
{
    ...
    *enable_network = 1;
    if (flags & WPA_SUPP_MODIFY_SSID) {
        snprintf(ncmd[nindex], sizeof(ncmd), "%s %d %s \"%s\"", WPA_SET_NETWORK_CMD_PREFIX, id,
    ...
    if (flags & WPA_SUPP_MODIFY_SEC_METHOD) {
        if (vap_params->security_params.sec_method ==
            ...
            snprintf(param, sizeof(param), "%s", WPA_SUPP_PROTO_OPEN);
        } else if (vap_params->security_params.sec_method ==
        ...
        if ((vap_params->security_params.sec_method ==
                ...
                (vap_params->security_params.sec_method ==
                 ACFG_WLAN_PROFILE_SEC_METH_OWE))
        {
            snprintf(ncmd[nindex], sizeof(ncmd), "%s %d %s %s",
                 ...
                 ACFG_WLAN_PROFILE_SEC_METH_WPAWPA2))
        {
            ...
            switch (vap_params->security_params.ieee80211w) {
                ...
                case ACFG_WLAN_PROFILE_IEEE80211W_REQUIRED:
                    snprintf(ncmd[nindex], sizeof(ncmd), "%s %d %s %s %s", WPA_SET_NETWORK_CMD_PREFIX, id,
            ...
            snprintf(ncmd[nindex], sizeof(ncmd), "%s %d %s %d", WPA_SET_NETWORK_CMD_PREFIX, id,
                    ...
                    ACFG_WLAN_PROFILE_SEC_METH_WPA3))
        {
            ...
            snprintf(ncmd[nindex], sizeof(param), "%s %d %s %s",
                     WPA_SET_NETWORK_CMD_PREFIX, id, "key_mgmt", param);
            nindex++;
            /* Set ieee80211w params */
            snprintf(ncmd[nindex], sizeof(ncmd), "%s %d %s %d", WPA_SET_NETWORK_CMD_PREFIX, id,
        ...
        } else if (ACFG_IS_SEC_WEP(vap_params->security_params))
        {
            ...
            nindex++;
            snprintf(ncmd[nindex], sizeof(ncmd), "%s %d %s %d",
                     WPA_SET_NETWORK_CMD_PREFIX, id,
                     "wep_tx_keyidx",
    ...
    if (ACFG_IS_SEC_ENABLED(vap_params->security_params.sec_method) && (flags & WPA_SUPP_MODIFY_CIPHER)) {
        ...
        acfg_get_cipher_str(vap_params->security_params.cipher_method,
                cipher);
        snprintf(ncmd[nindex], sizeof(ncmd), "%s %d %s %s", WPA_SET_NETWORK_CMD_PREFIX,
                 id, "pairwise", cipher);
        nindex++;
        ...
        int8_t force_enable, int8_t *sec)
{
    ...
    if (!g_sock_ctx.cfg80211) {
        ...
        acfg_os_strcpy((uint8_t *)driver_name, (uint8_t *)WPA_DRIVER_NL80211, sizeof(driver_name));
    }
#if HAPD_CONF_FILE_UPDATE
...
acfg_update_hapd_params(acfg_wlan_profile_vap_params_t *vap_params, acfg_hapd_param_t *hapd_params)
{
    ...
    snprintf(htcap_file, sizeof(htcap_file), "/sys/class/net/%s/cfg80211_htcaps", vap_params->vap_name);
    snprintf(vhtcap_file, sizeof(vhtcap_file), "/sys/class/net/%s/cfg80211_vhtcaps", vap_params->vap_name);
    ...
    switch (vap_params->phymode) {
        case ACFG_PHYMODE_AUTO:
            acfg_os_strcpy((uint8_t *)hapd_params->hw_mode, (uint8_t *)"any", sizeof(hapd_params->hw_mode));
            break;
        case ACFG_PHYMODE_11B:
            acfg_os_strcpy((uint8_t *)hapd_params->hw_mode, (uint8_t *)"g", sizeof(hapd_params->hw_mode));
            break;
        ...
        case ACFG_PHYMODE_TURBO_A:
            acfg_os_strcpy((uint8_t *)hapd_params->hw_mode, (uint8_t *)"a", sizeof(hapd_params->hw_mode));
            break;
