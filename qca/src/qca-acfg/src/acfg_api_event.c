...

{
    ...
    switch(msg->id)
    {
        ...
        case ACFG_EV_HW_MODE_CHANGE_STATUS:
            if(event->hw_mode_change_status)
                status = event->hw_mode_change_status(ifname,
                            &pdata->hw_mode_change_status);
            break;
    }
...
{
    struct acfg_rtnl_hdl    rth;
    struct wsupp_ev_handle wsupp_h[ACFG_MAX_VAPS * ACFG_MAX_RADIO];
    uint32_t status = QDF_STATUS_SUCCESS;
    int ret = 0 ;
    ...
    loop_for_events = 1;
    while (1) {
        acfg_get_vap_iface_names(&list, &list.num_iface);
