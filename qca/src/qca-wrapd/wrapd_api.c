...
void send_message_to_brd(int success_flag, const u8 *addr, int operation)
{
    ...
    buffer[0] = INTERNAL_MESSAGE;
    buffer[1] = operation;
    buffer[2] = success_flag;
...
wrapd_psta_add(struct wrap_wifi_iface *aptr, const char *parent, char *child, const u8 *addr, u_int32_t flags)
{
    ...
    if(addr[0] & 0x01) {
        wrapd_printf("Donot create psta in mcast mac address");
        return PROXY_STA_CREATION_ERROR;
    }
    ...
    if (fst_unused == -1) {
	    ...
	    send_message_to_brd(FLAG_FAIL,addr,WIRED_PSTA_ADD);
        }
 ...
 */
	...
	if(res==5)
	{
        ...
        if (flags & WRAPD_PSTA_FLAG_WIRED) {
	    send_message_to_brd(FLAG_SUCCESS,addr,WIRED_PSTA_ADD);
        }
    }
...
wrapd_psta_remove(struct wrap_wifi_iface *aptr, const u8 *addr, int msgtobrd, char *child)
{
    ...
    if (res < 0) {
	    ...
	    send_message_to_brd(FLAG_FAIL,addr,WIRED_PSTA_REMOVE);
        }
        ...
        if ((psta->flags & WRAPD_PSTA_FLAG_WIRED) && msgtobrd) {
	    send_message_to_brd(FLAG_SUCCESS,addr,WIRED_PSTA_REMOVE);
        }
    }
    psta->added = 0;
    psta->flags = 0;
