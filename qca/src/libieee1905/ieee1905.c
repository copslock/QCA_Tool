...
#undef QCA_MOD_INPUT
#define QCA_MOD_INPUT QCA_MOD_LIBIEEE1905
...
static void ieee1905EventDispatch( u_int8_t *frame, u_int32_t len, u_int32_t type )
{
    ...
    switch( type )
    {
        ...
        case IEEE1905_MSG_TYPE_CHANNEL_SCAN_REPORT:
            ieee1905S.ieee1905ReceiveCallback( IEEE1905_EVENT_CHANNEL_SCAN, frame, len );
            break;
        case IEEE1905_MSG_TYPE_CAC_REQUEST:
        case IEEE1905_MSG_TYPE_CAC_TERMINATION:
            ieee1905S.ieee1905ReceiveCallback( IEEE1905_EVENT_CAC, frame, len );
             break;
        case IEEE1905_MSG_TYPE_TUNNELED_MESSAGE:
            ieee1905S.ieee1905ReceiveCallback( IEEE1905_EVENT_TUNNELED_MESSAGE, frame, len );
            break;
        case IEEE1905_MSG_TYPE_ASSOC_STATUS_NOTIFICATION:
            ieee1905S.ieee1905ReceiveCallback( IEEE1905_EVENT_ASSOC_STATUS_NOTIFICATION_MESSAGE, frame, len );
            break;
        case IEEE1905_MSG_TYPE_BACKHAUL_STA_CAP_QUERY_MESSAGE:
        case IEEE1905_MSG_TYPE_BACKHAUL_STA_CAP_REPORT_MESSAGE:
            ieee1905S.ieee1905ReceiveCallback( IEEE1905_EVENT_ASSOC_STATUS_NOTIFICATION_MESSAGE, frame, len );
            break;
        case IEEE1905_MSG_TYPE_FAILED_CONNECTION_MESSAGE:
            ieee1905S.ieee1905ReceiveCallback( IEEE1905_EVENT_FAILED_CONNECTION, frame, len );
            break;
        default:
            break;
    }
...
static IEEE1905_STATUS ieee1905FragmentedMessageAppend( u_int32_t index, ieee1905Message_t *message, u_int32_t contentLen )
{
    ...
    if( (fragmentedMessage[ index ].dataLen + contentLen + IEEE1905_ETH_HEAD_LEN + IEEE1905_HEAD_LEN) > fragmentedMessage[ index ].bufferLen )
    {
...
static IEEE1905_STATUS ieee1905FrameAssembly( u_int8_t *frame, u_int32_t len )
{
    ...
    if( ieee1905IsMessageFragmented(message->ieee1905Header.flags) )
    {
        ...
        if( index < 0 )
        {
            ...
            {
                ieee1905Debug( DBGDEBUG, "%s: Not a valid Index = %d for fragmented message FID = %d, MID = %d",
                    __func__, index, message->ieee1905Header.fid, htons( message->ieee1905Header.mid ) );
                return IEEE1905_NOK;
            }
        }
