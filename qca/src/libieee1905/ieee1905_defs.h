...
typedef enum ieee1905MessageType_e {
    ...
    IEEE1905_MSG_TYPE_SP_REQUEST = 0x8023,
    ...
    IEEE1905_MSG_TYPE_DPP_CCE_INDICATION = 0x8034,
...
typedef struct ieee1905Encrypt_t {
    ...
    u_int8_t encryptTxCounter[6];

    u_int16_t length;

    /// AES-SIV Encryption Output field (i.e., SIV concatenated with
    /// all the encrypted TLVs);
    u_int8_t output[0];
} ieee1905Encrypt_t;
...
/**
 * @brief Representation of Service Prioritization Interface exception
 */
typedef struct ieee1905SPrioritizationIFException_t {
    u_int8_t numIf;

    struct {
        struct ether_addr addr;
    } Interfaces[IEEE1905_QCA_VENDOR_MAX_INTERFACE];

} ieee1905SPrioritizationIFException_t;
...
#define IEEE1905_MAX_NUM_AKM_SUITES 1
...
typedef struct ieee1905AKMSuiteCapabilities_t {
    /// Num bSTA AKM Suite Selectors
    u_int8_t numBstaAKM;

    /// bSTA AKM Suite
    ieee1905AKMSuite_t bstaAKMSuite[IEEE1905_MAX_NUM_AKM_SUITES];
...
typedef struct ieee1905DPPEncap_t {
    ...
    IEEE1905_BOOL channelListPresent : 1;
    ...
    struct {
        ...
        /// Number of Channels for this class
        u_int8_t numChan;

        /// Channel
        u_int8_t channel[IEEE1905_MAX_OP_CLASS_CHAN_PAIRS];
    } opClass[IEEE1905_MAX_OPERATING_CLASSES];
