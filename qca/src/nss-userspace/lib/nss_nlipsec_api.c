...
/*
 * nss_nlipsec_sock_send_sync()
 *	send the IPsec message asynchronously through the socket
 */
 ...
 */
void nss_nlipsec_init_cmd(struct nss_nlipsec_ctx *ctx, struct nss_nlipsec_rule *rule, void *data, nss_nlipsec_resp_t cb,
				enum nss_nlipsec_cmd type)
{
	...
	struct nss_nlipsec_resp *resp = nss_nlcmn_get_cb_data(&rule->cm, family_id);
	assert(resp);
