...
int apacHyfi20ReceiveResponseE(apacHyfi20Data_t *pData, u8 *msg, u32 msgLen) {
    ...
    apacBool_e isRoleMatching = APAC_FALSE;
    apacBool_e isContMapV2 = APAC_FALSE;
    int num_tlv = 2;
    ...
    while (processedLen <= msgLen) {
            ...
            *pTlvVal == APAC_REGISTRAR)
        {
            ...
            if (*pTlvVal == APAC_MAP_VERSION_2) {
                isContMapV2 = APAC_TRUE;
            }
            if ((apacHyfiMapIsEnabled(HYFI20ToMAP(pData)) == APAC_MAP_VERSION_2) && isContMapV2) {
                dprintf(MSG_DEBUG, " Controller and Enrollee MAP Version %d Use DPP\n", *pTlvVal);
            }
        } else if (isContMapV2) {
            if (tlvType == IEEE1905_TLV_TYPE_SECURITY_CAP) {
...
int apacHyfi20ReceiveSearchR(apacHyfi20Data_t *ptrData, u8 *msg, u32 msgLen) {
    ...
    u8 freq, mapVersion, searchReqFreq = 0;
    int num_tlv = 3;
...
int apacHyfi20SendResponseR(apacHyfi20Data_t *pData, u8 *dest_mac, u8 freq, u16 mid,
    ...
    u_int16_t tlvLen = 0;
    u_int8_t *content;
    apacMapData_t *map = HYFI20ToMAP(pData);
    ...
    if (isMapCapable) {
        ...
        *content = tlvValue;
        ...
        if (apacHyfiMapIsEnabled(HYFI20ToMAP(pData)) == APAC_MAP_VERSION_2) {
            ...
            *content = APAC_MAP_VERSION_2;
            content++; tlvLen++;

            ieee1905TLVLenSet(TLV, tlvLen, frameLen);

            /* Add Device 1905 Layer Security Capability TLV */
            TLV = ieee1905TLVGetNext(TLV);
            ieee1905TLVTypeSet(TLV, IEEE1905_TLV_TYPE_SECURITY_CAP);

            content = ieee1905TLVValGet(TLV);
            tlvLen = 0;

            /* Onboarding protocols supported */
            *content = map->onboardingType;
            content++; tlvLen++;

            /* Message integrity algorithms supported */
            *content = map->integrityAlgoType;
            content++; tlvLen++;

            /* Message encryption algorithms supported */
            *content = map->encryptionAlgotype;
            content++; tlvLen++;
