...
int apacGetPIFMapCap(apacHyfi20Data_t *pData) {
    ...
    for (i = 0; i < APAC_NUM_WIFI_FREQ; i++) {
        ...
        if (strlen(pAP[i].ifName) &&
            wlanIfWd->ops->getDbgreq(wlanIfWd->ctx, pAP[i].ifName, (void *)&req, (sizeof(struct ieee80211req_athdbg))) < 0)  {
...
apacBool_e apacHyfiMapParse1905TLV(struct apac_wps_session *sess, u8 *content, u32 contentLen,
