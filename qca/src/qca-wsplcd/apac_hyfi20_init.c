        ...
        apacHyfi20Data_t *pData/* output */ )
{
    ...
    int i, j, k, band = APAC_WIFI_FREQ_INVALID;
        ...
        )
{
    ...
    } else if (strcmp(tag, "MapOnboardingType") == 0) {
        mapData->onboardingType = atoi(value);
    } else if (strcmp(tag, "MapIntegrityAlgo") == 0) {
        mapData->integrityAlgoType = atoi(value);
    } else if (strcmp(tag, "MapEncryptionAlgo") == 0) {
        mapData->encryptionAlgotype = atoi(value);
...
void apacHyfi20ConfigInit(apacHyfi20Data_t *ptrData)
{
    ...
    map->onboardingType = APAC_MAP_PROVISION_SUPPORT_RESERVED_MAX;
    map->integrityAlgoType = APAC_MAP_INTRIGITY_ALGO_RESERVED_MAX;
    map->encryptionAlgotype = APAC_MAP_ENCRYPTION_ALGO_RESERVED_MAX;
...
void apacHyfi20CmdLogFileModeName(int argc, char **argv)
{
    ...
    for (;;) {
        c = getopt(argc, argv, "f:r:l:o:c:m:M:i:wa");
        if (c < 0)
            break;
        switch (c) {
        case 'w':
