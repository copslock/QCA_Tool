		...
		struct cnss_cli_qdss_trace_load_and_start_data *data)
{
	...
	ret = wlfw_send_qdss_trace_config_download_req(data->instance_id);
	if (ret) {
		wsvc_printf_err("Failed to send QDSS Trace Load Config");
		return;
	}

	/* After qdss_trace_config_download, FW initiates QDSS mem req
	 * handshake with platform driver then configures the QDSS address
	 * to HW Trace control. If qdss_trace_start is sent before the
	 * address is configured in HW Trace control, the FW asserts.
	 *
	 * Adding 1 second sleep as WAR until this is fixed on FW.
	 */
	sleep(1);
	wlfw_qdss_trace_start(data->instance_id);
}

static void handle_daemon_cold_boot_support(enum cnss_cli_cmd_type type,
				struct daemon_coldboot_support_data *data)
{
	if (type == DAEMON_SUPPORT) {
		cnss_genl_send_data(CNSS_GENL_MSG_TYPE_DAEMON_SUPPORT,
				    data->instance_id, data->value);
	} else if (type == COLD_BOOT_SUPPORT) {
...
void handle_cnss_user_event(int fd)
{
	...
	case COLD_BOOT_SUPPORT:
		if (hdr->len != sizeof(struct daemon_coldboot_support_data)) {
			wsvc_printf_err("Invalid data length %d for type %d\n",
					hdr->len, hdr->type);
			goto out;
		}
		data = (char *)hdr + sizeof(struct cnss_cli_msg_hdr);
		handle_daemon_cold_boot_support(hdr->type,
				(struct daemon_coldboot_support_data *)data);
		break;
	default:
