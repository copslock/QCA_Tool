...
static void usage(char *progname)
{
	fprintf(stderr, "usage: %s [options] \n"
		...
		"   -i, --interface Interface selected, like integrated pci0 pci1 \n"
				       ...
				       int argc, char **argv)
{
	...
	struct daemon_coldboot_support_data daemon_coldboot_data;
	...
	static struct option options[] = {
		...
		{"instance_id", required_argument, NULL, 'i'},
	...
	while (1) {
		c = getopt_long(argc, argv, "hi:sx:d:c:", options, NULL);
	...
	case COLD_BOOT_SUPPORT:
		hdr->len = sizeof(daemon_coldboot_data);
		daemon_coldboot_data.instance_id = instance_id;
		daemon_coldboot_data.value = value;
		cnss_cli_data = (char *)hdr + sizeof(struct cnss_cli_msg_hdr);
		memcpy(cnss_cli_data, &daemon_coldboot_data,
		       sizeof(daemon_coldboot_data));
		break;
	default:
		usage(progname);
		return;
