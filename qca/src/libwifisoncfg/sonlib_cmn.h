...
void wlanifLinkEventsProcessChannelChange(
        int sysIndex, u_int8_t channel, u_int16_t freq);
...
/**
 * @brief Radio CAC information
 */
#define SONCMN_MAX_CAC_TIME_SEC_SIZE 3
struct soncmn_cac_capabilities_t {
    ...
    struct {
        ...
        u_int8_t secRequiredForCAC[SONCMN_MAX_CAC_TIME_SEC_SIZE];
        ...
        struct {
            ...
            u_int8_t channelNum[SONCMN_MAX_CHANNELS_PER_OP_CLASS];
        } opClass[SONCMN_MAX_OP_CHANNELS];

    } cacType[SONCMN_MAX_CAC_MODES];
};

struct soncmn_cac_info_t {
    u_int8_t cacCapabilitiesValid;

    struct soncmn_cac_capabilities_t cacCapabilities;
};
...
#define SONCMN_IEEE80211_NWID_LEN 32
typedef struct soncmn_neigh_scan_result_t {
    ...
    char ssid[SONCMN_IEEE80211_NWID_LEN];
    ...
    /// Length of Channel Bandwidth field
    u_int8_t channelBw;

    /// BSS Load Element Present
    u_int8_t bssLoadPresent : 1;

    /// The value of the "Channel Utilization" field as reported by the
    /// neighboring BSS in the BSS Load element.
    u_int8_t chUtil;

    /// The value of the "Station Count" field reported by the neighboring BSS in the BSS Load
    /// element.
    u_int16_t staCnt;
} soncmn_neigh_scan_result_t;
...
struct soncmnOps {
    ...
    int (*getCACCap)(const void *ctx, const char *ifname, struct soncmn_cac_info_t *cac);

    int (*getCountryCode)(const void *, const char *, size_t size, char *countrycode);
     ...
     *
     */
    int (*setAcsState)(const void *, const char *, int32_t);
     ...
     *
     */
    int (*getMapBssRole)(const void *ctx, const char *ifname, u_int8_t *mapBssRole);
