       ...
       const uint8_t *destAddr, struct soncmn_ieee80211_bsteering_datarate_info_t *datarateInfo)
{
    ...
    dbgf(soncmnDbgS.dbgModule, DBGERR,
         "%s: Get data rate info failed (errno=%d) on %s for " qtiMACAddFmt(":"),
         __func__, errno, ifname, qtiMACAddData(destAddr));
    return -1;
}
    ...
    *cfreq = 0;
    ...
    for (i = 0; i < chanInfoList.ic_nchans; ++i) {
        if (chanInfoList.ic_chans[i].ic_ieee == channum) {
            *cfreq = chanInfoList.ic_chans[i].ic_vhtop_ch_num_seg1;
            break;
        }
    }
...
/// @see setCenterFreqSeg1(const void *, const char *, int)
static int setCenterFreqSeg1_qti(const void *ctx, const char *ifname,
                                 int chanNum) {
    int ret;
    struct iwreq Wrq;
    struct priv_qti *priv;
    int param[2] = {IEEE80211_PARAM_SECOND_CENTER_FREQ, chanNum};
...
static int getCenterFreqSeg0_qti(const void *ctx, const char *ifname, uint8_t channum, uint8_t *cFreqSeg0)
{
    ...
    *cFreqSeg0 = 0;
    ...
    for (i = 0; i < chanInfoList.ic_nchans; ++i) {
        if (chanInfoList.ic_chans[i].ic_ieee == channum) {
            *cFreqSeg0 = chanInfoList.ic_chans[i].ic_vhtop_ch_num_seg1;
            break;
        }
    }
...
/// @see getOpClassInfo(const void *, const char*, u_int8_t, u_int8_t*,
///                     u_int8_t channels[], u_int8_t*, int8_t*)
int getOpClassInfo_qti(const void *ctx, const char *ifname, u_int8_t opclass,
                       u_int8_t *num_chan, u_int8_t channels[],
                       u_int8_t *ch_width, int8_t *sc_loc) {
    struct ieee80211req_athdbg req = {0};
    int32_t ret;
    struct priv_qti *priv;
    ...
    req.data.map_op_class.opclass = opclass;
    req.cmd = IEEE80211_DBGREQ_MAP_GET_OP_CLASS_INFO;

    if ((ret = sendDbgReqSetCmn(ctx, ifname, &req,
                                sizeof(struct ieee80211req_athdbg))) < 0) {
        dbgf(soncmnDbgS.dbgModule, DBGERR,
             "%s: Failed to get op class info for iface[%s] opclass %d error = %d",
             __func__, ifname, opclass, ret);
        goto err;
    }

    if (ch_width) {
        *ch_width = req.data.map_op_class.ch_width;
    }

    if (sc_loc) {
        if (req.data.map_op_class.sc_loc == IEEE80211_SEC_CHAN_OFFSET_SCA) {
            *sc_loc = 1; // sec above primary
        } else if (req.data.map_op_class.sc_loc == IEEE80211_SEC_CHAN_OFFSET_SCB) {
            *sc_loc = -1; // sec below primary
        } else {
            *sc_loc = 0; // no secondary chan
        }
    }

    if (num_chan && channels) {
        if (req.data.map_op_class.num_chan < *num_chan) {
            *num_chan = req.data.map_op_class.num_chan;
        }
        memcpy(channels, req.data.map_op_class.channels, *num_chan);
    }
...
int getCACCap_qti(const void *ctx, const char *ifname, struct soncmn_cac_info_t *cac_info) {
    struct ieee80211req_athdbg req = {0};
    int32_t ret = 0;
    struct priv_qti *priv;
    ...
    req.cmd = IEEE80211_DBGREQ_MAPV2_GET_RADIO_CAC_CAP;
    if ((ret = sendDbgReqSetCmn(ctx, ifname, &req, sizeof(struct ieee80211req_athdbg))) < 0) {
        dbgf(soncmnDbgS.dbgModule, DBGERR, "%s: Failed to get apcap for iface[%s] error = %d",
             __func__, ifname, ret);
        goto err;
    ...
    // Warning:Types are assumed to be identical and thus any change made in the
    // driver,
    // needs to be reflected in the user space type and vice versa.
    if (req.data.mapv2_cac_info.cac_capabilities_valid) {
        memcpy(&cac_info->cacCapabilities, &req.data.mapv2_cac_info.cac_cap,
               sizeof(cac_info->cacCapabilities));
        cac_info->cacCapabilitiesValid = 1;
    }
    TRACE_EXIT();
    return 0;
...
struct iw_priv_args *getPrivArgs_qti(const void *ctxt, const char *ifname,
                                            size_t *numIoctls) {
    struct iwreq wrq;
    struct priv_qti *priv;
    ...
    *numIoctls = 0;
    ...
    if (!privArgs) {
        dbgf(soncmnDbgS.dbgModule, DBGERR, "%s: Initial memory allocation failed",
             __func__);
        TRACE_EXIT();
        return NULL;
    }
    ...
    while (ioctl(priv->Sock, SIOCGIWPRIV, &wrq) < 0) {
        if (E2BIG == errno) {
            ...
            if (!privArgs2) {
                dbgf(soncmnDbgS.dbgModule, DBGERR, "%s: Failed to realloc for %zu entries",
            ...
            dbgf(soncmnDbgS.dbgModule, DBGERR, "%s: ioctl failed on %s for unknown reason: %d",
...
/// @see setParam(void *, const char *, int, void *, u_int32_t)
int setParam_qti(void *ctx, const char *ifname, int cmd, void *param, u_int32_t len)
{
    int ret;
    struct iwreq Wrq;
    struct priv_qti *priv;
...
int getACSReport_qti(void *ctx, const char *ifName,
                u_int8_t* numChans, ieee80211_acs_report_t chanData[],
                u_int8_t* numNeighbors, ieee80211_neighbor_info_t neighborData[],
                u_int8_t neighborChans[]) {
    int ret;
    struct iwreq iwr;
    struct priv_qti *priv = (struct priv_qti *)ctx;
...
/**
 * @brief Function to get country code
 *
 * @param [in] ctx       opaque pointer to private vendor struct
 * @param [in] ifname    interface name
 * @param [in] size      size of output buffer
 *
 * @param [out] countrycode pointer to be filled by this function
 *
 * @return Success: 0, Failure: -1
 *
 */
static int getCountryCode_qti(const void *ctx, const char *ifname, size_t size, char *countryCode) {
    TRACE_ENTRY();
...
/**
 * @brief Function to set ACS State
 *
 * @param [in] ctx       opaque pointer to private vendor struct
 * @param [in] ifname    interface name
 * @param [in] acsstate value to set
 *
 * @return Success: 0, Failure: -1
 *
 */
static int setAcsState_qti(const void *ctx, const char *ifname, int acsstate) {
    int ret;
    struct iwreq Wrq;
    struct priv_qti *priv;
    int iwparam[2] = {IEEE80211_PARAM_START_ACS_REPORT, acsstate};
...
int setAcsChanList_qti(void *ctx, const char *ifName, u_int8_t numChans, u_int8_t *channels) {
    TRACE_ENTRY();
...
static int getMapBssRole_qti(const void *ctx, const char *ifname, u_int8_t *mapBssRole) {
    int ret,iwparam;
    struct iwreq Wrq;
    struct priv_qti * priv;
...
int setCACTimeout_qti(void *ctx, const char *ifname, int cac_timeout) {
    TRACE_ENTRY();
...
static struct soncmnOps qti_ops = {
    ...
    .getOpClassInfo = getOpClassInfo_qti,
    ...
    .setCenterFreqSeg1 = setCenterFreqSeg1_qti,
    .getPrivArgs = getPrivArgs_qti,
    .setParam = setParam_qti,
    .getACSReport = getACSReport_qti,
    .getEspInfo = getEspInfo_qti,
    .getCACCap = getCACCap_qti,
    .getCountryCode = getCountryCode_qti,
    .setAcsState = setAcsState_qti,
    .setAcsChanList = setAcsChanList_qti,
    .getMapBssRole = getMapBssRole_qti,
    .setCACTimeout = setCACTimeout_qti,
};
