...
pktlog_txctl(struct ath_softc *sc, struct log_tx *log_data, u_int32_t iflags)
{
    ...
    if(dir == IEEE80211_FC1_DIR_TODS) {
        tx_log->bssid_tail = (wh->i_addr1[QDF_MAC_ADDR_SIZE-2] << 8) |
                             (wh->i_addr1[QDF_MAC_ADDR_SIZE-1]);
        tx_log->sa_tail    = (wh->i_addr2[QDF_MAC_ADDR_SIZE-2] << 8) |
                             (wh->i_addr2[QDF_MAC_ADDR_SIZE-1]);
        tx_log->da_tail    = (wh->i_addr3[QDF_MAC_ADDR_SIZE-2] << 8) |
                             (wh->i_addr3[QDF_MAC_ADDR_SIZE-1]);
    }
    else if(dir == IEEE80211_FC1_DIR_FROMDS) {
        tx_log->bssid_tail = (wh->i_addr2[QDF_MAC_ADDR_SIZE-2] << 8) |
                             (wh->i_addr2[QDF_MAC_ADDR_SIZE-1]);
        tx_log->sa_tail    = (wh->i_addr3[QDF_MAC_ADDR_SIZE-2] << 8) |
                             (wh->i_addr3[QDF_MAC_ADDR_SIZE-1]);
        tx_log->da_tail    = (wh->i_addr1[QDF_MAC_ADDR_SIZE-2] << 8) |
                             (wh->i_addr1[QDF_MAC_ADDR_SIZE-1]);
    }
    else {
        tx_log->bssid_tail = (wh->i_addr3[QDF_MAC_ADDR_SIZE-2] << 8) |
                             (wh->i_addr3[QDF_MAC_ADDR_SIZE-1]);
        tx_log->sa_tail    = (wh->i_addr2[QDF_MAC_ADDR_SIZE-2] << 8) |
                             (wh->i_addr2[QDF_MAC_ADDR_SIZE-1]);
        tx_log->da_tail    = (wh->i_addr1[QDF_MAC_ADDR_SIZE-2] << 8) |
...
pktlog_rx(struct ath_softc *sc, struct log_rx *log_data, u_int32_t iflags)
{
    ...
    if(log_data->status->rs_datalen > sizeof(struct ieee80211_frame)) {
        ...
        dir = (wh->i_fc[1] & IEEE80211_FC1_DIR_MASK);
        if(dir == IEEE80211_FC1_DIR_TODS) {
            rx_log->bssid_tail = (wh->i_addr1[QDF_MAC_ADDR_SIZE-2] << 8) |
                                 (wh->i_addr1[QDF_MAC_ADDR_SIZE-1]);
            rx_log->sa_tail    = (wh->i_addr2[QDF_MAC_ADDR_SIZE-2] << 8) |
                                 (wh->i_addr2[QDF_MAC_ADDR_SIZE-1]);
            rx_log->da_tail    = (wh->i_addr3[QDF_MAC_ADDR_SIZE-2] << 8) |
                                 (wh->i_addr3[QDF_MAC_ADDR_SIZE-1]);
        } else if(dir == IEEE80211_FC1_DIR_FROMDS) {
            rx_log->bssid_tail = (wh->i_addr2[QDF_MAC_ADDR_SIZE-2] << 8) |
                                 (wh->i_addr2[QDF_MAC_ADDR_SIZE-1]);
            rx_log->sa_tail    = (wh->i_addr3[QDF_MAC_ADDR_SIZE-2] << 8) |
                                 (wh->i_addr3[QDF_MAC_ADDR_SIZE-1]);
            rx_log->da_tail    = (wh->i_addr1[QDF_MAC_ADDR_SIZE-2] << 8) |
                                 (wh->i_addr1[QDF_MAC_ADDR_SIZE-1]);
        } else {
            rx_log->bssid_tail = (wh->i_addr3[QDF_MAC_ADDR_SIZE-2] << 8) |
                                 (wh->i_addr3[QDF_MAC_ADDR_SIZE-1]);
            rx_log->sa_tail    = (wh->i_addr2[QDF_MAC_ADDR_SIZE-2] << 8) |
                                 (wh->i_addr2[QDF_MAC_ADDR_SIZE-1]);
            rx_log->da_tail    = (wh->i_addr1[QDF_MAC_ADDR_SIZE-2] << 8) |
