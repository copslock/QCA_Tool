...
qdf_export_symbol(__adf_net_pseudo_attach);
qdf_export_symbol(__adf_net_pseudo_detach);
...
__adf_net_pseudo_attach(const char *mod_name)
{
    ...
    qdf_nofl_info("ADF_NET:Attaching the pseudo driver\n");
    /**
     * Allocate the sc & zero down
     */
    osdev[num] = kzalloc(sizeof(struct __adf_device), GFP_KERNEL);
    if (!osdev[num]) {
        qdf_nofl_info("Cannot malloc softc\n");
    ...
    if(osdev[num]->drv_hdl == NULL) {
        qdf_nofl_info("ADF_NET:Pseudo device is not created, asserting\n");
        adf_os_assert(0);
        goto attach_fail;
    }
    num++;
...
__adf_net_pseudo_detach(const char *mod_name)
{
    ...
    qdf_nofl_info("ADF_NET:detaching the pseudo driver\n");
