...
__giwscan_cb(void *arg, struct acfg_scan_result * sr)
{
    ...
    if (0){
        adf_os_mem_copy(iwe.u.ap_addr.sa_data, se_macaddr,QDF_MAC_ADDR_SIZE);
    } else {
        adf_os_mem_copy(iwe.u.ap_addr.sa_data, se_bssid,QDF_MAC_ADDR_SIZE);
    }
                ...
                union iwreq_data *iwreq, char *extra)
{
    ...
    if (scan == NULL || data == NULL) {
        qdf_nofl_info("%s : Mem Allocation Fail\n",__FUNCTION__);
        if (scan)   vfree(scan);
        if (data)   vfree(data);
                    ...
                    union iwreq_data *iwreq, char *extra)
{
    ...
    if (status == A_STATUS_OK ) {
        ...
        if(len <= (ADF_CHANINFO_MAX * sizeof(unsigned int)))
            ...
            qdf_nofl_info("%s : Not enough memory\n",__FUNCTION__);
    }
