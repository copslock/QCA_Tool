                        ...
                        adf_os_pci_devid_t *data)
{
    ...
    if ((err = pci_request_region(pdev, ADF_OS_PCI_BAR_0, drv_name))){
        qdf_nofl_info("ADF_PCI:request region failed\n");
        goto req_reg_fail;
    }
    ...
    if ((err = pci_set_dma_mask(pdev, DMA_32BIT_MASK))) {
        qdf_nofl_info("ADF_PCI:no usable DMA config\n");
        goto dma_fail;
    }
...
__adf_os_pci_attach(struct pci_dev *pdev, const struct pci_device_id *ent)
{
    ...
    pdrv = to_pci_driver(pdev->dev.driver);
     if (pdrv == NULL) {
         qdf_nofl_info("ADF_PCI:no driver found!\n");
         return EIO;
     }
    /**
     * Allocate the softc
    ...
    if(!osdev){
        qdf_nofl_info("ADF_PCI: unable to allocate osdev \n");
        goto sc_fail;
    }
    ...
    if((error = __adf_os_pci_setup_res(osdev, osdev->drv_name, ent, &pci_data.pci_id))){
        qdf_nofl_info("ADF_PCI:Bus setup failed\n");
        goto pci_fail;
    }

    if((error = __adf_os_pci_setup_iomap(&osdev->res, &res))){
        qdf_nofl_info("ADF_PCI:resource mapping failed\n");
        goto pci_fail;
    }
    ...
    if(!osdev->drv_hdl) {
        qdf_nofl_info("ADF_PCI:attach failed \n");
        goto attach_fail;
    }
    return 0;
...
__adf_os_pci_drv_unreg(uint8_t *name)
{
    ...
    if (!elem) {
        qdf_nofl_info("ADF_PCI:driver doesn't exist %s\n", name);
        return;
    }
...
qdf_export_symbol(__adf_os_pci_drv_reg);
qdf_export_symbol(__adf_os_pci_drv_unreg);
