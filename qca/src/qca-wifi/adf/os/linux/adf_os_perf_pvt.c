/*
* Copyright (c) 2012, 2018-2019 Qualcomm Innovation Center, Inc.
* All Rights Reserved.
...
{
    qdf_nofl_info("Perf Debug Module Init\n");
...
{
    qdf_nofl_info("Perf Debug Module Exit\n");
                   ...
                   adf_os_perf_cntr_t    type)
{
    ...
    if (type >= CNTR_LAST) {
        qdf_nofl_info("%s:%s Invalid perf-type\n", __FILE__, __func__);
        goto done;
    }
...
qdf_export_symbol(__adf_os_perf_init);
...
__adf_os_perf_destroy(adf_os_perf_id_t  id)
{
    ...
    if (!list_empty(&entry->child)) {
        qdf_nofl_info("Child's are alive, Can't delete \n");
        return A_FALSE;
    }
...
qdf_export_symbol(__adf_os_perf_destroy);
...
qdf_export_symbol(__adf_os_perf_start);
...
qdf_export_symbol(__adf_os_perf_end);
