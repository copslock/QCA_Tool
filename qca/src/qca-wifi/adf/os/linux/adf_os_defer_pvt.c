...
__adf_os_defer_func(struct work_struct *work)
{
    ...
    if (ctx->fn == NULL)
    {
        qdf_nofl_info("BugCheck: Callback is not initilized while creating work queue\n");
        return;
    }
    ctx->fn(ctx->arg);
#endif
...
__adf_os_defer_delayed_func(struct work_struct *dwork)
{
    ...
    if (ctx->fn == NULL)
    {
        qdf_nofl_info("BugCheck: Callback is not initilized while creating delayed work queue\n");
        return;
    }
    ctx->fn(ctx->arg);
#endif
...
qdf_export_symbol(__adf_os_defer_func);
qdf_export_symbol(__adf_os_defer_delayed_func);
