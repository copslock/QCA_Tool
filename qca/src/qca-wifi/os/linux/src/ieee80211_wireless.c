/*
 * Copyright (c) 2013-2020 Qualcomm Innovation Center, Inc.
 *
 * All Rights Reserved
...
#define IOCTL_DPRINTF(_fmt, ...) do {   \
    if ((ath_ioctl_debug))          \
        qdf_nofl_info(_fmt, __VA_ARGS__);  \
    } while (0)
...
int
ieee80211dbg_frameinjector(struct net_device *dev,
                            struct ieee80211req_athdbg *req)
{
    osif_dev *osifp = ath_netdev_priv(dev);
    wlan_if_t vap = osifp->os_if;

    wlan_enable_frame_injector(vap, req->data.param[0], req->data.param[1],
                                            req->data.param[2], req->dstmac);
    return 0;
}
...
static const struct iw_priv_args ieee80211_priv_args[] = {
...
#endif
    { IEEE80211_IOCTL_SETPARAM,
    IW_PRIV_TYPE_BYTE | IW_PRIV_SIZE_FIXED | QDF_MAC_ADDR_SIZE, 0, "" },
    { IEEE80211_IOCTL_GETPARAM,
    0, IW_PRIV_TYPE_INT | IW_PRIV_SIZE_FIXED | 1, "" },
    { IEEE80211_IOCTL_GETPARAM,
    IW_PRIV_TYPE_INT | IW_PRIV_SIZE_FIXED | 1, IW_PRIV_TYPE_INT | IW_PRIV_SIZE_FIXED | 1, "" },
    { IEEE80211_IOCTL_GETPARAM, 0,
    IW_PRIV_TYPE_BYTE | IW_PRIV_SIZE_FIXED | QDF_MAC_ADDR_SIZE, "" },
    { IEEE80211_IOCTL_GETPARAM, 0,
    IW_PRIV_TYPE_CHAR | IW_PRIV_SIZE_FIXED | 16, "" },
    { IEEE80211_IOCTL_GETPARAM, 0,
    IW_PRIV_TYPE_CHAR | IW_PRIV_SIZE_FIXED | (QDF_MAC_ADDR_SIZE * 2), "" },
    /*
    ...
    */
    ...
    IW_PRIV_TYPE_BYTE | IW_PRIV_SIZE_FIXED | QDF_MAC_ADDR_SIZE, 0, "wapi_rkupdate" },
  ...
  { IEEE80211_PARAM_RXINFO_PERPKT,
    ...
    IW_PRIV_TYPE_CHAR | IW_PRIV_SIZE_FIXED | (QDF_MAC_ADDR_SIZE * 2), "get_whc_bssid" },
    { IEEE80211_PARAM_WHC_APINFO_CAP_BSSID, 0,
    IW_PRIV_TYPE_CHAR | IW_PRIV_SIZE_FIXED | (QDF_MAC_ADDR_SIZE * 2), "g_whc_cap_bssid" },
    { IEEE80211_PARAM_WHC_APINFO_BEST_UPLINK_OTHERBAND_BSSID, 0,
    IW_PRIV_TYPE_CHAR | IW_PRIV_SIZE_FIXED | (QDF_MAC_ADDR_SIZE * 2), "g_best_ob_bssid" },
    { IEEE80211_PARAM_WHC_APINFO_OTHERBAND_UPLINK_BSSID, 0,
    IW_PRIV_TYPE_CHAR | IW_PRIV_SIZE_FIXED | (QDF_MAC_ADDR_SIZE * 2), "g_whc_ob_bssid" },
    ...
    { IEEE80211_PARAM_SAE_PWID,
    IW_PRIV_TYPE_INT | IW_PRIV_SIZE_FIXED | 1, 0, "en_sae_pwid" },
    { IEEE80211_PARAM_SAE_PWID,
    0,IW_PRIV_TYPE_INT | IW_PRIV_SIZE_FIXED | 1, "g_en_sae_pwid" },
    ...
    IW_PRIV_TYPE_INT | IW_PRIV_SIZE_FIXED | 2, 0, "ser_history" },
#endif
#if WLAN_SCHED_HISTORY_SIZE
    { IEEE80211_PARAM_WLAN_SCHED_HISTORY, 0,
    IW_PRIV_TYPE_INT | IW_PRIV_SIZE_FIXED | 1, "sched_history" },
#endif
    ...
    { IEEE80211_PARAM_CONFIG_CAPTURE_LATENCY_ENABLE, 0,
    IW_PRIV_TYPE_INT | IW_PRIV_SIZE_FIXED | 1, "g_tx_lat_cptr" },
#if SM_ENG_HIST_ENABLE
    { IEEE80211_PARAM_SM_HISTORY,
    0, IW_PRIV_TYPE_INT | IW_PRIV_SIZE_FIXED | 1, "sm_history" },
#endif
    { IEEE80211_PARAM_DEC_BCN_LOSS,
      IW_PRIV_TYPE_INT | IW_PRIV_SIZE_FIXED | 1, 0,"dec_bcn_loss" },
    { IEEE80211_PARAM_VHT_MCS_12_13_SUPP,
      IW_PRIV_TYPE_INT | IW_PRIV_SIZE_FIXED | 1, 0, "mcs12_13_supp" },
    { IEEE80211_PARAM_VHT_MCS_12_13_SUPP, 0,
      IW_PRIV_TYPE_INT | IW_PRIV_SIZE_FIXED | 1, "g_mcs12_13_supp" },
    { IEEE80211_PARAM_SET_STATS_UPDATE_PERIOD,
      IW_PRIV_TYPE_INT | IW_PRIV_SIZE_FIXED | 1, 0, "stats_period" },
    { IEEE80211_PARAM_HE_DL_MU_OFDMA_BFER,
    IW_PRIV_TYPE_INT | IW_PRIV_SIZE_FIXED | 1, 0, "he_dlofdma_bf" },
    { IEEE80211_PARAM_HE_DL_MU_OFDMA_BFER,
...
*/
void
set_quality(void *quality, u_int rssi, int16_t chan_nf)
{
    struct ieee80211_quality *iq = (struct ieee80211_quality *)quality;
    if(rssi >= 42)
        iq->qual = 94 ;
    else if(rssi >= 30)
    ...
    ieee80211_iw_getstats(struct net_device *dev)
{
    ...
    bool is_aggr = 1;
    int retval = 0;
    if ((retval = ieee80211_ucfg_get_quality(vap, (void *)&is->qual)) < 0) {
        qdf_err("Fail to get quality");
        return is;
    }
    ...
    struct iw_param *param, char *extra)
{
    ...
    {
        qdf_nofl_info("WARNING: Fragmentation with HT mode NOT ALLOWED!!\n");
        return -EINVAL;
    }
    return 0;
}
    ...
    struct iw_freq *freq, char *extra)
{
    ...
    */
    if (freq->e == 1) {
        ...
        i = freq->m;
        if (i != 0) {
            ifreq = wlan_reg_legacy_chan_to_freq(vap->iv_ic->ic_pdev_obj, i);
            if (ifreq < 0)
                return -EINVAL;
        } else {
            ifreq = 0;
    ...
    if (wlan_vdev_is_up(vap->vdev_obj) != QDF_STATUS_SUCCESS) {
          qdf_info("VAP is not ready.");
          return -EINVAL;
    }

    return ieee80211_ucfg_set_freq(vap, ifreq);
}
...
#if 0
		qdf_nofl_info("%s() spydata %pK, num %d\n", __func__, spydata, wrqu->data.length);
		for (i = 0; i < wrqu->data.length; i++)
			qdf_nofl_info(KERN_DEBUG
    ...
    __u32 *mode, char *extra)
{
    ...
    qdf_nofl_info(" %s: imr.ifm_active=%d, new mode=%d, valid=%d \n",
...
waplist_cb(void *arg, wlan_scan_entry_t se)
{
    ...
    set_quality((void *)&req->qual[i], se_snr, 161); /* -95dBm */
    ...
    */
...
#if ATH_SUPPORT_DFS
    if (ic->ic_scan_over_cac && (mlme_dfs_is_ap_cac_timer_running(pdev)
        ...
        if(wlan_pdev_wait_to_bringdown_all_vdevs(ic) == QDF_STATUS_SUCCESS) {
    ...
    if (ic->ic_nl_handle) {
        ...
        if (mu_in_progress || scan_in_progress) {
            qdf_nofl_info("%s: Currently doing %s %d\n", __func__,
...
#if QCA_LTEU_SUPPORT
            if (ic->ic_nl_handle) {
                qdf_nofl_info("%s: Unable to start scan\n", __func__);
        ...
        if (data->flags & IW_SCAN_THIS_FREQ) {
            ...
            for (i = 0, j = 0; i < scan_num_channels; i++) {
                if (req.channel_list[i].e > 1)
                    continue;
                if (req.channel_list[i].e == 1)
                    scan_chan_list[j] = (u_int8_t)wlan_reg_freq_to_chan(ic->ic_pdev_obj, req.channel_list[i].m / 100000);
                else
...
#define TU_TO_MS(_tu)        (((_tu) * 1024) / 1000)
    ...
    case IEEE80211_M_HOSTAP:
        ...
        if (scan_type == IEEE80211_SCAN_PASSIVE) {
            ...
            /* XXX tunables */
            if (min_channel_time && max_channel_time) {
                if(vap->iv_dec_bcn_loss)
                    scan_params->scan_req.dwell_time_passive = DWELL_TIME_PASSIVE_LESS_BCN_LOSS;
                else
                    scan_params->scan_req.dwell_time_passive = max_channel_time;
                scan_params->scan_req.dwell_time_active = max_channel_time;
            }
...
#define MAX_IE_LENGTH 30 + 255 * 2 /* "xxx_ie=" + encoded IE */
    ...
    last_ev = current_ev;
    iwe.cmd = IWEVQUAL;
    set_quality((void *)&iwe.u.qual, se_snr, 161); /* -95dBm */
    current_ev = iwe_stream_add_event(CURRENT_EV,
 ...
 *         u_int8_t	im_macaddr[QDF_MAC_ADDR_SIZE];\n
 *          };\n
        ...
        void *w, char *extra)
{
    ...
    if (!(dev->flags & IFF_UP)) {
        qdf_nofl_info(" DEVICE IS DOWN ifname=%s\n", dev->name);
        return -EINVAL;     /* XXX */
    }
...
wlan_set_debug_mac_filtering_flags(wlan_if_t vaphandle, unsigned char* extra)
{
    ...
    if (ni) {
        ...
        qdf_nofl_info("dbgLVLmac: Node (%s): Not found\n", ether_sprintf(macaddr));
    }
    ...
    void *w, char *extra)
{
    ...
    if(mgmt_frm->buflen >= 37 && mgmt_frm->buf[24] == 0xff)
        {
            qdf_nofl_info("unknown action frame\n");
...
error:
    qdf_nofl_info("usage: acparams ac <0|3> RTS <0|1> aggr scaling <0..4> min mbps <0..250>\n");
    return -EINVAL;
}
...
hbr_error:
    qdf_nofl_info("usage: hbrparams ac <2> enable <0|1> per_low <0..49>\n");
    return -EINVAL;
}
 ...
 *           u_int8_t	ik_flags;\n
 *           u_int8_t	ik_macaddr[QDF_MAC_ADDR_SIZE];\n
 ...
 *           u_int8_t	idk_macaddr[QDF_MAC_ADDR_SIZE];\n
 *           };\n
    ...
    if (kid == IEEE80211_KEYIX_NONE)
    {
            ...
            osifp->os_opmode == IEEE80211_M_P2P_CLIENT)
        {
            ...
            if (i == QDF_MAC_ADDR_SIZE) {
                qdf_mem_set(ik->ik_macaddr, QDF_MAC_ADDR_SIZE, 0xFF);
            }
        }
    }
    else
    ...
    void *w, char *extra)
{
    ...
    switch(u->data.flags)
    {
        ...
        case IEEE80211_PARAM_GET_MAC_LIST_SEC:
            mac_list = (u_int8_t *)OS_MALLOC(osifp->os_handle, (QDF_MAC_ADDR_SIZE * 256), GFP_KERNEL);
            ...
            for (i = 0; i < num_mac; i++) {
                memcpy(&(sa[i]).sa_data, &mac_list[i * QDF_MAC_ADDR_SIZE], QDF_MAC_ADDR_SIZE);
        ...
        case IEEE80211_PARAM_GET_CONFIG_BSSID:
            memcpy(sa->sa_data,vap->iv_conf_des_bssid, QDF_MAC_ADDR_SIZE);
        ...
        case IEEE80211_PARAM_ACL_GET_BLOCK_MGMT:
            ...
            for (i = 0; i < num_mac; i++) {
                if (wlan_acl_is_block_mgmt_set(vap,
                    ...
                    memcpy(&(sa[i]).sa_data, &mac_list[i * QDF_MAC_ADDR_SIZE],
                           QDF_MAC_ADDR_SIZE);
                    sa[i].sa_family = ARPHRD_ETHER;
                    num_block_mgmt++;
                }
    ...
    void *w, char *extra)
{
    ...
    freq = wlan_get_wlan_band_id_chan_to_freq(ic->ic_pdev_obj, param[0], band);
    qdf_nofl_info("chan: %d, tbtt: %d, ch_width: %d, band: %d, freq: %d",
                  param[0], param[1], param[2], band, freq);
    if (!freq)
        return -EINVAL;

    qdf_nofl_info("Enabling Channel Switch Announcement on current channel\n");
    return (ieee80211_ucfg_set_chanswitch(vap, freq, param[1], 0));
}
    ...
    void *w, char *extra)
{
    ...
    qdf_nofl_info("Enabling Channel and channel width Switch Announcement on current channel\n");

    return (ieee80211_ucfg_set_chanswitch(vap, freq, param[1], param[2]));
}
    ...
    struct iw_request_info *info, struct iw_param *erq, char *buf)
{
    ...
    if (args[1] < 0) {
        qdf_nofl_info(KERN_WARNING "%s: unknown pairwise cipher %d\n",
                dev->name, iwciph);
        return -EINVAL;
    }
    return ieee80211_ioctl_setparam(dev, NULL, NULL, (char*)args);
    ...
    struct iw_request_info *info, struct iw_param *erq, char *buf)
{
    ...
    args[1] = iwcipher2ieee80211cipher(iwciph);
    if (args[1] < 0) {
        qdf_nofl_info(KERN_WARNING "%s: unknown group cipher %d\n",
                dev->name, iwciph);
        return -EINVAL;
    }
    return ieee80211_ioctl_setparam(dev, NULL, NULL, (char*)args);
...
#define IW_AUTH_WAPI_ENABLED 0x20
    ...
    case IW_AUTH_PRIVACY_INVOKED:
        ...
        break;
    default:
        qdf_nofl_info(KERN_WARNING "%s: unknown SIOCSIWAUTH flag %d\n",
            dev->name, erq->flags);
        break;
    }
    ...
    struct iw_request_info *info, struct iw_param *erq, char *buf)
{
    ...
    case IW_AUTH_PRIVACY_INVOKED:
        ...
        break;
    default:
        qdf_nofl_info(KERN_WARNING "%s: unknown SIOCGIWAUTH flag %d\n",
            dev->name, erq->flags);
        break;
    }
    ...
    struct iw_request_info *info, struct iw_point *erq, char *extra)
{
    ...
    if (ext->alg == IW_ENCODE_ALG_NONE) {
        ...
        qdf_mem_zero(&dk, sizeof(dk));
        dk.idk_keyix = kid;
        qdf_mem_copy(&dk.idk_macaddr, ext->addr.sa_data, QDF_MAC_ADDR_SIZE);

        return ieee80211_ioctl_delkey(dev, NULL, NULL, (char*)&dk);
    }

    if (ext->ext_flags & IW_ENCODE_EXT_GROUP_KEY){
        qdf_nofl_info(" WAPI :: %s[%d] group key \n",__func__,__LINE__);
    } else {
        if(!kid){
            kid = IEEE80211_KEYIX_NONE;
            qdf_nofl_info("WAPI :: %s[%d] \n",__func__,__LINE__);
        }
            qdf_nofl_info("WAPI :: %s[%d] unicast key \n",__func__,__LINE__);
    }
    ...
    * can be implemented in terms of ieee80211_ioctl_setkey */
    if (ext->alg == IW_ENCODE_ALG_WEP &&
        qdf_mem_cmp(ext->addr.sa_data, "\xff\xff\xff\xff\xff\xff",
            QDF_MAC_ADDR_SIZE) == 0) {
        ...
        * iw_encode_ext structure */
        ...
        qdf_mem_zero(&oldinfo, sizeof(oldinfo));
        oldinfo.cmd = SIOCSIWENCODE;
        oldinfo.flags = info->flags;

        qdf_mem_zero(&olderq, sizeof(olderq));
...
#define IW_ENCODE_ALG_SMS4       0x20
    ...
    case IW_ENCODE_ALG_SMS4:
        ...
        break;
    default:
        qdf_nofl_info(KERN_WARNING "%s: unknown algorithm %d\n",
                dev->name, ext->alg);
        error = -EINVAL;
        goto exit;
    }
    ...
    if (ext->key_len > sizeof(kr.ik_keydata)) {
        qdf_nofl_info(KERN_WARNING "%s: key size %d is too large\n",
                dev->name, ext->key_len);
        error = -E2BIG;
        goto exit;
    }
    qdf_mem_copy(kr.ik_keydata, ext->key, ext->key_len);
    ...
    if (ext->ext_flags & IW_ENCODE_EXT_SET_TX_KEY) {
        ...
        qdf_mem_copy(kr.ik_macaddr, ext->addr.sa_data, QDF_MAC_ADDR_SIZE);
    }

    if (ext->ext_flags & IW_ENCODE_EXT_RX_SEQ_VALID) {
        qdf_mem_copy(&kr.ik_keyrsc, ext->rx_seq, sizeof(kr.ik_keyrsc));
    }

    error = ieee80211_ioctl_setkey(dev, NULL, NULL, (char*)&kr);

exit:
    /* Zero-out local key variables */
    qdf_mem_zero(&kr, sizeof(struct ieee80211req_key));
    return error;
}
...
wlan_debug_dump_nodequeues_tgt(struct ieee80211_node *ni)
{
    ...
    qdf_nofl_info("Node TID Queues");
    qdf_nofl_info( " (PS dql: ");
    ps_q = IEEE80211_NODE_SAVEQ_DATAQ(ni);
    if (ps_q) {
        IEEE80211_NODE_SAVEQ_LOCK(ps_q);
        qdf_nofl_info("%d", IEEE80211_NODE_SAVEQ_QLEN(ps_q));
        IEEE80211_NODE_SAVEQ_UNLOCK(ps_q);
    } else {
        qdf_nofl_info("0");
    }
    qdf_nofl_info( " PS mql: ");
    ...
    for (t = 0; t < WME_NUM_TID; t++) {
        ...
        if (tid->seq_start != 0 ||
            ...
            QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "ss=%d sn=%d "
...
qdf_export_symbol(wlan_debug_dump_queues_tgt_register);
...
wlan_debug_dump_nodes_tgt(void)
{
    ...
    for_each_netdev(&init_net, dev) {
        ...
        /* wifi information */
        qdf_nofl_info(KERN_ERR "%s: flags=0x%08x, chan=%02d, freq=%d opmode=%d\n",
                ((osdev_t)ic->ic_osdev)->netdev->name, ic->ic_flags,
                ieee80211_chan2ieee(ic, ic->ic_curchan),
                ieee80211_chan2freq(ic, ic->ic_curchan),ic->ic_opmode);

        /* vap information */
        TAILQ_FOREACH(vap, &ic->ic_vaps, iv_next) {
            qdf_nofl_info("%s: peer count=%d, deleted=%d\n",
                          (((osif_dev *)vap->iv_ifp)->netdev)->name,
                          wlan_vdev_get_peer_count(vap->vdev_obj),
                          ieee80211_vap_deleted_is_set(vap));
            qdf_nofl_info("ps_pending=%d, ps_sta=%d, sta_assoc=%d, aid_count=%d, iv_bss=%pK, is_deleted=%d\n",
                          vap->iv_ps_pending, vap->iv_ps_sta,
                          vap->iv_sta_assoc, get_aid_count(vap),
                          vap->iv_bss, ((osif_dev *)vap->iv_ifp)->is_deleted);
        }

        /* node information */
        OS_BEACON_READ_LOCK(&nt->nt_nodelock, &lock_state, flags);
        TAILQ_FOREACH_SAFE(ni, &nt->nt_node, ni_list, next) {
            qdf_nofl_info(KERN_ERR "0x%pK: %pM flags=0x%08x refcnt=%03d (%s)\n",
        ...
        TAILQ_FOREACH(ni, &ic->ic_nodes, ni_alloc_list) {
            qdf_nofl_info(KERN_ERR "0x%pK: %pM flags=0x%08x refcnt=%03d (%s) bss=%d temp=%d in_tbl=%d dqlen=%u, mqlen=%u, bss_node=%pK, is_obss=%d, assoc_msec=%d\n",
                ni, ni->ni_macaddr, ni->ni_flags, ieee80211_node_refcnt(ni),
                (((osif_dev *)ni->ni_vap->iv_ifp)->netdev)->name,
...
void ieee80211_register_vap_wext_handlers(osif_dev *osifp,
                                          bool is_register)
{
    ...
    if (is_register) {
        if ((wlan_psoc_nif_fw_ext_cap_get(psoc,
            ...
            qdf_err("WEXT-ifce disabled by current "
                    "dynamic_hw_mode state: "
                    "in-progress or DBS");
            return;
        }
        ieee80211_ioctl_vattach(osifp->netdev);
    } else {
        ieee80211_ioctl_vdetach(osifp->netdev);
    }
}
qdf_export_symbol(ieee80211_register_vap_wext_handlers);
...
qdf_export_symbol(wlan_update_pdev_cp_stats);
                              ...
                              struct ieee80211_mac_stats *mcast_stats_usr)
{
    ...
    vap_stats_usr->peer_delete_all_req =
                                    vdev_cp_stats.stats.cs_peer_delete_all_req;
    vap_stats_usr->peer_delete_all_resp =
                                    vdev_cp_stats.stats.cs_peer_delete_all_resp;
#endif
                            ...
                            struct ieee80211_mac_stats *vap_multicast_stats)
{
    ...
    vap_multicast_stats->ims_tx_bcast_data_bytes = vdev_stats->tx.bcast.bytes;
...
static void update_peer_protocol_stats(struct ieee80211_nodestats *ni_stats,
                                   const struct cdp_peer_stats *peer_stats)
{
    ni_stats->icmp_tx_ingress =
        peer_stats->tx.protocol_trace_cnt[CDP_TRACE_ICMP].ingress_cnt;
    ni_stats->icmp_tx_egress  =
        peer_stats->tx.protocol_trace_cnt[CDP_TRACE_ICMP].egress_cnt;
    ni_stats->icmp_rx_ingress =
        peer_stats->rx.protocol_trace_cnt[CDP_TRACE_ICMP].ingress_cnt;
    ni_stats->icmp_rx_egress  =
        peer_stats->rx.protocol_trace_cnt[CDP_TRACE_ICMP].egress_cnt;

    ni_stats->arp_tx_ingress =
        peer_stats->tx.protocol_trace_cnt[CDP_TRACE_ARP].ingress_cnt;
    ni_stats->arp_tx_egress  =
        peer_stats->tx.protocol_trace_cnt[CDP_TRACE_ARP].egress_cnt;
    ni_stats->arp_rx_ingress =
        peer_stats->rx.protocol_trace_cnt[CDP_TRACE_ARP].ingress_cnt;
    ni_stats->arp_rx_egress  =
        peer_stats->rx.protocol_trace_cnt[CDP_TRACE_ARP].egress_cnt;

    ni_stats->eap_tx_ingress =
        peer_stats->tx.protocol_trace_cnt[CDP_TRACE_EAP].ingress_cnt;
    ni_stats->eap_tx_egress  =
        peer_stats->tx.protocol_trace_cnt[CDP_TRACE_EAP].egress_cnt;
    ni_stats->eap_rx_ingress =
        peer_stats->rx.protocol_trace_cnt[CDP_TRACE_EAP].ingress_cnt;
    ni_stats->eap_rx_egress  =
        peer_stats->rx.protocol_trace_cnt[CDP_TRACE_EAP].egress_cnt;
}
#else
static void update_peer_protocol_stats(struct ieee80211_nodestats *ni_stats,
                                   const struct cdp_peer_stats *peer_stats)
...
int wlan_peer_stats_prepare(struct wlan_objmgr_peer *peer,
                            const struct cdp_peer_stats *peer_stats,
                            struct ieee80211_nodestats *ni_stats)
{
    ...
    if (peer_stats->rx.to_stack.num >= peer_stats->rx.multicast.num)
    ...
    ni_stats->ns_rx_decryptcrc = peer_stats->rx.err.decrypt_err +
                                  peer_cp_stats_rx_decryptcrc_get(peer);
    ni_stats->ns_tx_data = peer_stats->tx.ucast.num;
    update_peer_protocol_stats(ni_stats, peer_stats);
    ...
    ni_stats->ns_tx_bcast = peer_stats->tx.bcast.num;
    ...
    ni_stats->ns_tx_bcast_bytes = peer_stats->tx.bcast.bytes;
#endif
                           ...
                           struct ieee80211_nodestats *ni_stats_user)
{
    ...
    if (QDF_IS_STATUS_ERROR(status)) {
        ...
        qdf_err("Failed to fetch peer_stats for peer mac %pM", peer->macaddr);
        return -ENOENT;
    }
    wlan_peer_stats_prepare(peer, peer_stats, ni_stats_user);
...
int ieee80211_set_conf_bssid(wlan_if_t vap, u_int8_t *macaddr)
{
    ...
    if (wlan_vap_get_opmode(vap) == IEEE80211_M_STA ) {
        if (ic->ic_strict_pscan_enable) {
            OS_MEMCPY(vap->iv_conf_des_bssid,macaddr,QDF_MAC_ADDR_SIZE);
...
ieee80211_add_localpeer(wlan_if_t vap, char *params)
{
    ...
    qdf_nofl_info ("local peer mac addr %x:%x:%x:%x:%x:%x caps %x\n",mac[0],mac[1],mac[2],mac[3],mac[4],mac[5],caps);
...
ieee80211_authorise_local_peer(wlan_if_t vap, char *params)
{
    ...
    qdf_nofl_info ("local peer mac addr %x:%x:%x:%x:%x:%x \n",mac[0],mac[1],mac[2],mac[3],mac[4],mac[5]);
...
int ieee80211_sendwowpkt(struct net_device *dev, u_int8_t *macaddr)
{
    ...
    struct ieee80211_node *ni = NULL;
    u_int8_t   src_mac_address[QDF_MAC_ADDR_SIZE] = { 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF };
    u_int8_t   dst_mac_address[QDF_MAC_ADDR_SIZE] = { 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF };
    ...
    if (vap != ni->ni_vap) {
        qdf_nofl_info("%s: is not assocated for this VAP \n" ,ether_sprintf(ni->ni_macaddr));
        ieee80211_free_node(ni, WLAN_MLME_SB_ID);
        return -EINVAL;
    }
    ...
    if (!mywbuf) {
        qdf_nofl_info("wbuf allocation failed in %s \n", __func__);
        ieee80211_free_node(ni, WLAN_MLME_SB_ID);
        return -ENOMEM;
    }
                           ...
                           struct ol_ath_radiostats *scn_stats)
{
    ...
    if (!(pdev_stats = qdf_mem_malloc(sizeof(struct cdp_pdev_stats)))) {
        return -1;
    }
    ...
    if ((target_type == TARGET_TYPE_QCA8074) ||
        ...
        if (!cdp_host_get_pdev_stats(soc_txrx_handle, pdev_id, pdev_stats)) {
...
int ieee80211_add_wdsaddr(wlan_if_t vap, union iwreq_data *u)
{
    ...
    if(vap->iv_opmode == IEEE80211_M_HOSTAP)
    {
        ...
        ni = ieee80211_find_wds_node(nt, sa->sa_data, WLAN_MLME_SB_ID);
        if(ni)
            ieee80211_free_node(ni, WLAN_MLME_SB_ID);
        else
            return rc = 1;
    ...
    ni_wds = ieee80211_find_wds_node(nt, sa->sa_data, WLAN_MLME_SB_ID);
    if(ni_wds)
    {
        ieee80211_free_node(ni_wds, WLAN_MLME_SB_ID); /* Decr ref count */
    }
    else
    {
...
ieee80211dbg_sendaddba(struct net_device *dev, struct ieee80211req_athdbg *req)
{
    ...
    if (wlan_get_device_param(ic, IEEE80211_DEVICE_ADDBA_MODE) == ADDBA_MODE_AUTO) {
        qdf_nofl_info("%s(): ADDBA mode is AUTO\n", __func__);
        return -EINVAL;
    }

    /* Valid TID values are 0 through 15 */
    if (req->data.param[1] < 0 || req->data.param[1] > (IEEE80211_TID_SIZE - 2)) {
        qdf_nofl_info("%s(): Invalid TID value\n", __func__);
        return -EINVAL;
    }
...
ieee80211dbg_getaddbastatus(struct net_device *dev, struct ieee80211req_athdbg *req)
{
    ...
    if (ad.status == 0xFFFF) {
        qdf_nofl_info("Addba status IDLE\n");
    }
...
ieee80211dbg_ioctl_neighrpt(wlan_if_t vap, struct ieee80211req_athdbg *req)
{
    ...
    nrresp = qdf_mem_malloc(sizeof(ieee80211_cust_nrresp_info_t) *
                            nrrpt_user.num_report);
    if (nrresp == NULL) {
        qdf_err("Unable to allocate memory");
        return -ENOMEM;
    }

    if (copy_from_user(nrresp, (ieee80211_cust_nrresp_info_t *)
                       nrrpt_user.custom_nrresp_info,
    ...
    nrrpt_user.custom_nrresp_info = nrresp;
    retv = wlan_send_neighbor_resp(vap, req->dstmac, &nrrpt_user);
...
ieee80211acs_ioctl_getchanlist(struct net_device *dev, struct ieee80211req_athdbg *req)
{
    ...
    qdf_freq_t freq_list[ACS_MAX_CHANNEL_COUNT] = {0};

    req->data.user_chanlist.n_chan = wlan_acs_get_user_chanlist(NETDEV_TO_VAP(dev), freq_list);
    for(i = 0; i < req->data.user_chanlist.n_chan; i++)
    {
        chans[i].band = reg_wifi_band_to_wlan_band_id(wlan_reg_freq_to_band(freq_list[i]));
        chans[i].chan = wlan_reg_freq_to_chan(pdev, freq_list[i]);
    }
...
ieee80211acs_ioctl_setchanlist(struct net_device *dev, struct ieee80211req_athdbg *req)
{
    ...
    uint16_t freq = 0;
    struct ieee80211_chan_def *chans = req->data.user_chanlist.chans;
    qdf_freq_t chan_list[ACS_MAX_CHANNEL_COUNT] = {0};

    append = (chans[0].chan == ACS_SET_CHAN_LIST_APPEND);
    if (append) chan_idx = 1;

    for (i = chan_idx; chans[i].chan; i++) {
        freq = wlan_get_wlan_band_id_chan_to_freq(ic->ic_pdev_obj,
                chans[i].chan, chans[i].band);
        if (!freq) {
            qdf_alert("%s Invalid input chan: %d, band: %d\n",
                    __func__, chans[i].chan, chans[i].band);
            return -EINVAL;
        }
        chan_list[i-chan_idx] = freq;
    }

    wlan_acs_set_user_chanlist(vap, append, &chan_list[0]);
    return 0;
}
...
ieee80211dbg_ioctl_get_last_beacon_rssi(struct net_device *dev, struct ieee80211req_athdbg *req)
{
    ...
    ni = ieee80211_find_node(&vap->iv_ic->ic_sta, info->macaddr, WLAN_MLME_SB_ID);
    if (ni == NULL) {
...
ieee80211dbg_ioctl_acs_block_channel(struct net_device *dev, struct ieee80211req_athdbg *req)
{
    ...
    uint16_t freq;
    struct ieee80211_chan_def *chans = req->data.user_chanlist.chans;
    uint16_t freq_list[ACS_MAX_CHANNEL_COUNT] = {0};
    ...
    for (i = chan_idx; chans[i].chan; i++) {
        ...
        freq_list[i-chan_idx] = freq;
    }

    return wlan_acs_block_channel_list(vap, freq_list, req->data.user_chanlist.n_chan);
}
...
static bool ieee80211tr69_ioctl_set_csa_deauth(struct net_device *dev,
        struct ieee80211req_athdbg *req)
{
    struct ieee80211vap *vap = NETDEV_TO_VAP(dev);
    ieee80211req_tr069_t * reqptr = &req->data.tr069_req;
    uint8_t csa_deauth_mode = reqptr->data_buff[0];

    if (csa_deauth_mode == CSA_DEAUTH_MODE_UNICAST)
        qdf_info("csa_deauth_mode set to unicast");
    else if (csa_deauth_mode == CSA_DEAUTH_MODE_BROADCAST)
        qdf_info("csa_deauth_mode set to broadcast");
    else if (csa_deauth_mode == CSA_DEAUTH_MODE_DISABLE) {
        qdf_info("Disabling csa_deauth_mode");
    } else {
        qdf_info("Invalid csa_deauth_mode\n");
        return false;
    }

    vap->iv_csa_deauth_mode = csa_deauth_mode;

    return true;
}

static uint8_t ieee80211tr69_ioctl_get_csa_deauth(struct net_device *dev,
        struct ieee80211req_athdbg *req)
{
    struct ieee80211vap *vap = NETDEV_TO_VAP(dev);
    ieee80211req_tr069_t * reqptr = &req->data.tr069_req;
    uint8_t *csa_deauth_mode = (uint8_t *)&reqptr->data_buff;
    *csa_deauth_mode = vap->iv_csa_deauth_mode;
    return 0;
}
	...
	switch(cmdid){
		case TR069_CHANHIST:
			...
			for(; i < IEEE80211_CHAN_MAXHIST; i++){
				...
				if (chan_data->chanlhist[i].chanid) /* Add  band only for valid channels */
					chan_data->chanlhist[i].chanband =
						reg_wifi_band_to_wlan_band_id(ic->ic_chanhist[i].chanband);
		...
		case TR069_GET_BSRATE:
			...
			break;
		case TR069_GET_CSA_DEAUTH:
			ieee80211tr69_ioctl_get_csa_deauth(dev, req);
			break;
		case TR069_SET_CSA_DEAUTH:
			ieee80211tr69_ioctl_set_csa_deauth(dev, req);
			break;
...
ieee80211dbg_ioctl_mu_scan(struct net_device *dev, struct ieee80211req_athdbg *req)
{
    ...
    if (reqptr->mu_channel == 0 || reqptr->mu_channel == (u_int8_t) IEEE80211_CHAN_ANY) {
        qdf_nofl_info("%s: Invalid channel specified "
    ...
    for (i = 0; i < sizeof(modes) / sizeof(modes[0]); i++) {
        ...
        channel = ieee80211_find_dot11_channel(ic, wlan_reg_chan_to_freq(ic->ic_pdev_obj, reqptr->mu_channel), 0,
        ...
        if (!ic->ic_nl_handle->force_vdev_restart && ic->ic_curchan) {
            if (old_mode == vap->iv_des_mode &&
                channel->ic_freq == ic->ic_curchan->ic_freq) {
                break;
            }
        ...
        retv = wlan_set_channel(vap, wlan_reg_chan_to_freq(ic->ic_pdev_obj, reqptr->mu_channel), 0);
        if (!retv) {
            TAILQ_FOREACH(tmpvap, &ic->ic_vaps, iv_next) {
    ...
    if (retv) {
        qdf_nofl_info("%s: Unable to start MU id=%d on channel %d\n",
        ...
        struct ieee80211vap *vap, ieee80211req_custom_chan_t *reqptr)
{
    ...
    struct ieee80211_node   *ni = NULL;
    struct ieee80211_ath_channel *channel;
    uint16_t band = 0, chan = 0;
    uint16_t freq = 0;

    if(vap->iv_opmode != IEEE80211_M_STA)
    {
        qdf_nofl_info("valid only in STA mode\n");
        return -EINVAL;
    }
    if(vap->iv_bss)
    {
    ...
    if((reqptr->scan_numchan_nonassociated > IEEE80211_CUSTOM_SCAN_ORDER_MAXSIZE) ||
            (reqptr->scan_numchan_associated > IEEE80211_CUSTOM_SCAN_ORDER_MAXSIZE))
    {
        qdf_nofl_info("List is bigger than supported %d",MAX_SCAN_CHANS);
        return -EINVAL;
    }
    if (reqptr->scan_numchan_nonassociated) {
        for (i = 0; i < reqptr->scan_numchan_nonassociated; i++)
        {
            band = reqptr->scan_channel_list_nonassociated[i].band;
            chan = reqptr->scan_channel_list_nonassociated[i].chan;
            freq = wlan_get_wlan_band_id_chan_to_freq(ic->ic_pdev_obj, chan, band);
            if (!freq) {
                qdf_alert("%s Invalid input chan: %d, band: %d\n",
                        __func__, chan, band);
                return -EINVAL;
            ...
            if(channel==NULL)
            {
                qdf_nofl_info("Couldn't find channel for freq:%d, des_mode:%d",
        ...
        for (i = 0; i < reqptr->scan_numchan_nonassociated; i++)
        {
            ic->ic_custom_chan_list_nonassociated[i] =
                wlan_get_wlan_band_id_chan_to_freq(ic->ic_pdev_obj,
                        reqptr->scan_channel_list_nonassociated[i].chan,
                        reqptr->scan_channel_list_nonassociated[i].band);
        }
    ...
    if (reqptr->scan_numchan_associated) {
        ...
        {
            band = reqptr->scan_channel_list_associated[i].band;
            chan = reqptr->scan_channel_list_associated[i].chan;
            freq = wlan_get_wlan_band_id_chan_to_freq(ic->ic_pdev_obj, chan, band);
            if (!freq) {
                qdf_alert("%s Invalid input chan: %d, band: %d\n",
                        __func__, chan, band);
        ...
        for (i = 0; i < reqptr->scan_numchan_associated; i++)
        {
            ic->ic_custom_chan_list_associated[i] =
                wlan_get_wlan_band_id_chan_to_freq(ic->ic_pdev_obj,
                        reqptr->scan_channel_list_associated[i].chan,
                        reqptr->scan_channel_list_associated[i].band);
        }
                                ...
                                struct ieee80211req_athdbg *req)
{
    ...
    freq = wlan_get_wlan_band_id_chan_to_freq(vap->iv_ic->ic_pdev_obj,
            offchan_test->ieee_chan, offchan_test->band);
    if (!freq) {
        qdf_alert("%s Invalid input chan: %d, band: %d\n",
                __func__, offchan_test->ieee_chan, offchan_test->band);
        return -EINVAL;
    }
                                 ...
                                 struct ieee80211req_athdbg *req)
{
    ...
    return ucfg_offchan_rx_test(vap->vdev_obj, freq,
...
{
    uint8_t i;
    uint8_t chan, band_6g = 0;
    uint16_t freq = 0;
    struct primary_allowed_chanlist *ic_primary_chanlist = ic->ic_primary_chanlist;

    for (i = 0; i < ic_primary_chanlist->n_freq; i++) {
        freq = ic_primary_chanlist->freq[i];
        chan = wlan_reg_freq_to_chan(ic->ic_pdev_obj, freq);
        band_6g = wlan_reg_is_6ghz_chan_freq(freq);

        if (!band_6g) {
            if (isclr(ic->ic_chan_avail_2g_5g, chan)) {
                qdf_info("%s: 2g or 5g Channel frequency %d is invalid ",
                                __func__, ic_primary_chanlist->freq[i]);
                return 0;
            }
        } else {
            if (isclr(ic->ic_chan_avail_6g, chan)) {
                qdf_info("%s: 6g Channel frequency %d is invalid ",
                                __func__, freq);
                return 0;
            }
        }
...
{
        struct wlan_objmgr_pdev *pdev = NULL;
        struct primary_allowed_chanlist *ic_chanlist = ic->ic_primary_chanlist;
        uint16_t *temp_freq = NULL;
        ...
        temp_freq = qdf_mem_malloc(ic_chanlist->n_freq * sizeof(uint16_t));
        if(temp_freq == NULL)
        {
                qdf_print("%s: Unable to allocate memory", __func__);
                return -ENOMEM;
        }
        OS_MEMZERO(temp_freq, ic_chanlist->n_freq * sizeof(uint16_t));

        for(i = 0; i < ic_chanlist->n_freq; i++)
        {
                freq = ic_chanlist->freq[i];
                if(mlme_dfs_is_freq_in_nol(pdev, freq)) {
                        qdf_print("%s: Cannot add NOL channel %d into primary allowed channel list ",
                                        __func__, ic_chanlist->freq[i]);
                } else {
                        temp_freq[temp_nchan++] = ic_chanlist->freq[i];
                }
        }
        OS_MEMZERO(ic_chanlist->freq, ic_chanlist->n_freq * sizeof(uint16_t));
        OS_MEMCPY(ic_chanlist->freq, temp_freq, temp_nchan * sizeof(uint16_t));
        ic_chanlist->n_freq = temp_nchan;
        qdf_mem_free(temp_freq);
                ...
                struct ieee80211req_athdbg *req)
{
        ...
        struct primary_allowed_chanlist *ic_chanlist = NULL;
        ...
        if(chanlist->n_chan == 0) {
            if(ic->ic_primary_allowed_enable) {
                qdf_info("Primary allowed channel list is cleared ");
                qdf_mem_free(ic->ic_primary_chanlist->freq);
                qdf_mem_free(ic->ic_primary_chanlist);
                ic->ic_primary_allowed_enable = false;
                ic->ic_primary_chanlist = NULL;
            } else {
                qdf_info("Primary allowed channel list is empty, nothing to clear ");
            }
            return EOK;
        }

        if(!ic->ic_primary_allowed_enable) {
                ic->ic_primary_chanlist = qdf_mem_malloc(sizeof(struct primary_allowed_chanlist));
                ...
                ic_chanlist->freq = qdf_mem_malloc(sizeof(uint16_t) * chanlist->n_chan);
                if(ic_chanlist->freq == NULL) {
                        qdf_print("%s: Unable to allocate memory", __func__);
        ...
        ic_chanlist->n_freq = chanlist->n_chan;
        for (i = 0; i < chanlist->n_chan; i++) {
            qdf_alert("input chan: %d, band: %d\n",
                    chanlist->chans[i].chan, chanlist->chans[i].band);

            freq = wlan_get_wlan_band_id_chan_to_freq(ic->ic_pdev_obj,
                    chanlist->chans[i].chan,
                    chanlist->chans[i].band);
            if (!freq) {
                qdf_alert("%s Invalid input chan: %d, band: %d\n",
                        __func__, chanlist->chans[i].chan, chanlist->chans[i].band);
                ic->ic_primary_chanlist = NULL;
                qdf_mem_free(ic_chanlist->freq);
                qdf_mem_free(ic_chanlist);
                ic->ic_primary_allowed_enable = false;
        ...
        if(remove_nol_chan(ic) || (!ic_chanlist->n_freq)) {
                ic->ic_primary_allowed_enable = false;
                ic->ic_primary_chanlist = NULL;
                qdf_mem_free(ic_chanlist->freq);
                qdf_mem_free(ic_chanlist);
        ...
        if(!ieee80211_check_allowed_prim_freqlist(ic, ic->ic_curchan->ic_freq)) {
                qdf_info("%s: Current freq %d is not a primary allowed channel ",
                ...
                if (ic->no_chans_available == 1) {
                        ...
                        /* mlme_dfs_random_channel must be fixed to return frequency */
                ...
                struct ieee80211req_athdbg *req)
{
    struct ieee80211vap *vap = NETDEV_TO_VAP(dev);
    wlan_dev_t ic = wlan_vap_get_devhandle(vap);
    ieee80211_primary_allowed_chanlist_t *chanlist_u;
    struct primary_allowed_chanlist *ic_chanlist = NULL;
    struct wlan_objmgr_pdev *pdev = wlan_vdev_get_pdev(vap->vdev_obj);
    int i = 0;
    ...
    if(ic->ic_primary_allowed_enable) {
        ic_chanlist = ic->ic_primary_chanlist;
        chanlist_u = &req->data.primary_allowed_chanlist;

        for (i = 0; i < ic_chanlist->n_freq; i++) {
            chanlist_u->chans[i].band = reg_wifi_band_to_wlan_band_id(
                    wlan_reg_freq_to_band(ic_chanlist->freq[i]));
            chanlist_u->chans[i].chan = wlan_reg_freq_to_chan(pdev, ic_chanlist->freq[i]);
            qdf_alert("band: %d, chan: %d\n", chanlist_u->chans[i].band, chanlist_u->chans[i].chan);
        }
        chanlist_u->n_chan = i;
    }
    return EOK;
...
static int
ieee80211_fake_mgmt_rx(wlan_if_t vap,
                       struct ieee80211req_athdbg *req)
{
    ...
    if (copy_from_user(wh, (u_int8_t *)req->data.mgmt_frm.buf,
        req->data.mgmt_frm.buflen)) {
        qdf_nofl_info("%s:%d Copy from user failed!\n", __func__, __LINE__);
    ...
    wbuf_set_pktlen(wbuf, req->data.mgmt_frm.buflen);
    ni = ieee80211_find_node(&ic->ic_sta, wh->i_addr2, WLAN_MLME_SB_ID);
    if (ni == NULL) {
        qdf_info("ni not found with provided macaddr, using bss peer\n");
        ni = vap->iv_bss;
        ni->ni_consecutive_xretries = 0;
    }

    if ((ni->ni_consecutive_xretries > 0) &&
...
ieee80211_ioctl_getkey(struct net_device *dev, struct iwreq *iwr)
{
    ...
    if (wlan_get_param(vap,IEEE80211_FEATURE_PRIVACY) == 0)
    {
        ...
        ik.ik_type = IEEE80211_CIPHER_NONE;
#endif
        err = (_copy_to_user(iwr->u.data.pointer, &ik, sizeof(ik)) ?
        -EFAULT : 0);
        goto exit;
    }
    ...
    err = wlan_get_key(vap, kid, ik.ik_macaddr, &kval,
                IEEE80211_KEYBUF_SIZE+IEEE80211_MICBUF_SIZE, GET_PN_ENABLE);
    if (err)
        goto exit;
    ...
    else
    {
        ik.ik_keyrsc = 0;
        ik.ik_keytsc = 0;
        qdf_mem_zero(ik.ik_keydata, sizeof(ik.ik_keydata));
    }
#else
    req_key.keyix = ik.ik_keyix;
    qdf_mem_copy(req_key.macaddr,ik.ik_macaddr,QDF_MAC_ADDR_SIZE);
    if (wlan_get_param(vap,IEEE80211_FEATURE_PRIVACY) == 0)
    {
        ik.ik_type = IEEE80211_CIPHER_NONE;
        err = (_copy_to_user(iwr->u.data.pointer, &ik, sizeof(ik)) ?
    ...
    qdf_mem_copy(ik.ik_macaddr, req_key.macaddr, QDF_MAC_ADDR_SIZE);
#if ATH_SUPPORT_WAPI
    ...
    err = ((_copy_to_user(iwr->u.data.pointer, &ik, sizeof(ik))) ? -EFAULT : 0);
...
exit:
    ...
    qdf_mem_zero(&req_key, sizeof(struct wlan_crypto_req_key));
#else
    qdf_mem_zero(&kval, sizeof(ieee80211_keyval));
#endif
    qdf_mem_zero(&ik, sizeof(struct ieee80211req_key));
    return err;
}
...
ieee80211_ioctl_getwpaie(struct net_device *dev, struct iwreq *iwr)
{
    ...
    if (wpaie == NULL)
    return -ENOMEM;
    if (__xcopy_from_user(wpaie, iwr->u.data.pointer, QDF_MAC_ADDR_SIZE)) {
    OS_FREE(wpaie);
        return -EFAULT;
    }
...
ieee80211_ioctl_getstastats(struct net_device *dev, struct iwreq *iwr)
{
    ...
    if (iwr->u.data.length < off)
        return -EINVAL;
    if (__xcopy_from_user(macaddr, iwr->u.data.pointer, QDF_MAC_ADDR_SIZE))
        return -EFAULT;
...
ieee80211_ioctl_getmac(struct net_device *dev, struct iwreq *iwr)
{
    ...
    if (iwr->u.data.length < sizeof(QDF_MAC_ADDR_SIZE))
    {
        return -EFAULT;
    }
    ...
    return (_copy_to_user(iwr->u.data.pointer, macaddr, QDF_MAC_ADDR_SIZE) ? -EFAULT : 0);
}
...
ieee80211_ioctl_hmwds(struct net_device *dev, struct iwreq *iwr)
{
    ...
    if (__xcopy_from_user(config, iwr->u.data.pointer, iwr->u.data.length)) {
        OS_FREE(config);
        qdf_nofl_info("%s: Copy from user failed\n", __func__);
        return -EFAULT;
    }
        ...
        struct iwreq *iwr)
{
    ...
    if(copy_to_user(buf, channel, channel->ic_nchans *
            sizeof(struct ieee80211_ath_channel) + sizeof(__u32))){
       qdf_nofl_info("%s: copy_to_user error\n", __func__);
       return -EFAULT;
    }
...
ieee80211_ioctl_nac(struct net_device *dev, struct iwreq *iwr)
{
    ...
    if(!vap->iv_smart_monitor_vap) {
        qdf_nofl_info("Nac commands not supported in non smart monitor vap!\n");
        return -EPERM;
    }
        ...
        u_int8_t *src_addr, u_int8_t *bssid, ieee80211_xmit_status *ts)
{
    ...
    memcpy(act_cb->dst_addr, dst_addr, QDF_MAC_ADDR_SIZE);
    memcpy(act_cb->src_addr, src_addr, QDF_MAC_ADDR_SIZE);
    memcpy(act_cb->bssid, bssid, QDF_MAC_ADDR_SIZE);
	...
	if (wlan_is_mpsta(vap)) {
        ...
        if (iwd->data.length) {
            ...
            if ((int) sizeof(*scan_extra) + scan_extra->ie_len > iwd->data.length) {
                qdf_nofl_info(" %s: scan data is greater than passed \n", __func__);
                retv = EINVAL;
                goto scanreq_fail;
            }
            ...
            if (scan_extra->num_ssid > MAX_SCANREQ_SSID ||
                ...
                qdf_nofl_info(" %s: num_ssid is greater:%d \n", __func__, scan_extra->num_ssid);
                retv = EINVAL;
                goto scanreq_fail;
            }
...
int ieee80211_set_vap_vendor_param(struct net_device *dev, void *data)
{
    ...
    {
        qdf_nofl_info("copy_from_user error\n");
        status = -1;
        goto done;
    }
...
int ieee80211_get_vap_vendor_param(struct net_device *dev, void *data)
{
    ...
    {
        qdf_nofl_info("copy_to_user error\n");
        status = -1;
    }
done:
    qdf_mem_free(req);
...
ieee80211_ioctl(struct net_device *dev, struct ifreq *ifr, int cmd)
{
    ...
    if (ic->ic_is_ifce_allowed_in_dynamic_mode &&
        !ic->ic_is_ifce_allowed_in_dynamic_mode(ic)) {
        qdf_err("WEXT-ifce disabled by current dynamic_hw_mode state"
                ": in-progress or DBS");
        return -EINVAL;
    }
    ...
    case ACFG_PVT_IOCTL:
        {
            ...
            error = acfg_handle_vap_ioctl(dev, (void *)req);
...
void wlan_noise_info(void *arg, wlan_node_t node)
{
    ...
    char *noise_stats = dest_addr + (node_info->traf_rate * QDF_MAC_ADDR_SIZE);

    if (node->ni_flags && (node_info->count <= node_info->traf_rate)) {
        IEEE80211_ADDR_COPY((dest_addr + (node_info->count * QDF_MAC_ADDR_SIZE)), node->ni_macaddr);
...
int wlan_get_traffic_stats(wlan_if_t vap, struct ieee80211req_athdbg *req, unsigned int size)
{
    ...
    if (size < (sizeof(*req) + (QDF_MAC_ADDR_SIZE * (node_info->count)) +
                             ...
                             void *wri_pointer, unsigned int data_size)
{
    ...
    switch (req->cmd) {
        ...
        case IEEE80211_DBGREQ_PEERNSS:
            ...
            break;
        case IEEE80211_DBGREQ_FRAME_INJECTOR:
            retv = ieee80211dbg_frameinjector(dev, req);
            break;
        ...
        case IEEE80211_DBGREQ_SENDNEIGRPT:
            ...
            break;
        case IEEE80211_DBGREQ_SEND_CUSTOM_NEIGRPT:
            retv = ieee80211dbg_ioctl_neighrpt(vap, req);
            break;
        ...
        case IEEE80211_DBGREQ_GET_RRM_STA_LIST:
            {
                if (IEEE80211_ADDR_EQ(req->dstmac, IEEE80211_GET_BCAST_ADDR(vap->iv_ic))) {
...
#endif
                    temp = qdf_mem_malloc(max_sta_count * QDF_MAC_ADDR_SIZE);
                    if (!temp) {
                    ...
                    if (buff) {
                        ...
                        retv = copy_to_user(buff, temp, (max_sta_count * QDF_MAC_ADDR_SIZE)) ?  -EFAULT : 0;
      ...
      case IEEE80211_DBGREQ_DISPLAY_TRAFFIC_STATISTICS:
        ...
        case IEEE80211_DBGREQ_FIPS:
        {
            ...
            if (ic->ic_get_tgt_type(ic) == TARGET_TYPE_AR9888) {
...
#define ATH_FIPS_TIMEOUT ((CONVERT_SEC_TO_SYSTEM_TIME(10)/10) + 1) /* 100 ms*/
                ...
                if(ic->ic_output_fips) {
                    qdf_nofl_info("%s received fips event %d %pK length %d\n", __func__,
    ...
    case IEEE80211_DBGREQ_MAP_GET_ESP_INFO:
        retv = ucfg_son_dispatcher(NETDEV_TO_VDEV(dev),
                  ...
                  (void *)req);
        break;
    case IEEE80211_DBGREQ_MAPV2_GET_RADIO_CAC_CAP:
        retv = ucfg_son_dispatcher(NETDEV_TO_VDEV(dev),
                  SON_DISPATCHER_CMD_MAP_GET_CAC_CAP,
                  SON_DISPATCHER_GET_CMD,
                  (void *)req);
        break;
    case IEEE80211_DBGREQ_MAP_GET_OP_CLASS_INFO:
        retv = ucfg_son_dispatcher(NETDEV_TO_VDEV(dev),
    ...
    case IEEE80211_DBGREQ_FAKE_MGMT_RX:
        retv = ieee80211_fake_mgmt_rx(vap, req);
        break;

    default:
        qdf_nofl_info("%s : Dbg command %d is not supported \n", __func__, req->cmd);
        break;
    }
