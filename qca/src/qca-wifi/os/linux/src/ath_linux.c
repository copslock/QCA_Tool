...
#define  WAIT_VAP_IDLE_INTERVALL 10
    ...
    do {
        ...
        if (delete_in_progress) {
            msleep(1000);
            if (--gracetime == 0) {
                qdf_nofl_info(KERN_ERR "timeout waiting for vap %s\n",
    ...
    if (!TAILQ_EMPTY(&ic->ic_vaps)) {
        TAILQ_FOREACH_SAFE(vap, &ic->ic_vaps, iv_next, tvap) {
            qdf_nofl_info(KERN_ERR "%s: vap %s still registered, cleaning up\n",
...
int32_t ath_ioctl_get_mpsta_mac_addr(struct ath_softc_net80211 *scn, caddr_t param)
{
    ...
    return _copy_to_user((caddr_t)param , mpsta_vap->iv_myaddr, QDF_MAC_ADDR_SIZE);
}
    ...
    case SIOCGSETCTLPOW:
        {
            ...
            qdf_nofl_info("ctl ioctl :\n");
            for (i = 0; i < 1024; i++) {
                if ((i % 16) == 15) {
                    qdf_nofl_info("\n");
                }
                qdf_nofl_info(" %02X", ctl[i]);
            }
            qdf_nofl_info("\n");
...
#endif
{
    ...
    if ((!scn) ||
        ...
        qdf_nofl_info ("%s : #### delete is in progress, scn %pK \n", __func__, scn);
...
OS_TIMER_FUNC(ath_int_status_poll_timer)
{
    ...
    if (scn->pre_int_scheduled_cnt !=0 && (scn->pre_int_scheduled_cnt == scn->int_scheduled_cnt)) {
        ath_bstuck_tasklet(sc);
                ATH_CLEAR_HANGS(sc);
        qdf_nofl_info("%s: issue internal reset to recover wifi interrupt stop, count=%d\n",__func__,scn->int_scheduled_cnt);
    }
                ...
                void *event_data)
{
    ...
    if (event == FLOWMAC_ENABLE) {
        qdf_nofl_info("%s Flowmac is enabled\n",__func__);
        sc->sc_osnetif_flowcntrl = 1;
    } else if (event == FLOWMAC_DISABLE)  {
        qdf_nofl_info("%s flowmac is disabled \n", __func__);
...
ath_flowmac_attach(ath_dev_t dev)
{
    ...
    if (!dev) {
        qdf_nofl_info("%s dev pointer is wrong \n", __func__);
        return -EINVAL;
    }
    ...
    if (sc->sc_flowmac_dev) {
        qdf_nofl_info("%s flowmac registration is successful %pK\n",
...
qdf_export_symbol(ath_flowmac_attach);
int
...
__ath_attach(u_int16_t devid, struct net_device *dev, HAL_BUS_CONTEXT *bus_context, osdev_t osdev)
{
...
#endif
    if (temp_dev == NULL) {
        qdf_nofl_info(KERN_ERR "ath: Cannot allocate soc object \n");
        error = -ENOMEM;
        goto bad0;
    }
    ...
    rtnl_unlock();
    qdf_nofl_info("In %s[%d]temp_dev->name: %s\n",__func__,__LINE__,temp_dev->name);
    ...
    if ((error = register_netdev(temp_dev)) != 0) {
        qdf_nofl_info(KERN_ERR "%s: unable to register device\n", temp_dev->name);
        goto bad_soc_free;
    }

    hal_conf_parm.calInFlash = bus_context->bc_info.cal_in_flash;
    qdf_nofl_info("%s: Set global_scn[%d]\n", __func__, num_global_scn);
    ...
    qdf_nofl_info("*** All the minfree values should be <= ATH_TXBUF-32, otherwise default value will be used instead ***\n");
    qdf_nofl_info("ACBKMinfree = %d\n", ath_params.ACBKMinfree);
    qdf_nofl_info("ACBEMinfree = %d\n", ath_params.ACBEMinfree);
    qdf_nofl_info("ACVIMinfree = %d\n", ath_params.ACVIMinfree);
    qdf_nofl_info("ACVOMinfree = %d\n", ath_params.ACVOMinfree);
    qdf_nofl_info("CABMinfree = %d\n", ath_params.CABMinfree);
    qdf_nofl_info("UAPSDMinfree = %d\n", ath_params.UAPSDMinfree);
    qdf_nofl_info("ATH_TXBUF=%d\n", ATH_TXBUF);
    ...
    if (ah == NULL) {
        qdf_nofl_info("%s: unable to attach hardware; HAL status %u\n",
               __func__, status);
        error = -ENXIO;
        goto bad_objmgr3;
    }
    ...
    dev->hard_header_len += sizeof (struct ieee80211_qosframe) + sizeof(struct llc) + QDF_MAC_ADDR_SIZE + IEEE80211_WEP_IVLEN + IEEE80211_WEP_KIDLEN;
   ...
   OS_MESGQ_INIT(osdev, &osdev->async_q, sizeof(os_async_q_mesg),
    ...
    if (error)
    {
        qdf_nofl_info(KERN_WARNING "%s: request_irq failed\n", dev->name);
        error = -EIO;
        goto bad4;
    }
...
#if PCI_INTERRUPT_WAR_ENABLE
    {
        qdf_nofl_info(KERN_INFO "%s : Interrupt WA timer is enabled\n", __func__);
...
__ath_remove_interface(struct net_device *dev)
{
    ...
    while (vap != NULL) {
        ...
        netdev = osifp->netdev;
        qdf_nofl_info("Remove interface on %s\n",netdev->name);
        rtnl_lock();
        dev_close(netdev);
...
qdf_export_symbol(__ath_detach);
qdf_export_symbol(init_wlan);
qdf_export_symbol(__ath_remove_interface);
qdf_export_symbol(__ath_attach);
qdf_export_symbol(ath_usb_vap_restart);
qdf_export_symbol(__ath_set_delete);
#endif
