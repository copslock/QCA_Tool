/*
 * Copyright (c) 2015-2018,2020 Qualcomm Innovation Center, Inc.
 * All Rights Reserved
...
osif_nss_ol_get_legacy_pdev_from_scn(struct ol_ath_softc_net80211 *scn)
{
    return (struct ol_txrx_pdev_t *)((struct ol_txrx_psoc_t *)wlan_psoc_get_dp_handle(scn->soc->psoc_obj))->pdev[0];
}

static inline struct ol_txrx_pdev_t *
osif_nss_ol_get_pdev_from_vap(struct ieee80211vap *vap)
{
    return osif_nss_ol_get_legacy_pdev_from_scn(osif_nss_ol_get_scn_from_vap(vap));
}

static inline struct ol_txrx_pdev_t *
osif_nss_ol_get_pdev_from_netdev(struct net_device *dev)
{
    return osif_nss_ol_get_pdev_from_vap(((osif_dev *)ath_netdev_priv(dev))->os_if);
}
...
bool osif_nss_ni_stats_update(struct ol_ath_softc_net80211 *scn, struct sk_buff * nbuf, void *rx_mpdu_desc, uint8_t htt_rx_status, struct nss_wifi_append_statsv2_metahdr *metaheader)
{
    ...
    struct ol_txrx_pdev_t *pdev = osif_nss_ol_get_legacy_pdev_from_scn(scn);
 ...
 */
void osif_nss_ol_be_vdev_tx_inspect_handler(struct cdp_soc_t *cdp_soc, uint8_t vdev_id, struct sk_buff *skb)
{
    struct ol_txrx_peer_t *peer;
...
bool osif_nss_ol_be_vdev_handle_monitor_mode(struct net_device *netdev, struct sk_buff *skb, uint8_t is_chain)
{
    ...
    vap = osdev->os_if;
    scn = osif_nss_ol_get_scn_from_vap(vap);
    pdev = osif_nss_ol_get_legacy_pdev_from_scn(scn);
    ...
    if (pdev->filter_neighbour_peers && vap->iv_smart_monitor_vap) {
        ...
        if (likely((not_mgmt_type && not_ctrl_type) != 0)) {
            if ((is_mcast && (pdev->mon_filter_mcast_data) == 1)
                    || ((!is_mcast) && (pdev->mon_filter_ucast_data) == 1)) {
...
void osif_nss_ol_be_vdev_spl_receive_exttx_compl(struct net_device *dev, struct sk_buff *skb, struct nss_wifi_vdev_tx_compl_metadata *tx_compl_metadata)
{
...
#endif
    struct ol_txrx_pdev_t *pdev = osif_nss_ol_get_pdev_from_netdev(dev);

    if (!pdev || !pdev->htt_pdev->htt_process_data_tx_frames){
        qdf_err("Drop frame !!..");
	qdf_nbuf_free(skb);
	return;
    }
    ...
    qdf_mem_copy(txhdr->ta, tx_compl_metadata->ta, QDF_MAC_ADDR_SIZE);
    qdf_mem_copy(txhdr->ra, tx_compl_metadata->ra, QDF_MAC_ADDR_SIZE);
    txhdr->ppdu_id = tx_compl_metadata->ppdu_id;
#endif
    pdev->htt_pdev->htt_process_data_tx_frames(skb);
    return;
}
...
void osif_nss_ol_be_vdev_spl_receive_ext_mesh(struct net_device *dev, struct sk_buff *skb, struct nss_wifi_vdev_mesh_per_packet_metadata *mesh_metadata)
{
    ...
    mhdr->band = pdev->rx_mon_recv_status->rs_band;
    mhdr->channel = pdev->rx_mon_recv_status->rs_channum;
...
uint8_t osif_nss_ol_be_vdev_call_monitor_mode(struct net_device *netdev, osif_dev  *osdev, qdf_nbuf_t skb_list_head, uint8_t is_chain)
{
    ...
    scn = osif_nss_ol_get_scn_from_netdev(netdev);
    pdev = osif_nss_ol_get_legacy_pdev_from_scn(scn);
    ic = &scn->sc_ic;

    if (!pdev || (!is_chain  && (qdf_nbuf_len(skb_list_head) == 0))) {
...
void osif_nss_ol_be_vdevcfg_set_offload_params(struct cdp_soc_t *soc, uint8_t vdev_id, struct nss_wifi_vdev_config_msg **p_wifivdevcfg)
{
...
{
    struct ol_txrx_peer_t *peer = NULL;
    struct ieee80211_node   *ni = list_entry->ni;
    struct ol_ath_softc_net80211 *scn = OL_ATH_SOFTC_NET80211(ni->ni_ic);
    struct ol_txrx_pdev_t *pdev_txrx_handle = osif_nss_ol_get_legacy_pdev_from_scn(scn);

    if (!pdev_txrx_handle)
        return;
#if ATH_SUPPORT_WRAP
    peer = ol_txrx_peer_find_hash_find(pdev_txrx_handle, ni->ni_macaddr, 1, wlan_vdev_get_id(ni->ni_vap->vdev_obj));
#else
    peer = ol_txrx_peer_find_hash_find(pdev_txrx_handle, ni->ni_macaddr, 1);
#endif
    if (!peer)
        return;
...
void *osif_nss_ol_be_get_vdev_from_vdev_id(struct cdp_soc_t *soc, uint8_t vdev_id)
{
    struct ol_txrx_pdev_t *pdev = (struct ol_txrx_pdev_t *)((struct ol_txrx_psoc_t *)soc)->pdev[0];

    return ol_txrx_get_vdev_from_pdev(pdev, vdev_id);
}

uint8_t osif_nss_ol_be_get_vdevid_fromosif(osif_dev *osifp, uint8_t *vdev_id)
{
    *vdev_id = wlan_vdev_get_id(osifp->os_if->vdev_obj);
    return 1;
}
...
{
    struct ol_txrx_pdev_t *pdev = osif_nss_ol_get_pdev_from_vap(vap);
    uint32_t peer_id;
    ...
    if (!(ast_entry = ol_txrx_ast_find_hash_find(pdev, peer_mac_addr, mac_addr_is_aligned))) {
...
#else
    peer = ol_txrx_peer_find_hash_find(pdev, peer_mac_addr, mac_addr_is_aligned);
#endif
    if (!(peer)) {
...
void
osif_nss_ol_be_rsim_rx_decap(os_if_t osif, qdf_nbuf_t *pdeliver_list_head, qdf_nbuf_t *pdeliver_list_tail, void *peer)
{
#if QCA_SUPPORT_RAWMODE_PKT_SIMULATION
    osif_rsim_rx_decap(osif, pdeliver_list_head, pdeliver_list_tail, ((struct ol_txrx_peer_t *)peer)->mac_addr.raw);
#endif
}
...
{
    struct ol_txrx_peer_t *peer;
    struct ol_txrx_pdev_t *pdev = osif_nss_ol_get_pdev_from_vap(osifp->os_if);
...
uint8_t osif_nss_ol_be_find_pstosif_by_id(struct net_device *netdev, uint32_t peer_id, osif_dev **psta_osifp)
{
    ...
    struct ol_txrx_pdev_t *pdev = osif_nss_ol_get_pdev_from_netdev(netdev);
...
int32_t osif_nss_be_vap_updchdhdr(osif_dev *osifp)
{
    ...
    if (nss_ifnum == -1) {
        qdf_err(" vap transmit called on invalid interface");
        return -1;
    }

    pdev = osif_nss_ol_get_pdev_from_vap(osifp->os_if);
    vdev_id = wlan_vdev_get_id(osifp->os_if->vdev_obj);
    if (!(vdev = ol_txrx_get_vdev_from_pdev(pdev, vdev_id))) {
        return -1;
    }
    ...
    qdf_mem_set(wifivdevmsg, 0, sizeof(struct nss_wifi_vdev_msg));
    wifivdevupdchdr = &wifivdevmsg->msg.vdev_updchdr;
    wifivdevupdchdr->vdev_id = vdev_id;
