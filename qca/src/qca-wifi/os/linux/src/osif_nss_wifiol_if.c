/*
 * Copyright (c) 2015-2020 Qualcomm Innovation Center, Inc.
 * All Rights Reserved
...
#define NSS_WIFIOL_DELAY_100MS 100
    ...
    for (i = 0 ; i < msg->nentries ; i++) {
        ...
        if (peer) {
            ...
            if ((vap) && ieee80211_vap_wnm_is_set(vap) && ieee80211_wnm_bss_is_set(vap->wnm)) {
                ni = ieee80211_find_node(&ic->ic_sta, peer->mac_addr.raw, WLAN_NSS_IF_ID);
                if (ni) {
                    if (ni != ni->ni_bss_node) {
                        ieee80211_wnm_bssmax_updaterx(ni, secured);
                    }
                    ieee80211_free_node(ni, WLAN_NSS_IF_ID);
                }
            }
            if (vap) {
...
int osif_nss_ol_process_ol_stats_msg(struct ol_ath_softc_net80211 *scn,  struct nss_wifi_ol_stats_msg *msg)
{
    ...
    struct ieee80211vap *vap = NULL;
    uint32_t num_msdus = 0, byte_cnt = 0, discard_cnt = 0, i = 0, tx_mgmt = 0, j = 0, bcast_pkt_cnt = 0, bcast_byte_cnt = 0;
    struct ol_txrx_peer_t *peer = NULL;
    ...
    for (i = 0 ; i < msg->npeers ; i++) {
        ...
        if (peer) {
            ...
            bcast_pkt_cnt = msg->peer_ol_stats[i].tx_bcast_pkts;
            bcast_byte_cnt = msg->peer_ol_stats[i].tx_bcast_bytes;
            ...
            tx_stats->bcast.num += bcast_pkt_cnt;
            tx_stats->bcast.bytes += bcast_byte_cnt;
            ...
            if (qdf_likely(vap)) {
                ni = ieee80211_vap_find_node(vap, peer->mac_addr.raw, WLAN_MLME_SB_ID);
                if (ni) {
#ifdef QCA_SUPPORT_CP_STATS
...
static void osif_nss_ol_wifi_ext_callback_func(ol_ath_soc_softc_t *soc, struct sk_buff * nbuf,  __attribute__((unused)) struct napi_struct *napi)
{
    ...
    pdev = (struct ol_txrx_pdev_t *)((struct ol_txrx_psoc_t *) soc_txrx_handle)->pdev[0];
    htt_pdev = pdev->htt_pdev;
                    ...
                    if (skb_list_head) {
                        ...
                        if (qdf_likely((not_mgmt_type && not_ctrl_type) != 0)) {
                            ...
                            if (qdf_likely(is_mcast != 1)) {
                                /*ucast data pkts*/
                                if (cdp_txrx_get_pdev_param(soc_txrx_handle, pdev->pdev_id, CDP_FILTER_UCAST_DATA, &val) &&
                                ...
                                /*mcast data pkts*/
                                if (cdp_txrx_get_pdev_param(soc_txrx_handle, pdev->pdev_id, CDP_FILTER_MCAST_DATA, &val) &&
                            ...
                            /*mgmt/ctrl etc. pkts*/
                            if (cdp_txrx_get_pdev_param(soc_txrx_handle, pdev->pdev_id, CDP_FILTER_NO_DATA, &val) &&
 ...
 */
static int osif_nss_ol_wds_extn_peer_cfg_send(struct ol_ath_softc_net80211 *scn, uint8_t *peer_mac, uint8_t vdev_id)
{
    struct ol_txrx_pdev_t *pdev = NULL;
    struct ol_txrx_peer_t *peer = NULL;
    int ifnum = -1;
...
static void osif_nss_ol_set_dbdc_fast_lane(struct ieee80211com* ic, bool enable)
{
    /*
     * This is not supported for Legacy systems.
     */
...
static void osif_nss_ol_store_other_pdev_stavap(struct ieee80211com* ic)
{
    ...
    for (i = 0 ;i < MAX_RADIO_CNT-1; i++) {
        ...
        if (tmp_ic && tmp_ic->ic_is_mode_offload(tmp_ic)) {
            ...
            tpdev = osif_nss_ol_get_legacy_pdev_from_scn(scn);
            if (!tpdev ) {
                continue ;
            }
...
static int osif_nss_tx_queue_cfg(struct ol_ath_softc_net80211 *scn, uint32_t range, uint32_t buf_size)
{
    ...
    if (!(pdev = osif_nss_ol_get_legacy_pdev_from_scn(scn))) {
        return 0;
    }
    nss_wifiol_ctx = pdev->nss_wifiol_ctx;
    if (!nss_wifiol_ctx) {
...
static void osif_nss_wifi_peer_ast_flowid_map(ol_ath_soc_softc_t *soc, uint16_t peer_id,
        uint8_t vdev, uint8_t *peer_mac_addr)
{
    qdf_err("NSS-WifOffoad: peer ast flowmap (nocode) ");
}
...
static void osif_nss_ol_post_recv_buffer(ol_ath_soc_softc_t *soc)
{
    ...
    /*
     * Adding delay between WiFi Init and Post Recv msg
     * to help NSS FW to prefetch payloads needed for CE init.
     */
    mdelay(NSS_WIFIOL_DELAY_100MS);
...
static int osif_nss_ol_pdev_add_wds_peer(struct ol_ath_softc_net80211 *scn, uint8_t vdev_id, uint8_t *peer_mac, uint16_t peer_id, uint8_t *dest_mac, uint8_t *next_node_mac, uint8_t ast_type)
{
    void *nss_wifiol_ctx;
    uint32_t radio_id;
    int ifnum = -1;
    ...
    pdev = osif_nss_ol_get_legacy_pdev_from_scn(scn);
    if (!pdev) {
        qdf_err("%s: pdev is null",__FUNCTION__);
        return 1;
    }
    ...
    memcpy(st->peer_mac, next_node_mac, 6);
...
static int osif_nss_ol_peer_set_vlan_id(struct ol_ath_softc_net80211 *scn, uint8_t *peer_mac,
                                        uint16_t vlan_id)
{
    return 0;
}

static int osif_nss_ol_pdev_update_lmac_n_target_pdev_id(struct ol_ath_softc_net80211 *scn, uint8_t *pdev_id,
                                        uint8_t *lmac_id, uint8_t *target_pdev_id)
{
    return 0;
}
...
static void osif_nss_wifi_nawds_enable(ol_ath_soc_softc_t *soc, uint8_t *peer_mac, uint16_t vdev_id, uint16_t is_nawds)
{
 ...
 */
static int osif_nss_ol_set_peer_security_info(struct ol_ath_softc_net80211 *scn, uint8_t *peer_mac, uint8_t vdev_id,
...
static struct nss_wifi_radio_ops nss_wifi_funcs_radio = {
    ...
    osif_nss_ol_peer_set_vlan_id,
    osif_nss_ol_set_dbdc_fast_lane,
    osif_nss_ol_pdev_update_lmac_n_target_pdev_id
};
...
static struct nss_wifi_internal_ops nss_wifi_funcs_ol_be = {
    ...
    osif_nss_ol_be_get_vdev_from_vdev_id,
...
struct nss_wifi_soc_ops nss_wifibe_soc_ops = {
    ...
    osif_nss_wifi_peer_ast_flowid_map,
};
