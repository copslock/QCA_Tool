...
static int
decap_dot11_to_8023(qdf_nbuf_t msdu, uint8_t *peer_mac, struct ieee80211vap *vap);
                             ...
                             uint8_t *peer_mac)
{
    ...
    /*
     * When order bit is set to 1 in frame control for QOS data frame. MAC header carries additional 4 bytes
     * corresponding to HT control information. The dot112dot3 conversion logic has to take
     * care of this additional length in MAC header size.
     */
    ...
    if (wh.i_fc[1] & IEEE80211_FC1_WEP) {
        ...
        sec_type = osif_get_sec_type(vap, peer_mac, sec_idx);
...
decap_dot11_to_8023(qdf_nbuf_t mpdu, uint8_t *peer_mac, struct ieee80211vap *vap)
{
     ...
     * care of this additional length in MAC header size.
     */
    if ((wh.i_fc[1] & IEEE80211_FC1_ORDER) && (subtype == IEEE80211_FC0_SUBTYPE_QOS)) {
            hdrsize += 4;
    }
    ...
    qdf_nbuf_t nbuf, enum ieee80211_opmode mode, u_int8_t *bssid,struct ieee80211vap *vap)
{
    ...
    if (mesh_vap_mode) {
        ...
        if (vap->iv_rawmodesim_debug) {
            qdf_nofl_info("dst %s ",ether_sprintf(eth_hdr.ether_dhost));
            qdf_nofl_info("src %s ",ether_sprintf(eth_hdr.ether_shost));
            qdf_nofl_info("bssid %s \n",ether_sprintf(bssid));
            qdf_nofl_info("is 4addr %d\n",addr4_frame);
        }
    }
#endif
...
osif_rsim_rx_decap(os_if_t osif,
        ...
        qdf_nbuf_t *pdeliver_list_tail, uint8_t *peer_mac)
{
...
qdf_export_symbol(osif_rsim_rx_decap);
...
osif_rsim_tx_encap(struct ieee80211vap *vap, qdf_nbuf_t *pnbuf)
{
    ...
    if(vap->iv_rawmodesim_debug) {
        qdf_nofl_info("Tx Ethernet II type packet hexdump before encap\n");
