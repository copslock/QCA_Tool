    ...
    if ( (sc->sc_reset_type ==  ATH_RESET_NOLOSS) ){
        if (sc->sc_opmode == HAL_M_IBSS) {
            if (ath_hal_getrxfilter(sc->sc_ah) & HAL_RX_FILTER_PHYRADAR) {
                qdf_nofl_info("%s: radar was enabled before reset\n", __func__);
                enable_radar = 1;
            }
        }
...
#endif
        if (enable_radar) {
            qdf_nofl_info("%s: enabling radar after reset\n", __func__);
...
qdf_export_symbol(ath_internal_reset);
...
ath_dynamic_sysctl_register(struct ath_softc *sc)
{
	...
	char *devname = sc->sc_osdev->netdev->name ;
        ...
        if (sc->sc_sysctls == NULL) {
                qdf_nofl_info("%s: no memory for sysctl table!\n", __func__);
                return;
        }
        ...
        if (!sc->sc_sysctl_header) {
                qdf_nofl_info("%s: failed to register sysctls!\n", devname);
...
qdf_export_symbol(ath_dynamic_sysctl_register);
qdf_export_symbol(ath_dynamic_sysctl_unregister);
...
qdf_export_symbol(ath_ignoredfs_enable);
#endif
...
int ath_proc_write_nodefixedrate(struct file *file, const char *buffer, unsigned long count, void *data)
{
    ...
    qdf_nofl_info("Proc write: macaddr = %s\n", ether_sprintf(macaddr));
    fixedratecode = (u_int32_t)simple_strtoul(buffer+15, NULL, 16);
    qdf_nofl_info("Proc write: fixedratecode = %x\n", fixedratecode);

    for (i=0; i<num_global_scn; i++) {
        qdf_nofl_info("Searching for node in wifi%d\n", i);
...
{
    qdf_nofl_info(KERN_INFO"Removing athdebug proc file\n");
...
#endif
qdf_export_symbol(ACBKMinfree);
qdf_export_symbol(ACBEMinfree);
qdf_export_symbol(ACVIMinfree);
qdf_export_symbol(ACVOMinfree);
qdf_export_symbol(CABMinfree);
qdf_export_symbol(UAPSDMinfree);
qdf_export_symbol(min_buf_resv);
#endif

qdf_export_symbol(init_sc_params);
qdf_export_symbol(init_sc_params_complete);
qdf_export_symbol(ath_dev_attach);
qdf_export_symbol(ath_dev_free);
qdf_export_symbol(ath_pkt_log_text);
qdf_export_symbol(ath_draintxq);
