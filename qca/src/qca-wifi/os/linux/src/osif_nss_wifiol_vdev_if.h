/*
 * Copyright (c) 2015-2016,2017-2018,2020 Qualcomm Innovation Center, Inc.
 * All Rights Reserved
...
enum osif_nss_vdev_cmd {
    ...
    OSIF_NSS_WIFI_VDEV_CFG_MULTIPASS,
    OSIF_NSS_WIFI_VDEV_MAX
};

/*
 * Struct MC_LIST_UPDATE is use for passing parameters to ListUpdate function
...
struct MC_LIST_UPDATE{
    ...
    union {
            ...
            u_int8_t            grpaddr_ip6[QDF_IPV6_ADDR_SIZE];
    }u;
    u_int32_t               timestamp;
    u_int8_t                grp_addr[QDF_MAC_ADDR_SIZE];
    u_int8_t                grp_member[QDF_MAC_ADDR_SIZE];
    struct ieee80211vap     *vap;
    struct ieee80211_node   *ni;
    u_int8_t                cmd;
    u_int8_t                nsrcs;
    u_int8_t                srcs[QDF_IPV6_ADDR_SIZE * DP_ME_MCAST_SRCS_MAX];
};
...
void osif_nss_vdev_me_update_hifitlb(osif_dev *osifp, dp_vdev_me_t *vdev_me);
#endif
...
int32_t osif_nss_ol_vap_create(struct ieee80211vap *vap, osif_dev *osif, uint32_t nss_ifnum);
...
int osif_nss_vdev_set_group_key(osif_dev *osifp, uint16_t vlan_id, uint16_t group_key);
...
struct nss_vdev_ops {
    ...
    int32_t (* ic_osif_nss_vap_create)(struct ieee80211vap *vap,
    ...
    int (*ic_osif_nss_vdev_set_group_key)(osif_dev *osifp, uint16_t vlan_id, uint16_t group_key);
};
...
void * osif_nss_ol_be_peer_find_hash_find(struct ieee80211vap *vap,
                                uint8_t *peer_mac_addr, int mac_addr_is_aligned);
void osif_nss_ol_be_rsim_rx_decap(os_if_t osif, qdf_nbuf_t *pdeliver_list_head,
                                  qdf_nbuf_t *pdeliver_list_tail, void *peer);
void osif_nss_wifili_rsim_rx_decap(os_if_t osif, qdf_nbuf_t *pdeliver_list_head,
                             qdf_nbuf_t *pdeliver_list_tail, void *peer);
...
void *osif_nss_wifili_get_vdev_from_vdev_id(struct cdp_soc_t *soc, uint8_t vdev_id);
...
uint8_t osif_nss_wifili_find_pstosif_by_id(struct net_device *netdev, uint32_t peer_id,
                                osif_dev **psta_osifp);
void osif_nss_ol_be_vdevcfg_set_offload_params(struct cdp_soc_t *soc, uint8_t vdev_id,
                                struct nss_wifi_vdev_config_msg **p_wifivdevcfg);
void osif_nss_wifili_vdevcfg_set_offload_params(struct cdp_soc_t *cdp_soc, uint8_t vdev_id,
                                struct nss_wifi_vdev_config_msg **p_wifivdevcfg);
...
void osif_nss_wifili_vdev_tx_inspect_handler(struct cdp_soc_t *cdp_soc, uint8_t vdev_id, struct sk_buff *skb);
...
struct nss_wifi_internal_ops {
    ...
    void *(* ic_osif_nss_ol_get_vdev_from_vdev_id)(struct cdp_soc_t *cdp_soc, uint8_t vdev_id);
    uint8_t (* ic_osif_nss_ol_find_pstosif_by_id)(struct net_device *netdev,
                                uint32_t peer_id, osif_dev **psta_osifp);
    void (* ic_osif_nss_ol_vdevcfg_set_offload_params)(struct cdp_soc_t *cdp_soc, uint8_t vdev_id,
                                struct nss_wifi_vdev_config_msg **p_wifivdevcfg);
    bool (* ic_osif_nss_ol_vdev_handle_monitor_mode)(struct net_device *netdev,
                                struct sk_buff *skb, uint8_t is_chain);
    void (* ic_osif_nss_ol_vdev_tx_inspect_handler)(struct cdp_soc_t *cdp_soc, uint8_t vdev_id, struct sk_buff *skb);
    ...
    void *(* ic_osif_nss_ol_peer_find_hash_find)(struct ieee80211vap *vap,
                                uint8_t *peer_mac_addr, int mac_addr_is_aligned);
    void (* ic_osif_nss_rsim_rx_decap)(os_if_t osif, qdf_nbuf_t *pdeliver_list_head, qdf_nbuf_t *pdeliver_list_tail, void *peer);
};
