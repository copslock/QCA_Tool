 ...
 */
void osif_nss_wifili_vdev_tx_inspect_handler(struct cdp_soc_t *soc, uint8_t vdev_id, struct sk_buff *skb)
{
    struct dp_peer *peer;
    ...
    vdev = dp_get_vdev_from_soc_vdev_id_wifi3((struct dp_soc *)soc, vdev_id);
...
bool osif_nss_ol_li_vdev_spl_receive_ext_wdsdata(struct net_device *dev, struct sk_buff *nbuf, struct nss_wifi_vdev_wds_per_packet_metadata *wds_metadata)
{
    ...
    vdev = dp_get_vdev_from_soc_vdev_id_wifi3(soc, wlan_vdev_get_id(osdev->ctrl_vdev));
...
bool osif_nss_ol_li_vdev_spl_receive_ppdu_metadata(struct net_device *dev, struct sk_buff *nbuf, struct nss_wifi_vdev_ppdu_metadata *ppdu_mdata)
{
    ...
    av_sc = (OL_ATH_VAP_NET80211(osdev->os_if))->av_sc;
...
void osif_nss_wifili_fill_mesh_stats(struct ieee80211vap *vap, qdf_nbuf_t nbuf, struct nss_wifi_vdev_meshmode_rx_metadata *mesh_metadata)
{
    ...
    if (pdev == NULL) {
        QDF_TRACE(QDF_MODULE_ID_DP, QDF_TRACE_LEVEL_ERROR,
                "Invalid pdev dp handle! Returning.");
        return;
    }

    /* Finding the current operating band */

    rx_info->rs_band = WLAN_BAND_UNSPECIFIED;
    if (pdev->soc && pdev->soc->cdp_soc.ol_ops->freq_to_band) {
        rx_info->rs_band = pdev->soc->cdp_soc.ol_ops->freq_to_band(pdev->soc->ctrl_psoc,
                                                            pdev->pdev_id,
...
void osif_nss_wifili_vdevcfg_set_offload_params(struct cdp_soc_t *soc, uint8_t vdev_id, struct nss_wifi_vdev_config_msg **p_wifivdevcfg)
{
    struct nss_wifi_vdev_config_msg *wifivdevcfg = *p_wifivdevcfg;
    struct dp_vdev *vdev_handle = dp_get_vdev_from_soc_vdev_id_wifi3((struct dp_soc *)soc, vdev_id);
...
{
    return dp_get_vdev_from_soc_vdev_id_wifi3((struct dp_soc *)soc, vdev_id);
}
...
void
osif_nss_wifili_rsim_rx_decap(os_if_t osif, qdf_nbuf_t *pdeliver_list_head, qdf_nbuf_t *pdeliver_list_tail, void *peer)
{
#if QCA_SUPPORT_RAWMODE_PKT_SIMULATION
    osif_rsim_rx_decap(osif, pdeliver_list_head, pdeliver_list_tail, ((struct dp_peer *)peer)->mac_addr.raw);
#endif
}
...
void * osif_nss_wifili_find_peer_by_id(osif_dev *osifp, uint32_t peer_id)
{
    ...
    peer = dp_peer_find_by_id(osif_nss_ol_li_dp_soc_from_vap(osifp->os_if), peer_id);
...
uint8_t osif_nss_wifili_find_pstosif_by_id(struct net_device *netdev, uint32_t peer_id, osif_dev **psta_osifp)
{
    ...
    pstapeer = dp_peer_find_by_id(osif_nss_ol_li_dp_soc_from_vap(osifp->os_if), peer_id);
    if (!pstapeer) {
...
uint32_t osif_nss_wifili_vdev_set_cfg(osif_dev *osifp, enum osif_nss_vdev_cmd osif_cmd)
{
    ...
    vdev = dp_get_vdev_from_soc_vdev_id_wifi3(osif_nss_ol_li_dp_soc_from_vap(osifp->os_if),
...
uint8_t osif_nss_wifili_vdev_get_stats(osif_dev *osifp, struct nss_cmn_msg *wifivdevmsg)
{
    ...
    soc = osif_nss_ol_li_dp_soc_from_scn(av_sc);
    pdev = dp_get_pdev_from_soc_pdev_id_wifi3(soc, pdev_id);
    vdev = dp_get_vdev_from_soc_vdev_id_wifi3(soc, wlan_vdev_get_id(osifp->ctrl_vdev));
...
uint8_t osif_nss_ol_li_vdev_qwrap_mec_check(osif_dev *mpsta_osifp, struct sk_buff *skb)
{
    ...
    vdev = dp_get_vdev_from_soc_vdev_id_wifi3(
                        osif_nss_ol_li_dp_soc_from_vap(psta_osdev->os_if),
                        wlan_vdev_get_id(psta_osdev->ctrl_vdev));
