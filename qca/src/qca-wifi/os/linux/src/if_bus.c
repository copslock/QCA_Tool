...
qdf_export_symbol(bus_map_single);
qdf_export_symbol(bus_unmap_single);
qdf_export_symbol(bus_dma_sync_single);
...
int bus_device_suspend(osdev_t osdev)
{
    ...
    if (bustype == HAL_BUS_TYPE_PCI) {
      ...
      qdf_nofl_info("Not supported on this bus\n");
      return 1;
    }
}
...
qdf_export_symbol(bus_device_suspend);
qdf_export_symbol(bus_device_resume);
#endif /* ATH_BUS_PM */

#if ATH_PERF_PWR_OFFLOAD
qdf_export_symbol(bus_alloc_consistent);
qdf_export_symbol(bus_free_consistent);
qdf_export_symbol(bus_alloc_nonconsistent);
qdf_export_symbol(bus_free_nonconsistent);
