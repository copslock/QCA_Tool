        ...
        qdf_nbuf_t nbuf, __attribute__((unused)) struct napi_struct *napi)
{
    ...
    if (ic->nss_iops && vap->iv_rawmode_pkt_sim) {
        ic->nss_iops->ic_osif_nss_rsim_rx_decap((os_if_t)osdev, &deliver_list_head, &deliver_list_tail, peer);
    }
#endif
...
osif_nss_vdev_data_receive(struct net_device *dev, struct sk_buff *skb, __attribute__((unused)) struct napi_struct *napi)
{
    ...
    if (!(ic->nss_iops && ic->nss_iops->ic_osif_nss_ol_get_vdev_from_vdev_id &&
        ic->nss_iops->ic_osif_nss_ol_get_vdev_from_vdev_id(soc, vdev_id))) {
        qdf_nbuf_free(skb);
        return;
    }
    osif = vap->iv_ifp;
    ...
    if (vap->iv_rx_decap_type == htt_cmn_pkt_type_raw) {
        ...
        if (ic->nss_iops) {
            peer = ic->nss_iops->ic_osif_nss_ol_peer_find_hash_find(vap, &pkt_taddr[0], 0);
        }
        ...
        if (vap->iv_rawmode_pkt_sim) {
            ic->nss_iops->ic_osif_nss_rsim_rx_decap(osif, &skb_list_head, &skb_list_tail, peer);
        }
#endif
...
int32_t osif_nss_ol_vap_create(struct ieee80211vap *vap, osif_dev *os_dev, uint32_t if_num)
{
...
qdf_export_symbol(osif_nss_vdev_me_delete_deny_list);
...
qdf_export_symbol(osif_nss_vdev_me_dump_denylist);
