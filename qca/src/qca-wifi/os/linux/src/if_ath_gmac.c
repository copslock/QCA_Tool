...
{
    qdf_nofl_info("skb->len = %d\n",skb->len);
    qdf_nofl_info("skb->dev = %#x\n",(uint32_t)skb->dev);
...
__dbg_print_ath(athhdr_t  *ath)
{
    ...
    qdf_nofl_info("ath[0] = %#x\n",data[0]);
    qdf_nofl_info("ath[1] = %#x\n",data[1]);
    qdf_nofl_info("ath[2] = %#x\n",data[2]);
    qdf_nofl_info("ath[3] = %#x\n",data[3]);

    qdf_nofl_info("ath->type.proto = %#x\n",ath->type.proto);
}
...
__gmac_create_wlan_dev(void)
{
    ...
    if (dev == NULL) {
        qdf_nofl_info(KERN_ERR "ath_gmac: no memory for device state\n");
        goto done;
    }
...
__gmac_attach(void *  arg)
{
    ...
    if(__ath_attach(DEVID_MAGPIE_MERLIN, dev, &bus_context, &sc->aps_osdev))
        qdf_nofl_info(KERN_ERR "%s: __ath_attach fail!\n", __func__);
}
...
os_gmac_init(void)
{
    ...
    if (ath_create_htc_thread(NULL) != 0)
        qdf_nofl_info(KERN_ERR "%s: create thread fail\n", __func__);
