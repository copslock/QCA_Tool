...
int wlan_proxy_arp(wlan_if_t vap, wbuf_t wbuf)
{
    ...
    if (IEEE80211_VAP_IS_DELIVER_80211_ENABLED(vap)) {
        qdf_nofl_info("%s: IEEE80211_FEXT_DELIVER_80211 is not supported\n", __func__);
        goto pass;
    }
    if (eh == NULL)
        goto drop;
    ...
    if (ether_type == htons(ETHERTYPE_ARP)) {
        ...
        if (arp->ar_op == htons(ARPOP_REQUEST)) {
            ...
            ni = ieee80211_find_node(nt, sha, WLAN_PROXY_ARP_ID);
            if (IS_MYSTA(vap, ni))
                bridge = 0;
            if (ni)
                ieee80211_free_node(ni, WLAN_PROXY_ARP_ID);

            /* Proxy ARP request for the clients within the BSS */
            ni = ieee80211_find_node_by_ipv4(nt, tip, WLAN_PROXY_ARP_ID);
            if (IS_MYSTA(vap, ni)) {
                    ...
                    ni->ni_ipv4_lease_timeout < jiffies_to_msecs(jiffies) / 1000)
                {
                ...
                } else if (memcmp(ni->ni_macaddr, sha, QDF_MAC_ADDR_SIZE)) {
                    int defense = 0;
                     ...
                     */
                    if (bridge && tip == sip &&
                        !memcmp(tha, eth_zero_addr, QDF_MAC_ADDR_SIZE))
                    {
                        /*
                    ...
                    if (bridge) {
                        ...
                        arp_send(ARPOP_REPLY, ETH_P_ARP, sip, dev, tip, sha,
                                 ni->ni_macaddr, tha);
#endif
                        sni = ieee80211_find_node_by_ipv4(nt, sip, WLAN_PROXY_ARP_ID);
                        if (!sni) {
                            sni = ieee80211_find_node(nt, sha, WLAN_PROXY_ARP_ID);
                            if (IS_MYSTA(vap, sni) &&
                                !memcmp(sha, sni->ni_macaddr, QDF_MAC_ADDR_SIZE)) {
                        ...
                        if (sni)
                            ieee80211_free_node(sni, WLAN_PROXY_ARP_ID);
                    }
                     ...
                     */
                     if (tip == sip && !memcmp(tha, eth_zero_addr, QDF_MAC_ADDR_SIZE) &&
        ...
        if (!IEEE80211_IS_BROADCAST(eh->ether_dhost)) {
            ni = ieee80211_find_node(nt, eh->ether_dhost, WLAN_PROXY_ARP_ID);
            if (!IS_MYNODE(vap, ni))
                goto pass_node;
            if (ni)
                ieee80211_free_node(ni, WLAN_PROXY_ARP_ID);
        ...
        if (ext_len >= 4 && !memcmp(dhcp->exten, ic_bootp_cookie, 4)) {
            ...
            if (mt == DHCPACK) {
                ni = ieee80211_find_node(nt, dhcp->hw_addr, WLAN_PROXY_ARP_ID);
                if (IS_MYSTA(vap, ni)) {
                    if (dhcp->your_ip == 0) {
        ...
        if (ip6->nexthdr == IPPROTO_ICMPV6) {
            ...
            case NDISC_NEIGHBOUR_SOLICITATION:
                ...
                if ((src_type == IPV6_ADDR_ANY) && (dst_type & IPV6_ADDR_MULTICAST)) {
                    ...
                    ni = ieee80211_find_node(nt, eh->ether_shost, WLAN_PROXY_ARP_ID);
                    ni1 = ieee80211_find_node_by_ipv6(nt, (u8 *)&msg->target, WLAN_PROXY_ARP_ID);
                    if (ni1) {
                        /* No interest in STA's own DAD transmit */
                        if (ni1 == ni) {
                            ieee80211_free_node(ni1, WLAN_PROXY_ARP_ID);
                            goto drop_node;
                        }
                    ...
                    if (IS_MYNODE(vap, ni)) {
                        ...
                        if (ieee80211_node_add_ipv6(nt, ni, (u8 *)&msg->target)) {
                            qdf_nofl_info("Maximum multiple IPv6 addresses exceeded "
...
send_na:
                ni = ieee80211_find_node_by_ipv6(nt, (u8 *)&msg->target, WLAN_PROXY_ARP_ID);
                if (IS_MYSTA(vap, ni)) {
                    struct sk_buff *skb;
                    struct ipv6hdr *nip6;
                    struct nd_msg *nmsg;
		    ...
		    if (is_dad_transmit) {
                    ...
                    nmsg->opt[1] = 1;
                    memcpy(&nmsg->opt[2], ni->ni_macaddr, QDF_MAC_ADDR_SIZE);
			...
			nbuf_debug_del_record(skb);
                        ...
                        ni1 = ieee80211_find_node(nt, lladdr, WLAN_PROXY_ARP_ID);
                        if (IS_MYSTA(vap, ni1)) {
                            ieee80211_free_node(ni1, WLAN_PROXY_ARP_ID);
                            /* Send it to the STA */
#ifdef HOST_OFFLOAD
            ...
            case NDISC_NEIGHBOUR_ADVERTISEMENT:
                ...
                ni = ieee80211_find_node_by_ipv6(nt, (u8 *)&msg->target, WLAN_PROXY_ARP_ID);
                if (ni && memcmp(ni->ni_macaddr, lladdr, QDF_MAC_ADDR_SIZE)) {
                    /*
                ...
                if (unsolicited && !msg->icmph.icmp6_override)
                    goto drop;
#if 0
                ni = ieee80211_find_node(nt, lladdr, WLAN_PROXY_ARP_ID);
                if (!IS_MYNODE(vap, ni)) {
                    /*
