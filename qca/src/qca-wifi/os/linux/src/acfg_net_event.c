...
static void acfg_event_work_process(void *arg)
{
    ...
    if (acfg_nl == NULL || acfg_nl->acfg_sock == NULL) {
       qdf_nofl_info("%s: acfg_nl (or) acfg_sock is NULL !!\n", __func__);
       return;
    }
#endif
    ...
    if (NULL == net){
        qdf_nofl_info("%s: mem alloc failed for net\n",__FUNCTION__);
        return;
    }

    nlh = (struct nlmsghdr *)qdf_mem_malloc_atomic(sizeof(struct nlmsghdr));
    if (NULL == nlh){
        qdf_nofl_info("%s: mem alloc failed for nlh\n",__FUNCTION__);
        qdf_mem_free(net);
        return;
    }
                    ...
                    u_int32_t event_type, int send_iwevent, u_int32_t pid)
{
    ...
    if(!osdev->acfg_netlink_wq_init_done){
        qdf_nofl_info("Work queue not ready, drop acfg event!\n");
        return QDF_STATUS_E_FAILURE;
    }
    ...
    if(event_len > NLMSG_DEFAULT_SIZE){
        qdf_nofl_info("%s: Event length bigger than NLMSG_DEFAULT_SIZE(%d)\n",__FUNCTION__,(int)NLMSG_DEFAULT_SIZE);
        kfree_skb(skb);
        return QDF_STATUS_E_FAILURE ;
    }
...
#endif
		if (acfg_ev_sock == NULL) {
			qdf_nofl_info("%s: netlink create failed\n", __func__);
			return QDF_STATUS_E_FAILURE;
		}
	}
	return QDF_STATUS_SUCCESS;
...
void osif_radio_evt_radar_detected_ap(void *event_arg, struct ieee80211com *ic)
{
    ...
    if (nolinfo == NULL){
        qdf_nofl_info("%s: mem alloc failed for nolinfo\n",__FUNCTION__);
    ...
    {
        acfg_event->freqs[count] = wlan_reg_freq_to_chan(pdev, nolinfo->dfs_nol[i].nol_freq);
        count++;
    }
