/*
 * Copyright (c) 2016-2020 Qualcomm Innovation Center, Inc.
 * All Rights Reserved.
        ...
        struct cfg80211_csa_settings *csa_params)
{
    ...
    /*
     * If current frequency is same as existing frequency retrun
     */

    if (freq == ath_channel->ic_freq) {
        qdf_info("%s: Channel freq:%d Configured already. ", __func__, freq);
        return 0;
    }
    ...
    if (tbtt > 0) {
        ieee80211_ucfg_set_chanswitch(vap, freq, tbtt, ch_width);
    } else {
        /* if tbtt is ZERO then do channel change immediately */
        ieee80211_ucfg_set_freq(vap, freq);
    }
    return 0;
}
        ...
        struct cfg80211_ap_settings *params)
{
    ...
    /* In MBSS case, we start transmitting VAP first followed by non-transmitting VAPs */
    if (wlan_pdev_nif_feat_cap_get(ic->ic_pdev_obj, WLAN_PDEV_F_MBSS_IE_ENABLE) &&
        ...
        struct cfg80211_connect_params *req)
{
    ...
    u_int8_t des_bssid[QDF_MAC_ADDR_SIZE] = {0x0,0x0,0x0,0x0,0x0,0x0};
    u_int8_t skip_bssid_set = 0;
    u_int8_t ret_val = 0;
    u_int8_t *rsn_ie = NULL;
    u_int8_t *wpa_ie = NULL;
    u_int8_t *ptr = (u_int8_t *)req->ie;
...
#define SEC_80_4 14
    ...
    if (primary_channel){
        ...
        if ((mode == IEEE80211_MODE_11AC_VHT160) || (mode == IEEE80211_MODE_11AXA_HE160)) {
        ...
        if ((mode == IEEE80211_MODE_11AC_VHT80_80) || (mode == IEEE80211_MODE_11AXA_HE80_80)) {
                             ...
                             bool *chan_6ghz_found)
{
    ...
    *ccount = chanlist->ic_nchans;
    ...
    for (i = 0; i < chanlist->ic_nchans; i++) {
        ...
        if (IEEE80211_IS_CHAN_11AC_VHT160(icv) ||
                        IEEE80211_IS_CHAN_11AXA_HE160(icv)) {
            ...
            if (NULL == chanlist_160) {
                ...
                if (wlan_get_chaninfo(vap, 1, chanlist_160->ic_chans,
                    ...
                    uint8_t reason)
{
    ...
    if (is_phymode_2G(phy_mode)) {
        ...
        band = WLAN_BAND_2GHZ;
    } else if (is_phymode_5G_or_6G(phy_mode)) {
        /*
...
static const struct
nla_policy qca_wlan_vendor_attr[QCA_WLAN_VENDOR_ATTR_VLAN_PARAM_MAX + 1 ] = {
        [QCA_WLAN_VENDOR_ATTR_STA_VLAN]       = {.type = NLA_U16},
        [QCA_WLAN_VENDOR_ATTR_VLAN_MAC_ADDR]  = {.type = NLA_BINARY,
                                                 .len = QDF_MAC_ADDR_SIZE},
};

static int wlan_cfg80211_set_sta_vlan_cb(struct wiphy *wiphy,
        struct wireless_dev *wdev,
        const void *data,
        int data_len)
{
    struct nlattr *attr[QCA_WLAN_VENDOR_ATTR_VLAN_PARAM_MAX + 1];
    struct cfg80211_context *cfg_ctx = NULL;
    struct ieee80211com *ic = NULL;
    struct ieee80211_node *ni;
    wlan_if_t vap = NULL;
    ...
    if (wlan_cfg80211_nla_parse(attr, QCA_WLAN_VENDOR_ATTR_VLAN_PARAM_MAX,
                                data, data_len,
    ...
    if(attr[QCA_WLAN_VENDOR_ATTR_VLAN_MAC_ADDR]) {
        mac_addr = nla_data(
                       attr[QCA_WLAN_VENDOR_ATTR_VLAN_MAC_ADDR]);
    }

    /* Extracting TID */
    if(attr[QCA_WLAN_VENDOR_ATTR_STA_VLAN]) {
        vlan_id = nla_get_u16(
                  attr[QCA_WLAN_VENDOR_ATTR_STA_VLAN]);
    }

    ni = ieee80211_find_node(&vap->iv_ic->ic_sta, mac_addr, WLAN_MLME_SB_ID);
    if (ni) {
        ni->vlan_id = vlan_id;
        ieee80211_free_node(ni, WLAN_MLME_SB_ID);
    }
...
static const struct
nla_policy qca_wlan_vendor_vlan_attr[QCA_WLAN_VENDOR_ATTR_VLAN_SETKEY_PARAM_MAX+ 1 ] = {
        [QCA_WLAN_VENDOR_ATTR_VLAN_SETKEY_KEYIX]       = {.type = NLA_U32},
        [QCA_WLAN_VENDOR_ATTR_VLAN_SETKEY_VLAN_ID]     = {.type = NLA_U16},
        [QCA_WLAN_VENDOR_ATTR_VLAN_SETKEY_MAC_ADDR]  = {.type = NLA_BINARY,
                                                 .len = QDF_MAC_ADDR_SIZE},
        [QCA_WLAN_VENDOR_ATTR_VLAN_SETKEY_KEYDATA]  = {.type = NLA_BINARY,
                                                 .len = 32},
};

static int wlan_cfg80211_vlan_set_key_cb(struct wiphy *wiphy,
                                             struct wireless_dev *wdev,
                                             const void *data, int data_len)
{
    struct cfg80211_context *cfg_ctx = NULL;
    ...
    struct nlattr *attr[QCA_WLAN_VENDOR_ATTR_VLAN_SETKEY_PARAM_MAX + 1];
    uint8_t *mac_addr = NULL;
    uint32_t vlan_id=0;
    uint8_t key_len=0;
    uint8_t key_ix=0;
    uint8_t *key = NULL;
    int cipher = IEEE80211_CIPHER_NONE;
    ...
    if (wlan_cfg80211_nla_parse(attr,
                                ...
                                data,
                                data_len,
                                qca_wlan_vendor_vlan_attr)) {
        return -EINVAL;
    }
    if (attr[QCA_WLAN_VENDOR_ATTR_VLAN_SETKEY_KEYDATA]) {
        key = nla_data(
                attr[QCA_WLAN_VENDOR_ATTR_VLAN_SETKEY_KEYDATA]);
        key_len = nla_len(attr[QCA_WLAN_VENDOR_ATTR_VLAN_SETKEY_KEYDATA]);
    }

    if(attr[QCA_WLAN_VENDOR_ATTR_VLAN_SETKEY_MAC_ADDR]) {
        mac_addr = nla_data(
                       attr[QCA_WLAN_VENDOR_ATTR_VLAN_SETKEY_MAC_ADDR]);
    }

    if(attr[QCA_WLAN_VENDOR_ATTR_VLAN_SETKEY_VLAN_ID]) {
        vlan_id = nla_get_u16(
                  attr[QCA_WLAN_VENDOR_ATTR_VLAN_SETKEY_VLAN_ID]);
    }

    if(attr[QCA_WLAN_VENDOR_ATTR_VLAN_SETKEY_KEYIX]) {
        key_ix = nla_get_u32(
                  attr[QCA_WLAN_VENDOR_ATTR_VLAN_SETKEY_KEYIX]);
    }

    qdf_mem_zero(&key_val, sizeof(ieee80211_keyval));
    key_ix= add_vlan(vap,vlan_id,key_ix);
    ieee80211_ucfg_getparam(vap, IEEE80211_PARAM_MCASTCIPHER , &cipher);
    key_val.keytype = cipher;
    key_val.macaddr = (u_int8_t *) mac_addr;
    key_val.keydir = IEEE80211_KEY_DIR_BOTH;
    key_val.keyindex = key_ix;
    key_val.keyrsc  = 0;
    key_val.keytsc  = 0;
    key_val.keylen = key_len ;
    key_val.rxmic_offset = IEEE80211_KEYBUF_SIZE + 8;
    key_val.txmic_offset =  IEEE80211_KEYBUF_SIZE;
    qdf_mem_copy(keydata, key, key_len);
    ...
    wlan_set_key(vap,key_val.keyindex,&key_val);
...
#define HE_SRG_BITMAP_STRING    20
    ...
    struct ieee80211com *ic = NULL;
    struct ol_ath_softc_net80211 *scn = NULL;
    uint32_t srg_field = params->value;
    int cmd_type ,ret = 0;
    void *cmd;
        ...
        struct wlan_cfg8011_genric_params *params)
{
    ...
    if (ic->ic_cfg80211_radio_handler.set_he_sr_config) {
        ret = ic->ic_cfg80211_radio_handler.set_he_sr_config(
                                (void *)scn, param, value,
    ...
    if (ic->ic_cfg80211_radio_handler.get_he_sr_config) {
        ...
        switch (params->value) {
            ...
            case HE_SR_NON_SRG_OBSSPD_ENABLE:
                ...
                cfg80211_reply_command(wiphy, strlen(string), string, 0);
            break;
...
qdf_export_symbol(wlan_cfg80211_httstats_event);
...
const struct wiphy_vendor_command wlan_cfg80211_vendor_commands[] = {
    ...
    {
        .info.vendor_id = QCA_NL80211_VENDOR_ID,
        .info.subcmd = QCA_NL80211_VENDOR_SUBCMD_VLAN,
        .flags = WIPHY_VENDOR_CMD_NEED_WDEV |
            WIPHY_VENDOR_CMD_NEED_NETDEV |
            WIPHY_VENDOR_CMD_NEED_RUNNING,
        .doit = wlan_cfg80211_set_sta_vlan_cb
    },

    {
            .info.vendor_id = QCA_NL80211_VENDOR_ID,
            .info.subcmd = QCA_NL80211_VENDOR_SUBCMD_KEY_VLAN_SET_KEY,
            .flags = WIPHY_VENDOR_CMD_NEED_WDEV |
                    WIPHY_VENDOR_CMD_NEED_NETDEV |
                    WIPHY_VENDOR_CMD_NEED_RUNNING,
            .doit = wlan_cfg80211_vlan_set_key_cb
    },
...
/**
 * wlan_cfg80211_get_num_6g_channels() - find number of 6 GHz channels
 * @ic: ieee80211com object
 * @phymode: ieee80211_phymode for which channels are to be checked
 *
 * API, Find the number of 6 GHz channels available on underlying pdev
 * in specified phy mode.
 *
 * Return: number of 6 GHz channels for @ic and @phymode combination
 */

int
wlan_cfg80211_get_num_6g_channels(struct ieee80211com *ic, enum ieee80211_phymode phymode)
...
static inline void
wlan_copy_ieee80211_channel_to_ath_channel(
        struct ieee80211_channel *ieee_chan,
        struct ieee80211_ath_channel *ath_chan,
        enum ieee80211_band phy_band)
{
    ieee_chan->band =  phy_band;
    ieee_chan->center_freq = ath_chan->ic_freq;
    ieee_chan->hw_value = ath_chan->ic_ieee;
    ieee_chan->flags = 0;
    ieee_chan->max_antenna_gain = ath_chan->ic_antennamax;
    ieee_chan->max_power = ath_chan->ic_maxregpower;
}
 ...
 *   @channel_list: pointer to channel list which needs to be filled.
        ...
        uint32_t max_channels)
{
    ...
    for (i = 0; i < ic->ic_nchans && nchan < max_channels; i++) {
        chan = &ic->ic_channels[i];
        if (phymode == ieee80211_chan2mode(chan)) {
            wlan_copy_ieee80211_channel_to_ath_channel(channel_list, chan, phy_band);
            channel_list++;
            nchan++;
        }
    }
...
/**
 *   wlan_cfg80211_populate_6g_channels: This function populates 6 GHz channels
 *   for given phymode and phyband.
 *
 *   @ic: pointer to ieee80211com
 *   @phymode: ieee80211_phymode
 *   @phy_band: underlying band
 *   @channel_list: pointer to channel list which needs to be filled.
 *   @max_channels: maximum channels allowed to be added to @channel_list
 *
 *   Return - number of channels populated
 */
int wlan_cfg80211_populate_6g_channels(struct ieee80211com *ic, enum ieee80211_phymode phymode,
        enum ieee80211_band phy_band,
        struct ieee80211_channel *channel_list,
        uint32_t max_channels)
{
        ...
        uint32_t max_channels)
{
    ...
    band->n_channels = wlan_cfg80211_populate_channels(ic,
            IEEE80211_MODE_11B, IEEE80211_BAND_2GHZ, channel_list, max_channels);
        ...
        uint32_t max_channels)
{
    ...
    band->channels = channel_list;
    band->n_channels = wlan_cfg80211_populate_channels(ic,
            IEEE80211_MODE_11A, IEEE80211_BAND_5GHZ, channel_list, max_channels);
    if (band->n_channels)
     ...
     * support 6GHz
     */
    band->n_channels += wlan_cfg80211_populate_6g_channels(ic, IEEE80211_MODE_11AXA_HE20,
            IEEE80211_BAND_5GHZ, (channel_list + band->n_channels),
 ...
 *   data struccture, ic contains list of channel that are supported for that radio.
 *   populate channels, bitrates, HT, VHT and HE caps for this radio.
 *   @ic: pointer to ieee80211com.
 *   @band: pointer to ieee80211_supported_band.
 *   @channel_list: pointer to channel list wihch needs to be filled.
 *   @max_channels: maximum channels allowed to be added to @channel_list
 ...
 *
 *  Note: 6GHz does not require HT/VHT but cfg80211 kernel subsystem is expecting ht,
 *  vht caps when setting channel and bandwidth (cfg80211_chandef_usable). due to this
 *  limitation HT & VHT caps are filled during registration.
 *
 */
        ...
        uint32_t max_channels)
{
    ...
    /* Fill band data struture */
    band->channels = channel_list;
    band->n_channels = wlan_cfg80211_populate_6g_channels(ic,
            IEEE80211_MODE_11AXA_HE20,
            IEEE80211_BAND_6GHZ, channel_list,
            max_channels);
    ...
    band->ht_cap.ampdu_factor = ic->ic_maxampdu;
    band->ht_cap.ampdu_density = ic->ic_mpdudensity;
    for (i = 0; i < ic->ic_num_rx_chain; i++) {
        band->ht_cap.mcs.rx_mask[i] = 0xFF;
    }
    band->ht_cap.mcs.tx_params = IEEE80211_HT_MCS_TX_DEFINED;
    ...
    mcs_map = 0xffff;
    for (i = 0; i < ic->ic_num_tx_chain; i++)
        mcs_map |= IEEE80211_VHT_MCS_SUPPORT_0_9 << (i*2);
    band->vht_cap.vht_mcs.tx_mcs_map = cpu_to_le16(mcs_map);
...
int wlan_cfg80211_update_channel_list(struct ieee80211com *ic)
{
    ...
    num_2g_channels = wlan_cfg80211_get_num_channels(ic, IEEE80211_MODE_11B);
    ...
    num_5g_channels = wlan_cfg80211_get_num_channels(ic, IEEE80211_MODE_11A);
    num_6g_channels = wlan_cfg80211_get_num_6g_channels(ic, IEEE80211_MODE_11AXA_HE20);
#if CFG80211_SUPPORT_11AX_HOSTAPD
...
int wlan_cfg80211_init(struct net_device *dev, struct wiphy *wiphy, struct ieee80211com *ic)
{
   ...
   wiphy->max_remain_on_channel_duration = IEEE80211_MAX_REMAIN_ON_CHANNEL;
    ...
    /* Band and Channel information */
    num_channels = wlan_cfg80211_get_num_channels(ic, IEEE80211_MODE_11B);
    if (num_channels) {
                  ...
                  struct vendor_chan_info *channel_list)
{
    ...
    switch (reason) {
        ...
        case QCA_WLAN_VENDOR_ACS_SELECT_REASON_CW_INTERFERENCE:
        {
            ...
            if (IEEE80211_MODE_AUTO == act_phymode) {
...
int wlan_cfg80211_get_channel_survey_stats(struct ieee80211com *ic)
{
    ...
    msglen = sizeof(ic->ic_channel_stats.home_chan_stats) +
             sizeof(ic->ic_channel_stats.scan_chan_stats);
    reply_skb = wlan_cfg80211_vendor_cmd_alloc_reply_skb(ic->ic_wiphy,
