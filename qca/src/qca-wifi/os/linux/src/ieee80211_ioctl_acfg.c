/*
 * Copyright (c) 2014-2015,2017-2020 Qualcomm Innovation Center, Inc.
 * All Rights Reserved
...
unsigned int netlink_acfg = NETLINK_ACFG;
module_param(netlink_acfg, uint, S_IRUSR | S_IWUSR | S_IRGRP | S_IROTH);
                                    ...
                                    acfg_radio_vap_info_t *acfg_profile)
{
	...
	acfg_profile->chan_band = reg_wifi_band_to_wlan_band_id(
            wlan_reg_freq_to_band(profile->freq));
                ...
                uint8_t *buffer, int32_t Length)
{
    ...
    u_int16_t vhtrates;
    u_int8_t stamac[QDF_MAC_ADDR_SIZE];
    uint16_t aid;
    uint8_t qos;
    int i;
                    ...
                    uint8_t *buffer, int32_t Length)
{
    ...
    freq = wlan_get_wlan_band_id_chan_to_freq(ic->ic_pdev_obj, req->data[0], band);
    if (!freq) {
        qdf_err("chan; %d, band: %d, freq: %d\n", req->data[0], band, freq);
        return -EINVAL;
    }
    return ieee80211_ucfg_set_chanswitch(vap, freq, ic->ic_chan_switch_cnt, 0);
}
             ...
             uint8_t *buffer, int32_t Length)
{
    ...
    *ptr = chan->ic_ieee;
    *++ptr = reg_wifi_band_to_wlan_band_id(
            wlan_reg_freq_to_band(chan->ic_freq));
                  ...
                  uint8_t *buffer, int32_t Length)
{
    ...
    return ieee80211_ucfg_setparam(vap, ptr->param, ptr->val, (char *)ptr);
}
                  ...
                  uint8_t *buffer, int32_t Length)
{
    ...
    retv = ieee80211_ucfg_getparam(vap, ptr->param, &value);
               ...
               uint8_t *buffer, int32_t Length)
{
    ...
    rc = wlan_get_acl_list(vap, macList, (QDF_MAC_ADDR_SIZE * 256), &num_mac, IEEE80211_ACL_FLAG_ACL_LIST_1);
	if(rc) {
    	if (macList) {
        	OS_FREE(macList);
        }
	...
	for (i = 0; i < num_mac; i++) {
    	memcpy(ptr->macaddr[i], &macList[i * QDF_MAC_ADDR_SIZE],
												QDF_MAC_ADDR_SIZE);
    }
    ptr->num = num_mac;
	if (macList) {
               ...
               uint8_t *buffer, int32_t length)
{
    ...
    for (i = 0; i < num_mac; i++) {
        memcpy(ptr->macaddr[i], &mac_list[i * QDF_MAC_ADDR_SIZE], QDF_MAC_ADDR_SIZE);
    }
    ptr->num = num_mac;
    OS_FREE(mac_list);
    return rc;
             ...
             acfg_event_type_t type, acfg_event_data_t *acfg_event)
{
    ...
    acfg_hw_mode_change_status_t *hw_mode_change;
	...
	case WL_EVENT_TYPE_DISASSOC_AP:
        ...
        case WL_EVENT_TYPE_SMART_LOG_FW_PKTLOG_STOP:
            ...
            break;
#endif /* defined(OL_ATH_SMART_LOGGING) && !defined(REMOVE_PKT_LOG) */
        case WL_EVENT_TYPE_HW_MODE_CHANGE_STATUS:
            event->id = ACFG_EV_HW_MODE_CHANGE_STATUS;
            hw_mode_change = (void *) &event->data;
            hw_mode_change->result = acfg_event->result;
            break;
        default:
            qdf_err("Unknown ACFG event");
            return;
    }
...
int acfg_frame_tx_offload(wlan_if_t vaphandle, void *frame)
{
    ...
    ni = ieee80211_try_ref_node(vap->iv_bss, WLAN_MLME_SB_ID);
    if (ni) {
...
int acfg_gpr_rate_check(struct ieee80211vap *vap, struct acfg_offchan_tx_hdr *acfg_frame)
{
    ...
    if( IEEE80211_IS_CHAN_5GHZ_6GHZ(chan) && acfg_frame->preamble == 1 ){
        IEEE80211_DPRINTF(vap, IEEE80211_MSG_ANY,
...
static
int acfg_offchan_tx_offload(wlan_if_t vaphandle, u_int32_t chan_freq,
                            u_int16_t scan_dur, u_int16_t freq,
                            ...
                            u_int32_t offchan_rx)
{
    ...
    req.wide_scan.chan_freq = chan_freq;
    ...
    if (ucfg_offchan_tx_request(vap->vdev_obj, &req) != QDF_STATUS_SUCCESS) {
        /* Append the list back to offchan_list so that the buffers in the list
           gets freed in the caller API */
        qdf_spin_lock_bh(&acfg_nl->offchan_lock);
        qdf_nbuf_queue_append(&acfg_nl->offchan_list, &req.offchan_tx_list);
        qdf_spin_unlock_bh(&acfg_nl->offchan_lock);
        return -EINVAL;
    }
...
void acfg_process_msg(wbuf_t msg)
{
    ...
    /* freq is not expected for ACFG_PKT_TYPE_GPR.
     * Need to handle messages based on msg_type instead of
     * handling in single function to avoid such checks.
     */
    if ((acfg_hdr.msg_type != ACFG_PKT_TYPE_GPR) &&
        ...
        freq = wlan_get_wlan_band_id_chan_to_freq(ic->ic_pdev_obj,
                                                  acfg_hdr.channel,
                                                  acfg_hdr.channel_band);
        if (!freq) {
            qdf_err("Invalid chan: %d, band: %d, freq: %d\n",
                    acfg_hdr.channel, acfg_hdr.channel_band, freq);
            goto bad;
        }
    }
    acfg_resp = &acfg_nl->acfg_resp;
    ...
    acfg_resp->hdr.channel_band = acfg_hdr.channel_band;
    ...
    chan = wlan_get_current_channel(vap, true);
    if ((freq == chan->ic_freq) && (acfg_hdr.msg_length > 0)) {
        /* Home channel */
        acfg_nl->home_chan = 1;
        ...
        if ((chan = ieee80211_find_dot11_channel(ic, freq, 0, IEEE80211_MODE_AUTO)) == NULL) {
            IEEE80211_DPRINTF(vap, IEEE80211_MSG_ANY,
...
qdf_export_symbol(acfg_offchan_cancel);
            ...
            for (scn_idx = 0; scn_idx < WLAN_UMAC_MAX_PDEVS; scn_idx++) {
                ...
                if (ic_tmp && ic_tmp->ic_acfg_handle != NULL) {
                    ...
                    if (acfg_nl_tmp->acfg_sock != NULL) {
                        ...
                        qdf_nofl_info("%s: Offload using existing sock %pK\n", __func__, acfg_nl->acfg_sock);
                          ...
                          acfg_diag_stainfo_t *prev_stainfo)
{
    ...
    prev_stainfo->diag.channel_band = curr_stainfo->diag.channel_band;
    ...
    if (rinfo.rate && rinfo.mcs) {
        ...
        if(sta_info) {
            ...
            if (chan != IEEE80211_CHAN_ANYC) {
                ...
                sta_info->diag.channel_band = reg_wifi_band_to_wlan_band_id(
                        wlan_reg_freq_to_band(chan->ic_freq));
            }
