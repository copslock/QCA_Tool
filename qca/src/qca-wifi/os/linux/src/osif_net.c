...
__qdf_net_start_tx(struct sk_buff  *skb, struct net_device  *netdev)
{
    ...
    if(unlikely((skb->len <= ETH_HLEN))) {
        qdf_nofl_info("QDF_NET:Bad skb len %d\n", skb->len);
        qdf_nbuf_free(skb);
        return NETDEV_TX_OK;
    }
...
qdf_export_symbol(__qdf_net_get_vlantag);
...
qdf_export_symbol(__qdf_net_get_vlanvid);
...
{
    qdf_nofl_info(" QDF_NET_IOCTL_SUPPORT is not On \n");
...
qdf_export_symbol(__qdf_net_create_wifidev);
                        ...
                        qdf_net_dev_info_t     *info, void          *vap_cfg)
{
    ...
    if(!is_valid_ether_addr(info->dev_addr)){
        qdf_nofl_info("QDF_NET:invalid MAC address\n");
        error = EINVAL;
    }
...
qdf_export_symbol(__qdf_net_create_vapdev);
...
qdf_export_symbol(__qdf_net_create_ethdev);
/**
...
qdf_export_symbol(__qdf_net_delete_dev);
...
qdf_export_symbol(__qdf_net_dev_exist_by_name);
...
#if 0
        qdf_nofl_info("addrs : sa : %x : da:%x\n", skb->nh.iph->saddr, skb->nh.iph->daddr);
        qdf_nofl_info("head : %pK tail : %pK iph %pK %pK\n", skb->head, skb->tail,
...
qdf_export_symbol(__qdf_net_indicate_packet);
...
__qdf_net_dev_tx(qdf_net_handle_t hdl, struct sk_buff  *skb)
{
    ...
    if(unlikely(!netdev)){
        qdf_nofl_info("QDF_NET:netdev not found\n");
        qdf_assert(0);
    }
...
qdf_export_symbol(__qdf_net_dev_tx);
...
qdf_export_symbol(__qdf_net_dev_open);
...
qdf_export_symbol(__qdf_net_dev_close);
...
qdf_export_symbol(__qdf_net_ifname);
...
qdf_export_symbol(__qdf_net_fw_mgmt_to_app);
...
qdf_export_symbol(__qdf_net_send_wireless_event);
...
qdf_export_symbol(__qdf_net_indicate_vlanpkt);
...
qdf_export_symbol(__qdf_net_new_wlanunit);
...
qdf_export_symbol(__qdf_net_alloc_wlanunit);
...
qdf_export_symbol(__qdf_net_delete_wlanunit);
...
qdf_export_symbol(__qdf_net_ifc_name2unit);
...
qdf_export_symbol(__qdf_net_free_wlanunit);
...
__sccfg_cb(int cpl, void *dev_id, struct pt_regs *regs, void *push_time)
{
    ...
    for(i = 0 ; i < QDF_MAX_WPS_CALLBACKS ; i++)
    {
            if( scCB[i].dev == (struct net_device *)dev_id )
            {
                ...
                //qdf_nofl_info("%s: Calling PBC callback for %s\n",__FUNCTION__,scCB[i].dev->name);
...
__qdf_net_register_wpscb(qdf_net_handle_t radiodev, void *ctx, qdf_net_wps_cb_t cb)
{
    ...
    if(ret == 0)
    {
        ...
        qdf_nofl_info("%s: Registerd wps callback for %s \n",__FUNCTION__,netdev->name);
    }
    else
    {
        status = QDF_STATUS_E_FAILURE ;
        qdf_nofl_info("%s: Failed to register wps callback for %s \n",__FUNCTION__,netdev->name);
    }

    return status ;
}
qdf_export_symbol(__qdf_net_register_wpscb);
...
__qdf_net_unregister_wpscb(qdf_net_handle_t radiodev)
{
    ...
    for(i = 0 ; i < QDF_MAX_WPS_CALLBACKS ; i++)
    {
        if(scCB[i].dev == netdev)
        {
            ...
            scCB[i].dev = NULL ;
            scCB[i].push_dur = 0 ;
            qdf_nofl_info("%s: Deregister wps callback for %s \n",__FUNCTION__,netdev->name);
            if(ret == 0)
                status = QDF_STATUS_SUCCESS ;
...
qdf_export_symbol(__qdf_net_unregister_wpscb);
...
qdf_export_symbol(__qdf_net_poll_schedule);
...
qdf_export_symbol(__qdf_net_poll_schedule_cpu);
...
qdf_export_symbol(__qdf_net_register_drv);
...
qdf_export_symbol(__qdf_net_unregister_drv);
