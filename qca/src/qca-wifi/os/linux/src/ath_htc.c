...
void ath_null_send_tx_event(void *Context, void *data)
{
    ...
    OS_FREE(data);        
    ni = ieee80211_find_node(nt,addr, WLAN_MLME_SB_ID);
...
{   
    ...
    switch(evt_id) {
        ...
        case WMI_BMISS_EVENTID:
            qdf_nofl_info("WMI_BMISS_EVENTID\n");
            ath_bmiss_event(scn, pBuffer, buf_len);       
            break;
        case WMI_WLAN_TXCOMP_EVENTID:
            /* Event for NULL data and pwrMgt bit = 1 TX Complete */
            ps_null_addr = OS_MALLOC(scn->sc_osdev, QDF_MAC_ADDR_SIZE, GFP_ATOMIC);
            if(ps_null_addr) {
                IEEE80211_ADDR_COPY(ps_null_addr, pBuffer);
                qdf_nofl_info(KERN_DEBUG "WMI_WLAN_TXCOMP_EVENTID: %02x %02x %02x %02x %02x %02x\n",
                ...
                qdf_nofl_info("##dbg [%s] fail to allocate buffer, line %d\n",__func__,__LINE__);
            }
            break;
        ...
        case WMI_GENTIMER_EVENTID:
            {
                ...
                if (gentimerEvt) {
                    //qdf_nofl_info("WMI_GENTIMER_EVENTID\n");
...
host_htc_rdy_handler(osdev_t osdev, u_int32_t TotalCredits)
{
    ...
    if (HTCWaitForHtcRdy(osdev->host_htc_handle) != A_OK) {
        qdf_nofl_info("HTCWaitForHtcRdy failed, init failed!\n");
        return A_ERROR;
    }
    ...
    if (A_FAILED(status)) {
        qdf_nofl_info("wmi connect failed!");
        return A_ERROR;
    }
    ...
    if (A_FAILED(status)) {
        qdf_nofl_info("Failed to connect services!");
        return A_ERROR;
    }
    ...
    if (A_FAILED(status)) {
        qdf_nofl_info("Failed to start HTC!");
        return A_ERROR;
    } 
    return A_OK;
...
void MpHtcAddNodeTarget(osdev_t osdev)
{
    ...
    TAILQ_FOREACH_SAFE(ni, &_nt->nt_node, ni_list, next) {
        qdf_nofl_info(KERN_INFO "%s: %02x %02x %02x %02x %02x %02x\n", __func__,
        ...
        if (i == HTC_MAX_NODE_NUM) {
            qdf_nofl_info("%s: can't find node in the node_bitmap\n", __func__);
            continue;
        }
        ...
        qdf_mem_copy(&nt.ni_macaddr, &(ni->ni_macaddr), (QDF_MAC_ADDR_SIZE * sizeof(u_int8_t)));
        qdf_mem_copy(&nt.ni_bssid, &(ni->ni_bssid), (QDF_MAC_ADDR_SIZE * sizeof(u_int8_t)));
...
#ifdef ATH_USB
qdf_export_symbol(MpHtcDetach);
qdf_export_symbol(MpHtcAttach);
qdf_export_symbol(MpHtcSuspend);
qdf_export_symbol(MpHtcResume);
#endif
