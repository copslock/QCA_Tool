...
//    struct iw_request_info *info = (struct iw_request_info *)vinfo;
    ...
    for ( i = 0; i < QDF_MAC_ADDR_SIZE; i++ ) {
        ...
        if ( i < QDF_MAC_ADDR_SIZE - 1 )
            buf[j++] = ':';
        else
            buf[j++] = '\0';
    }

    wri->length = (QDF_MAC_ADDR_SIZE * 3) - 1;
...
qdf_export_symbol(ath_iw_attach);
...
#define N(a) (sizeof(a)/sizeof(a[0]))
    ...
    if (sc == NULL) {
        qdf_nofl_info("   Hardware queue dump not yet supported on %s.\n", ic->ic_osdev->netdev->name);
        return;
    }

#if 1
    /* HW queues */
    qdf_nofl_info("HW TX Queues, %s:\n", ic->ic_osdev->netdev->name);
    ...
    /* AC queues*/
    qdf_nofl_info("HW AC Queues:\n");
    qdf_nofl_info("   ac-queue mapping: AC_BK(#%d)->txq %d, AC_BE(#%d)->txq %d, ",
        WME_AC_BK, scn->sc_ac2q[WME_AC_BK], WME_AC_BE, scn->sc_ac2q[WME_AC_BE]);
    qdf_nofl_info("AC_VI(#%d)->txq %d, AC_VO(#%d)->txq %d\n",
    ...
    for (i = 0; i < HAL_NUM_TX_QUEUES; i++) {
        ...
        TAILQ_FOREACH(ac, &txq->axq_acq, ac_qelem) {
            ...
            if (tid) {
                ...
                qdf_nofl_info("   ac_q=%d, ac=0x%pK, hw_q=%d, node=%pK, an_flags=0x%x, sched=%d, hwqcnt=%d, ",
                    ac->qnum, ac, i, ni, tid->an->an_flags, ac->sched, ac->hwqcnt);
            } else {
                qdf_nofl_info("   ac_q=%d, ac=0x%pK, hw_q=%d, node=<null>, sched=%d, hwqcnt=%d, ",
                    ac->qnum, ac, i, ac->sched, ac->hwqcnt);
            }
            qdf_nofl_info("filtered=%d, queued tids=%d\n",
                ac->filtered, depth);
            empty = 0;
        }
        ATH_TXQ_UNLOCK(txq);
    }
    if (empty) qdf_nofl_info("   AC queues empty\n");
    qdf_nofl_info("\n");

    /* TID queues overview */
    qdf_nofl_info("HW TID Queues:\n");
    for (i = 0; i < HAL_NUM_TX_QUEUES; i++) {
        txq = &sc->sc_txq[i];
        if (!txq) { continue; }
        ...
        TAILQ_FOREACH(ac, &txq->axq_acq, ac_qelem) {
            TAILQ_FOREACH_SAFE(tid, &ac->tid_q, tid_qelem, next_tid) {
                ...
                TAILQ_FOREACH(bf, &tid->buf_q, bf_list) {
                    ...
                    qdf_nofl_info("      bf: node=%x, fc0=0x%x, fc1=0x%x, status=%08x, flags=%08x, nframes=%d, isaggr=%d, ",
                        bf->bf_node, wh->i_fc[0], wh->i_fc[1], bf->bf_status, bf->bf_flags, bf->bf_nframes,
                        (bf->bf_isaggr ? 1 : 0));
                    qdf_nofl_info("isampdu=%d, data=%d, mc=%d, al=%d, frmlen=%d, retries=%d, seqno=%d\n",
                ...
                qdf_nofl_info("   vap=%d, tidno=%d, tid=0x%pK, node=%pK, an_flags=0x%x, ac_q=%d, ",
...
qdf_export_symbol(wlan_debug_dump_queues_tgt);
#endif
#endif
