...
#define WIFILI_NSS_PEER_BYTE_SIZE 1280
...
static int osif_nss_wifili_wds_extn_peer_cfg_send(struct ol_ath_softc_net80211 *scn, uint8_t *peer_mac, uint8_t vdev_id)
{
    ...
    if (peer->delete_in_progress) {
        QDF_TRACE(QDF_MODULE_ID_NSS, QDF_TRACE_LEVEL_ERROR,"[nss-wifili]: delete in progress for peer %pK : mac %pM", peer, peer_mac);
        goto fail;
    }
                                             ...
                                             uint8_t *dest_mac, uint8_t *next_mac, uint8_t ast_type)
{
    ...
    if (peer_id == HTT_INVALID_PEER) {
        peer = dp_peer_find_hash_find(dpsoc, peer_mac, 0, vdev_id);
    ...
    vdev = dp_get_vdev_from_soc_vdev_id_wifi3(dpsoc, vdev_id);
    if (!vdev) {
        QDF_TRACE(QDF_MODULE_ID_NSS, QDF_TRACE_LEVEL_ERROR,
    ...
    if (next_mac == NULL && ast_type == CDP_TXRX_AST_TYPE_WDS_HM_SEC) {
        ...
        memcpy(wlwdspeermsg->peer_mac, next_mac, 6);
        if (ast_type == CDP_TXRX_AST_TYPE_MEC) {
            ast_type = NSS_PEER_MAP_AST_TYPE_MEC;
	} else {
    ...
    if (ast_type == CDP_TXRX_AST_TYPE_MEC) {
        wlwdspeermsg->peer_id = NSS_WIFILI_MEC_PEER_ID;
    } else {
                                                     ...
                                                     uint8_t pkt_type, uint8_t cipher_type, uint8_t mic_key[])
{
    ...
    if (peer->delete_in_progress) {
       QDF_TRACE(QDF_MODULE_ID_NSS, QDF_TRACE_LEVEL_ERROR,
                    "[nss-wifili]: %s, peer %pK delete is in progress", __func__, peer);
        goto fail;
    }
 ...
 */
static int osif_nss_wifili_peer_set_vlan_id(struct ol_ath_softc_net80211 *scn, uint8_t *peer_mac,
                                                     uint16_t vlan_id)
{
    ...
    soc = scn->soc;
    pdev = (struct cdp_pdev *) dp_get_pdev_from_soc_pdev_id_wifi3(
                                wlan_psoc_get_dp_handle(scn->soc->psoc_obj),
                                wlan_objmgr_pdev_get_pdev_id(scn->sc_pdev));

    if (!pdev) {
       QDF_TRACE(QDF_MODULE_ID_NSS, QDF_TRACE_LEVEL_ERROR,
                 "[nss-wifili]: %s, pdev NULL for id %d", __func__,
                 wlan_objmgr_pdev_get_pdev_id(scn->sc_pdev));
       return QDF_STATUS_E_FAILURE;
    }

    nss_contex = scn->nss_radio.nss_rctx;
    peer = (struct dp_peer *)dp_find_peer_by_addr(pdev, peer_mac);
    ...
    vlan_msg->peer_id = peer->peer_ids[0];
...
static struct nss_wifi_internal_ops nss_wifili_iops = {
    ...
    osif_nss_wifili_get_vdev_from_vdev_id,
 ...
 */
static enum nss_dynamic_interface_type osif_nss_wifili_get_di_type(struct ol_ath_softc_net80211 *scn, uint32_t soc_if_num)
{
...
static QDF_STATUS nss_wifili_pdev_attach(struct ol_ath_softc_net80211 *scn, uint32_t  *radio_id)
{
    ...
    int ret = 0;
    uint32_t rx_desc_pool_weight = 0;
     ...
     */
    scn->nss_radio.nss_rctx = nss_register_wifili_radio_if((uint32_t)radio_if_num, NULL, NULL, NULL, (struct net_device *)pdev, 0);
    if (!scn->nss_radio.nss_rctx) {
        QDF_TRACE(QDF_MODULE_ID_NSS, QDF_TRACE_LEVEL_ERROR,
    ...
    rx_desc_pool_weight = wlan_cfg_get_dp_soc_rx_sw_desc_weight(dpsoc->wlan_cfg_ctx);
    ring_size =  wlan_cfg_get_dp_soc_rxdma_refill_ring_size(dpsoc->wlan_cfg_ctx);

    wlpdevinitmsg->num_rx_swdesc = ring_size * rx_desc_pool_weight;
                                         ...
                                         struct nss_wifili_jitter_stats *jt_stats)
{
     ...
     */
    for (peer_idx = 0; peer_idx < npeers; peer_idx++) {
        peer = dp_peer_find_by_id(dpsoc, jt_stats[peer_idx].peer_id);
        if (!peer) {
            QDF_TRACE(QDF_MODULE_ID_NSS, QDF_TRACE_LEVEL_WARN,
        ...
        for (tid_idx = 0; tid_idx < QDF_MIN(CDP_DATA_TID_MAX, NSS_WIFILI_MAX_TID); tid_idx++) {
            ...
            rx_tid->tx_avg_jitter = tid_stats->avg_jitter;
            rx_tid->tx_avg_delay = tid_stats->avg_delay;
            rx_tid->tx_avg_err += tid_stats->avg_err;
            rx_tid->tx_total_success += tid_stats->success;
            rx_tid->tx_drop += tid_stats->drop;
        }
    }
}
        ...
        case NSS_WIFILI_PEER_DELETE_MSG:
            {
                ...
                if (response == NSS_CMN_RESPONSE_EMSG) {
                    QDF_TRACE_ERROR_RL(QDF_MODULE_ID_NSS,
                            "[nss-wifili]:NSS Wifili peer delete configuration message failed with error = %u per_id = %d", error, peer_id);
                    return;
			    }
...
uint32_t osif_nss_wifili_pmem_alloc(ol_ath_soc_softc_t *soc, struct nss_wifi_peer_mem *nwpmem, uint16_t peer_id)
{
    void *mem = NULL;
    uint32_t pmem = 0;
