...
/**
 * struct queue entry - queue entry structure
 * @value: Value to store in queue
 * @tailq: Tailq pointers
 */
struct queue_entry {
    void *value;
...
/**
 * q_init()    - queue initialization
 * @q: queue pointer
 *
 * Return 0 - OK Error code on failure
 */
...
void q_insert(struct queue *q, void *value)
{
    ...
    printf("QLD::Inserting a new node\n");
    entry->value = value;
...
/**
 * q_remove() - queue removal
 * @q: queue pointer
 * @value: value to remove from queue
 *
