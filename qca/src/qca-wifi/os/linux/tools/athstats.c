...
static void printphy_stats_curmode(FILE *fd, const struct ath_phy_stats *phy_stats)
{
    ...
    fprintf(fd, "received phy error packets = %"PRIu64"\n",
            phy_stats->ast_rx_phyerr);
    fprintf(fd, "received crc error packets = %"PRIu64"\n",
            phy_stats->ast_rx_crcerr);
    fprintf(fd, "received decrypt error packets = %"PRIu64"\n",
            phy_stats->ast_rx_decrypterr);
    fprintf(fd, "received demic error packets = %"PRIu64"\n",
            phy_stats->ast_rx_demicerr);
    fprintf(fd, "received fifo error packets = %"PRIu64"\n",
            phy_stats->ast_rx_fifoerr);
    fprintf(fd, "received decrypt busy error packets = %"PRIu64"\n",
           ...
           int stats_level)
{
    ...
    if (stats_level == TXRX_STATS_LEVEL_BASIC) {
        fprintf(fd,"  tx: %u msdus (%"PRIu64" B)\n",
                pdev_stats->tx.comp_pkt.num,
                pdev_stats->tx.comp_pkt.bytes);
    } else { /* full */
        fprintf(fd,
            "  tx: sent %u msdus (%"PRIu64" B), "
            "rejected %u ( %"PRIu64" B), dropped %"PRIu64" (%"PRIu64" B)\n",
            pdev_stats->tx.comp_pkt.num,
            pdev_stats->tx.comp_pkt.bytes,
        ...
        fprintf(fd,
            "    download fail: %"PRIu64" (%"PRIu64" B), "
            "target discard: %u (%"PRIu64" B), "
            "no ack: %u (%"PRIu64" B)\n",
            stats->tx.dropped.download_fail.pkts,
            stats->tx.dropped.download_fail.bytes,
        ...
        const ol_dbg_stats *wlan_pdev_stats, FILE *fd)
{
    ...
    fprintf(fd,"ast_rx_bytes        :\t%"PRIu64"\n",radio_stats->rx_bytes);
    fprintf(fd,"ast_tx_bytes        :\t%"PRIu64"\n",radio_stats->tx_bytes);
...
#define PRINT_ROW_ELEM 5
    ...
    fprintf(fd, "\n");
    fprintf(fd,"wmi_tx_mgmt                :\t%"PRIu64"\n",radio_stats->wmi_tx_mgmt);
    fprintf(fd,"wmi_tx_mgmt_completions    :\t%"PRIu64"\n",radio_stats->wmi_tx_mgmt_completions);
...
static void dp_pdev_printstats(FILE *fd, const struct cdp_pdev_stats *ath_stats)
{
    ...
    fprintf(fd, "\ttoal bytes replenished                               :\t%"PRIu64"\n",
    ...
    /*tx_i */
    fprintf(fd, "\nTx ingress stats\n");
    fprintf(fd, "\tTotal bytes received for transmission                :\t%"PRIu64"\n",
    ...
    fprintf(fd, "\tTotal Raw bytes                                      :\t%"PRIu64"\n",
            ath_stats->tx_i.raw.raw_pkt.bytes);
    fprintf(fd, "\tDMA map error                                        :\t%u\n",
            ath_stats->tx_i.raw.dma_map_error);
    fprintf(fd, "\tTotal no of TSO bytes                                :\t%"PRIu64"\n",
            ath_stats->tso_stats.num_tso_pkts.bytes);
    fprintf(fd, "\tTotal no of TSO packets                              :\t%u\n",
            ath_stats->tso_stats.num_tso_pkts.num);
    fprintf(fd, "\tTSO packets dropped by host                          :\t%u\n",
            ath_stats->tso_stats.dropped_host.num);
    fprintf(fd, "\tTSO packets dropped by target                        :\t%u\n",
            ath_stats->tso_stats.dropped_target);
    fprintf(fd, "\tTotal scatter gather bytes                           :\t%"PRIu64"\n",
    ...
    fprintf(fd, "\ttotal no of multicast conversion bytes               :\t%"PRIu64"\n",
    ...
    fprintf(fd, "\tNumber of sniffer bytes                              :\t%"PRIu64"\n",
    ...
    fprintf(fd, "\tBytes Info for which completions were received       :\t%"PRIu64"\n",
            ath_stats->tx.comp_pkt.bytes);
    fprintf(fd, "\tUnicast bytes Count                                  :\t%"PRIu64"\n",
            ath_stats->tx.ucast.bytes);
    fprintf(fd, "\tUnicast Packet Count                                 :\t%u\n",
            ath_stats->tx.ucast.num);
    fprintf(fd, "\tMulticast bytes Count                                :\t%"PRIu64"\n",
    ...
    fprintf(fd, "\tSuccessful Tx bytes                                  :\t%"PRIu64"\n",
    ...
    fprintf(fd, "\tRSSI of received signal                              :\t%u\n",
            ath_stats->rx.rssi);
    fprintf(fd, "\tTotal multipass Rx packets dropped                    :\t%u\n",
            ath_stats->rx.multipass_rx_pkt_drop);
}
