/*
 * Copyright (c) 2013,2019-2020 Qualcomm Innovation Center, Inc.
 * All Rights Reserved.
...
#define	STAT(x,fmt) \
	if (stats->is_##x) fprintf(fd, "%"PRIu64 fmt "\n", stats->is_##x)
...
static void
print_sta_stats(FILE *fd, const u_int8_t macaddr[QDF_MAC_ADDR_SIZE])
{
#define	STAT(x,fmt) \
...
#define	STAT64(x,fmt) \
	...
	memcpy(stats.is_u.macaddr, macaddr, QDF_MAC_ADDR_SIZE);
	...
	STAT(rx_ucast, "%u");
	STAT(rx_mcast, "%u");
	STAT(rx_bytes, "%"PRIu64);
#if UMAC_SUPPORT_STA_STATS_ENHANCEMENT
	STAT(rx_ucast_bytes, "%"PRIu64);
	STAT(rx_mcast_bytes, "%"PRIu64);
#endif
	STAT(rx_dup, "%u");
	STAT(rx_noprivacy, "%u");
	STAT(rx_wepfail, "%u");
	...
	STAT(tx_ucast, "%u");
	STAT(tx_mcast, "%u");
	STAT(tx_bytes, "%"PRIu64);
#if UMAC_SUPPORT_STA_STATS_ENHANCEMENT
	STAT(tx_bytes_success, "%"PRIu64);
	STAT(tx_ucast_bytes, "%"PRIu64);
	STAT(tx_mcast_bytes, "%"PRIu64);
#endif
	STAT(last_per, "%"PRIu64);
	STAT(tx_novlantag, "%u");
