...
#define WPS_NL80211_CMD_SOCK_ID DEFAULT_NL80211_CMD_SOCK_ID
#define WPS_NL80211_EVENT_SOCK_ID DEFAULT_NL80211_EVENT_SOCK_ID
...
static int stop_wps_on_sta(char *vap, char *radio)
{
    ...
    char wps_cmpl_sta[IFNAMSIZ];

    f = fopen(cmpl_file, "r");
    if (f) {
        if (fgets(wps_cmpl_sta, IFNAMSIZ, f) != NULL) {
            ...
            log_msg(WPS_DBG, "wps completed "
                    "on %s vap\n", wps_cmpl_sta);
            if (strncmp(vap, wps_cmpl_sta, sizeof(wps_cmpl_sta)) == 0) {
                   log_msg(WPS_DBG, "wps completed hence, wps_cancel "
                        "is not sent for %s vap\n", vap);
                fclose(f);
                return 0;
            }
        } else {
...
static int parse_radiolist(char *linebuf)
{
    ...
    memset(radiohead_ptr, 0, sizeof(struct wps_radiolist_t));
    pline = linebuf;
    len = strlen(pline) - 1;
    if (pline[len] == '\n')
        pline[len] = '\0';
    ...
    while (pname != NULL) {
        ...
        memset(radio_p, 0, sizeof(struct wps_radio_t));
        count++;
...
static void insert_vap_to_radiolist(char *vap, int ioctl_sock)
{
    ...
    if (sock_ctx.cfg80211) {
#if UMAC_SUPPORT_CFG80211
	    buffer.data = &parent_ifindex;
	    buffer.length = sizeof(uint32_t);
	    buffer.parse_data = 0;
	    buffer.callback = NULL;
	    buffer.parse_data = 0;
	    msg = wifi_cfg80211_send_getparam_command(&(sock_ctx.cfg80211_ctxt),
			    QCA_NL80211_VENDOR_SUBCMD_WIFI_PARAMS, IEEE80211_PARAM_PARENT_IFINDEX, vap, (char *)&buffer, sizeof(uint32_t));
	    ...
	    if(strlcpy(iwr.ifr_name, vap, IFNAMSIZ) >= IFNAMSIZ) {
    ...
    memset(&ifr, 0, sizeof(ifr));
    if(strlcpy(ifr.ifr_name, vap, IFNAMSIZ) >= IFNAMSIZ) {
        printf("%s:Interface name too long.%s\n",__func__,vap);
        return;
    }
    ...
    if(strlcpy(p_vap->vap_iface, vap, IFNAMSIZ) >= IFNAMSIZ) {
        printf("%s:Interface name too long.%s\n",__func__,vap);
        return;
    }
    ...
    log_msg(WPS_INF, "vap(%s)  %d\n",vap,radiohead_ptr->num_all_sta_vaps);
...
next_line:
        ...
        if (wps_use_cases_start) {
            ...
            if ((found_case != prev_use_case) || (wps_use_cases_end)) {
			    ...
			    if(strlen(p_dent->d_name) >= IFNAMSIZ) {
				    printf("%s:Interface name too long.%s\n",__func__,p_dent->d_name);
				    return -1;
			    }
...
{
    int ret = EXIT_SUCCESS, items = 0, sock = -1;
    ...
    init_socket_context(&sock_ctx, WPS_NL80211_CMD_SOCK_ID, WPS_NL80211_EVENT_SOCK_ID);
