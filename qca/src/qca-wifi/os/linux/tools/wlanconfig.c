...
int main(int argc, char *argv[])
{
    ...
    else if (streq(cmd,"rx_flow_tag_op")) {
        handle_command = handle_command_flow_tag;
    }
...
void usage(void)
{
    ...
    fprintf(stderr, "wlanconfig athX cfr list\n") ;
#if WLAN_ENH_CFR_ENABLE
    fprintf(stderr, "wlanconfig athx cfr m_ta_ra_filter <enable/disable>\n");
    fprintf(stderr, "wlanconfig athx cfr m_directed_ndpa_ndp <enable/disable>\n");
    fprintf(stderr, "wlanconfig athx cfr m_ndpa_ndp_all <enable/disable>\n");
    fprintf(stderr, "wlanconfig athx cfr disable_all\n");
    fprintf(stderr, "wlanconfig athx cfr ta_ra_addr <group_id> <ta> <ta_mask> <ra> <ra_mask>\n");
    fprintf(stderr, "wlanconfig athx cfr bw_nss <group_id> <bw> <nss>\n");
    fprintf(stderr, "wlanconfig athx cfr subtype <group_id> <mgmt> <ctrl> <data>\n");
    fprintf(stderr, "wlanconfig athx cfr en_cfg <bitmask>\n");
    fprintf(stderr, "wlanconfig athx cfr reset_cfg <bitmask>\n");
    fprintf(stderr, "wlanconfig athx cfr capture_dur <value>\n");
    fprintf(stderr, "wlanconfig athx cfr capture_intval <value>\n");
    fprintf(stderr, "wlanconfig athx cfr ul_mu_user_mask <mask_lower_32> <mask_upper_32>\n");
    fprintf(stderr, "wlanconfig athx cfr freeze_tlv_delay_cnt <freeze_tlv_delay_cnt_en> <threshold>\n");
    fprintf(stderr, "wlanconfig athx cfr commit\n");
    fprintf(stderr, "wlanconfig athx cfr rcc_config_details\n");
    fprintf(stderr, "wlanconfig athx cfr rcc_debug_counters\n");
    fprintf(stderr, "wlanconfig athx cfr clr_dbg_counters\n");
    fprintf(stderr, "wlanconfig athx cfr rcc_dump_lut\n");
#endif
#endif
#ifdef WLAN_SUPPORT_RX_FLOW_TAG
    fprintf(stderr, "usage: wlanconfig wifiX rx_flow_tag_op <opcode> <ipversion> <source_ip> <dest_ip> <src_port> <dest_port> <L4_protocol_num> <tag>\n");
#endif /* WLAN_SUPPORT_RX_FLOW_TAG */
    exit(-1);
}
        ...
        char *addr, u_int8_t maxrate)
{
    ...
    memcpy(config.smr.mac, temp, QDF_MAC_ADDR_SIZE);
    config.smr.maxrate = maxrate;
    send_command(sock_ctx, ifname, &config, sizeof(struct ieee80211_wlanconfig),
           NULL, QCA_NL80211_VENDOR_SUBCMD_SET_MAXRATE, IEEE80211_IOCTL_CONFIG_GENERIC);
