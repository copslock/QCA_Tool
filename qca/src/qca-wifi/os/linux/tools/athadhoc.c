...
ieee80211_security_set_groupkey(const char *ifname)
{
	...
	memset(setkey.ik_macaddr, 0xff, QDF_MAC_ADDR_SIZE);

    set80211priv(ifname, IEEE80211_IOCTL_SETKEY, &setkey, sizeof(struct ieee80211req_key), 1);
}

static void
ieee80211_security_set_ucastkey(const char *ifname, unsigned char macaddr[QDF_MAC_ADDR_SIZE])
{
	struct ieee80211req_key setkey;
	...
	memcpy(setkey.ik_macaddr, macaddr, QDF_MAC_ADDR_SIZE);
