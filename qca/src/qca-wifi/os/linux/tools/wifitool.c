/*
 * Copyright (c) 2013-2015, 2017,2019-2020 Qualcomm Innovation Center, Inc.
 *
 * All Rights Reserved.
...
static int if_name_filter_set(char *ifname)
{
    if (if_name_filter == NULL) {
        return 0;
    } else if (strncmp(if_name_filter, ifname, MAX_IFNAME_SIZE) == 0) {
        return 1;
    } else {
        return 0;
    }
...
static void usage(void)
{
    ...
    fprintf(stderr, "cmd: [frame_injector_en]\n");
...
{
    fprintf(stderr, "usage: wifitool athX setprimarychans [-band X] ch1 ch2….chN [-Band Y] ch1, ch2,.. chN\\n");
}
...
{
    fprintf(stderr, "usage: wifitool athN setchanlist [-band X] ch1 ch2….chN [-Band Y] ch1, ch2,.. chN\n");
}
...
{
    fprintf(stderr, "usage: wifitool athX offchan_tx_test [-band X] chan dwell_time \n");
    exit(-1);
}

static void usage_offchan_rx_test(void)
{
    fprintf(stderr, "usage: wifitool athX offchan_rx_test [-band X] chan dwell_time bw_mode(optional) \n");
    exit(-1);
}
...
{
    fprintf(stderr, "usage: wifitool athX acsreport value(optional)\n");
    fprintf(stderr, "usage: wifitool athX acsreport : to display default acsreport\n");
    fprintf(stderr, "usage: wifitool athX acsreport -nfall : to display per chain NF table along with default acsreport\n");
}
...
static void usage_frame_injector(void)
{
    fprintf(stderr, "usage: wifitool athX frame_injector_en <frametype> <enable|disable> <inject_period> <dest_macaddr> \n");
    exit(-1);
}
...
static void usage_send_customneigrpt(void)
{
    fprintf(stderr, "usage: wifitool athX sendcustneigrpt <dst mac> <num rep> <bssid> <dialog_token> <info> <operating class> \n");
    fprintf(stderr, "       <channel> <phy type> [-t <1/0] [-c <1/0>] [-p <1/0] [-ter <1/0] [-b <1/0] \n");
    fprintf(stderr, "       [-w <1/0] [-htc <2/0>] [-hto <1/0] [-sec <1/0] [-pil <1/0] [-rm <1/0] \n");
    fprintf(stderr, "       [-vhc <1/0] [-vho <1/0> [-ven <1/0>] \n");
    fprintf(stderr, "       optional ies: -t:tsf,-c:country, -p:preference, -ter:termination duration \n");
    fprintf(stderr, "       -b:bearing, -w:widebandwidth chan, -htc:ht caps, -hto: ht op, -sec:sec channel \n");
    fprintf(stderr, "       -pil: meas pilot, -rm: RM caps -vhc: vht caps, -vh0: vhtops, -ven: vendor\n");
    exit(-1);
}
...
{
    fprintf(stderr, "usage: wifitool athX sendbstmreq_target <mac_addr> [-di <disassoc_duration> <disassoc_imminent>] (optional)\n[<candidate_bssid> <candidate channel> <candidate_preference> <operating class> <PHY type>...]\n");
    exit(-1);
}
...
static void usage_fake_mgmt_rx(void)
{
    fprintf(stderr, "usage: wifitool athX fake_mgmt_rx <buflen> <buffer>\n");
    exit(-1);
}
...
/*
 * send_custom_neigh_resp: send Custom neighbor report response
 * @ifname: interface name
 * @argc: total number of command line arguments
 * @argv: values of command line arguments
 * @req_cmd: command to be sent
 * return -1 on error; otherwise return 0 on success
 */
static int
send_custom_neigh_resp(const char *ifname, int argc, char *argv[], int req_cmd)
{
#define MIN_ARG_CUST_NEIGH_RESP  11
#define ARG_INDEX_DST_NEIGH_RESP 3
    int8_t arg_index = 0, cmd_index = 0, enable = 0;
    /* Maintain same order as the bit map indication in rrm.h */
    int8_t keywords[14][5] = {"-t\0", "-c\0", "-p\0", "-ter\0", "-b\0", "-w\0", "-htc\0", \
                              "-hto\0", "-sec\0", "-pil\0", "-rm\0", "-vhc\0", "-vho\0", "-ven\0"};
    int count = 0, retval = 0;
    struct ieee80211req_athdbg req = {0};
    ieee80211_user_nrresp_info_t   nrreports;
    ieee80211_cust_nrresp_info_t *nrresp = NULL;

    if (argc < MIN_ARG_CUST_NEIGH_RESP) {
           usage_send_customneigrpt();
    } else {
        arg_index = ARG_INDEX_DST_NEIGH_RESP;
        req.cmd = IEEE80211_DBGREQ_SEND_CUSTOM_NEIGRPT;
        req.needs_reply = DBGREQ_REPLY_IS_NOT_REQUIRED;
        req.data.neighrpt_custom = (ieee80211_user_nrresp_info_t *)&nrreports;

        if (!wifitool_mac_aton(argv[arg_index++], req.dstmac, QDF_MAC_ADDR_SIZE)) {
             errx(1, "Invalid destination mac address");
             return -1;
        }

        nrreports.num_report = atoi(argv[arg_index++]);
        if (!nrreports.num_report) {
             errx(1, "Number of reports cannot be zero\n");
             return -1;
        }
        nrresp = (ieee80211_cust_nrresp_info_t *)malloc(sizeof(ieee80211_cust_nrresp_info_t) *
                                                        nrreports.num_report);
        if (nrresp == NULL) {
            errx(1, "Memory could not be allocated for nrresp\n");
            return -1;
        }
        nrreports.custom_nrresp_info = nrresp;
        while (count < nrreports.num_report) {
            arg_index = 5;
            cmd_index = 0;

            if (!wifitool_mac_aton(argv[arg_index++], nrresp->bssid, QDF_MAC_ADDR_SIZE)) {
                errx(1, "Invalid BSSID");
                goto fail;
            }
            nrreports.dialog_token = atoi(argv[arg_index++]);
            nrresp->bssi_info = atoi(argv[arg_index++]);
            nrresp->regclass = atoi(argv[arg_index++]);
            nrresp->channum = atoi(argv[arg_index++]);
            nrresp->phy_type = atoi(argv[arg_index++]);
            nrresp->subie_pres = 0;
            while (arg_index < argc) {
                enable = 0;
                if (strcmp(argv[arg_index], (char *)keywords[cmd_index]) == 0) {
                    arg_index++;
                    enable = atoi(argv[arg_index]);
                    if (enable) {
                        nrresp->subie_pres |= (1 << cmd_index);
                    }
                } else {
                    arg_index++;
                }
            ...
            if (nrresp->subie_pres & IEEE80211_NR_SUBIE_PRES_TSF) {
                nrresp->nr_tsf.tsf_offset = 0;
                nrresp->nr_tsf.bcn_int = 100;
            }
            if (nrresp->subie_pres & IEEE80211_NR_SUBIE_PRES_COUNTRY) {
                strlcpy((char *)(&nrresp->nr_country.country_string), "US\0", 3);
            }
            if (nrresp->subie_pres & IEEE80211_NR_SUBIE_PRES_CAND_PREF) {
                nrresp->nr_cand_pref.preference = 1;
            }
            if (nrresp->subie_pres & IEEE80211_NR_SUBIE_PRES_TERM_DUR) {
                nrresp->nr_term_duration.tsf = 0;
                nrresp->nr_term_duration.duration = 0;
            }
            if (nrresp->subie_pres & IEEE80211_NR_SUBIE_PRES_BEARING) {
                nrresp->nr_bearing.bearing = 1;
                nrresp->nr_bearing.distance = 1;
                nrresp->nr_bearing.height = 1;
            }
            if (nrresp->subie_pres & IEEE80211_NR_SUBIE_PRES_WIDEBAND) {
                nrresp->nr_wb_chan.chan_width = 1;
                nrresp->nr_wb_chan.centerfreq0 = 42;
                nrresp->nr_wb_chan.centerfreq1 = 0;
            }
            if (nrresp->subie_pres & IEEE80211_NR_SUBIE_PRES_HT_CAPS) {
                nrresp->nr_ht_cap.htcap_info = 0;
                nrresp->nr_ht_cap.ampdu_param = 0;
                memset(&nrresp->nr_ht_cap.mcs, 0, IEEE80211_MAX_NR_MCS_SET);
                nrresp->nr_ht_cap.ht_extcap = 0x0500;
                nrresp->nr_ht_cap.asel_caps = 1;
            }
            if (nrresp->subie_pres & IEEE80211_NR_SUBIE_PRES_HT_OP) {
                nrresp->nr_ht_op.chan = 36;
                memset(&nrresp->nr_ht_op.htop_info, 0, IEEE80211_NR_HTOP_LEN);
                memset(&nrresp->nr_ht_op.htop_mcs, 0, IEEE80211_MAX_NR_MCS_SET);
            }
            if (nrresp->subie_pres & IEEE80211_NR_SUBIE_PRES_SEC_CHAN_OFFSET) {
                nrresp->nr_sec_chan.sec_chan_offset = 4;
            }
            if (nrresp->subie_pres & IEEE80211_NR_SUBIE_PRES_MEAS_PILOT) {
                nrresp->nr_meas_pilot.pilot = 0;
                nrresp->pilot_vndroui_len = 4;
                strlcpy((char *)(&nrresp->nr_meas_pilot.vendor.oui), "OUI\0", 4);
                nrresp->pilot_vndrbuf_len = 10;
                strlcpy((char *)(&nrresp->nr_meas_pilot.vendor.buf), "123456789\0", 10);
            }
            if (nrresp->subie_pres & IEEE80211_NR_SUBIE_PRES_RM_EN_CAPS) {
                memset(&nrresp->nr_rm_en_caps.rm_cap, 1, IEEE80211_NR_RM_CAP_LEN);
            }
            if (nrresp->subie_pres & IEEE80211_NR_SUBIE_PRES_VHT_CAPS) {
                nrresp->nr_vhtcaps.vhtcap_info = 0;
                nrresp->nr_vhtcaps.mcs = 0x32;
                nrresp->nr_vhtcaps.rx_highest_rate = 0;
                nrresp->nr_vhtcaps.tx_vht_mcs = 0;
                nrresp->nr_vhtcaps.tx_highest_rate = 0;
            }
            if (nrresp->subie_pres & IEEE80211_NR_SUBIE_PRES_VHT_OP) {
                nrresp->nr_vht_op.vhtop_info.chan_width = 1;
                nrresp->nr_vht_op.vhtop_info.centerfreq0 = 42;
                nrresp->nr_vht_op.vhtop_info.centerfreq1 = 0;
            }
            if (nrresp->subie_pres & IEEE80211_NR_SUBIE_PRES_VENDOR) {
                nrresp->vendor_oui_len = 4;
                strlcpy((char *)(&nrresp->nr_vendor.oui), "OUI\0", 4);
                nrresp->vendor_buf_len = 10;
                strlcpy((char *)(&nrresp->nr_vendor.buf), "123456789\0", 10);
            }
            count++;
            if (count < nrreports.num_report) {
                nrresp = nrresp + 1;
            }
        }
...
fail:
    free(nrreports.custom_nrresp_info);
    return retval;
#undef MIN_ARG_CUST_NEIGH_RESP
#undef ARG_INDEX_DST_NEIGH_RESP
}
    ...
    if (argc < arg_num) {
        ...
        switch (req_cmd) {
            case IEEE80211_DBGREQ_SENDBCNRPT:
                ...
                if (argc < 15) {
                    memset(bcnrpt->bssid,0xff,QDF_MAC_ADDR_SIZE);
                } else {
...
static int send_bstmreq_target(const char *ifname, int argc, char *argv[])
{
    ...
    if (argc < fixed_length_args) {
        ...
        if (streq(argv[4], "-di")) {
            /* Add disassoc timer and imminent to reqinfo */
            if (argv[5]) {
                reqinfo->disassoc_timer = atoi(argv[5]);
            }
            if (argv[6]) {
                reqinfo->disassoc = atoi(argv[6]);
                /* If the Disassociation Imminent field is 0, the Disassociation
                 * Timer field is reserved.
                 */
                if (!reqinfo->disassoc)
...
void print_channel_survey_stats (struct cfg80211_data *arg)
{
    ...
    printf("Home channel survey stats msg_len: %d, num_elements: %d\n",
           (uint32_t)msg_len, num_elements);
    printf("freq: %4d, rx_bss: %12"PRIu64", total: %12"PRIu64", tx: %12"PRIu64", "
...
#define MIN_PARAM 4
    ...
    chanlist = &req.data.primary_allowed_chanlist;
    s = socket(AF_INET, SOCK_DGRAM, 0);
    (void) memset(&iwr, 0, sizeof(iwr));
    ...
    for (i = 3, j = 0; i < argc; i++) {
        if (streq(argv[i], "-band") || streq(argv[i], "--band")) {
            band = atoi(argv[++i]);
            continue;
        }
        chanlist->chans[j].band =  band;
        chanlist->chans[j].chan =  atoi(argv[i]);
        j++;
    }
    chanlist->n_chan = j;
...
#define MAX_PARAM  3
    ...
    if(chanlist->n_chan == 0) {
        ...
        for(i = 0; i < chanlist->n_chan; i++) {
            if (is_new_channel_display_format() &&
                (band != chanlist->chans[i].band)) {
                band = chanlist->chans[i].band;
                printf("%s: ", get_wlan_band_id_to_string(band));
            }
            printf("%d    ",chanlist->chans[i].chan);
        }
        printf("\n");
    }
    close(s);
...
/*
 * parse_band_channel_idx: parses remaining command line arguments and
 *         populates chan index and band info
 * @argc:  argc points to the arg from where band and channel number needs
 *         parsing after skipping other fields
 * @argv: values of command line arguments
 * @chanlist: pointer where channel and band info to be updated post parsing
 * return: number of channels parsed
 */
static int
parse_band_channel_idx(int argc, char *argv[], ieee80211_user_chanlist_t *chanlist)
{
    u_int8_t band = 0;
    u_int16_t i, count = 0;
    u_int16_t max = sizeof(chanlist->chans)/sizeof(chanlist->chans[0]);

    for (i = argc; argv[i] && (count < max); i++) {
        if (streq(argv[i], "-band") || streq(argv[i], "--band")) {
            i++;
            band = atoi(argv[i]);
            continue;
        }
        chanlist->chans[count].band = band;
        chanlist->chans[count].chan = atoi(argv[i]);
        count++;
    }
...
#define MAX_PARAM  3
    ...
    switch (req_cmd) {
        case IEEE80211_DBGREQ_SETACSUSERCHANLIST:
            parse_band_channel_idx(3, argv, &req.data.user_chanlist);
            send_command(&sock_ctx, ifname, &req, sizeof(req), NULL, QCA_NL80211_VENDOR_SUBCMD_DBGREQ, IEEE80211_IOCTL_DBGREQ);
            break;
        ...
        case IEEE80211_DBGREQ_GETACSUSERCHANLIST:
            ...
            for(i = 0; i < req.data.user_chanlist.n_chan; i++) {
                if (is_new_channel_display_format() &&
                    (band != req.data.user_chanlist.chans[i].band)) {
                    band = req.data.user_chanlist.chans[i].band;
                    printf("%s:", get_wlan_band_id_to_string(band));
                }
                printf(" %d ",req.data.user_chanlist.chans[i].chan);
            }
            printf("\n");
            break;
...
{
#if UMAC_SUPPORT_CFG80211
    u_int32_t len = arg->length;/*QDF_MAC_ADDR_SIZE*/
...
#define MAX_PARAM 3
    ...
    req.needs_reply = DBGREQ_REPLY_IS_REQUIRED;
    memset(req.dstmac,0xff,QDF_MAC_ADDR_SIZE); /* Assigning broadcast address to get the STA count */
    ...
    bufflen = sta_count * QDF_MAC_ADDR_SIZE;
    if (!sock_ctx.cfg80211) {
...
#if UMAC_SUPPORT_CFG80211
        arg.data = addr_list;
        arg.length = req.data.param[0] * QDF_MAC_ADDR_SIZE;
        if (sta_type == RRM_STA) {
...
#define MAX_PARAM 3
    ...
    if (!(temp = (void *) realloc(req, (sizeof(*req) + (QDF_MAC_ADDR_SIZE * (count)) + (sizeof(ieee80211_noise_stats_t) * bin_number * count))))) {
        printf("Memory allocation failed  __investigate__\n");
        goto MEMORY_FAIL;
    }
    ...
    if (send_command(&sock_ctx, ifname, req, (sizeof(*req) + (QDF_MAC_ADDR_SIZE * (count)) +
    ...
    noise_stats = (ieee80211_noise_stats_t *) ((char *)addr_list + (QDF_MAC_ADDR_SIZE * (count)));
    ...
    for (index = 0;index<(QDF_MAC_ADDR_SIZE*count);index++){
        if (index % QDF_MAC_ADDR_SIZE == 0) {
            printf("Mac address \t");
        }
        printf("%02x:",addr_list[index]);

        if (index % 6 == (QDF_MAC_ADDR_SIZE-1)) {
            printf("\n");
 ...
 */
/*
 * LTEU not enabled and also supported only in 5G so
 * not adding 6G support as of now.
 */
...
static void block_acs_channel(const char *ifname, int argc, char *argv[])
{
    ...
    u_int8_t channel[ACS_MAX_CHANNEL_COUNT] = {0}, valid[ACS_MAX_CHANNEL_COUNT] = {0};
    u_int8_t band[ACS_MAX_CHANNEL_COUNT] = {0};
    char *p = NULL;
    struct ieee80211req_athdbg req = { 0 };
    u_int8_t chan_band = 0;
    u_int32_t arg_idx = 0;
    struct ieee80211_chan_def *chans;

    if ((argc < 4)) {
        fprintf(stderr, "usage: wifitool athX block_acs_channel [-band X] ch1,ch2,.chN [-Band Y] ch1,ch2,. chN\n");
    ...
    for (arg_idx = 3; arg_idx < argc; arg_idx++) {
        if (streq(argv[arg_idx], "-band") || streq(argv[arg_idx], "--band")) {
            arg_idx++;
            chan_band = atoi(argv[arg_idx]);
            continue;
        }
        p = argv[arg_idx];
        while(*p != '\0') {
    ...
    if(count) {
        chans = &req.data.user_chanlist.chans[0];
        req.data.user_chanlist.n_chan = count;
        for (i = 0; i < count; i++) {
            chans[i].band = band[i];
            chans[i].chan = channel[i];
        }
        send_command(&sock_ctx, ifname, &req, sizeof(req), NULL, QCA_NL80211_VENDOR_SUBCMD_DBGREQ, IEEE80211_IOCTL_DBGREQ);
        if(!chans[0].chan) {
            printf("List Flushed \n");
...
static int acs_report(const char *ifname, int argc, char *argv[])
{
    ...
    fprintf(stdout," Channel | BSS  | minrssi | maxrssi | NF | Ch load | spect load | sec_chan | SR bss | SR load | Ch avil | Chan eff | NearBSS | Med BSS | Far BSS | Eff BSS | grade | Rank | Unused | Radar \n");
    fprintf(stdout,"-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------\n");
    ...
    for (i = 0; i < acs->nchans; i++) {
        ...
        fprintf(stdout," %4d(%3d) %4d     %4d      %4d   %4d    %4d        %4d       %4d     %4d      %4d     %4u      %4u      %4u       %4d      %4d      %4d      %4u   %4d      %s     %4d\n",
                acs->chan_freq,
                acs->ieee_chan,
                acs->chan_nbss,
                acs->chan_minrssi,
    ...
    if (print_nf_table) {
        ...
        fprintf(stdout,"-----------------------------------------------------------------------------------------------------------------\n");
        fprintf(stdout,"Per chain NF value in dBm \n");
        fprintf(stdout,"-----------------------------------------------------------------------------------------------------------------\n");
        fprintf(stdout," Channel | NF chain-0 | NF chain-1 | NF chain-2 | NF chain-3 | NF chain-4 | NF chain-5 | NF chain-6 | NF chain-7 \n");
        fprintf(stdout,"-----------------------------------------------------------------------------------------------------------------\n");

        for (i = 0; i < acs->nchans; i++) {
            acs->entry_id = i;
            acs->acs_type = ACS_CHAN_NF_STATS;
            req.cmd = IEEE80211_DBGREQ_GETACSREPORT;
            req.needs_reply = DBGREQ_REPLY_IS_REQUIRED;
            send_command(&sock_ctx, ifname, &req, sizeof(req), NULL, QCA_NL80211_VENDOR_SUBCMD_DBGREQ, IEEE80211_IOCTL_DBGREQ);

            fprintf(stdout," %4d(%3d)     %4d         %4d        %4d        %4d         %4d         %4d         %4d         %4d\n",
                    acs->chan_freq,
                    acs->ieee_chan,
                    acs->perchain_nf[0],
                    acs->perchain_nf[1],
...
static void get_rrmutil(const char *ifname, int argc, char *argv[])
{
    ...
    if (req.data.tr069_req.data_size > sizeof(req.data.tr069_req.data_buff)) {
        fprintf(stderr, "error: %s: buff_size: %"UINT", response len: %u\n",
...
/*
 * fake_mgmt_rx: specify parameters set during the sending of a mgmt frame
 * @ifname: interface name
 * @argc: total number of command line arguments
 * @argv: values of command line arguments
 * return -1 on error; otherwise return 0 on success
 */
static int fake_mgmt_rx(const char *ifname, int argc, char *argv[])
{
    struct ieee80211req_athdbg req;
    int err = 0;
    ...
    req.cmd = IEEE80211_DBGREQ_FAKE_MGMT_RX;
    req.needs_reply = DBGREQ_REPLY_IS_NOT_REQUIRED;

    req.data.mgmt_frm.buflen = atoi(argv[3]);
    req.data.mgmt_frm.buf = (uint8_t *) malloc(req.data.mgmt_frm.buflen);

    if (!wifitool_mac_aton(argv[4], req.data.mgmt_frm.buf, req.data.mgmt_frm.buflen))
    {
        errx(1, "Invalid packet input");
        return -1;
    }

    err = send_command(&sock_ctx, ifname, &req, sizeof(req), NULL,
            QCA_NL80211_VENDOR_SUBCMD_DBGREQ, IEEE80211_IOCTL_DBGREQ);
    free(req.data.mgmt_frm.buf);
    return err;
}
...
static void tr069_chan_history(const char *ifname, int argc, char *argv[])
{
    ...
    if (sizeof(ieee80211_channelhist_t) > sizeof(req.data.tr069_req.data_buff)) {
        fprintf(stderr, "error: %s: buff_size: %"UINT", response len: %"UINT"\n", __func__,
    ...
    fprintf(stdout," Channel |   band   | Selection Time \n");
    fprintf(stdout,"----------------------------------\n");
    /* print from latest to first */
    for(i = act_idx; i >= 0 ; i--) {
        clock_gettime(CLOCK_REALTIME, &nowtime);
        ts = &(chandata->chanlhist[i].chan_time);
        if (abs(prev_sec - ts->tv_sec) <= 1) {
            continue;
        }
        prev_sec = ts->tv_sec;
        ...
        fprintf(stdout," %4d      %s      %s \n",
                chandata->chanlhist[i].chanid,
                get_wlan_band_id_to_string(chandata->chanlhist[i].chanband),
                chanSelTime);
    }
    prev_sec = 0;
...
int tr069_chan_inuse(const char *ifname, int argc, char *argv[])
{
    ...
    struct ieee80211req_athdbg req = { 0 };
    struct ieee80211_acs_dbg *acs = NULL;
    enum wlan_band_id band = WLAN_BAND_MAX;
    char buff[5];
    char bandstr[10];
    char chanbuff[1024]; /*max according to the spec*/
    ...
    for (i = 0; i < acs->nchans; i++) {
        ...
        if (acs->chan_band != band) {
            band = acs->chan_band;
            if (strlen(chanbuff)) {
                /* Clear last comma (,) added as part of non terminal channel */
                chanbuff[strlen(chanbuff)-1] = '\0';
            }
            snprintf(bandstr, sizeof(bandstr), " %s: ",
                    get_wlan_band_id_to_string(band));
            if (strlcat(chanbuff, bandstr, sizeof(chanbuff)) >=
                    sizeof(chanbuff)) {
        ...
        if (strlcat(chanbuff, buff, sizeof(chanbuff)) >=
                sizeof(chanbuff)) {
            fprintf(stderr, "Channel number string too long to handle\n");
            free(acs);
            return -1;
...
int enable_frame_injector(const char *ifname, int argc, char *argv[])
{
    struct ieee80211req_athdbg req;

    if (argc < 7) {
        usage_frame_injector();
    } else {
        memset(&req, 0, sizeof(struct ieee80211req_athdbg));
        req.cmd = IEEE80211_DBGREQ_FRAME_INJECTOR;
        req.needs_reply = DBGREQ_REPLY_IS_NOT_REQUIRED;
        req.data.param[0] = atoi(argv[3]);
        req.data.param[1] = atoi(argv[4]);
        req.data.param[2] = atoi(argv[5]);
        if (!wifitool_mac_aton(argv[6], req.dstmac, 6)) {
            errx(1, "Invalid destination mac address");
            return -1;
        }
        send_command(&sock_ctx, ifname, &req, sizeof(req), NULL,
...
int tr069_get_rate(const char *ifname, int argc, char *argv[], int req_cmdid)
{
    ...
    fprintf(stdout," List of %s Rates (Mbps)\n", ratetype);
    fprintf(stdout,"------------------------------\n");

    for (i = 1; i < ratelist[0]; i++) {
      fprintf(stdout,"%5.2f, ",((float)ratelist[i]) / 1000);
      if (i % 10 == 0)
          fprintf(stdout,"\n");
    }
    fprintf(stdout,"\n");
 ...
 * return -1 on error; otherwise return 0 on success
 */
int set_csa_deauth(const char *ifname, int argc, char *argv[])
{
    int err;
    struct ieee80211req_athdbg req = { 0 };
    uint8_t csa_deauth_mode;

    if ((argc != 4)) {
        fprintf(stderr, "usage: wifitool athX csa_deauth_mode <1 - unicast | 2 - broadcast>\n");
        return -1;
    }

    csa_deauth_mode = atoi(argv[3]);
    if (csa_deauth_mode != CSA_DEAUTH_MODE_UNICAST && csa_deauth_mode != CSA_DEAUTH_MODE_BROADCAST &&
            csa_deauth_mode != CSA_DEAUTH_MODE_DISABLE) {
        fprintf(stderr, "Invalid csa deauth mode, Use '1' for unicast or '2' for broadcast\n");
        return -1;
    }

    req.cmd = IEEE80211_DBGREQ_TR069;
    req.needs_reply = DBGREQ_REPLY_IS_NOT_REQUIRED;
    req.data.tr069_req.cmdid = TR069_SET_CSA_DEAUTH;
    req.data.tr069_req.data_size = sizeof(uint8_t);
    req.data.tr069_req.data_buff[0] = csa_deauth_mode;
...
/**
 * get_csa_deauth(): get csa deauth
 * @ifname: interface name
 * @argc: total number of command line arguments
 * @argv: values of command line arguments
 * return -1 on error; otherwise return 0 on success
 */
int get_csa_deauth(const char *ifname, int argc, char *argv[])
{
    int err;
    struct ieee80211req_athdbg req = { 0 };
    uint8_t *csa_deauth_mode = NULL;

    if ((argc != 3)) {
        fprintf(stderr, "usage: wifitool athX g_csa_deauth");
        return -1;
    }

    req.cmd = IEEE80211_DBGREQ_TR069;
    req.needs_reply = DBGREQ_REPLY_IS_REQUIRED;
    req.data.tr069_req.cmdid = TR069_GET_CSA_DEAUTH;
    req.data.tr069_req.data_size = sizeof(uint8_t);

    err = send_command(&sock_ctx, ifname, &req, sizeof(req), NULL, QCA_NL80211_VENDOR_SUBCMD_DBGREQ, IEEE80211_IOCTL_DBGREQ);

    csa_deauth_mode = (uint8_t *)&(req.data.tr069_req.data_buff);

    if (*csa_deauth_mode == CSA_DEAUTH_MODE_UNICAST)
        fprintf(stdout, "csa_deauth_mode was set to unicast\n");
    else if (*csa_deauth_mode == CSA_DEAUTH_MODE_BROADCAST)
        fprintf(stdout, "csa_deauth_mode was set to broadcast\n");
    else if (*csa_deauth_mode == CSA_DEAUTH_MODE_DISABLE)
        fprintf(stdout, "csa_deauth_mode is disabled\n");
...
#define PRINT_STRUCTURE_VALUE(structure, parameter) \
    printf(#parameter "=%d\n", structure->parameter);

#define GET_STRUCTURE_VALUE(structure, parameter, input) \
    structure->parameter = atoi(input);

void print_acl_params(ieee80211_acl_cli_param_t *cli)
{
    PRINT_STRUCTURE_VALUE(cli, probe_rssi_hwm);
    PRINT_STRUCTURE_VALUE(cli, probe_rssi_lwm);
    PRINT_STRUCTURE_VALUE(cli, inact_rssi_xing);
    PRINT_STRUCTURE_VALUE(cli, low_rssi_xing);
    PRINT_STRUCTURE_VALUE(cli, low_rate_rssi_xing);
    PRINT_STRUCTURE_VALUE(cli, high_rate_rssi_xing);
    PRINT_STRUCTURE_VALUE(cli, auth_block);
    PRINT_STRUCTURE_VALUE(cli, auth_rssi_hwm);
    PRINT_STRUCTURE_VALUE(cli, auth_rssi_lwm);
    PRINT_STRUCTURE_VALUE(cli, auth_reject_reason);

}

void get_acl_params(ieee80211_acl_cli_param_t *cli, char *argv[], int start_arg)
{
    GET_STRUCTURE_VALUE(cli, probe_rssi_hwm, argv[start_arg++]);
    GET_STRUCTURE_VALUE(cli, probe_rssi_lwm, argv[start_arg++]);
    GET_STRUCTURE_VALUE(cli, inact_rssi_xing, argv[start_arg++]);
    GET_STRUCTURE_VALUE(cli, low_rssi_xing, argv[start_arg++]);
    GET_STRUCTURE_VALUE(cli, low_rate_rssi_xing, argv[start_arg++]);
    GET_STRUCTURE_VALUE(cli, high_rate_rssi_xing, argv[start_arg++]);
    GET_STRUCTURE_VALUE(cli, auth_block, argv[start_arg++]);
    GET_STRUCTURE_VALUE(cli, auth_rssi_hwm, argv[start_arg++]);
    GET_STRUCTURE_VALUE(cli, auth_rssi_lwm, argv[start_arg++]);
    GET_STRUCTURE_VALUE(cli, auth_reject_reason, argv[start_arg++]);
}


int acl_get_cmd(const char *ifname, int argc, char *argv[])
{
    struct ieee80211req_athdbg req = { 0 };
    ...
    req.cmd = IEEE80211_DBGREQ_ACL_GET_CLI_PARAMS;
    req.needs_reply = DBGREQ_REPLY_IS_REQUIRED;
    if (!wifitool_mac_aton(argv[3], &req.dstmac[0], 6)) {
        errx(1, "Invalid mac address");
        return -1;
    ...
    send_command(&sock_ctx, ifname, &req, sizeof(req), NULL, QCA_NL80211_VENDOR_SUBCMD_DBGREQ, IEEE80211_IOCTL_DBGREQ);
    print_acl_params(&req.data.acl_cli_param);

    return 0;
}

int acl_set_cmd(const char *ifname, int argc, char *argv[])
{
    struct ieee80211req_athdbg req = { 0 };

    if (argc != 14) {
        printf("ERROR! %s wrong number of args\n", __func__);
        return -1;
    }
    req.cmd = IEEE80211_DBGREQ_ACL_SET_CLI_PARAMS;
    req.needs_reply = DBGREQ_REPLY_IS_NOT_REQUIRED;
    if (!wifitool_mac_aton(argv[3], &req.dstmac[0], 6)) {
        errx(1, "Invalid mac address");
        return -1;
    }
    get_acl_params(&req.data.acl_cli_param, argv, 4);
    print_acl_params(&req.data.acl_cli_param);
    send_command(&sock_ctx, ifname, &req, sizeof(req), NULL, QCA_NL80211_VENDOR_SUBCMD_DBGREQ, IEEE80211_IOCTL_DBGREQ);
...
void print_custom_chan_list_request(struct ieee80211req_athdbg *req)
{
    int i;
    u_int8_t len = req->data.custom_chan_req.scan_numchan_associated;
    struct ieee80211_chan_def *chans = &req->data.custom_chan_req.scan_channel_list_associated[0];

    fprintf(stderr,"Associated list len: %d\n", len);
    for (i = 0; i < len; i++) {
        fprintf(stderr,"band: %d, chan: %d\n", chans[i].band, chans[i].chan);
    }

    len = req->data.custom_chan_req.scan_numchan_nonassociated;
    chans = &req->data.custom_chan_req.scan_channel_list_nonassociated[0];

    fprintf(stderr,"Nonassociated list len: %d\n", len);
    for (i = 0; i < len; i++) {
        fprintf(stderr,"band: %d, chan: %d\n", chans[i].band, chans[i].chan);
    }
}
...
void print_custom_chan_list_help (struct ieee80211req_athdbg *req)
{
    print_custom_chan_list_request(req);
    printf("wifitool athX custom_chan_list\n");
    printf("\t -a followed by optional \'-band <band value>\' \n");
    printf("\t <number of channels> <ch1> <ch2> ..<chN>\n");
    printf("\t -n followed by optional \'-band <band value>\' \n");
    printf("\t <number of channels> <ch1> <ch2> ..<chN>\n");
    printf("\t **NOTE: If band option is not provided, channel numbers default to 2G/5G band\n");
    printf("\t **NOTE: Band value definition: 1 -> 2GHz, 2 -> 5 GHz, 3 -> 6 GHz\n");
    printf("Example: \n");
    printf("\tset channel 6, 9 from band 1, channel 36, 40 from band 2 in assoc list\n");
    printf("\t\twifitool ath0 custom_chan_list -a -band 1 2 6 9 -band 2 2 36 40\n");
    printf("\tset channel 219, 233 from band 3, channel 36, 40 from band 2 in non assoc list\n");
    printf("\t\twifitool ath0 custom_chan_list -n -band 3 2 219 233 -band 2 2 36 40\n");
    printf("\tabove can be combined as below\n");
    printf("\t\twifitool ath0 custom_chan_list -a -band 1 2 6 9 -band 2 2 36 40\n");
    printf("\t\t\t-n -band 3 2 219 233 -band 2 2 36 40\n");
}
...
void custom_chan_list(const char *ifname, int argc, char *argv[])
{
    struct ieee80211req_athdbg req = { 0 };
    int i, j;
    u_int8_t band = 0, nchan = 0;
    u_int8_t *size = NULL;
    int val = 0;
    struct ieee80211_chan_def *chans = NULL;
    bool alist = false, nlist = false;
    #define MAX_VALID_CHANNEL_NUMBER 253

    if (argc < 6) {
        print_custom_chan_list_help(&req);
        return;
    }
    ...
    for (i = 3; i < argc; i++) {
        if (streq(argv[i], "-a")) {
            alist = true;
            band = 0;
            size = &req.data.custom_chan_req.scan_numchan_associated;
            chans = &req.data.custom_chan_req.scan_channel_list_associated[0];
            *size = 0;
            i++;
        } else if (streq(argv[i], "-n")) {
            nlist = true;
            band = 0;
            size = &req.data.custom_chan_req.scan_numchan_nonassociated;
            chans = &req.data.custom_chan_req.scan_channel_list_nonassociated[0];
            *size = 0;
            i++;
        } else {
        ...
        if (alist || nlist) {
            ...
            val = strtoul(argv[i], NULL, 10);
            if (val < 1 || val > MAX_CUSTOM_CHANS) {
                print_custom_chan_list_help(&req);
                return;
            }
            nchan = val;
            i++;
            /* next val elements are channel numbers for previous band */
            for (j = 0; j < nchan && i < argc; j++) {
                val = strtoul(argv[i], NULL, 10);
                if (val < 1 || val > MAX_VALID_CHANNEL_NUMBER) {
                    print_custom_chan_list_help(&req);
                    return;
                }
                i++;
                chans[*size].band = band;
                chans[*size].chan = val;
                ++*size;
            }
            i--; /* put back so that top for loop takes it back. */
        }
    }
    print_custom_chan_list_request(&req);
    send_command(&sock_ctx, ifname, &req, sizeof(req), NULL, QCA_NL80211_VENDOR_SUBCMD_DBGREQ, IEEE80211_IOCTL_DBGREQ);
...
enum {
    ...
    FIPS_MALLOC_FAIL=9,
    FIPS_TEST_FAIL=10,
    FIPS_INVALID_PNLEN=11,
    FIPS_INVALID_HDRLEN=12,
    FIPS_INVALID_PAYLOADLEN=13,
    FIPS_INVALID_EXPOP_LEN=14,
    FIPS_INVALID_KEYIX=15,
    FIPS_INVALID_CIPHER=16,
    FIPS_INVALID_PN_MISMATCH=17,
    FIPS_INVALID_HDR_MISMATCH=18,
    FIPS_INVALID_PAYLOAD_MISMATCH=19,
};
...
char *fips_error[]={
    ...
    "fips_invalid_pnlen",
    "fips_invalid_hdrlen",
    "fips_invalid_payloadlen",
    "fips_invalid_expoplen",
    "fips_invalid_keyix",
    "fips_invalid_cipher",
    "fips_invalid_pn_mismatch",
    "fips_invalid_hdr_mismatch",
    "fips_invalid_payload_mismatch",
};
...
static void fips_ccmp_encr_err(u_int8_t* exp_op,
                               int header_len, int data_len,
                               int fips_op_len, u_int8_t* fips_op)
{
    int i = 0;
    int err_byte = 0;

    for (i = 0; i < fips_op_len; i++) {
        if (exp_op[i] != *(fips_op+i)) {
            printf("\nmismatching byte is %d\n", i+1);
            err_byte = i+1;
            if (err_byte <= header_len)
                printf("%s : possible encr err in header\n", __func__);
            else if ((err_byte > header_len) &&
                     (err_byte <= header_len + 3))
                printf("%s : possible encr err in PN\n", __func__);
            else if (err_byte == header_len + 4)
                printf("%s : possible encr err in KeyId\n", __func__);
            else if ((err_byte > header_len + 8) &&
                     (err_byte <= header_len + data_len))
                printf("%s : possible encr err in payload\n", __func__);
            else if (err_byte > header_len + data_len)
                printf("%s : possible encr err in MIC/FCS\n", __func__);
        }
    }
}
...
int set_fips(const char *ifname, int argc, char *argv[])
{
    ...
    u_int8_t pn[8] = {0};
    u_int8_t header[32] = {0};
    u_int8_t *payload = NULL;
    unsigned int pn_len, header_len, exp_op_len;
    unsigned int key_idx, cipher_type;
    ...
    if(argc < 4) {
        ...
        while (ftell(fp) < size)
        {
            ...
            if ((cmd < 0) || (cmd > 1)  || (mode <1) || (mode > 2)) {
                en=FIPS_RANGE_CHECK_FAIL;
                goto close_n_exit;
            }
            if (mode == FIPS_MODE_ECB_AES) {
            ...
            } else if (mode == FIPS_MODE_CCM_GCM) {
                if (-1 == get_number(fp, 0, &pn_len, &eol) || eol) {
                    en=FIPS_INVALID_PNLEN;
                    goto close_n_exit;
                }
                if (-1 == get_number(fp, 0, &header_len, &eol) || eol) {
                    en=FIPS_INVALID_HDRLEN;
                    goto close_n_exit;
                }
                if (-1 == get_number(fp, 0, &datalen, &eol) || eol) {
                    en=FIPS_INVALID_PAYLOADLEN;
                    goto close_n_exit;
                }
                ...
                if (-1 == get_number(fp, 0, &exp_op_len, &eol) || eol) {
                    en=FIPS_INVALID_EXPOP_LEN;
                    goto close_n_exit;
                }
                if (-1 == get_number(fp, 0, &key_idx, &eol) || eol) {
                    en=FIPS_INVALID_KEYIX;
                    goto close_n_exit;
                }
                if (-1 == get_number(fp, 0, &cipher_type, &eol) || eol) {
                    en=FIPS_INVALID_CIPHER;
                    goto close_n_exit;
                }
                ...
                if (pn_len != get_xstream_2_xbytes(fp, pn, pn_len*2, &eol) || eol){
                    en=FIPS_INVALID_PN_MISMATCH;
                    goto close_n_exit;
                }
                if (header_len != get_xstream_2_xbytes(fp, header, header_len*2, &eol) || eol){
                    en=FIPS_INVALID_HDR_MISMATCH;
                    goto close_n_exit;
                }
                if (datalen != get_xstream_2_xbytes(fp, payload, datalen*2, &eol) || eol) {
                    en=FIPS_INVALID_PAYLOAD_MISMATCH;
                    goto close_n_exit;
                }
                if (exp_op_len != get_xstream_2_xbytes(fp, exp_op, (exp_op_len)*2, &eol)) {
                    en=FIPS_INVALID_EXPOP_MISMATCH;
                    goto close_n_exit;
                }
            ...
            req = (struct ath_ioctl_fips *) malloc( sizeof(struct ath_ioctl_fips)
                    + (datalen - sizeof(u_int32_t)));
            if (!req)  {
                en=FIPS_MALLOC_FAIL;
                goto close_n_exit;
            }
            ...
            if (mode == FIPS_MODE_ECB_AES) {
                ...
                req->key_idx = key_idx;
                req->key_cipher = cipher_type;
                req->pn_len = pn_len;
                memcpy(req->pn, pn, req->pn_len);
                req->header_len = header_len;
                memcpy(req->header, header, req->header_len);
                req->data_len = datalen;
                memcpy(req->data, payload, req->data_len);
            }
            ...
            if (req) {
                ...
                if (output_fips->error_status == 0) {
                    ...
                    if ((argc > 4) && streq(decrypt, "decrypt") && (mode == FIPS_MODE_CCM_GCM)) {
                        /* for FIPS_MODE_CCM_GCM decrypt case, we donot check MIC and FCS bytes */
#define FIPS_MODE_CCM 4

#define FIPS_CCM_GCM_128 16
#define FIPS_CCM_GCM_256 32

#define CCM_128_MIC_LEN 8
#define GCM_MIC_LEN 16
#define FCS_LEN 4
                        if ((cipher_type == FIPS_MODE_CCM) && (keylen == FIPS_CCM_GCM_128)) {
                            /* 8 bytes mic + 4 bytes fcs */
                            output_fips->data_len -= CCM_128_MIC_LEN + FCS_LEN;
                        } else {
                            /* 16 bytes mic + 4 bytes fcs */
                            output_fips->data_len -= GCM_MIC_LEN + FCS_LEN;
                        }
                    }
                    ...
                    /* check error bits for ccmp fips in case of failure */
                    if ((output_fips->data_len < sizeof(exp_op)) &&
                        (fips_fail == 1) && (mode == FIPS_MODE_CCM_GCM)) {
                        fips_op = (uint8_t *)malloc(output_fips->data_len);
                        if (fips_op) {
                            memcpy(fips_op, output_fips->data, output_fips->data_len);
                            fips_ccmp_encr_err(exp_op, header_len, datalen, output_fips->data_len, fips_op);
                        }
                    }

                    if ((argc < 5) || (mode == FIPS_MODE_CCM_GCM)) {
                        printf("\n Expected Output \n");
                        if (mode == FIPS_MODE_ECB_AES)
                            exp_op_len = datalen;
                        for (i = 0; i < exp_op_len; i++)
                            printf("%02x ", exp_op[i]);
                        if (fips_fail == 1)
                            printf("\n Known Answer Test Failed\n");
                        else
                            printf("\n Known Answer Test Passed\n");
                    } else if ((argc > 4) && (mode == FIPS_MODE_ECB_AES)) {
                        /* for FIPS_MODE_ECB_AES, block output to be xor with
                         * plaintext (encr) or ciphertext (decr):CTR_AES */
            ...
            /* Freeing allocated payload in FIPS_MODE_CCM_GCM mode*/
            if(payload) {
                free(payload);
                payload=NULL;
            }
            /* Freeing allocated fips_op in FIPS_MODE_CCM_GCM mode error */
            if(fips_op) {
                free(fips_op);
                fips_op=NULL;
            }
...
#define MAX_OFFCHAN_PARAM_COUNT  7
    struct ieee80211req_athdbg req = { 0 };
    ...
    if (streq(argv[3], "-band") || streq(argv[3], "--band")) {
        if (argc < MAX_OFFCHAN_PARAM_COUNT) {
            usage_offchan_tx_test();
            return -1;
        }
        offchan_req->band = atoi(argv[4]);
        offchan_req->ieee_chan = atoi(argv[5]);
        offchan_req->dwell_time = atoi(argv[6]);
    } else {
        offchan_req->band = 0;
...
{
#define MIN_OFFCHAN_PARAM_COUNT  5
#define MAX_OFFCHAN_PARAM_COUNT  8
    int status;
    struct ieee80211req_athdbg req = { 0 };
    ieee80211_offchan_tx_test_t *offchan_req;
    ...
    offchan_req->ieee_chan = atoi(argv[i]); i++;
    offchan_req->dwell_time = atoi(argv[i]); i++;

    if(argv[i]) {
        status = offchan_rx_get_params(argv[i], &bw_mode, &sec_chan_offset);
        if (status == -1) {
...
int ap_twt_dialog_req_cmd (const char *ifname, int argc, char *argv[], int req_cmd)
{
    ...
    switch (req_cmd) {
        case IEEE80211_DBGREQ_TWT_ADD_DIALOG:
            ...
            req.data.twt_add.flags = (uint32_t)strtoul(argv[10], NULL, 0);
            break;
...
static void get_last_four_beacon_rssi(const char *ifname, int argc, char *argv[])
{
    ...
    mask = 0xFF;
    for (i = 0; i < 4; i++) {
        fprintf(stdout, "%-10d%-10u%-10u%-10u%-10"PRIu64"\n", (i + 1), info->stats.ni_last_bcn_rssi & mask,
...
static void get_innetwork_2g_mac(const char *ifname, int argc, char *argv[]) {
    ...
    if (!param) {
        ...
        u_int8_t mac_addr[128][QDF_MAC_ADDR_SIZE] = {0};
        ...
        if (num_entries != 0) {
            ...
            for (i = 0; i < num_entries; i++) {
                if (!memcmp(zeroMAC, mac_addr[i], QDF_MAC_ADDR_SIZE)) {
                    continue;
                }
...
int main(int argc, char *argv[])
{
    ...
    } else if (streq(cmd, "sendcustneigrpt")) {
        send_custom_neigh_resp(ifname, argc, argv, IEEE80211_DBGREQ_SEND_CUSTOM_NEIGRPT);
    ...
    } else if (streq(cmd, "frame_injector_en")) {
        enable_frame_injector(ifname, argc, argv);
    ...
    } else if (streq(cmd, "acl_get_cmd")) {
        acl_get_cmd(ifname, argc, argv);
    } else if (streq(cmd, "acl_set_cmd")) {
        acl_set_cmd(ifname, argc, argv);
    ...
    } else if (streq(cmd, "g_csa_deauth")) {
        get_csa_deauth(ifname, argc, argv);
    } else if (streq(cmd, "csa_deauth")) {
        set_csa_deauth(ifname, argc, argv);
    } else if (streq(cmd, "fake_mgmt_rx")) {
        fake_mgmt_rx(ifname, argc, argv);
    } else {
        usage();
    }
