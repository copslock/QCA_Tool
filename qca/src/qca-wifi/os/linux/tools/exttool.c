/*
 * Copyright (c) 2016-2018, 2020 Qualcomm Innovation Center, Inc.
 *
 * All Rights Reserved.
...
int validate_scan_param( wifi_scan_custom_request_cmd_t *scan_req)
{
    if ((scan_req->min_dwell_time <= 0) || (scan_req->min_dwell_time < 50)) {
        fprintf(stdout, "Invalid min dwell time %d\n", scan_req->min_dwell_time);
        return INVALID_ARG;
    }

    if ((scan_req->max_dwell_time < scan_req->min_dwell_time) || (scan_req->max_dwell_time > 10000)) {
        fprintf(stdout, "Invalid max dwell time %d (min dewll: %d)\n",
                scan_req->max_dwell_time , scan_req->min_dwell_time);
        return INVALID_ARG;
    }
    ...
    if (scan_req->max_scan_time < scan_req->rest_time) {
        fprintf(stdout, "Invalid max_scan_time: %d (rest_time: %d)\n",
                scan_req->max_scan_time, scan_req->rest_time);
        return INVALID_ARG;
    }
    if (scan_req->idle_time < 50) {
        fprintf(stdout, "Invalid idle_time (must be >= 50)\n");
        return INVALID_ARG;
    }
    return 0;
}
        ...
        switch (option) {
            ...
            case 'e': /* channel to scan  */
                ...
                while (count < extended_cmd.ext_data.channel_scan_req.chanlist.n_chan) {
                    ...
                    extended_cmd.ext_data.channel_scan_req.chanlist.chans[count].band = band;
                    extended_cmd.ext_data.channel_scan_req.chanlist.chans[count].chan =
