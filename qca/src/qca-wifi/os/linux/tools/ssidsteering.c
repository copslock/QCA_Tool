...
static void
ieee80211_vap_send_mac_addr(const char *ifname, unsigned char macaddr[QDF_MAC_ADDR_SIZE])
{
#if UMAC_SUPPORT_WEXT
    struct iwreq iwr = { 0 };
    struct sockaddr addr;
    memset(&addr, 0, sizeof(addr));
    memcpy(&addr.sa_data, macaddr, QDF_MAC_ADDR_SIZE);
    int sck_fd = 0;
#endif
    ...
    err = init_socket_context(&sock_ctx, DEFAULT_NL80211_CMD_SOCK_ID, DEFAULT_NL80211_EVENT_SOCK_ID);
    if (err == -EIO) {
    ...
    if (sock_ctx.cfg80211) {
        ...
        if ((ret=wifi_cfg80211_send_setparam_command(&(sock_ctx.cfg80211_ctxt),
            QCA_NL80211_VENDORSUBCMD_ADDMAC, 0,
            ifname, (char *)&buffer, sizeof(int))) < 0) {
            perror(" Error in sending vdev config with NL \n");
...
ieee80211_vap_send_ssid_config(char *ifname,  int vap_config)
{
    ...
    if (sock_ctx.cfg80211) {
        ...
        if ((ret=wifi_cfg80211_send_setparam_command(&(sock_ctx.cfg80211_ctxt),
            QCA_NL80211_VENDOR_SUBCMD_WIFI_PARAMS, IEEE80211_PARAM_VAP_SSID_CONFIG,
            ifname, (char *)&buffer, sizeof(int))) < 0) {
            perror(" Error in sending vdev config with NL \n");
            destroy_socket_context(&sock_ctx);
            return;
        ...
        if (vap_config == SON_SSID_STEERING_PUBLIC_VDEV) {
            buffer.data = &maccmd_val;
            buffer.length = sizeof(int);
            buffer.parse_data = 0;
            buffer.callback = NULL;
            buffer.parse_data = 0;
            if ((ret=wifi_cfg80211_send_setparam_command(&(sock_ctx.cfg80211_ctxt),
                QCA_NL80211_VENDOR_SUBCMD_WIFI_PARAMS, IEEE80211_PARAM_MACCMD,
                ifname, (char *)&buffer, sizeof(int))) < 0) {
                perror(" Error in sending maccmd with NL \n");
                destroy_socket_context(&sock_ctx);
                return;
            }
        }
        ...
        if (ioctl(sck_fd, IEEE80211_IOCTL_SETPARAM, &iwr) < 0) {
    ...
    while(fgets(temp, 512, fp) != NULL) {
        ...
        if((strstr(temp, private)) != NULL) {
            ...
            for(ssid_config_data.private_num_vaps = 0; ssid_config_data.private_num_vaps < i; ssid_config_data.private_num_vaps++){
                ieee80211_vap_send_ssid_config(private_iface[ssid_config_data.private_num_vaps], SON_SSID_STEERING_PRIVATE_VDEV);
            }
        }
            ...
            for(ssid_config_data.public_num_vaps = 0; ssid_config_data.public_num_vaps < i; ssid_config_data.public_num_vaps++){
                ieee80211_vap_send_ssid_config(public_iface[ssid_config_data.public_num_vaps], SON_SSID_STEERING_PUBLIC_VDEV);
            }
        }
        find_result++;
    }
