...
#define IOCTL_DPRINTF(_fmt, ...) do { qdf_nofl_info(_fmt, __VA_ARGS__); } while (0)
#else
...
struct _vendor_generic_ioctl {
    u_int32_t   tm_is_rx                :1,
                ...
                scan_is_set             :1;
    u_int8_t    tm_channel;
    u_int8_t    tm_bssid[QDF_MAC_ADDR_SIZE];
...
static int _vendor_ioctl_getparam_versinfo(struct net_device * dev, athcfg_wcmd_data_t * iiData)
{
    ...
    strlcpy(iiData->version_info.version, ATH_DRIVER_VERSION, strlen(ATH_DRIVER_VERSION));
    strlcpy(iiData->version_info.fw_version, ATH_DRIVER_VERSION, strlen(ATH_DRIVER_VERSION));
    strlcpy(iiData->version_info.driver, dev_info, strlen(dev_info));
...
static int _vendor_ioctl_getparam_prodinfo(struct net_device * dev, athcfg_wcmd_data_t * iiData)
{
...
#ifdef ATH_USB
    {
        ...
        if (udev->manufacturer)
            strlcpy(iiData->product_info.manufacturer, udev->manufacturer, strlen(udev->manufacturer));
        if (udev->product)
            strlcpy(iiData->product_info.product, udev->product, strlen(udev->product));
        if (udev->serial)
            strlcpy(iiData->product_info.serial, udev->serial, strlen(udev->serial));
                                        ...
                                        ieee80211_recv_status *rs)
{
    ...
    if ((osvsp->tm_is_rx) &&
        ...
        (OS_MEMCMP(osvsp->tm_bssid, wh->i_addr2, QDF_MAC_ADDR_SIZE) == 0))
    {
