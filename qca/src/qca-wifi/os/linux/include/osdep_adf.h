/*
 * Copyright (c) 2013-2016,2017,2019,2020 Qualcomm Innovation Center, Inc.
 * All Rights Reserved
...
cmpxchg(int32_t *_patomic_arg, int32_t _comparand, int32_t _exchange)
{
...
if (!spin_is_locked(x)) { \
   WARN_ON(1); \
   qdf_nofl_info(KERN_EMERG " %s:%d unlock addr=%pK, %s \n", __func__, __LINE__, x, \
...
if (!spin_is_locked(x)) { \
  WARN_ON(1); \
  qdf_nofl_info(KERN_EMERG " %s:%d unlock_irqrestore addr=%pK, flags=%lx, %s\n", \
...
static INLINE int OS_MESGQ_SEND(os_mesg_queue_t *queue,u_int16_t type, u_int16_t len,  void *msg)
{
    ...
    if (queue->is_synchronous ) {
        ...
        if (mesg) {
            ...
            spin_unlock(&(queue->lock));
            qdf_nofl_info("No more message queue buffers !!! \n");
            return -ENOMEM;
        }
