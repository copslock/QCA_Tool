...
qdf_export_symbol(sa_default_antenna);
...
qdf_export_symbol(sa_enable_test_mode);
...
int32_t sa_update_ratestats(struct sa_node_ccp *node, struct sa_tx_feedback *feedback)
{
    ...
    SA_DEBUG_PRINT(SA_DEBUG_LVL4,node->node_info.radio_id," %s:  Mac: %s, nFrame:%d nBad: %d SR: [%d %d %d %d %d %d] LR: [%d %d %d %d %d %d]"
            ...
            ,feedback->is_trainpkt, feedback->ratemaxphy[0]
            ,feedback->goodput);
    ...
    if ((feedback->ratemaxphy[0] & SA_BYTE_MASK) && (feedback->ratemaxphy[1] & SA_BYTE_MASK) && (feedback->ratemaxphy[2] & SA_BYTE_MASK)) {
        node->rate_stats.rate_maxphy = (uint32_t)((feedback->ratemaxphy[0] & SA_BYTE_MASK) ||
						  ((feedback->ratemaxphy[1] & SA_BYTE_MASK) << 8) ||
						  ((feedback->ratemaxphy[2] & SA_BYTE_MASK) << 16));
    } else {
        SA_DEBUG_PRINT(SA_DEBUG_LVL3,node->node_info.radio_id, " Error: %s invalid ratemax phy in normal pkt 0x%04x",print_mac_addr((const uint8_t *)node->node_info.mac_addr),
			feedback->ratemaxphy[0]);
    }
    return 0;
}
...
int __sa_update_txfeedback(void *ccp, struct sa_tx_feedback *feedback, uint8_t *status)
{
    ...
    if (sa_enable_test_mode) {
        SA_DEBUG_PRINT(SA_DEBUG_LVL4,node->node_info.radio_id,"%s:  Mac: %s, nFrame:%d nBad: %d SR: [%d %d %d %d %d %d] LR: [%d %d %d %d %d %d]"
                "Rate:[0x%x 0x%x] RateIdx: %d Antenna: [0x%06x 0x%06x] RSSI: [%d %d %d]"
                "Trainpkt: %d ratemaxphy: [0x%03x|0x%03x|0x%03x|0x%03x] goodput:%d \n"
                ,__func__ , print_mac_addr((const uint8_t *)&(node->node_info.mac_addr)), feedback->nPackets, feedback->nBad
                ,feedback->nshort_retries[0], feedback->nshort_retries[1], feedback->nshort_retries[2]
                ,feedback->nshort_retries[4], feedback->nshort_retries[5], feedback->nshort_retries[6]
                ,feedback->nlong_retries[0], feedback->nlong_retries[1], feedback->nlong_retries[2]
                ...
                ,feedback->tx_antenna[0], feedback->tx_antenna[1]
                ,feedback->rssi[0], feedback->rssi[1], feedback->rssi[2]
                ,feedback->is_trainpkt, feedback->ratemaxphy[0], feedback->ratemaxphy[1], feedback->ratemaxphy[2], feedback->ratemaxphy[3]
                ,feedback->goodput);
        return 0;
    }
...
int  __sa_update_rxfeedback(void  *ccp, struct sa_rx_feedback *rx_feedback, uint8_t *status)
{
    ...
    SA_DEBUG_PRINT(SA_DEBUG_LVL5,node->node_info.radio_id,"%s: Mac: %s Rate:0x%x Rate Index:%d Antenna:0x%06x npakcets:%d", __func__,
                   print_mac_addr((const uint8_t *)&(node->node_info.mac_addr)), rx_feedback->rx_rate_mcs, rx_feedback->rx_rate_index,
                   rx_feedback->rx_antenna, rx_feedback->npackets);
    SA_DEBUG_PRINT(SA_DEBUG_LVL5,node->node_info.radio_id,"%s: Rssi   bw0 bw1 bw2 bw3", __func__);
    for (chain = 0; chain < SA_MAX_RECV_CHAINS; chain++) {
        for (bw = 0; bw < 4; bw++) {
            SA_DEBUG_PRINT(SA_DEBUG_LVL5,node->node_info.radio_id,"%s: Chain%d [%d][%d][%d][%d]", __func__, chain,
                   rx_feedback->rx_rssi[chain][bw], rx_feedback->rx_rssi[chain][bw + 1],rx_feedback->rx_rssi[chain][bw + 2], rx_feedback->rx_rssi[chain][bw + 3]);
	    bw += 3;
	}
    }
    SA_DEBUG_PRINT(SA_DEBUG_LVL5,node->node_info.radio_id,"\n");
    return SA_STATUS_SUCCESS;
}
...
int  __sa_get_training_info(void *ccp , uint32_t *rate_array, uint32_t *antenna_array, uint32_t *numpkts)
{
    ...
    if (sa_enable_test_mode) {
        rate_array[0] = (g_train_params.ratecode[0] | (g_train_params.ratecode[1] << 16));
        rate_array[1] = (g_train_params.ratecode[2] & SA_WORD_MASK);
        rate_array[2] = (g_train_params.ratecode[0] | (g_train_params.ratecode[1] << 16));
        rate_array[3] = (g_train_params.ratecode[2] & SA_WORD_MASK);
...
/**
 *   __sa_peer_connect: api function that indicates node addition
 *   @ccp: pointer to pointer of node
 *   @node_info: node info
 *
 *   returns success or failure
 */
int __sa_peer_connect(void  **ccp, struct  sa_node_info *node_info)
{
    ...
    SA_DEBUG_PRINT(SA_DEBUG_LVL2,node->node_info.radio_id,"%s Mac: %s RadioId: %d max bw: %d chainmask: %d opmode: %d  channel : %d band : %d\n",
            __func__, print_mac_addr((const uint8_t *)node_info->mac_addr), node_info->radio_id, node_info->max_ch_bw,
            node_info->chainmask, node_info->opmode, node_info->channel_num, node_info->band);
...
/**
 *   __sa_peer_disconnect: api function that indicates node disconnect
 *   @ccp: pointer to node
 *
 *   returns success or failure
 */
int  __sa_peer_disconnect (void *ccp)
{
    struct sa_node_ccp *node = (struct sa_node_ccp *) ccp;
    struct radio_config *radio;
    ...
    if (ccp) {
        ...
        SA_DEBUG_PRINT(SA_DEBUG_LVL2,node->node_info.radio_id,"%s Mac: %s RadioId: %d max bw: %d chainmask: %d opmode: %d  channel : %d  band : %d\n",
                ...
                node->node_info.chainmask, node->node_info.opmode, node->node_info.channel_num, node->node_info.band);
...
void sa_update_ant_map(struct sa_node_ccp *node,uint32_t selected_antenna)
{
    ...
    if (i == g_sa.radio[node->node_info.radio_id].sa_params.num_tx_antennas) {
        SA_DEBUG_PRINT(SA_DEBUG_LVL2,node->node_info.radio_id," %s: Changing selected antenna to ant_map[0] = %d\n",
                       __func__, node->train_info.train_state.ant_map[i]);
    }
...
int  __sa_set_param (struct sa_config_params *params)
{
    ...
    switch (params->param_id)
    {
        ...
        case SMART_ANT_PARAM_TRAIN_RATE_TESTMODE:
            if (sa_enable_test_mode) {
                rate_index = SA_GET_BW_FROM_RC_PARAM(params->param_value >> 16);
		if (rate_index >= SA_MAX_BW) {
                    SA_DEBUG_PRINT(SA_DEBUG_LVL1,params->radio_id,"Incorrect Param; Rate code from bit 0-11 & bit 16-17 for Bandwidth\n");
                    SA_DEBUG_PRINT(SA_DEBUG_LVL1,params->radio_id,"00 - Legacy\\20MHz\n01 - 40MHz\n10 - 80MHz\n");
                    return SA_STATUS_FAILURE;
		}
		g_train_params.ratecode[rate_index] = (uint16_t)(params->param_value & SA_WORD_MASK);
            } else {
                return SA_STATUS_FAILURE;
            }
            break;
...
int  __sa_get_param (struct sa_config_params *params)
{
    ...
    switch (params->param_id)
    {
        ...
        case SMART_ANT_PARAM_TRAIN_RATE_TESTMODE:
            if (sa_enable_test_mode) {
		SA_DEBUG_PRINT(SA_DEBUG_LVL1,params->radio_id,"Legacy\\20Mhz 0x%x 40Mhz 0x%x 80Mhz 0x%x\n",
                               g_train_params.ratecode[0], g_train_params.ratecode[1], g_train_params.ratecode[2]);
            } else {
                retval = SA_STATUS_FAILURE;
            }
            break;
...
int __sa_init (struct sa_config *sa_config, int new_init)
{
    ...
    SA_DEBUG_PRINT(SA_DEBUG_LVL1,(SA_GET_RADIO_ID(sa_config->radio_id)),"%s %s: RadioId: %d max_fallback_rates: %d channel: %d band: %d bss mode: %d nss: %d chainmask: 0x%02x radiotype %d \n",
            __func__, (new_init ? "New Init" : "Re Init"), SA_GET_RADIO_ID(sa_config->radio_id),
            sa_config->max_fallback_rates, sa_config->channel_num, sa_config->band, sa_config->bss_mode,
...
struct smartantenna_ops sa_ops = {
    ...
    sa_peer_connect:__sa_peer_connect,
    sa_peer_disconnect:__sa_peer_disconnect,
