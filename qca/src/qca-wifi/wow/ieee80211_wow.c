...
ieee80211_wow_magic_parser(struct ieee80211_node *ni, wbuf_t wbuf)
{
    ...
    uint8_t    i_addr[QDF_MAC_ADDR_SIZE];
    ...
    while (cur_ptr && (left_len>0) && (!is_match)) {
        ...
        while (IEEE80211_IS_BROADCAST(cur_ptr) && (left_len>=IEEE80211_WOW_MAGIC_PKTLEN)) {
            is_bcast = TRUE;
            cur_ptr += QDF_MAC_ADDR_SIZE;
            left_len -= QDF_MAC_ADDR_SIZE;
        }
        ...
        if (is_bcast) {
            ...
            while (cur_ptr && (left_len>=QDF_MAC_ADDR_SIZE) && IEEE80211_ADDR_EQ(cur_ptr, i_addr)) {
                cur_ptr += QDF_MAC_ADDR_SIZE;
                left_len -= QDF_MAC_ADDR_SIZE;
                dup_cnt++;
