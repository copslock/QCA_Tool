 ...
 * evolving.
 *
 * Example usage: insmod umac.ko <other params...> ol_scan_freqlist=2412,2437,
 *                2462,5180 ..
 */
unsigned short ol_scan_freqlist[IEEE80211_CUSTOM_SCAN_ORDER_MAXSIZE];
int ol_scan_chanlist_size = 0;
module_param_array(ol_scan_freqlist,
        ushort, &ol_scan_chanlist_size, S_IRUSR | S_IWUSR | S_IRGRP | S_IROTH);
MODULE_PARM_DESC(ol_scan_freqlist,
        "offload: Specifies list of IEEE channels to scan in client mode");
qdf_export_symbol(ol_scan_freqlist);
qdf_export_symbol(ol_scan_chanlist_size);
...
qdf_export_symbol(ol_dual_band_5g_radios);
...
qdf_export_symbol(ol_dual_band_2g_radios);
...
extern void ieee80211_register_vap_wext_handlers(osif_dev *osifp,
                                                 bool is_register);
#endif
#if !NO_SIMPLE_CONFIG
...
#define IC_UINT64_FIELD(x) seq_printf(m, #x ": %llu\n", ic->x)
    ...
    IC_UINT_FIELD(ic_chanchange_chan_freq);
...
#if ATH_SUPPORT_DFS
    IC_UINT_FIELD(ic_scan_over_cac);
#endif
   return 0;
}
...
/**
 * wifi_supported_bands_show() - sysfs handler to read supported bands
 * @dev: device object
 * @attr:device_attribute object
 * @buf: output buffer
 *
 * Return: number of characters in output buf
 */
static ssize_t wifi_supported_bands_show(struct device *dev,
                               struct device_attribute *attr, char *buf)
{
    struct net_device *net = to_net_dev(dev);
    struct ol_ath_softc_net80211 *scn = ath_netdev_priv(net);
    ...
    struct wlan_supported_freq_bands *supported_bands = &ic->ic_supported_freq_bands;

    if(supported_bands->band_2ghz)
        strlcat(buf, "2GHz/", strlen("2GHz/") + strlen(buf) + 1);
    if(supported_bands->band_5ghz)
        strlcat(buf, "5GHz/", strlen("5GHz/") + strlen(buf) + 1);
    if(supported_bands->band_6ghz)
        strlcat(buf, "6GHz/", strlen("6GHz/") + strlen(buf) + 1);

    return strlen(buf);
}
static DEVICE_ATTR(supported_bands, S_IRUGO, wifi_supported_bands_show, NULL);
...
/**
 * Handler for sysfs entry edge channel deprioritize - returns edge channel
 * deprioritize applicable for the device
 */
static ssize_t wifi_edge_ch_deprioritize_show(struct device *dev,
                               struct device_attribute *attr, char *buf)
{
    struct ol_ath_softc_net80211 *scn = NULL;
    struct ieee80211com *ic = NULL;
    ...
    if (ic->ic_edge_ch_dep_applicable) {
        strlcpy(buf, "1", strlen("1") + strlen(buf) + 1);
    } else {
        strlcpy(buf, "0", strlen("0") + strlen(buf) + 1);
    }

    return strlen(buf);
}
static DEVICE_ATTR(edge_ch_dep_applicable, S_IRUGO, wifi_edge_ch_deprioritize_show, NULL);
...
/**
 * wifi_6g_maxchwidth_show() - Handler for sysfs entry 6g_maxchwidth
 * @dev: pointer to device object
 * @attr: pointer to device_attribute
 * @buf: output buffer
 *
 * sets buf with maximim chwidth supported in 6G band
 *
 * Return: size of buffer returned
 */
static ssize_t wifi_6g_maxchwidth_show(struct device *dev,
                               struct device_attribute *attr, char *buf)
{
    struct ol_ath_softc_net80211 *scn = NULL;
    struct ieee80211com *ic = NULL;
    ...
    supported_bands = &ic->ic_supported_freq_bands;
    if (!supported_bands->band_6ghz)
        return 0;

    hw_caps = ic->ic_modecaps;

    if (hw_caps & 1ULL << IEEE80211_MODE_11AXA_HE160) {
        strlcpy(buf, "160", sizeof("160"));
    }
    else if (hw_caps & 1 << IEEE80211_MODE_11AXA_HE80) {
        strlcpy(buf, "80", sizeof("80"));
    }
    else if (hw_caps &
        (1 << IEEE80211_MODE_11AXA_HE40 |
...
static DEVICE_ATTR(6g_maxchwidth, S_IRUGO, wifi_6g_maxchwidth_show, NULL);
...
/* Handler for sysfs entry is_512M_e_build - returns if the workspace compiled
 * for 512M E build or not.
 */
static ssize_t wifi_is_e_build_show(struct device *dev,
        struct device_attribute *attr, char *buf)
{
    strlcpy(buf, "1", strlen("1") + 1);
    return strlen(buf);
...
static DEVICE_ATTR(is_e_build, S_IRUGO, wifi_is_e_build_show, NULL);
#endif
...
/* Handler for sysfs entry txchains - returns the txchain value */
static ssize_t wifi_get_txchains(struct device *dev,
                                 struct device_attribute *attr, char *buf)
{
    struct ol_ath_softc_net80211 *scn = NULL;
    struct ieee80211com *ic = NULL;
    ...
    snprintf(buf, len, "%d", ic->ic_num_tx_chain);

    return strlen(buf);
}

static DEVICE_ATTR(txchains, S_IRUGO, wifi_get_txchains, NULL);

/* Handler for sysfs entry rxchains - returns the rxchain value */
static ssize_t wifi_get_rxchains(struct device *dev,
                                 struct device_attribute *attr, char *buf)
{
    struct ol_ath_softc_net80211 *scn = NULL;
    struct ieee80211com *ic = NULL;
    ...
    snprintf(buf, len, "%d", ic->ic_num_rx_chain);

    return strlen(buf);
}

static DEVICE_ATTR(rxchains, S_IRUGO, wifi_get_rxchains, NULL);
...
static struct attribute *wifi_device_attrs[] = {
    ...
    &dev_attr_edge_ch_dep_applicable.attr,   /*sysfs entry for edge channel deprioritize*/
    &dev_attr_5g_maxchwidth.attr, /*sysfs entry for displaying the max channel width supported in 5Ghz*/
    &dev_attr_6g_maxchwidth.attr, /*sysfs entry for displaying the max channel width supported in 6Ghz*/
...
#endif /* OL_ATH_SMART_LOGGING */
    &dev_attr_txchains.attr,
    &dev_attr_rxchains.attr,
#ifdef QCA_IPQ807X_E_BUILD
    &dev_attr_is_e_build.attr,
#endif
    &dev_attr_supported_bands.attr, /* sysfs entry for supported bands */
    NULL
};
...
qdf_export_symbol(ol_ath_thermal_mitigation_attach);
...
qdf_export_symbol(ol_ath_thermal_mitigation_detach);
...
static QDF_STATUS cfg_get_ini_file_name(uint32_t target_type, struct ini_file *ini)
{
    QDF_STATUS status = QDF_STATUS_SUCCESS;
    switch (target_type) {
        case TARGET_TYPE_QCA8074:
            ini->external = "QCA8074.ini";
        ...
        case TARGET_TYPE_QCA8074V2:
            ...
            break;
        case TARGET_TYPE_QCA6018:
            ini->external = "QCA6018.ini";
            ini->internal = "QCA6018_i.ini";
            break;
        case TARGET_TYPE_QCA9984:
            /* External ini file is common for all AR900B family */
            ini->external = "QCA9984.ini";
            /* Have specific internal ini file for target QCA9984 */
            ini->internal = "QCA9984_i.ini";
            break;
        case TARGET_TYPE_IPQ4019:
            /* External ini file is common for all AR900B family */
            ini->external = "QCA9984.ini";
            /* Have specific internal ini file for target IPQ4019*/
            ini->internal = "IPQ4019_i.ini";
            break;
        case TARGET_TYPE_AR900B:
            /* External ini file is common for all AR900B family */
            ini->external = "QCA9984.ini";
            /* Have specific internal ini file for target AR900B */
            ini->internal = "AR900B_i.ini";
            break;
        case TARGET_TYPE_AR9888:
            /* External ini file is common for all AR900B family */
            ini->external = "QCA9984.ini";
            /* Have specific internal ini file for target AR9888 */
            ini->internal = "QCA9888_i.ini";
            break;
        case TARGET_TYPE_QCN9000:
            ini->external = "QCN9000.ini";
            ini->internal = "QCN9000_i.ini";
            break;
        case TARGET_TYPE_QCA5018:
            ini->external = "QCA5018.ini";
            ini->internal = "QCA5018_i.ini";
            break;
        case TARGET_TYPE_QCA9888:
        case TARGET_TYPE_AR9888V2:
            /*
             * For some targets, target specific ini file is not required and
             * all parameters can be used from global ini
             */
        default:
            ini->external = NULL;
            ini->internal = NULL;
            status = QDF_STATUS_E_INVAL;
            qdf_info("%s: Target specific INI not defined for tgt_type %d",  __func__, target_type);
            break;
    }
    return status;
}
...
static ssize_t wifi_soc_ini_file_show(struct device *dev,
                               struct device_attribute *attr, char *buf)
{
    struct net_device *net = to_net_dev(dev);
    ol_ath_soc_softc_t *soc = ath_netdev_priv(net);
    ...
    status = cfg_get_ini_file_name(target_type, &ini);

    buf[0]='\0';

    if (QDF_IS_STATUS_SUCCESS(status)) {
        qdf_str_lcopy(buf, ini.internal, qdf_str_len(ini.internal) + 1);
        return qdf_str_len(buf);
    }

    return 0;

}
static DEVICE_ATTR(ini_file, S_IRUGO, wifi_soc_ini_file_show, NULL);


static ssize_t wifi_soc_pdev_map_show(struct device *dev,
                                      struct device_attribute *attr,
                                      char *buf)
{
    struct net_device *net = to_net_dev(dev);
    ...
    snprintf(str, sizeof(str), "hw_mode=%s %s\n-------------\n",
             (hw_mode_ctx->current_mode == WMI_HOST_HW_MODE_DBS)    ? "DBS"     :
             (hw_mode_ctx->current_mode == WMI_HOST_HW_MODE_DBS_SBS)? "DBS_SBS" :
             (hw_mode_ctx->current_mode == WMI_HOST_HW_MODE_MAX)    ? "MAX"     :
             (hw_mode_ctx->current_mode == WMI_HOST_HW_MODE_DETECT) ? "DETECT"  : "UNKNOWN",
             (hw_mode_ctx->recover_mode == WMI_HOST_HW_MODE_DBS)    ? "(recover_mode=1)" :
             (hw_mode_ctx->recover_mode == WMI_HOST_HW_MODE_DBS_SBS)? "(recover_mode=4)" : "");
    strlcat(buf, str, strlen(buf) + strlen(str) + 1);
    ...
    for (i = 0; i < WMI_HOST_MAX_PDEV; i++) {
        ...
        if (hw_mode_ctx->pdev_map[i] != WMI_HOST_PDEV_ID_INVALID) {
            snprintf(str, sizeof(str), "%c%s  [%d:%d]\n",
                     (pdev == hw_mode_ctx->primary_pdev)? '*' : ' ',
                     scn->netdev->name, i, hw_mode_ctx->pdev_map[i]);
        } else {
            snprintf(str, sizeof(str), "%c%s  [%d:x]\n",
                     (pdev == hw_mode_ctx->primary_pdev)? '*' : ' ',
                     scn->netdev->name, i);
        }
...
static DEVICE_ATTR(pdev_map, S_IRUGO, wifi_soc_pdev_map_show, NULL);

static struct attribute *wifi_soc_attrs[] = {
    &dev_attr_hw_modes.attr,
    /* sysfs entry to store the name of ini file*/
    &dev_attr_ini_file.attr,
    /* pdev mapping for dynamic mode switch */
    &dev_attr_pdev_map.attr,
    NULL
};
...
ath_netdev_open(struct net_device *dev)
{
    ...
    if ((soc->recovery_enable == RECOVERY_ENABLE_WAIT)
...
ath_ioctl(struct net_device *dev, struct ifreq *ifr, int cmd)
{
    ...
    case SIOCGATHPHYSTATS:
        ...
        if (cdp_ath_getstats(soc_txrx_handle,
                         wlan_objmgr_pdev_get_pdev_id(scn->sc_pdev),
                         &cdp_stats,
                         UPDATE_PDEV_STATS) != QDF_STATUS_SUCCESS)
            return -EFAULT;
    ...
    case ACFG_PVT_IOCTL:
        {
            ...
            error = ol_acfg_handle_ioctl(dev, (void *)req);
...
#endif
{
...
#endif
    if (cdp_ath_getstats(soc_txrx_handle, wlan_objmgr_pdev_get_pdev_id(scn->sc_pdev), &cdp_stats,
                    UPDATE_PDEV_STATS) != QDF_STATUS_SUCCESS)
        goto stats_done;
    ...
    stats->rx_errors = cdp_stats.rx_errors;
    stats->rx_dropped = cdp_stats.rx_dropped;
...
static void ol_ath_init_deinit_hw_mode_ctx(ol_ath_soc_softc_t *soc,
                                           bool is_init)
{
    ol_ath_hw_mode_ctx_t *hw_mode_ctx  = &soc->hw_mode_ctx;
    int i;
    ...
    hw_mode_ctx->is_boot_in_progress   = false;
    hw_mode_ctx->target_band           = WMI_HOST_WLAN_5G_CAPABILITY;
    hw_mode_ctx->current_mode          = WMI_HOST_HW_MODE_MAX;
    hw_mode_ctx->target_mode           = WMI_HOST_HW_MODE_MAX;
    hw_mode_ctx->is_fw_resp_success    = false;

    if (is_init)
        qdf_event_create(&hw_mode_ctx->event);
    else
        qdf_event_destroy(&hw_mode_ctx->event);

    if (is_init) {
        hw_mode_ctx->dynamic_hw_mode = cfg_get(soc->psoc_obj,
                                               CFG_OL_DYNAMIC_HW_MODE);
        if (hw_mode_ctx->dynamic_hw_mode == WMI_HOST_DYNAMIC_HW_MODE_FAST) {
            hw_mode_ctx->primary_pdev = NULL;

            for (i = 0; i < WMI_HOST_MAX_PDEV; i++ ) {
                hw_mode_ctx->pdev_map[i] = WMI_HOST_PDEV_ID_INVALID;
...
static void ol_ath_assign_master(ol_ath_soc_softc_t *soc)
{
    ...
    for (pdev_id = 0; pdev_id < num_radios; pdev_id++) {
        ...
        if (hw_mode_ctx->target_band & ic->ic_supported_bands) {
            if (ic->ic_master_radio) {
                wlan_objmgr_pdev_release_ref(pdev, WLAN_MLME_NB_ID);
                break;
            } else if (temp_pdev_id == -1)
                /* identify the first radio that supports
                 * target_band
                 */
                temp_pdev_id = pdev_id;
        }
    ...
    if (pdev_id == num_radios) {
        /* no master radio assigned as previous
         * attempt to identify master based on
         * max chains would have failed
         */

        /* assign the first identified 5G radio
         * as master to recover from this failure
         */
        pdev = NULL;
        pdev = wlan_objmgr_get_pdev_by_id(psoc, temp_pdev_id, WLAN_MLME_NB_ID);
...
exit:
    if (temp_pdev_id > -1)
        QDF_TRACE(QDF_MODULE_ID_DYNAMIC_MODE_CHG,
                  ...
                  FL("pdev_idx: %d is assigned master_radio"),
                  temp_pdev_id);
    return;
}

/**
 * ol_ath_hw_mode_setup_ctx(): Setup hw mode context
...
QDF_STATUS ol_ath_hw_mode_setup_ctx(ol_ath_soc_softc_t *soc)
{
    ...
    primary_if = cfg_get(soc->psoc_obj, CFG_OL_DYNAMIC_HW_MODE_PRIMARY_IF);
    ...
    if ((hw_mode_ctx->current_mode == WMI_HOST_HW_MODE_MAX) ||
        (hw_mode_ctx->current_mode == WMI_HOST_HW_MODE_DETECT)) {
        hw_mode_ctx->current_mode = target_psoc_get_preferred_hw_mode(tgt_hdl);
        target_psoc_set_preferred_hw_mode(tgt_hdl, hw_mode_ctx->target_mode);
    }
    ...
    for (i = 0; i < num_radios; i++) {
        ...
        hw_mode_ctx->pdev_map[i] = mac_phy_cap[i].tgt_pdev_id;
        hw_mode_ctx->next_pdev_map[i] = mac_phy_cap[i].tgt_pdev_id;
        ...
        if (!found) {
            if (primary_if && strlen(primary_if)) {
                if (!strncmp(scn->netdev->name, primary_if,
                             strlen(primary_if))) {
...
exit:
    if (status != EOK) {
        wlan_psoc_nif_feat_cap_clear(psoc, WLAN_SOC_F_DYNAMIC_HW_MODE);
        QDF_TRACE(QDF_MODULE_ID_DYNAMIC_MODE_CHG,
                  QDF_TRACE_LEVEL_INFO,
                  FL("DMS: primary pdev can't be identified, disabling dynamic mode switch"));
    }

    return status;
}

static void ol_ath_release_vap_ifce(void *arg)
{
    qdf_work_t *work = (qdf_work_t *) (*(uintptr_t *) arg);
    struct ol_ath_softc_net80211 *scn = (struct ol_ath_softc_net80211 *)
                        (*(uintptr_t *)((uint8_t *) arg + sizeof(uintptr_t)));
    struct ieee80211com           *ic = &scn->sc_ic;
    ol_ath_hw_mode_ctx_t *hw_mode_ctx = &scn->soc->hw_mode_ctx;
    struct ieee80211vap *vap;

    if (qdf_wait_for_event_completion(&hw_mode_ctx->event,
            MAX_HW_MODE_SWITCH_ACCOMPLISH_TIME_IN_HOST)) {
        QDF_TRACE(QDF_MODULE_ID_DYNAMIC_MODE_CHG,
            QDF_TRACE_LEVEL_ERROR, FL("Failed waiting on switch complete."
                                      " Vap retry suggested. pdev_id: %d"), ol_ath_get_pdev_idx(ic));
    } else {
        QDF_TRACE(QDF_MODULE_ID_DYNAMIC_MODE_CHG,
            QDF_TRACE_LEVEL_INFO, FL("Mode switch complete. Re-attaching"
                                     " vap ifce_handlers. pdev_id: %d"), ol_ath_get_pdev_idx(ic));

        /* attach vap specific handlers */
        TAILQ_FOREACH(vap, &ic->ic_vaps, iv_next) {
            osif_dev  *osifp = (osif_dev *)((os_if_t)vap->iv_ifp);
            ieee80211_register_vap_wext_handlers(osifp, 1 /* register */);
        }
        /* release it for the next waiting ic */
...
/**
 * ol_ath_ifce_setup(): Setup input interface as per requested
 * state
...
void ol_ath_ifce_setup(struct ol_ath_softc_net80211 *scn,
                       ifce_status ifce_up)
{
    struct net_device *pdev_netdev = scn->netdev;
    struct ieee80211com        *ic = &scn->sc_ic;
    struct ieee80211vap                     *vap;
    qdf_work_t                             *work; /* work to wait on switch
                                                   * complete to unblock vap
                                                   * ifce */
    void                                    *arg;

#if UMAC_SUPPORT_WEXT
    /* attach/detach radio specific handlers */
    ifce_up ? ol_ath_iw_attach(pdev_netdev) : ol_ath_iw_detach(pdev_netdev);

    if (ifce_up) {
        if (scn->soc->hw_mode_ctx.dynamic_hw_mode != WMI_HOST_DYNAMIC_HW_MODE_FAST) {
            work = (qdf_work_t *) qdf_mem_malloc(sizeof(qdf_work_t));

            if (work) {
                arg = qdf_mem_malloc(2*sizeof(uintptr_t));
                if (arg) {
                    *(uintptr_t *) arg = (uintptr_t) work;
                    *(uintptr_t *) ((uint8_t *) arg + sizeof(uintptr_t))
                                   = (uintptr_t) scn;
                    qdf_create_work(NULL, work, ol_ath_release_vap_ifce, arg);
                    qdf_sched_work(NULL, work);
                } else {
                    qdf_mem_free(work);
                }
            }
        ...
        /* detach vap specific handlers */
        TAILQ_FOREACH(vap, &ic->ic_vaps, iv_next) {
            osif_dev  *osifp = (osif_dev *)((os_if_t)vap->iv_ifp);

            ieee80211_register_vap_wext_handlers(osifp, 0 /* un-register */);
        }
    }
#endif
    ...
    if (ic->ic_cfg80211_config) {
        if (ifce_up) {
            wlan_cfg80211_register_vendor_cmd_event_handlers(ic);
        } else {
    ...
    if ((!ifce_up) &&
        (scn->soc->hw_mode_ctx.dynamic_hw_mode ==
         WMI_HOST_DYNAMIC_HW_MODE_SLOW)) {
        /* call netdev function of the relevant pdev */
        /* This will clear IFF_UP and result in a call to ath_netdev_stop */
        dev_change_flags(pdev_netdev, pdev_netdev->flags & ~IFF_UP);
    }
}

/**
 * ol_ath_is_ifce_allowed_in_dynamic_hw_mode(): Checks if input interface is
 * allowed. Interface is not allowed if:
 * 1. Dynamic hw-mode enable check passes as returned by
 * ol_ath_is_dynamic_hw_mode_enabled() and
 * 2. If the corresponding radio supports target
 * band configured in hw_mode_ctx and
 * 3. 1 and 2 above and if the corresponding radio is a
 * master radio and hw_mode_swtich is in-progress
 * 4. 1 and 2 above and if the corresponging radio is not
 * a master radio and hw_mode_switch is in-progress or
 * current mode is DBS
 * 5. In all other cases interface is allowed
 * @soc: soc handle
 *
...
bool ol_ath_is_ifce_allowed_in_dynamic_hw_mode(void *i_scn)
{
    ...
    ol_ath_soc_softc_t  *soc          = scn->soc;
    ol_ath_hw_mode_ctx_t *hw_mode_ctx = &soc->hw_mode_ctx;
    bool is_cmn_cond; /* is common condition satisfied */

    /* Both for a master or a non-master radio interface
     * is allowed if dynamic_hw_mode switch feature is
     * not supported or if the radio does not supprt the
     * target_band configured in hw_mode_ctx. These two
     * are common conditions below.
     */
    is_cmn_cond = ol_ath_is_dynamic_hw_mode_enabled(soc) &&
                    (ic->ic_supported_bands & hw_mode_ctx->target_band);

    return (ic->ic_master_radio || (scn->sc_pdev == hw_mode_ctx->primary_pdev)) ?
            !(is_cmn_cond &&
            /* if master radio then ifce is not allowed
             * if switch is in-progress */
            hw_mode_ctx->is_switch_in_progress) :
            !(is_cmn_cond &&
             /* if not a master radio then ifce is not allowed
              * if either switch is in-progress or if current
              * mode is DBS */
            (hw_mode_ctx->is_switch_in_progress ||
             hw_mode_ctx->current_mode == WMI_HOST_HW_MODE_DBS));
}
...
void ol_ath_check_and_reconfig_hw_mode(ol_ath_soc_softc_t *soc)
{
    ...
    /* Check feature support only after retrieving
     * tgt_hdl as it is required before to obtain
     * the current mode before returing to caller.
     * Refer to label 'exit'
     */
    if (!ol_ath_is_dynamic_hw_mode_enabled(soc)) {
        goto exit;
    ...
    /* Assign master-radio if previous attempt failed
     * to identify during ol_ath_pdev_attach()
     */
    ol_ath_assign_master(soc);

    /* retrieve pdev for idx 0 */
    pdev = wlan_objmgr_get_pdev_by_id(psoc, 0, WLAN_MLME_NB_ID);
    if (pdev == NULL) {
         QDF_TRACE(QDF_MODULE_ID_DYNAMIC_MODE_CHG,
                 QDF_TRACE_LEVEL_ERROR,
                 FL("pdev object (id: %d) is NULL"), 0);
         goto exit;
    }

    /* retrieve scn - required to caller mode_switch handler */
    ...
    target_mode = hw_mode_ctx->target_mode;

    if ((target_mode < WMI_HOST_HW_MODE_MAX) &&
        (target_mode != target_psoc_get_preferred_hw_mode(tgt_hdl))) {

        if (!rtnl_is_locked()) {
            rtnl_lock();
            rtnl_lock_acquired = true;
        }

        status = ol_ath_handle_hw_mode_switch(scn, target_mode);
        ...
        if (status) {
            QDF_TRACE(QDF_MODULE_ID_DYNAMIC_MODE_CHG,
                    QDF_TRACE_LEVEL_ERROR,
                    FL("Unable to switch hardware mode to %d"), target_mode);
        }
...
exit:
    ...
    QDF_TRACE(QDF_MODULE_ID_DYNAMIC_MODE_CHG,
            QDF_TRACE_LEVEL_INFO, FL("Running with HW mode = %d"),
                                    hw_mode_ctx->current_mode);
    return;
}
...
int ol_ath_configure_cong_ctrl_max_msdus(struct ol_ath_softc_net80211 *scn)
{
    ...
    if (cfg_get(soc->psoc_obj, CFG_NSS_WIFI_OL)) {
        if (pdev_idx == 0)
            tx_desc_limit = soc->num_tx_desc_0;
        else if (pdev_idx == 1)
            tx_desc_limit = soc->num_tx_desc_1;
        else if (pdev_idx == 2)
            tx_desc_limit = soc->num_tx_desc_2;
        else {
            qdf_err("Invalid pdev_idx");
            return -1;
        }
    } else
#endif
    {
        tx_desc_limit = soc->num_tx_desc;
    }

    qdf_info("pdev_idx = %d tx_desc_limit = %d", pdev_idx, tx_desc_limit);

    return ol_ath_pdev_set_param(scn, wmi_pdev_param_set_cong_ctrl_max_msdus,
            tx_desc_limit);
}
...
__ol_ath_attach(void *hif_hdl, struct ol_attach_t *ol_cfg, osdev_t osdev, qdf_device_t qdf_dev)
{
...
#if defined(ATH_AHB) && !defined(BUILD_X86)
    ...
    qdf_register_self_recovery_callback(ol_ath_self_recovery);
    ...
    if (QDF_IS_STATUS_SUCCESS(ret)) {
        cfg_psoc_parse(psoc, ini.external);
        strlcat(ini_buf, ini.internal, qdf_str_len(ini.internal) + qdf_str_len(ini_buf) + 1);
        cfg_parse_to_psoc_store(psoc, ini_buf);
    }
    ...
    soc->max_group_keys = cfg_get(psoc, CFG_OL_MAX_GROUP_KEYS);
    ...
    soc->re_ul_resp = cfg_get(psoc, CFG_OL_RE_UL_RESP);

    /* initialize hw_mode_ctx */
    ol_ath_init_deinit_hw_mode_ctx(soc, true);

    if (cfg_get(psoc, CFG_OL_DYNAMIC_HW_MODE)) {
        qdf_info("Booting with Dynamic HW Mode feature enabled");
        preferred_hw_mode = WMI_HOST_HW_MODE_DETECT;
        wlan_psoc_nif_feat_cap_set(psoc, WLAN_SOC_F_DYNAMIC_HW_MODE);
        ...
        if (cfg_get(psoc, CFG_OL_HW_MODE_ID) < WMI_HOST_HW_MODE_MAX) {
            if (cfg_get(psoc, CFG_OL_DYNAMIC_HW_MODE) ==
                ...
                soc->hw_mode_ctx.target_mode = cfg_get(psoc, CFG_OL_HW_MODE_ID);
            }
        }
    } else {
        if (cfg_get(psoc, CFG_OL_HW_MODE_ID) < WMI_HOST_HW_MODE_MAX) {
    ...
    /* Initialize cfg command logging in debugfs */
    config_cmd_resp_init(soc);

#if CONFIG_WIFI_EMULATION_WIFI_3_0
	qdf_info("preferred_hw_mode %d", preferred_hw_mode);
#endif
...
#define LTEU_SUPPORT2    0x4
    ...
    if ((error = cdp_mempools_attach(soc_txrx_handle)) != 0) {
    ...
    spectral_legacy_cbacks.vdev_get_chan_freq_seg2 = wlan_vdev_get_chan_freq_seg2;
    ...
    for (i = 0; i < num_radios; i++) {
        ...
        if (pdev_ospriv == NULL) {
                qdf_info("%s: pdev_ospriv is NULL", __func__);
                break;
        }

        pdev_ospriv->nif = (struct qdf_net_if *)pdev_netdev;
        ...
        wlan_minidump_log((void *)scn, sizeof(*scn), (void *)psoc,
...
#endif
        ic->ic_num_clients = ol_ath_get_num_clients(pdev);
	...
	ic->ic_cfg80211_config = cfg_get(psoc, CFG_OL_CFG80211_CONFIG);
        ...
        ic->ic_re_ul_resp = ((soc->re_ul_resp >> lmac_get_pdev_idx(scn->sc_pdev)) & 0x01);
	if (ol_scan_chanlist_size > 0) {
            ...
            for (j = 0;
                 ...
                 j++)
            {
                ic->ic_custom_scan_order[i] = ol_scan_freqlist[i];
     ...
     */
        ...
        if (lmac_is_target_ar900b(soc->psoc_obj)|| ol_target_lithium(soc->psoc_obj)) {
            pdev_netdev->needed_headroom = sizeof (struct ieee80211_qosframe) +
                sizeof(struct llc) + QDF_MAC_ADDR_SIZE +
#if MESH_MODE_SUPPORT
...
// Enable ethtool support
    ...
    if (i < num_radios) {
        ...
        if (soc->hw_mode_ctx.dynamic_hw_mode == WMI_HOST_DYNAMIC_HW_MODE_FAST) {
            ...
            hw_mode_ctx->is_boot_in_progress  = true;
    ...
    ol_num_global_soc++;
    wlan_minidump_log((void *)soc, sizeof(*soc), psoc,
                      WLAN_MD_CP_EXT_PSOC, "ol_ath_soc_softc");
#if QLD
    qld_register((void *)soc, sizeof(*soc), "ol_ath_soc_softc");
#endif
...
__ol_ath_detach(struct net_device *dev)
{
    ...
    qdf_mem_zero(soc->tso_vdev_bitmap, sizeof(soc->tso_vdev_bitmap));
    psoc = soc->psoc_obj;
...
#if QLD
        qld_register((void *)scn, sizeof(*scn), "ol_ath_softc_net80211");
#endif
    ...
    /* de-initialize hw_mode_ctx */
    ol_ath_init_deinit_hw_mode_ctx(soc, false);
    ...
    wlan_objmgr_psoc_check_for_leaks(psoc);

    target_if_free_psoc_tgt_info(psoc);
    wlan_objmgr_psoc_obj_delete(psoc);
    wlan_minidump_remove((void *)soc);
    free_netdev(dev);
...
__ol_ath_check_wmi_ready(ol_ath_soc_softc_t *soc)
{
    ...
    if (!(target_psoc_get_wmi_ready(tgt_hdl))) {
        qdf_info("WMI is not ready\n");
        return -EIO;
    }
...
qdf_export_symbol(war1);
qdf_export_symbol(war1_allow_sleep);
qdf_export_symbol(WAR_PCI_WRITE32);
...
qdf_export_symbol(__ol_ath_attach);
qdf_export_symbol(__ol_ath_detach);
qdf_export_symbol(__ol_ath_target_status_update);
qdf_export_symbol(__ol_ath_check_wmi_ready);
qdf_export_symbol(ol_ath_diag_user_agent_init);
qdf_export_symbol(ol_ath_diag_user_agent_fini);
qdf_export_symbol(ol_ath_host_config_update);
...
qdf_export_symbol(ol_ath_request_firmware);
...
qdf_export_symbol(ol_ath_release_firmware);
