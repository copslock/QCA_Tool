...
extern int ol_ath_ucfg_get_vap_iface_names(struct ol_ath_softc_net80211 *scn,
...
int
ol_acfg_get_all_vap_info(void *ctx, uint16_t cmdid,
               uint8_t *buffer, int32_t Length)
{
    struct net_device *dev = (struct net_device *) ctx;
    struct ol_ath_softc_net80211 *scn = ath_netdev_priv(dev);
    ...
    if (scn->soc->down_complete) {
        qdf_err("Target stopped: cannot get wifi param");
        return -1;
    }
    ...
    error = ol_ath_ucfg_get_vap_iface_names(scn, ptr);
...
acfg_dispatch_entry_t ol_acfgdispatchentry[] =
{
    ...
    { ol_acfg_get_all_vap_info,   ACFG_REQ_GET_VAP_NAMES  , 0 },
