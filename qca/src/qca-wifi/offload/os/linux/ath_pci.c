...
struct net_device *pci_dev_to_net_dev(struct pci_dev *pcidev)
{
    struct net_device *dev = NULL;
    pci_priv_data *pci_priv = NULL;
    struct pld_plat_data *wlan_sc =  NULL;
    struct pld_plat_data *cnss_plat = NULL;

    if(!pcidev)
        return NULL;

#ifdef CONFIG_CNSS2_SUPPORT
    if (pcidev->device == QCN9000_DEVICE_ID) {
...
int
ol_ath_pci_probe(struct pci_dev *pcidev, const struct pci_device_id *id)
{
...
#ifdef CONFIG_CNSS2_SUPPORT
    pld_wait_for_fw_ready(&pcidev->dev);
#endif //CONFIG_CNSS2_SUPPORT

    if ((id->device == QCN9000_DEVICE_ID) &&
         (testmode == QDF_GLOBAL_COLDBOOT_CALIB_MODE ||
          testmode == QDF_GLOBAL_FTM_COLDBOOT_CALIB_MODE)) {
        ret = ol_ath_do_cold_boot_calibration(&pcidev->dev);
        return ret;
    }

    if((testmode == QDF_GLOBAL_WALTEST_MODE) &&
        (id->device == QCN9000_DEVICE_ID)) {
        ret = ol_ath_do_waltest(&pcidev->dev);
        return ret;
    }

    aps_osdev = qdf_mem_malloc(sizeof(*aps_osdev));
    if(aps_osdev == NULL) {
	qdf_nofl_info("Error while allocating aps_osdev\n");
	ret =-1;
	goto cleanup1;
    }
    OS_MEMSET(aps_osdev, 0, sizeof(*(aps_osdev)));

    aps_osdev->bdev = pcidev;
    aps_osdev->device = &pcidev->dev;
    ...
    qdf_dev->drv_hdl = pcidev; /* bus handle */
    qdf_dev->dev = &pcidev->dev; /* device */
    qdf_dev->drv = aps_osdev;

#ifndef CONFIG_CNSS2_SUPPORT
    pci_set_drvdata(pcidev, NULL);
#endif
    hif_context  = ol_hif_open(&pcidev->dev, pcidev, (void *)id, QDF_BUS_TYPE_PCI, 0,qdf_dev);
    ...
    if (id->device == QCN9000_DEVICE_ID) {
       ...
       pci_set_drvdata(pcidev, aps_osdev->netdev);
    }
#endif
#if !defined(BUILD_X86) && !defined(CONFIG_X86)
        ...
        enum qdf_bus_type bus_type, bool reinit, qdf_device_t qdf_dev)
{
    ...
    struct hif_driver_state_callbacks cbk;
        ...
        if (testmode == 1) {
          hif_ctx = hif_open(qdf_dev, QDF_GLOBAL_FTM_MODE,
                             bus_type, &cbk, NULL);
        } else if (testmode == QDF_GLOBAL_COLDBOOT_CALIB_MODE) {
          hif_ctx = hif_open(qdf_dev, QDF_GLOBAL_COLDBOOT_CALIB_MODE,
                             bus_type, &cbk, NULL);
        } else if (testmode == QDF_GLOBAL_FTM_COLDBOOT_CALIB_MODE) {
          hif_ctx = hif_open(qdf_dev, QDF_GLOBAL_FTM_COLDBOOT_CALIB_MODE,
                             bus_type, &cbk, NULL);
        } else {
          hif_ctx = hif_open(qdf_dev, QDF_GLOBAL_MISSION_MODE,
                             bus_type, &cbk, NULL);
        }

        if (!hif_ctx) {
            qdf_nofl_info("%s: hif_open error", __func__);
            return NULL;
        }
	...
	if (!QDF_IS_STATUS_SUCCESS(status)) {
		qdf_nofl_info("hif_enable error = %d, reinit = %d",
				status, reinit);
...
#define NAPI_ANY -1
	...
	if (ret < 0) {
		qdf_nofl_info("%s: NAPI creation error, rc: 0x%x", __func__, status);
		return NULL;
	}
...
void
ol_ath_pci_remove(struct pci_dev *pcidev)
{
    struct net_device *dev = pci_dev_to_net_dev(pcidev);
    ol_ath_soc_softc_t *soc;
    ...
    if (!(soc->down_complete)) {
        ...
        if (!ol_ath_suspend_target(soc, 1)) {
            ...
            timeleft = wait_event_interruptible_timeout(soc->sc_osdev->event_queue,
                    (soc->is_target_paused == TRUE), PDEV_SUSPEND_TIMEOUT);
    ...
    if(pcidev->device == QCN9000_DEVICE_ID) {
        pci_priv_data *pci_priv = pci_get_drvdata(pcidev);
        cnss_plat = pci_priv->wlan_priv;
        wlan_sc = cnss_plat->wlan_priv;
    ...
    qdf_nofl_info(KERN_INFO "ath_pci_remove\n");
}
...
int
ol_ath_pci_suspend(struct pci_dev *pcidev, pm_message_t state)
{
    ...
    pci_read_config_dword(pcidev, OL_ATH_PCI_PM_CONTROL, &val);
    if ((val & 0x000000ff) != 0x3) {
        PCI_SAVE_STATE(pcidev,
            ((struct ath_pci_softc *)ath_netdev_priv(dev))->aps_pmstate);
        pci_disable_device(pcidev);
        pci_write_config_dword(pcidev, OL_ATH_PCI_PM_CONTROL, (val & 0xffffff00) | 0x03);
    }
    return 0;
}
...
int
ol_ath_pci_resume(struct pci_dev *pcidev)
{
    struct net_device *dev = pci_get_drvdata(pcidev);
    u32 val;
    int err;
    ...
    if ((val & 0x000000ff) != 0) {
        ...
        pci_write_config_dword(pcidev, OL_ATH_PCI_PM_CONTROL, val & 0xffffff00);
         ...
         *
         */
        pci_read_config_dword(pcidev, 0x40, &val);

        if ((val & 0x0000ff00) != 0)
            pci_write_config_dword(pcidev, 0x40, val & 0xffff00ff);
    }
...
void
ol_ath_pci_recovery_remove(struct pci_dev *pcidev)
{
    struct net_device *dev = pci_get_drvdata(pcidev);
    struct ol_ath_soc_softc *soc;
    struct pdev_op_args oper;
...
int
ol_ath_pci_recovery_probe(struct pci_dev *pcidev, const struct pci_device_id *id)
{
    struct net_device *dev = pci_get_drvdata(pcidev);
