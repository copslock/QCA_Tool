...
#endif
{
    ...
    if(tgt_info->target_type == TARGET_TYPE_QCA6018 ||
       tgt_info->target_type == TARGET_TYPE_QCA8074 ||
       tgt_info->target_type == TARGET_TYPE_QCA8074V2 ||
       tgt_info->target_type == TARGET_TYPE_QCN9000) {

        rv = pld_athdiag_write(soc->qdf_dev->dev,
                 (uint32_t)pos, 0, count,(uint8_t *)buf);
...
#endif
{
    ...
    if(tgt_info->target_type == TARGET_TYPE_QCA6018 ||
       ...
       tgt_info->target_type == TARGET_TYPE_QCN9000) {
        ...
        rv = pld_athdiag_read(soc->qdf_dev->dev,
                 (uint32_t)pos, 0, count,(uint8_t *)buf);
...
ath_sysfs_diag_init(struct ol_ath_soc_softc *soc)
{
    ...
    if (!diag_fsattr) {
        qdf_nofl_info("%s: Memory allocation failed\n", __func__);
		return;
    }
    ...
    if (ret) {
        qdf_nofl_info("%s: sysfs create failed\n", __func__);
        OS_FREE(diag_fsattr);
