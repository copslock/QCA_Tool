                            ...
                            struct ieee80211_node *ni, int isnew )
{
    ...
    if ((ni->ni_flags & IEEE80211_NODE_AUTH) &&
        ...
        cdp_peer_authorize(soc_txrx_handle, wlan_vdev_get_id(vap->vdev_obj), ni->peer_obj->macaddr, 1);
    } else {
        cdp_peer_authorize(soc_txrx_handle, wlan_vdev_get_id(vap->vdev_obj), ni->peer_obj->macaddr, 0);
    }
    ...
    IEEE80211_DPRINTF(vap, IEEE80211_MSG_ASSOC,
            ...
            " Peer NSS = %d\n", __func__, param.peer_he_caps_6ghz,
                        ...
                        u_int16_t statuscode)
{
    ...
    cdp_set_addbaresponse(soc_txrx_handle, ni->ni_macaddr, wlan_vdev_get_id(vap->vdev_obj),
                                   tidno, statuscode);
    /* send the management frame buffer to the target */
                ...
                struct ieee80211_rx_status *rs)
{
    ...
    rs->rs_channel  = rx_status->ch_num;
    rs->rs_flags  = 0;
...
ol_ath_mgmt_rx_event_handler(ol_scn_t sc, u_int8_t *data, u_int32_t datalen)
{
    ...
    if (!rx_event.chan_freq) {
        rx_event.chan_freq = wlan_reg_chan_band_to_freq(pdev, rx_event.channel,
    ...
    /* Update RX status */
    rx_status.ch_num = rx_event.channel;
    rx_status.freq = rx_event.chan_freq;
        ...
        bool null_data_handler)
{
    ...
    if (scn->soc->hw_mode_ctx.dynamic_hw_mode == WMI_HOST_DYNAMIC_HW_MODE_FAST) {
        ...
        if (wmi_is_blocked(wmi_handle)) {
            qdf_nofl_warn("%s: mode switch in progress, dropping mgmt frame subtype 0x%x (wmi_handle %pK)\n",
                          __func__, subtype, wmi_handle);
            wbuf_free(wbuf);
            return;
        }
    }
...
mgmt_crypto_encap(wbuf_t wbuf, struct ieee80211_node *ni)
{
    ...
    if (wh->i_fc[1] & IEEE80211_FC1_WEP) {
        ...
        if (k == NULL) {
            qdf_nofl_info("Mgmt encap Failed \n");
            return -1;
        }
        if (tid != OFFCHAN_EXT_TID_NONPAUSE) {
