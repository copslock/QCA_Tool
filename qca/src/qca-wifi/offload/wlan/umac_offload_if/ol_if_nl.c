...
ol_ath_mu_report_event_handler(ol_scn_t sc, u_int8_t *data, u_int32_t datalen)
{
    ...
    if (!ic->ic_nl_handle) {
        qdf_nofl_info("%s: MU report (status=%d id=%d), but on non NL radio\n",
    ...
    if (atomic_read(&nl_handle->mu_in_progress) == 0) {
        ...
        qdf_nofl_info("%s: MU report id=%d (%d), but MU not active\n",
    ...
    if (event->status_reason) {
        qdf_nofl_info("%s: MU report id=%d failed with status=%d\n", __func__, event->mu_request_id, event->status_reason);
    }
    ...
    if (event->num_TA_entries > MU_DATABASE_MAX_LEN) {
        qdf_nofl_info("%s: num_TA_entries value =%d, exceeds MAX limit of %d",
