...
/**
 * target_if_fd_offload() - API to offload the FD frame
 * vdev: Pointer to vdev object
 * vdev_id: Vdev ID
 * This function is invoked from ol_if layer during VDEV up.
 * It invokes FD RX ops function pointers to offload the FD frame.
 */
QDF_STATUS target_if_fd_offload(struct wlan_objmgr_vdev * vdev, uint32_t vdev_id);

/**
 * target_if_fd_tmpl_update() - API to update the FD template
 * vdev: Pointer to vdev object
 * This funtion is invoked from ol_if layer during FD parameter update.
 * It invokes FD RX ops function pointers to update the FD template
 */
 ...
 * @vdev: Pointer to vdev object
 * @offload: Flag to offload FD frame.
 *
 * This function is invoked from ol_if layer during VDEV up.
