...
static void process_ieee_hdr(void *data)
{
    ...
    if (dir == IEEE80211_FC1_DIR_TODS) {
        frm_hdr.bssid_tail = (wh->i_addr1[QDF_MAC_ADDR_SIZE-2] << 8) |
                             (wh->i_addr1[QDF_MAC_ADDR_SIZE-1]);
        frm_hdr.sa_tail    = (wh->i_addr2[QDF_MAC_ADDR_SIZE-2] << 8) |
                             (wh->i_addr2[QDF_MAC_ADDR_SIZE-1]);
        frm_hdr.da_tail    = (wh->i_addr3[QDF_MAC_ADDR_SIZE-2] << 8) |
                             (wh->i_addr3[QDF_MAC_ADDR_SIZE-1]);
    } else if (dir == IEEE80211_FC1_DIR_FROMDS) {
        frm_hdr.bssid_tail = (wh->i_addr2[QDF_MAC_ADDR_SIZE-2] << 8) |
                             (wh->i_addr2[QDF_MAC_ADDR_SIZE-1]);
        frm_hdr.sa_tail    = (wh->i_addr3[QDF_MAC_ADDR_SIZE-2] << 8) |
                             (wh->i_addr3[QDF_MAC_ADDR_SIZE-1]);
        frm_hdr.da_tail    = (wh->i_addr1[QDF_MAC_ADDR_SIZE-2] << 8) |
                             (wh->i_addr1[QDF_MAC_ADDR_SIZE-1]);
    } else {
        frm_hdr.bssid_tail = (wh->i_addr3[QDF_MAC_ADDR_SIZE-2] << 8) |
                             (wh->i_addr3[QDF_MAC_ADDR_SIZE-1]);
        frm_hdr.sa_tail    = (wh->i_addr2[QDF_MAC_ADDR_SIZE-2] << 8) |
                             (wh->i_addr2[QDF_MAC_ADDR_SIZE-1]);
        frm_hdr.da_tail    = (wh->i_addr1[QDF_MAC_ADDR_SIZE-2] << 8) |
                             ...
                             void *data, ath_pktlog_hdr_t *pl_hdr)
{
    ...
    u_int8_t *addr = NULL;
    u_int8_t vap_addr[QDF_MAC_ADDR_SIZE];
    u_int8_t ret_val = 0;
    u_int8_t vdev_id = 0;
    ...
    for (i = 0; i < pl_msdu_info.num_msdu; i++) {
        ...
        tx_desc = ol_tx_desc_find((struct ol_txrx_pdev_t *)txrx_pdev, tx_desc_id);
        ...
        if (!(pl_hdr->flags & (1 << PKTLOG_FLG_FRM_TYPE_CLONE_S))){
            ...
            frm_hdr.sa_tail    = (addr[2 * QDF_MAC_ADDR_SIZE-2] << 8) |
            ...
            if (ret_val) {
                frm_hdr.bssid_tail = (vap_addr[QDF_MAC_ADDR_SIZE-2] << 8) |
                        ...
                        ath_pktlog_hdr_t *pl_hdr)
{
    ...
    struct ol_ath_softc_net80211 *scn =
        (struct ol_ath_softc_net80211 *)(pl_dev->scn);
    struct ol_txrx_pdev_t *txrx_pdev = (struct ol_txrx_pdev_t *)
        ((struct ol_txrx_psoc_t *)wlan_psoc_get_dp_handle(scn->soc->psoc_obj))->pdev[0];
    ...
    if (pl_hdr->log_type == PKTLOG_TYPE_TX_VIRT_ADDR) {
        ...
        if (txrx_pdev->is_ar900b) {
            ...
            if ((desc_id >> 24) == BCN_DESC_ID) {
                vdev_id &= 0x00FFFFFF;
                        pl = ol_ath_get_bcn_header(txrx_pdev->scnctx, vdev_id);
                if (pl)
                    process_ieee_hdr(pl);
            } else {
                /*TODO*/
    ...
    if (pl_hdr->log_type == PKTLOG_TYPE_TX_STAT) {
        if (pl_info->tx_capture_enabled &&
            ...
            if (beacon) {
                beacon_buf = ol_ath_get_bcn_buffer(txrx_pdev->scnctx, vdev_id);
                if (beacon_buf)
...
process_rx_cbf_remote(void *context, qdf_nbuf_t amsdu)
{
    ...
    struct ol_ath_softc_net80211 *scn =
        (struct ol_ath_softc_net80211 *)(pl_dev->scn);
    struct ol_txrx_pdev_t *pdev = (struct ol_txrx_pdev_t *)
        ((struct ol_txrx_psoc_t *) wlan_psoc_get_dp_handle(scn->soc->psoc_obj))->pdev[0];
    struct ath_pktlog_info *pl_info;
    void *rx_desc;
    ath_pktlog_hdr_t pl_hdr;
