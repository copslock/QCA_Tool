/*
 * Copyright (c) 2012, 2015, 2017-2020 Qualcomm Innovation Center, Inc.
 *
 * All Rights Reserved.
        ...
        uint32_t log_state)
{
    ...
    if (log_state & ATH_PKTLOG_PHY_LOGGING)
        types |= WMI_HOST_PKTLOG_EVENT_PHY_LOGGING;
#if 0
        ...
        struct ol_pktlog_dev_t *pl_dev, int32_t log_state)
{
    ...
    if(pl_dev->pl_info->peer_based_filter == 1) {
        if(cdp_enable_peer_based_pktlog(soc_txrx_handle, pdev_id,
    ...
    if ((log_state & ATH_PKTLOG_TX)       ||
        ...
        (log_state & ATH_PKTLOG_PHY_LOGGING)) {
...
__pktlog_enable(struct ol_ath_softc_net80211 *scn, int32_t log_state)
{
    ...
    if (log_state != 0 && log_state != pl_info->log_state) {
        ...
        if (pl_info->buf == NULL) {
            ...
            if (pl_dev->checksum) {
                pl_info->buf->bufhdr.version = pl_dev->checksum;
                pl_info->buf->bufhdr.magic_num =
                                          PKTLOG_MAGIC_NUM_VERSION_IS_CHECKSUM;
            } else {
                pl_info->buf->bufhdr.version = CUR_PKTLOG_VER;
                pl_info->buf->bufhdr.magic_num = PKTLOG_MAGIC_NUM_LEGACY;
            }
