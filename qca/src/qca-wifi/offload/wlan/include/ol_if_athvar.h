 ...
 * switch cmd
 */
#define MAX_HW_MODE_FW_RESP_TIME 1500
...
#define DCS_ENABLE_TIME         (30*60) /* Duration after which DCS should be enabled back after disabling it due to 3 triggers in 5 minutes */
...
struct ol_ath_vap_net80211 {
    ...
    wbuf_t                          av_pr_rsp_wbuf;     /* 20 TU pr Resp buffer */
    uint32_t                  vdev_param_capabilities; /*vdev param capabilities state*/
                                                        /*   event to bringup vap*/
#if ATH_SUPPORT_WRAP
    u_int8_t                   av_is_psta:1,   /* is ProxySTA VAP */
                                av_is_mpsta:1,  /* is Main ProxySTA VAP */
                                av_is_wrap:1,   /* is WRAP VAP */
                                av_use_mat:1;   /* use MAT for this VAP */
    u_int8_t                    av_mat_addr[QDF_MAC_ADDR_SIZE];    /* MAT addr */
#endif
    uint8_t                        av_restart_in_progress:1, /* vdev restart in progress */
                                   is_dma_mapped:1,
                                   av_ol_resmgr_wait:1;  /* UMAC waits for target */
};
 ...
 * @SR_TYPE_SRP: Spatial Reuse Parameter based Spatial reuse
 */
enum srtype {
 ...
 * Return: Integer value indicating status
 */
int ol_ath_node_delete_multiple_wds_entries(struct cdp_ctrl_objmgr_psoc *psoc, uint8_t vdev_id,
                                            u_int8_t *wds_macaddr,
                                            u_int8_t *peer_macaddr, u_int32_t flags);
...
void ol_ath_prb_rsp_stop(struct ol_ath_softc_net80211 *scn,
                                struct ol_ath_vap_net80211 *avn);
...
void ol_ath_prb_rsp_free(struct ieee80211com *ic, int if_id,
                                        struct ieee80211vap *vap);

int ol_ath_prb_resp_tmpl_send(struct ol_ath_softc_net80211 *scn, int vdev_id,
                                                    struct ieee80211vap *vap);
