    ...
    else if ( param & OL_SPECIAL_PARAM_SHIFT )
    {
        ...
        switch (param) {
            case OL_SPECIAL_PARAM_COUNTRY_ID:
                ...
                if (orig_cc == value) {
                    qdf_info("New country code is same as current country code %d, skip set country code",
                            orig_cc);
                    return -EINVAL;
                }
            ...
            case OL_SPECIAL_PARAM_REGDOMAIN:
                {
                    ...
                    if (orig_rd == value) {
                        qdf_info("New regdomain is same as current regdomain %d, skip set regdomain",
                                orig_rd);
                        return -EINVAL;
                    }
...
static bool ol_ath_ucfg_validate_aid(wlan_if_t vaphandle, u_int32_t value)
{
    ...
    TAILQ_FOREACH(node, &nt->nt_node, ni_list) {
        if (!ieee80211_try_ref_node(node, WLAN_MLME_SB_ID))
            continue;

        aid = IEEE80211_AID(node->ni_associd);
        ieee80211_free_node(node, WLAN_MLME_SB_ID);
        if (aid == value ) {
            return TRUE;
        }
    }
...
int ol_ath_ucfg_set_country(void *vscn, char *cntry)
{
    ...
    if (&scn->sc_ic) {
        qdf_mem_zero(ctry_iso, sizeof(ctry_iso));
        ieee80211_getCurrentCountryISO(&scn->sc_ic, ctry_iso);

        if (!(qdf_str_ncmp(ctry_iso, cntry, REG_ALPHA2_LEN))) {
            qdf_info("New country is same as current country %s, skip set country",
                    ctry_iso);
            return -EINVAL;
        }
        ...
        if (retval) {
            qdf_print("%s: Unable to set country code", __func__);
            retval = wlan_set_countrycode(&scn->sc_ic, ctry_iso, 0, CLIST_NEW_COUNTRY);
        }
    } else {
        retval = -EOPNOTSUPP;
    }
        ...
        struct packet_power_info_params *param)
{
    if(ol_ath_packet_power_info_get(scn, param)) {
        qdf_nofl_info("%s:Unable to send request to get packet power info\n", __func__);
        return -1;
    }
    return 0;
}
                    ...
                    uint32_t param)
{
    ...
    wlan_pdev_beacon_update(ic);
    ic->ic_is_spatial_reuse_updated = false;
    return retv;
}
                          ...
                          struct ieee80211_pkt_capture_enh *peer_info)
{
    ...
    if (!soc_handle) {
        qdf_err("psoc handle is NULL", soc_handle);
        return -EFAULT;
    }
                                        ...
                                        uint32_t threshold)
{
    ...
    if(ol_ath_pdev_set_param(scn, wmi_pdev_param_nav_override_config,
                config_val) != EOK) {
        qdf_err("%s: NAV config value %d could not be set", __func__, config_val);
        return -EINVAL;
    } else {
                                 ...
                                 uint8_t param, uint8_t value, uint8_t data1, uint8_t data2)
{
    ...
    switch (param) {
        ...
        case HE_SR_SRP_ENABLE:
             ...
             */
            if(value == ic->ic_he_srctrl_srp_disallowed) {
                ic->ic_is_spatial_reuse_updated = true;
                ic->ic_he_srctrl_srp_disallowed = !value;
            } else {
                qdf_info("SRP Enabled already set to: %d", value);
            }
        break;
        ...
        case HE_SR_ENABLE_PER_AC:
             ...
             */
            if(value > 15) {
                qdf_err("%s: Value: %d is not a valid value for setting SR ACs",
                        __func__, value);
                return -EINVAL;
            }

            if (value != ic->ic_he_sr_enable_per_ac) {
                if (0 == ol_ath_pdev_set_param(scn,
                            wmi_pdev_param_set_cmd_obss_pd_per_ac,
                            value)) {
                    ic->ic_he_sr_enable_per_ac = value;
                } else {
                    qdf_err("Error sending WMI for OBSS PD per AC");
                    return -EINVAL;
                }
            } else {
                qdf_info("SR tx already enabled for: 0x%x", value);
            }
        break;
...
int ol_ath_ucfg_get_he_sr_config(void *vscn, uint8_t param, uint32_t *value)
{
    ...
    switch (param) {
        ...
        case HE_SR_ENABLE_PER_AC:
            value[0] = ic->ic_he_sr_enable_per_ac;
        break;
...
static enum srtype ol_ath_extact_sr_type(void *data, uint32_t data_len)
{
	...
	void *data, uint32_t data_len, uint32_t value)
{
    ...
    switch (param)
    {
        ...
        case SR_SELF_OBSS_PD_THRESHOLD:
...
int ol_ath_ucfg_set_non_ht_dup(void *vscn, uint8_t frametype, bool enable)
{
    ...
    switch(frametype) {
        ...
        case NON_HT_DUP_BEACON:
            ic->ic_non_ht_dup &= ~IEEE80211_NON_HT_DUP_BEACON_M;
            ic->ic_non_ht_dup |= (enable << IEEE80211_NON_HT_DUP_BEACON_S);
            break;
        case NON_HT_DUP_BCAST_PROBE_RESP:
            ic->ic_non_ht_dup &= ~IEEE80211_NON_HT_DUP_BCAST_PROBE_RESP_M;
            ic->ic_non_ht_dup |= (enable << IEEE80211_NON_HT_DUP_BCAST_PROBE_RESP_S);
            break;
        case NON_HT_DUP_FILS_DISCOVERY:
            ic->ic_non_ht_dup &= ~IEEE80211_NON_HT_DUP_FILS_DISCOVERY_M;
            ic->ic_non_ht_dup |= (enable << IEEE80211_NON_HT_DUP_FILS_DISCOVERY_S);
            break;
        default:
            qdf_err("Invalid input");
            return -EINVAL;
