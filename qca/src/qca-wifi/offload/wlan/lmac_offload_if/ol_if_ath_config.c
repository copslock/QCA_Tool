        ...
        uint32_t chainmask, int direction, int phymode)
{
    ...
    if (is_phymode_5G_or_6G(phymode)) {
...
int config_rxchainmask(struct ieee80211com *ic, struct ieee80211_ath_channel *chan)
{
    ...
    if (retval == EOK) {
        /* Update the ic_chainmask */
        ol_ath_update_fw_adfs_support(ic, scn->user_config_val);
    ...
    case OL_ATH_PARAM_ARPDHCP_AC_OVERRIDE:
        ...
        case OL_ATH_PARAM_IGMPMLD_OVERRIDE:
            if ((0 == value) || (value == 1)) {
                ...
                cdp_txrx_set_pdev_param(soc_txrx_handle, pdev_id, CDP_CONFIG_IGMPMLD_OVERRIDE, val);
#ifdef QCA_NSS_WIFI_OFFLOAD_SUPPORT
                if (ic->nss_radio_ops)
        ...
        case OL_ATH_PARAM_DCS:
            {
                ...
                if ((value & OL_ATH_CAP_DCS_WLANIM) && (IEEE80211_IS_CHAN_2GHZ(ic->ic_curchan))) {
...
#define ANTENNA_GAIN_5G_MASK    0x8000
       ...
       case OL_ATH_PARAM_FW_RECOVERY_ID:
            if (value >= RECOVERY_DISABLE && value <= RECOVERY_ENABLE_WAIT) {
                ...
                if (lmac_get_tgt_type(scn->soc->psoc_obj) ==
                    TARGET_TYPE_QCN9000) {
                    dev = scn->sc_osdev->device;
                    if (value == RECOVERY_DISABLE)
                        pld_set_ramdump_enabled(dev, false);
                    else
                        pld_set_ramdump_enabled(dev, true);
                }
            } else {
...
#undef ARP_RESET
	...
	case OL_ATH_PARAM_PDEV_TO_REO_DEST:
        ...
        case OL_ATH_PARAM_TX_CAPTURE:
            ...
            {
               qdf_err("value %u is not allowed when current setting is %u. "
