        ...
        uint8_t target_hw_mode)
{
    ...
    ol_if_dfs_deinit_nol(scn, target_hw_mode);
    ieee80211_dfs_nol_reset(&scn->sc_ic);
    status = ol_if_dfs_reset_agile_cac(&scn->sc_ic);
    if (status)
        return status;
    ol_if_dfs_psoc_deinit_pre_hw_mode_switch(scn->soc->psoc_obj);
        ...
        uint8_t target_hw_mode)
{
    ...
    if (dfs_rx_ops->dfs_agile_precac_start)
