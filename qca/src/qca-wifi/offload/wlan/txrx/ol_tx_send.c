/*
 * Copyright (c) 2011-2015, 2017-2018, 2020 Qualcomm Innovation Center, Inc.
 * All Rights Reserved
...
static void tx_info_dump(struct ieee80211com *ic, struct ieee80211_tx_status *ts, uint8_t *dest_addr)
{
    ...
    qdf_nofl_info("==radio=%s, STA=%s\n", scn->netdev->name, ether_sprintf((const uint8_t *)dest_addr));
    qdf_nofl_info("==ts->ppdu_rate=0x%x preamble=%d, nss=%d, mcs=%d \n",ts->ppdu_rate,
                    HTT_GET_HW_RATECODE_PREAM(ts->ppdu_rate),
                    HTT_GET_HW_RATECODE_NSS(ts->ppdu_rate),
                    HTT_GET_HW_RATECODE_RATE(ts->ppdu_rate));
    qdf_nofl_info("==ts->ts_rateKbps=%dkbps\n",ts->ts_rateKbps);
    qdf_nofl_info("==ts->ppdu_num_mpdus_success=0x%x\n",ts->ppdu_num_mpdus_success);
    qdf_nofl_info("==ts->ppdu_num_mpdus_fail=0x%x\n",ts->ppdu_num_mpdus_fail);
    qdf_nofl_info("==ts->ppdu_num_msdus_success=0x%x\n",ts->ppdu_num_msdus_success);
    qdf_nofl_info("==ts->ppdu_bytes_success=0x%x\n",ts->ppdu_bytes_success);
    qdf_nofl_info("==ts->ppdu_duration=0x%x\n",ts->ppdu_duration);
    qdf_nofl_info("==ts->ppdu_retries=0x%x\n",ts->ppdu_retries);
    qdf_nofl_info("==ts->ppdu_is_aggregate=0x%x\n",ts->ppdu_is_aggregate);
    qdf_nofl_info("==ts->ts_flags=0x%x\n",ts->ts_flags);
    qdf_nofl_info("==ts->msdu_length=%d\n",ts->msdu_length);
    qdf_nofl_info("==ts->msdu_priority=%d\n",ts->msdu_priority);
    qdf_nofl_info("==ts->ch_width=%d\n",ts->ch_width);
    qdf_nofl_info("==ts->msdu_q_time=%d ms\n",(int)(ts->msdu_q_time));
...
#ifdef QCA_NSS_WIFI_OFFLOAD_SUPPORT
        pdev = (struct ol_txrx_pdev_t *) ((struct ol_txrx_psoc_t *)wlan_psoc_get_dp_handle(scn->soc->psoc_obj))->pdev[0];
        ...
        qdf_nbuf_t netbuf, struct ol_tx_desc_t *tx_desc, u_int16_t *ack_rssi, int i)
{
    ...
    mhdr->band = wlan_reg_freq_to_band(pdev->rx_mon_recv_status->rs_freq);
    mhdr->channel = wlan_reg_freq_to_chan(ic->ic_pdev_obj, pdev->rx_mon_recv_status->rs_freq);
    return mhdr_len;
    ...
    uint16_t tid)
{
    ...
    for (i = 0; i < num_msdus; i++) {
        ...
        if (!tx_desc->allocated) {
           qdf_nofl_info("%s: MSDU is not allocated  MSDU ID is %d \n", __func__, tx_desc_id);
           continue;
        }
#if MESH_MODE_SUPPORT
        ...
        if (pdev->ap_stats_tx_cal_enable) {
            if (tx_desc->tx_encap_type == htt_pkt_type_ethernet) {
                ...
                if (IEEE80211_IS_BROADCAST(eh->h_dest)) {
                    bcastcnt++;
                    bcastbytes += qdf_nbuf_len(netbuf);
                }
            }
        }
            ...
            if (ppdu_peer_id_filled) {
                ...
                if (peer && qdf_nbuf_headroom(netbuf) >= sizeof(struct tx_capture_hdr)) {
                    ...
                    if (newdata_ptr) {
                        ...
                        qdf_mem_copy(tcaphdr->ta, (eh->h_dest), QDF_MAC_ADDR_SIZE);
                        tcaphdr->ppdu_id = *ppdu_peer_id_ptr;
                        txcap_peer = pdev->peer_id_to_obj_map[peer_id];
                        qdf_mem_copy(tcaphdr->ra, peer->mac_addr.raw, QDF_MAC_ADDR_SIZE);
                        pdev->tx_cap_hdr = 1;
                    }
                }
            }
    ...
    if (qdf_unlikely(pdev->ap_stats_tx_cal_enable)) {
        ...
        case htt_tx_status_ok:
            ...
            */
                ...
                peer->stats.tx.bcast.bytes += bcastbytes;
            }
            break;
