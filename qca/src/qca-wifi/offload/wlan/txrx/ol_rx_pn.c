    ...
    qdf_nbuf_t msdu_list)
{
	...
	if (last_pn_valid) {
        ...
        if (pn_is_replay) {
            ...
            if (scn) {
                ...
                if (!(scn->soc->dbg.err_types.pn_errors % scn->soc->dbg.print_rate_limit)) {
#if !(TXRX_PRINT_ENABLE)
                    qdf_nofl_info("=== Reducing pace for error prints ==== \n");
                    qdf_nofl_info("PN check failed - TID %d, peer %pK "
    ...
    if(peer->pn_replay_counter > MAX_ALLOWED_CONSECUTIVE_PN_FAILURE) {
        qdf_nofl_info("Kicking out node %x:%x:%x:%x:%x:%x due to excessive PN replays\n", peer->mac_addr.raw[0], peer->mac_addr.raw[1],
