/*
 * Copyright (c) 2011-2015,2017-2020 Qualcomm Innovation Center, Inc.
 * All Rights Reserved
...
void ol_rx_mesh_mode_per_pkt_rx_info(qdf_nbuf_t nbuf, struct ol_txrx_peer_t *peer, struct ol_txrx_vdev_t *vdev)
{
    ...
    rx_info->rs_band = ieee80211_get_ath_channel_band(ni->ni_chan);
    ...
    if (wlan_get_key(vap, kid, peer->mac_addr.raw, &k,
    ...
    )
{
    ...
    TAILQ_FOREACH(vdev, &pdev->vdev_list, vdev_list_elem) {
        if (qdf_mem_cmp(a1, vdev->mac_addr.raw, QDF_MAC_ADDR_SIZE) == 0) {
            break;
        }
    }
    if (!vdev)
 ...
 */
    ...
    if (vdev->opmode == wlan_op_mode_ap) {
        /* qdf_nofl_info("AP m %d 3A %d 4A %d UP %d MP %d ", rx_mcast, rx_3addr, rx_4addr, rx_policy_ucast, rx_policy_mcast); */
        ...
        /* qdf_nofl_info("STA m %d 3A %d 4A %d UP %d MP %d ", rx_mcast, rx_3addr, rx_4addr, rx_policy_ucast, rx_policy_mcast); */
                                ...
                                void *rx_mpdu_desc )
{
    ...
    ni = ieee80211_find_node(&ic->ic_sta, peer->mac_addr.raw, WLAN_MLME_SB_ID);
                    ...
                    void *rx_desc)
{
...
#if UNIFIED_SMARTANTENNA
        || target_if_sa_api_is_rx_feedback_enabled((struct wlan_objmgr_psoc *)soc->psoc_obj,
              ((struct ol_ath_softc_net80211 *)txrx_pdev->scnctx)->sc_pdev)
#endif
       ))
    ...
    if (ppdu) {
	    ...
	    ratekbps = dp_getrateindex(sgi, mcs, nss, preamble, bw, &rix, &ratecode);
	    if (ratekbps)
		    peer->stats.rx.avg_rx_rate = dp_ath_rate_lpf(peer->stats.rx.avg_rx_rate, ratekbps);
	    peer->stats.rx.rnd_avg_rx_rate = dp_ath_rate_out(peer->stats.rx.avg_rx_rate);
...
#if UNIFIED_SMARTANTENNA
static void
ol_rx_populate_evm_details(struct ol_txrx_pdev_t *txrx_pdev,
        void *rx_desc, struct cdp_rx_indication_ppdu *cdp_rx_ppdu)
{
    uint32_t *ppdu;
    uint8_t pilot;

    ppdu = txrx_pdev->htt_pdev->ar_rx_ops->get_ppdu_end(rx_desc);
    for (pilot = 0; pilot < MAX_EVM_SIZE_LEGACY; pilot++) {
        cdp_rx_ppdu->evm_info.pilot_evm[pilot] = ppdu[pilot];
    }
    cdp_rx_ppdu->evm_info.nss_count = 4;
    cdp_rx_ppdu->evm_info.pilot_count = 4;
    cdp_rx_ppdu->rx_antenna = (ppdu[RX_DESC_ANTENNA_OFFSET] & SMART_ANT_ANTENNA_MASK);
}
#endif
        ...
        enum htt_cmn_rx_status status)
{
    ...
    struct cdp_rx_indication_ppdu *cdp_rx_ppdu;
    if (!msdu) {
        qdf_err("Invalid data for pdev %pK", txrx_pdev);
        return A_ERROR;
    }

    scn = (struct ol_ath_softc_net80211 *)txrx_pdev->scnctx;
    if (!scn) {
        qdf_err("Invalid scn for pdev %pK", txrx_pdev);
        return A_ERROR;
    }

    pdev_obj = (struct wlan_objmgr_pdev *)scn->sc_pdev;
...
#if UNIFIED_SMARTANTENNA
                    if (target_if_sa_api_is_rx_feedback_enabled(wlan_pdev_get_psoc(pdev_obj),
                                                        pdev_obj)) {
                        ol_rx_populate_evm_details(txrx_pdev, rx_desc, cdp_rx_ppdu);
                    }
#endif
        ...
        int num_mpdu_ranges)
{
    ...
    if (qdf_unlikely(!scn)) {
        qdf_err("Invalid scn for pdev %pK", pdev);
        return;
    }
    ...
    spdev = (struct wlan_objmgr_pdev *)scn->sc_pdev;
...
out_stats:
                ...
                if (CE_is_directlink(((struct ol_txrx_pdev_t*)htt_pdev->txrx_pdev)->ce_tx_hdl)) {
                        qdf_nofl_info("ERROR: %s called over Directlink\n",__func__);
                        A_ASSERT(0);
                }
                ...
                if ( vdev && vdev->opmode != wlan_op_mode_sta) {
                    ...
                    vap = ((osif_dev *)(vdev->osif_vdev))->os_if;
            ...
            for (i = 0; i < num_mpdus; i++) {
                ...
                if (status == htt_rx_status_tkip_mic_err && vdev != NULL &&  peer != NULL) {
                    ol_rx_err((ol_pdev_handle) ((struct ol_ath_softc_net80211 *)pdev->scnctx)->sc_pdev, vdev->vdev_id, peer->mac_addr.raw,
...
normalproc:
                ...
                if ((status == htt_rx_status_ctrl_mgmt_null) && (peer == NULL)) {
                    ...
                    type = wh->i_fc[0] & IEEE80211_FC0_TYPE_MASK;
                    subtype = wh->i_fc[0] & IEEE80211_FC0_SUBTYPE_MASK;
                    if (msdu->len >= sizeof(struct ieee80211_frame)) {
        ...
        if (discard || filter) {
            ...
            if (tid == 0) {
                if (qdf_nbuf_data(msdu)[0x17] == 0x6) {
                    ...
                    HTT_PRINT_BUF(qdf_nofl_info, qdf_nbuf_data(msdu),
                            0x41,__func__);
                }
            }
    ...
    if(scn->sc_arp_dbg_conf) {
        ...
        if (iph->protocol == IPPROTO_ICMP ) {
            ...
            if ((iph->daddr == ntohl(scn->sc_arp_dbg_srcaddr)) && (iph->saddr == ntohl(scn->sc_arp_dbg_dstaddr))) {
                qdf_nofl_info("%s: ICMP type: %d: Sequence: %u From Source Address: 0x%08x To Destination Address: 0x%08x \n", __func__, icmp->type, icmp->un.echo.sequence
                        , iph->saddr , iph->daddr);
...
void ol_ath_add_vow_extstats(rx_handle_t rxh, qdf_nbuf_t msdu)
{
    ...
    scn = (struct ol_ath_softc_net80211 *)((struct ol_txrx_pdev_t *) pdev->txrx_pdev)->scnctx;
    if (!scn) {
        qdf_warn("scn is NULL for pdev:%pK", pdev);
        return;
    }
