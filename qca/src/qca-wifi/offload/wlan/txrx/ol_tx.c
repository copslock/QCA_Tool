...
QDF_STATUS
ol_tx_rawsim_getastentry(struct cdp_soc_t *soc, uint8_t vdev_id, qdf_nbuf_t *pnbuf, struct cdp_raw_ast *raw_ast_handle)
{
    u_int8_t ftype = 0;
    u_int8_t *hdr_ptr;
            ...
            if (ether_type == __constant_htons(ETHERTYPE_8021AD)) {
                ...
                if (dir == tx_direction) {
                    ...
                    if (map_prio == OL_TX_SVLAN_CVLAN_DSCP ||
                        ...
                        cvlan = (u_int16_t)*(nbuf->data + hdr_offset + 2);
...
qdf_export_symbol(ol_txrx_classify);
...
u_int32_t
ol_pflow_update_pdev_params(ol_txrx_soc_handle soc, uint8_t pdev_id,
                            enum _dp_param_t param,
                            u_int32_t value, void *buff)
{
    struct ol_txrx_pdev_t *pdev = (struct ol_txrx_pdev_t *) ((struct ol_txrx_psoc_t *)soc)->pdev[0];
#ifdef QCA_NSS_WIFI_OFFLOAD_SUPPORT
    ...
    switch(param)
    {
        ...
        case DP_PARAM_QFLUSHINTERVAL:
         ...
         case DP_PARAM_TOTAL_Q_SIZE:
            ...
            break;
         case DP_PARAM_TOTAL_Q_SIZE_RANGE0:
         case DP_PARAM_TOTAL_Q_SIZE_RANGE1:
         case DP_PARAM_TOTAL_Q_SIZE_RANGE2:
         case DP_PARAM_TOTAL_Q_SIZE_RANGE3:
            {
                if(buff) {
                   *(int*)buff = pdev->pflow_ctl_max_buf_global;
                }
         ...
         case DP_PARAM_MAX_Q_LIMIT:
            ...
            break;
         case DP_PARAM_CONG_CTRL_TIMER_INTV:
            {
                if(!buff) {
                    if(value != 0) {
            ...
            break;
         case DP_PARAM_STATS_TIMER_INTV:
            {
                if(!buff) {
                    if(value != 0) {
            ...
            break;
         case DP_PARAM_ROTTING_TIMER_INTV:
            {
                if(!buff) {
                    if(value != 0) {
...
ol_tx_ll(struct cdp_soc_t *soc, uint8_t vdev_id, qdf_nbuf_t msdu_list)
{
    ...
    if (qdf_unlikely(!(vdev = ol_txrx_get_vdev_from_pdev(pdev, vdev_id))))
        goto out;

    osdev = (osif_dev *)(vdev->osif_vdev);
    vap = osdev->os_if;
...
#if PEER_FLOW_CONTROL_DEBUG
        qdf_nofl_info("%s dequeued %d tx_pkts and %d tx_bytes \n", num_tx_pkts, num_tx_bytes);
#endif
...
QDF_STATUS
ol_txrx_per_peer_stats(struct cdp_soc_t *soc, char *addr)
{
    struct ol_txrx_pdev_t *pdev = (struct ol_txrx_pdev_t *)((struct ol_txrx_psoc_t *)soc)->pdev[0];
    struct ol_txrx_ast_entry_t *ast_entry;
    ...
    qdf_nofl_info("Found peer_id=%d\n", peer_id);

    if (peer_id == HTT_INVALID_PEER) {
        qdf_nofl_info("No peer found\n");
        return QDF_STATUS_E_FAILURE;
    }

    if (peer_id >= OL_TXRX_MAX_PEER_IDS) {
        qdf_nofl_info("Invalid Peer Id=%d\n", peer_id);
        return QDF_STATUS_E_FAILURE;
    }

    ni = ieee80211_find_node(&ic->ic_sta, addr, WLAN_MLME_SB_ID);
    if (ni == NULL) {
...
void
ol_tx_flush_buffers(struct cdp_soc_t *soc, uint8_t vdev_id)
{
    struct ol_txrx_vdev_t *vdev;
    struct ol_txrx_pdev_t *pdev = (struct ol_txrx_pdev_t *) ((struct ol_txrx_psoc_t *)soc)->pdev[0];
...
#if !PEER_FLOW_CONTROL
    {
        ...
        while (scn_qcnt) {
            ...
            if (netbuf) {
                ...
                qdf_nofl_info("%s: Error: Broken acnbufq, remaining qlen: %d\n",
                    __func__, scn_qcnt);
                break;
            }
            scn_qcnt--;
