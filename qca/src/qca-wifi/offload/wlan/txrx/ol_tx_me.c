/*
 * Copyright (c) 2014-2015, 2017-2018, 2020 Qualcomm Innovation Center, Inc.
 * All Rights Reserved
...
ol_tx_me_alloc_descriptor(struct cdp_soc_t *soc, uint8_t pdev_id)
{
    ...
    if (qdf_unlikely(!(pdev = (struct ol_txrx_pdev_t *) ((struct ol_txrx_psoc_t *)soc)->pdev[0]))) {
        return;
    }
...
ol_tx_me_free_descriptor(struct cdp_soc_t *soc, uint8_t pdev_id)
{
    ...
    if (atomic_read(&pdev->mc_num_vap_attached)) {
       if (qdf_atomic_dec_and_test(&pdev->mc_num_vap_attached)) {
           ...
           qdf_info("Disable MCAST_TO_UCAST");
       }
    }
}
...
ol_tx_me_init(ol_txrx_pdev_handle pdev_handle)
{
    ...
    if (pdev->me_buf.vaddr == NULL) {
        ...
        if (pdev->me_buf.vaddr == NULL) {
            ...
            qdf_nofl_info("Error allocating memory pool\n");
            return 1;
        }
        ...
        p->next = NULL;
        qdf_spin_unlock_bh(&pdev->tx_mutex);
        qdf_nofl_info("ME Pool succesfully initialized vaddr - %pK paddr - %x\n"
...
#endif
            qdf_nofl_info("%s:Already Enabled!!\n", __func__);
    }
    return 0;
}
...
ol_tx_me_exit(ol_txrx_pdev_handle pdev_handle)
{
    ...
    num_pool_elems = ol_cfg_target_tx_credit((ol_soc_handle)dp_soc->psoc_obj);
    /* Add flow control buffer count */
    ...
    if (pdev->me_buf.vaddr) {
        uint16_t wait_cnt = 0;
        qdf_nofl_info("Disabling Mcastenhance. This may take some time...\n");
        qdf_spin_lock_bh(&pdev->tx_mutex);
        ...
        if (pdev->me_buf.buf_in_use > 0) {
            qdf_nofl_info("Tx-comp pending for %d ME frames after waiting %ds!!\n",
        ...
        qdf_nofl_info(KERN_INFO"%s: Already Disabled !!!\n", __func__);
    }
}
...
extern uint16_t
ol_tx_me_convert_ucast(ol_txrx_soc_handle soc, uint8_t vdev_id,
                       qdf_nbuf_t wbuf,
                       u_int8_t newmac[][6], uint8_t new_mac_cnt)
{
    struct ol_txrx_pdev_t *pdev;
    struct ol_txrx_vdev_t *dp_vdev;
    struct ether_header *eh;
    u_int8_t *data;
    u_int16_t len, dwnld_len;
    u_int8_t *dstmac;                           /* reference to frame dst addr */
    u_int8_t srcmac[QDF_MAC_ADDR_SIZE];    /* copy of original frame src addr */
    u_int8_t empty_entry_mac[QDF_MAC_ADDR_SIZE];
    ...
    if (qdf_unlikely(WLAN_UMAC_PDEV_MAX_VDEVS <= vdev_id) ||
        qdf_unlikely(!(dp_vdev = pdev->vdev_id_to_obj_map[vdev_id]))) {
        qdf_nbuf_free(wbuf);
        return 1;
    }
    ...
    if ((CE_is_directlink(pdev->ce_tx_hdl))) {
        ol_tx_me_convert_ucast_partner( dp_vdev, wbuf, newmac, new_mac_cnt);
        return 1;
    }
#endif
    TXRX_VDEV_STATS_ADD(dp_vdev, tx_i.mcast_en.mcast_pkt.num, 1);

    OS_MEMSET(empty_entry_mac, 0, QDF_MAC_ADDR_SIZE);
    ...
    for( new_mac_idx = 0; new_mac_idx < new_mac_cnt; new_mac_idx++ ) {
        ...
        && (ol_tx_ll_pflow_ctrl((ol_txrx_vdev_handle)dp_vdev, wbuf_cpy) == 0)
#else
        && (ol_tx_ll_fast((ol_txrx_vdev_handle)dp_vdev, &wbuf_cpy, 1) == 0)
#endif
#endif
        ) {
