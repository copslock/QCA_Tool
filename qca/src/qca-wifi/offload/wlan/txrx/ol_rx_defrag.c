...
#define	DEFRAG_IEEE80211_ADDR_EQ(a1, a2) \
    (qdf_mem_cmp(a1, a2, QDF_MAC_ADDR_SIZE) == 0)
    ...
    int seq_num)
{
    ...
    if(peer->tids_rx_reorder[tid].array == NULL) {
        qdf_nofl_info("%s:tids_rx_reorder.array for tid %d is NULL for peer %pK,"
    ...
    qdf_nbuf_t frag)
{
    ...
    if( peer->tids_rx_reorder[tid].win_sz_mask != 0 ) {
        qdf_nofl_info("Recieved Frags in BA session - Dropping the frames\n");
        return -1;
    }
    ...
    qdf_nbuf_t frag_list)
{
    ...
    struct wlan_objmgr_pdev *obj_pdev = ((struct ol_ath_softc_net80211 *)pdev->scnctx)->sc_pdev;
    htt_pdev_handle htt_pdev = pdev->htt_pdev;
    vdev = peer->vdev;
    ...
    if (!pdev->host_80211_enable) {
        if (ol_cfg_frame_type((ol_pdev_handle) obj_pdev) == wlan_frm_fmt_802_3) {
     ...
     */
    if (ol_rx_intrabss_fwd_check((ol_pdev_handle) obj_pdev,
    ...
    u_int8_t hdr[])
{
    ...
    case IEEE80211_FC1_DIR_NODS:
        ...
        DEFRAG_IEEE80211_ADDR_COPY(hdr + QDF_MAC_ADDR_SIZE, wh->i_addr2);
        break;
    ...
    case IEEE80211_FC1_DIR_FROMDS:
        DEFRAG_IEEE80211_ADDR_COPY(hdr, wh->i_addr1); /* DA */
        DEFRAG_IEEE80211_ADDR_COPY(hdr + QDF_MAC_ADDR_SIZE, wh->i_addr3);
        break;
    case IEEE80211_FC1_DIR_DSTODS:
        DEFRAG_IEEE80211_ADDR_COPY(hdr, wh->i_addr3); /* DA */
        DEFRAG_IEEE80211_ADDR_COPY(hdr + QDF_MAC_ADDR_SIZE, wh->i_addr4);
        break;
    }
    /*
...
ol_rx_defrag_nwifi_to_8023(qdf_nbuf_t msdu)
{
    ...
    if ((wh1->i_fc[1] & IEEE80211_FC1_DIR_MASK) == IEEE80211_FC1_DIR_DSTODS)
        ...
        qdf_mem_copy(&wh, qdf_nbuf_data(msdu), (sizeof(wh)-QDF_MAC_ADDR_SIZE));
    ...
    switch (wh.i_fc[1] & IEEE80211_FC1_DIR_MASK) {
    case IEEE80211_FC1_DIR_NODS:
        qdf_mem_copy(eth_hdr->dest_addr, wh.i_addr1, QDF_MAC_ADDR_SIZE);
        qdf_mem_copy(eth_hdr->src_addr, wh.i_addr2, QDF_MAC_ADDR_SIZE);
        break;
    case IEEE80211_FC1_DIR_TODS:
        qdf_mem_copy(eth_hdr->dest_addr, wh.i_addr3, QDF_MAC_ADDR_SIZE);
        qdf_mem_copy(eth_hdr->src_addr, wh.i_addr2, QDF_MAC_ADDR_SIZE);
        break;
    case IEEE80211_FC1_DIR_FROMDS:
        qdf_mem_copy(eth_hdr->dest_addr, wh.i_addr1, QDF_MAC_ADDR_SIZE);
    ...
    case IEEE80211_FC1_DIR_DSTODS:
        qdf_mem_copy(eth_hdr->dest_addr, wh.i_addr3, QDF_MAC_ADDR_SIZE);
        qdf_mem_copy(eth_hdr->src_addr,  wh.i_addr4, QDF_MAC_ADDR_SIZE);
        break;
    }
    qdf_mem_copy(
