...
#ifdef ATH_MAT_TEST
	qdf_nofl_info(KERN_ERR "%s: src %s type 0x%x",__func__,ether_sprintf(src_mac),ether_type);
	qdf_nofl_info(KERN_ERR "des %s\n",ether_sprintf(des_mac));
#endif
...
#ifdef ATH_DEBUG_MAT
			qdf_nofl_info(KERN_ERR "%s:%d sport %d dport %d\n",__func__,__LINE__,p_udp->source,p_udp->dest);
#endif
			/*
...
#ifdef ATH_DEBUG_MAT
							qdf_nofl_info(" Make the dhcp flag to broadcast ... \n");
#endif
