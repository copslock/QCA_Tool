/*
 * Copyright (c) 2011-2105,2017-2018,2020 Qualcomm Innovation Center, Inc.
 * All Rights Reserved
 ...
 */
#define CFG_TGT_NUM_VDEV_AR900B 17
...
struct ol_txrx_psoc_t {
    ...
    /* PSoC object */
    struct cdp_ctrl_objmgr_psoc *psoc_obj;
...
struct ol_txrx_vdev_t {
    ...
    uint8_t tidmap_prty;
    struct cdp_ext_vdev *vdev_dp_ext_handle; /* Extended data path handle */
};
...
static inline struct ol_txrx_vdev_t *
ol_txrx_get_vdev_from_pdev(struct ol_txrx_pdev_t *pdev, uint8_t vdev_id)
{
    if (qdf_unlikely(!pdev || (WLAN_UMAC_PDEV_MAX_VDEVS <= vdev_id))) {
        qdf_debug("Invalid parameters : pdev = %pK , vdev = %d", pdev, vdev_id);
        return NULL;
    }

    return pdev->vdev_id_to_obj_map[vdev_id];
}
