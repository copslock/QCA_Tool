/*
 * Copyright (c) 2011, 2015, 2020 Qualcomm Innovation Center, Inc.
 *
 * All Rights Reserved
...
extern int dp_me_mcast_convert(struct cdp_soc_t *soc,
                               uint8_t vdev_id,
                               uint8_t pdev_id,
                               wbuf_t wbuf);

#if defined(WLAN_FEATURE_FASTPATH) && (!QCA_OL_TX_PDEV_LOCK)
#define OL_VDEV_TX_MCAST_ENHANCE(_soc, _vdev, _pdev_id, _msdu, _vap) \
{ \
        ol_vap_tx_lock(_vdev->osif_vdev); \
        if( dp_me_mcast_convert(_soc, _vdev->vdev_id, _pdev_id, _msdu) > 0 ) { \
            ol_vap_tx_unlock(_vdev->osif_vdev); \
