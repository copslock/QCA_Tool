...
wds_entry_add(struct ol_ath_softc_net80211 * scn, struct wds_table *table, unsigned char *dest_mac, unsigned char *peer_mac)
{
    ...
    if (wds) {
        ...
        wds->vap = NULL;
        ni = ieee80211_find_node(&((struct ol_ath_softc_net80211 *)scn)->sc_ic.ic_sta,
                peer_mac, WLAN_WDS_ID);
        if (ni) {
            wds->vap = ni->ni_vap;
...
wds_peer_event_handler(ol_scn_t sc, u_int8_t *datap, u_int32_t len)
{
    ...
    if(wmi_extract_wds_addr_event(wmi_handle, datap, len, &wds_addr_event)) {
        ...
        if (wds_addr_event->event_type[0] == 1) {
            qdf_nofl_info(KERN_ERR "Adding WDS entry through(%02x:%02x:%02x:%02x:%02x:%02x) of mac(%02x:%02x:%02x:%02x:%02x:%02x)\n",
            ...
            if (scn->sc_ic.nss_radio_ops) {
                scn->sc_ic.nss_radio_ops->ic_nss_ol_pdev_add_wds_peer(scn, vdev_id, peer_obj->macaddr, HTT_INVALID_PEER,
                                  dest_mac, peer_mac, CDP_TXRX_AST_TYPE_NONE);
            }
#endif

        } else if (wds_addr_event->event_type[0] == 2) {
            qdf_nofl_info(KERN_ERR "Deleting WDS entry through(%02x:%02x:%02x:%02x:%02x:%02x) of mac(%02x:%02x:%02x:%02x:%02x:%02x)\n",
                    peer_mac[0], peer_mac[1],
                    peer_mac[2], peer_mac[3],
                    peer_mac[4], peer_mac[5],
                    dest_mac[0], dest_mac[1],
...
wds_peer_event_handler(ol_scn_t sc, u_int8_t *datap, u_int32_t len)
{
    ...
    uint8_t pdev_id;
    struct wmi_unified *wmi_handle;
    struct ol_txrx_psoc_t *soc_txrx_handle = (struct ol_txrx_psoc_t *)wlan_psoc_get_dp_handle(soc->psoc_obj);
    /* Extracting pdev from pdev[0], since in Legacy there is only one pdev per soc */
    struct ol_txrx_pdev_t *pdev_txrx_handle = (struct ol_txrx_pdev_t *)soc_txrx_handle->pdev[0];
    ...
    if (wmi_extract_wds_addr_event(wmi_handle, datap, len, &wds_addr_event))
    {
        ...
        if (wds_addr_event.event_type[0] == 1) {
            ...
            ast_entry =  ol_txrx_ast_find_hash_find(pdev_txrx_handle, wds_addr_event.dest_mac, 0);
            if (ast_entry) {
                if (ol_txrx_ast_find_hash_remove(pdev_txrx_handle, wds_addr_event.dest_mac)) {
            ...
            ol_txrx_ast_find_hash_add(pdev_txrx_handle,
                    wds_addr_event.peer_mac, wds_addr_event.dest_mac, HTT_INVALID_PEER, 0);
#ifdef QCA_NSS_WIFI_OFFLOAD_SUPPORT
            if (scn->sc_ic.nss_radio_ops) {
                scn->sc_ic.nss_radio_ops->ic_nss_ol_pdev_add_wds_peer(scn, wds_addr_event.vdev_id, peer_obj->macaddr, HTT_INVALID_PEER,
                          (uint8_t *)wds_addr_event.dest_mac, (uint8_t *)wds_addr_event.peer_mac, CDP_TXRX_AST_TYPE_NONE);
            }
#endif
