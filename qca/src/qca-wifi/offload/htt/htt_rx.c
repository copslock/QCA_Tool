...
htt_rx_netbuf_pop(rx_handle_t rxh)
{
     ...
     */
    if (CE_is_directlink(((struct ol_txrx_pdev_t*)pdev->txrx_pdev)->ce_tx_hdl)) {
        qdf_nofl_info("ERROR: %s called in Directlink path\n",__func__);
        A_ASSERT(0);
	return NULL;
    } else
#endif /* QCA_PARTNER_DIRECTLINK_RX */
...
dump_pkt(qdf_nbuf_t msdu, int len)
{
    ...
    qdf_nofl_info("++++++++++++++++++++++++++++++++++++++++++++\n");
    qdf_nofl_info("Dumping %d bytes of data\n", len);

    for (i = 0; i < len; i++) {
        qdf_nofl_info("0x%x ", *buf++);
        if (i && (i % 8 == 0))
            qdf_nofl_info("\n");
    }
...
dump_htt_rx_desc(htt_pdev_handle pdev, void *rx_desc)
{
    ...
    qdf_nofl_info("++++++++++++++++++++++++++++++++++++++++++++\n");
    qdf_nofl_info("Dumping %d words of rx_desc of the msdu \n", len);

    for (i = 0; i < len; i++) {
        qdf_nofl_info("0x%08x ", *buf);
        buf++;
        if (i && (i % 4 == 0))
            qdf_nofl_info("\n");
    }
    ...
    if (!msdu_done) {
            ...
            if (pdev->htt_rx_donebit_assertflag == 0) {
                if ((pdev->g_htt_status != htt_rx_status_ok) && ((*npackets) == 0)) {
                    ...
                    qdf_nofl_info("DONE BIT NOT SET FOR FIRST MSDU and PEER IS INVALID \n");
                    qdf_nofl_info("rx_desc %pK msdu %pK rx_ind_msg %pK \n", rx_desc, msdu, rx_ind_msg);
                    qdf_nofl_info("status %d peer_id %d tid %d no. of pkts %d\n", pdev->g_htt_status, pdev->g_peer_id, pdev->g_tid, *npackets);
                    qdf_nofl_info("peer_mac 0x%x:0x%x:0x%x:0x%x:0x%x:0x%x\n",
                            pdev->g_peer_mac[0], pdev->g_peer_mac[1], pdev->g_peer_mac[2],
                            pdev->g_peer_mac[3], pdev->g_peer_mac[4], pdev->g_peer_mac[5]);
                    qdf_nofl_info("num_mpdus %d mpdu_ranges %d msdu_chained %d\n",
                            pdev->g_num_mpdus, pdev->g_mpdu_ranges, pdev->g_msdu_chained);
                qdf_nofl_info("Rx Attention 0x%x\n", pdev->ar_rx_ops->get_attn_word(rx_desc));

                    qdf_nbuf_map_nbytes(pdev->osdev, msdu, QDF_DMA_FROM_DEVICE, HTT_RX_BUF_SIZE);
                    return NULL;
                } else {
                    qdf_nofl_info("DONE BIT NOT SET but EITHER PEER IS VALID (OR) NOT A FIRST MSDU\n");
                    qdf_nofl_info("rx_desc %pK msdu %pK rx_ind_msg %pK \n", rx_desc, msdu, rx_ind_msg);
                    qdf_nofl_info("status %d peer_id %d tid %d no. of pkts %d\n", pdev->g_htt_status, pdev->g_peer_id, pdev->g_tid, *npackets);
                    qdf_nofl_info("peer_mac 0x%x:0x%x:0x%x:0x%x:0x%x:0x%x\n",
                            pdev->g_peer_mac[0], pdev->g_peer_mac[1], pdev->g_peer_mac[2],
                ...
                qdf_nofl_info("next_rx_desc %pK next_msdu %pK \n", rx_desc, next_msdu);
                qdf_nofl_info("Rx Attention of next_msdu 0x%x\n",
                    pdev->ar_rx_ops->get_attn_word(rx_desc));

            if (pdev->ar_rx_ops->attn_msdu_done(rx_desc)) {
                    qdf_nofl_info("RECOVERED FROM DONE BIT NOT SET ERROR. USING NEXT MSDU\n");
                ...
                msdu = htt_rx_netbuf_pop((rx_handle_t)pdev);
                    ...
                    qdf_nofl_info("DONE BIT NOT SET FOR CURRENT AND NEXT MSDU\n");
                HTT_ASSERT_ALWAYS(pdev->ar_rx_ops->attn_msdu_done(rx_desc));
                }
            }
        } else {
            if (pdev->htt_rx_donebit_assertflag == 1) {
                qdf_nofl_info("rx_desc %pK msdu %pK \n", rx_desc, msdu);
                qdf_nofl_info("Rx Attention of msdu 0x%x\n",
                    pdev->ar_rx_ops->get_attn_word(rx_desc));
                qdf_nofl_info("RECOVERED FROM DONE BIT NOT SET ERROR. USING SAME MSDU\n");
                pdev->htt_rx_donebit_assertflag = 0;
            }
            if ((*npackets) == 0) {
            msdu = htt_rx_netbuf_pop((rx_handle_t)pdev);
...
htt_rx_attach(struct htt_pdev_t *pdev)
{
    ...
    if (!pdev->cfg.is_high_latency) {
        ...
        if(pdev->ar_rx_ops == NULL) {
            qdf_nofl_info("%s:%d Invalid target",__func__,__LINE__);
            goto fail1;
        }
