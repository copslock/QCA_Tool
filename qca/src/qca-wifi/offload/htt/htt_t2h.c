                                        ...
                                        uint32_t *num_htt_cmpls)
{
    ...
    case HTT_T2H_MSG_TYPE_TX_INSPECT_IND:
        {
            ...
            if (CE_is_directlink(((struct ol_txrx_pdev_t*)pdev->txrx_pdev)->ce_tx_hdl)) {
                qdf_nofl_info("ERROR: TX_INSPECT_IND over Directlink\n");
                A_ASSERT(0);
            }
#endif /* QCA_PARTNER_DIRECTLINK_TX */
            ol_tx_inspect_handler(pdev->txrx_pdev, num_msdus, msg_word + 1);
    ...
    case HTT_T2H_MSG_TYPE_TX_MODE_SWITCH_IND:
        {
            ...
            if(mode_enable) {
                ...
                if(mode == HTT_TX_MODE_PUSH_NO_CLASSIFY) {
                    ...
                    qdf_nofl_debug("Switching to Tx Mode-%u", mode);
                    ...
                    qdf_nofl_debug("Switching to Tx Mode-%u Threshold %d", mode, threshold);
    ...
    case HTT_T2H_MSG_TYPE_CFR_DUMP_COMPL_IND:
	    ...
	    qdf_debug("CFR:%s HTT event received from FW\n");
            ...
            scn = (struct ol_ath_softc_net80211 *)prdev->scnctx;

            if(scn && scn->sc_pdev)
                ol_txrx_htt_cfr_rx_ind_handler(scn->sc_pdev, msg_word, qdf_nbuf_len(htt_t2h_msg));
    ...
    for (i = 0; i < num_cmpls; i++) {
        ...
        if (pdev->nss_wifiol_ctx) {
            if ((msg_type == HTT_T2H_MSG_TYPE_TX_INSPECT_IND) ||
                ...
                qdf_nofl_info("NSS Wifi Offload : Unhandled %x\n", msg_type);
                qdf_nbuf_free(htt_t2h_msg);
                continue;
...
#if QCA_PARTNER_DIRECTLINK_TX
                    if (CE_is_directlink(((struct ol_txrx_pdev_t*)pdev->txrx_pdev)->ce_tx_hdl)) {
                        qdf_nofl_info("ERROR: TX_COMPL_IND received over Directlink\n");
                        A_ASSERT(0);
                    }
#endif /* QCA_PARTNER_DIRECTLINK_TX */
                    ol_tx_completion_handler(
