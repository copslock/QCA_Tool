				  ...
				  u_int8_t peer_index, u_int8_t peer_index_fw)
{
	...
	if (cfg->peer_id[peer_index].index_group == 0xff) {
		ext_req->atf_peer_ext_info[peer_index_fw].group_index = 0;
		return retv;
	}
	if (pa->atf_ssidgroup) {
		  ...
		  struct pdev_atf_group_wmm_ac_req * grp_ac_req)
{
	...
	if (pa->atf_ssidgroup) {
		...
		if (grp_ac_req->num_groups > ATF_ACTIVED_MAX_ATFGROUPS)
			grp_ac_req->num_groups = ATF_ACTIVED_MAX_ATFGROUPS;
		atf_debug("Num groups : %d \n",grp_ac_req->num_groups);
	...
	if (cfg->peer_num_cal != 0) {
		...
		for (i = 0; (i < ATF_ACTIVED_MAX_CLIENTS) &&
			...
			if (cfg->peer_id[i].sta_assoc_status == 1) {
				...
				atf_debug("Peer MAC: %s, GroupIndex: %u, atf_units_reserved: 0x%x airtime: %d\n",
