				     ...
				     uint32_t index)
{
	...
	/* if group being deleted is not the last,
	 * move other groups (following the deleted group index)
	 * to the index of the deleted group */
	if ((index != (cfg->grp_num_cfg -1)) &&
	    (index < ATF_ACTIVED_MAX_ATFGROUPS)) {
		qdf_mem_copy(&cfg->atfgroup[index], &cfg->atfgroup[index + 1],
			     (sizeof(cfg->atfgroup[0]) *
			     (cfg->grp_num_cfg -index -1)));
	}
			   ...
			   uint32_t peer_index)
{
	...
	if  (!val->ssid) {
		...
		cfg->peer_id[peer_index].sta_cfg_value[cfg_index + 1] =
								   val->value;
		cfg->peer_id[peer_index].index_group = cfg_index + 1;
...
static int32_t atf_build_alloc_tbl(struct wlan_objmgr_pdev *pdev)
{
	...
	if (pa->atfcfg_set.peer_num_cal != 0) {
		...
		atf_info("Empty table,no setting to pass firmware!\n");
		retv = -1;
	}
