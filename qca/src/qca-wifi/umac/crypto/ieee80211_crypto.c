...
ieee80211_crypto_register(struct ieee80211com *ic, const struct ieee80211_cipher *cip)
{
    ...
    if (cip->ic_cipher >= IEEE80211_CIPHER_MAX) {
        qdf_nofl_info("%s: cipher %s has an invalid cipher index %u\n",
               __func__, cip->ic_name, cip->ic_cipher);
        return;
    }
    if (ic->ciphers[cip->ic_cipher] != NULL && ic->ciphers[cip->ic_cipher] != cip) {
        qdf_nofl_info("%s: cipher %s registered with a different template\n",
        ...
        if (ret) {
            ...
            if (vap->iv_key_map) {
                ...
                if(ni && ni->ni_table == NULL) {
                    ...
                    IEEE80211_CRYPTO_KEY_UNLOCK_BH(key);
                    qdf_nofl_info("Warning: node not in table 0x%pK\n", ni);
                    return ret;
                }
                           ...
                           wbuf_t wbuf)
{
    ...
    if (!IEEE80211_IS_MULTICAST(wh->i_addr1)) {
        ni = ieee80211_vap_find_node(vap, wh->i_addr1, WLAN_MLME_SB_ID);
        if (!ni) {
            return NULL;
        }
    }
