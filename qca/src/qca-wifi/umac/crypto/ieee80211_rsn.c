...
ieee80211_set_persta_default_key(struct ieee80211vap *vap, u_int16_t keyix, ieee80211_keyval *kval)
{
    ...
    ni = ieee80211_vap_find_node(vap, kval->macaddr, WLAN_MLME_SB_ID);
    if (ni == NULL) {
...
ieee80211_set_default_key(struct ieee80211vap *vap, u_int16_t keyix, ieee80211_keyval *kval)
{
    ...
    if ((keyix >= IEEE80211_WEP_NKID)
        ...
        qdf_nofl_info("%s: invalid key index %d\n", __func__, keyix);
        return -EINVAL;
    }
...
ieee80211_set_keymapping_key(struct ieee80211vap *vap, ieee80211_keyval *kval)
{
    ...
    if (ni == NULL) {
         ...
         */
        if (kval->keytype == IEEE80211_CIPHER_WEP)
            qdf_nofl_info("%s:node not found, dropping WEP keys. check.\n", __FUNCTION__);
...
wlan_set_key(wlan_if_t vaphandle, u_int16_t keyix, ieee80211_keyval *key)
{
    ...
    if (keyix == IEEE80211_KEYIX_NONE)
        status = ieee80211_set_keymapping_key(vap, key);
    else
        status = ieee80211_set_default_key(vap, keyix, key);
#endif
             ...
             u_int8_t get_pn_flag)
{
    ...
    qdf_mem_copy(kval->keydata, k->wk_key, k->wk_keylen);
    ...
    if (kval->keytype == IEEE80211_CIPHER_TKIP) {
        ...
        qdf_mem_copy(kval->keydata + kval->keylen, k->wk_key + k->wk_keylen, IEEE80211_MICBUF_SIZE);
...
bad:
    ...
    /* Zero-out local key variables */
    qdf_mem_zero(k, sizeof(struct ieee80211_key));
    return error;
#endif
}
...
#if QCA_SUPPORT_SON
	    wlan_acl_apply_node_rssi_thresholds(vap, macaddr);
        son_send_node_associated_event(vap->vdev_obj, ni->peer_obj);
#endif

#if ATH_SUPPORT_HYFI_ENHANCEMENTS
        /* send node authorized event */
        ald_assoc_notify(vap, macaddr, ALD_ACTION_ASSOC, ni->ni_assocstatus);
#endif
