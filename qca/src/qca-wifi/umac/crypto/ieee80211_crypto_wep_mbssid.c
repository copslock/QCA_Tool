...
static int ieee80211_crypto_keymiss(struct ieee80211_node *ni, wbuf_t wbuf, struct ieee80211_rx_status *rs)
{
    ...
    if (wh->i_fc[1] & IEEE80211_FC1_WEP) {
        ...
        sender = ieee80211_find_node(nt, wh->i_addr2, WLAN_MLME_SB_ID);
        if(sender == NULL) {
        ...
        if (cip->ic_cipher != IEEE80211_CIPHER_WEP) {
            ieee80211_free_node(sender, WLAN_MLME_SB_ID);
            return 1;
        }
                                    ...
                                    struct ieee80211_rx_status *rs)
{
    ...
    if ((rs->rs_flags & IEEE80211_RX_KEYMISS)) {
        ni = ieee80211_find_rxnode(ic,
                (const struct ieee80211_frame_min *) wbuf_header(wbuf), WLAN_MLME_SB_ID);
