                           ...
                           uint8_t secch_2)
{
    ...
    switch (phymode) {
        ...
        case IEEE80211_MODE_11AC_VHT80_80:
            bcn->is_dot11acplus = 1;
            bcn->htcap.hc_ie.hc_cap |= WLAN_HTCAP_C_CHWIDTH40;
            bcn->vhtop.vht_op_chwidth = (phymode == IEEE80211_MODE_11AC_VHT160) ?
                                        WLAN_VHTOP_CHWIDTH_160 : WLAN_VHTOP_CHWIDTH_80_80;
            bcn->vhtop.vht_op_ch_freq_seg1 = secch_1;
...
#if SUPPORT_11AX_D3
        heop_param.co_located_bss = 1;
        heop_param.op_6g_info_present = 1;
#endif
                      ...
                      uint32_t ieee_chan_freq)
{
   ...
   for (ix = 0; !ret && ix < bcn->nbss; ix++) {
       ...
       if (!rx_param) {
           qdf_info("%s: Rx params could not be allocated", __func__);
           ret = ACSDBG_ERROR;
           continue;
       }
                             ...
                             ieee80211_acs_t ic_acs)
{
    ...
    *chan_nf = chan->event[ix].noise_floor;
    ...
    /* Adding all RF characterization entries in one go */
    if (!chan->acs_debug_is_rf_char_entries_loaded &&
