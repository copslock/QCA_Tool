...
wlan_find_full_channel(struct ieee80211com *ic, uint32_t freq)
{
    ...
    c = NULL;
    if (band_flag & IEEE80211_CHAN_2GHZ) {
        if (IEEE80211_SUPPORT_PHY_MODE(ic, IEEE80211_MODE_11NG_HT20)) {
            c = ieee80211_find_channel(ic, freq, 0, (band_flag | IEEE80211_CHAN_HT20));
        }
        if (c == NULL) {
    ...
    } else if (band_flag & IEEE80211_CHAN_5GHZ) {
    ...
    } else if (band_flag & IEEE80211_CHAN_6GHZ) {
        if (IEEE80211_SUPPORT_PHY_MODE(ic, IEEE80211_MODE_11AXA_HE20)) {
            c = ieee80211_find_channel(ic, freq, 0, (band_flag | IEEE80211_CHAN_HE20));
        }
    }
    return c;
}
        ...
        struct scan_cache_entry *scan_entry)
{
    ...
    struct ieee80211_ath_channel *bcn_chan =
        wlan_find_full_channel(ic, se_chan->chan_freq);
        ...
        struct scan_cache_entry *scan_entry)
{
    ...
    uint16_t bcn_chan_freq = util_scan_entry_channel(scan_entry)->chan_freq;
    uint16_t vht_op_freq_seg2 = 0;
    ...
    struct ieee80211_ath_channel *bcn_recv_chan =
        wlan_find_full_channel(ic, bcn_chan_freq);
    ...
    if (ap_vhtop) {
        ...
        vht_op_freq_seg2 = wlan_reg_chan_band_to_freq(ic->ic_pdev_obj, vht_op_ch_freq_seg2, BIT(wlan_reg_freq_to_band(bcn_chan_freq)));

        ch_width = ieee80211_get_chan_width_from_phymode(ap_phymode);
        mode = get_mode_from_phymode(ap_phymode);

        if (wlan_psoc_nif_fw_ext_cap_get(psoc, WLAN_SOC_RESTRICTED_80P80_SUPPORT) &&
                (ch_width == CH_WIDTH_80P80MHZ) &&
                !(CHAN_WITHIN_RESTRICTED_80P80(bcn_chan_freq, vht_op_freq_seg2))) {
            ch_width = CH_WIDTH_80MHZ;
            ap_phymode = ieee80211_get_composite_phymode(mode, ch_width, 0);
        }

        chan = ieee80211_find_dot11_channel(ic, bcn_chan_freq,
                vht_op_freq_seg2, ap_phymode | halfquarter);
        ...
        chan = ieee80211_find_dot11_channel(ic, bcn_chan_freq, 0,
        ...
        struct scan_cache_entry* scan_entry)
{
    ...
    if (!new_chan) {
        qdf_nofl_info("new_chan is null");
        return;
    }
...
static const u_int16_t default_scan_order[] = {
    ...
    /* 6G channels */
    5945, 5965, 5985, 6005, 6025, 6045, 6065, 6085, 6105, 6125,
    6145, 6165, 6185, 6205, 6225, 6245, 6265, 6285, 6305, 6325,
    6345, 6365, 6385, 6405, 6425, 6445, 6465, 6485, 6505, 6525,
    6545, 6565, 6585, 6605, 6625, 6645, 6665, 6685, 6705, 6725,
    6745, 6765, 6785, 6805, 6825, 6845, 6865, 6885, 6905, 6925,
    6945, 6965, 6985, 7005, 7025, 7045, 7065, 7085, 7105,
};
...
#define IEEE80211_2GHZ_FREQUENCY_THRESHOLD    3000  /* in kHz */
    ...
    if (ic->ic_custom_scan_order_size > 0) {
        qdf_nofl_info("Using custom scan order\n"),
    ...
    if (wlan_wide_band_scan_enabled(ic)) {
        ...
        for (i = 0; i < scan_order_size; ++i) {
            /* Add 20 MHz channel */
            c = NULL;
            band_flag  = ieee80211_get_band_flag(scan_order[i]);
            if (IEEE80211_SUPPORT_PHY_MODE(ic, IEEE80211_MODE_11AXA_HE20)) {
                c = ieee80211_find_channel(ic, scan_order[i], 0, band_flag | IEEE80211_CHAN_HE20);
                ...
                c = ieee80211_find_channel(ic, scan_order[i], 0, band_flag | IEEE80211_CHAN_VHT20);
            } else if (IEEE80211_SUPPORT_PHY_MODE(ic, IEEE80211_MODE_11NA_HT20)) {
                c = ieee80211_find_channel(ic, scan_order[i], 0, band_flag | IEEE80211_CHAN_HT20);
            ...
            if (IEEE80211_SUPPORT_PHY_MODE(ic, IEEE80211_MODE_11AXA_HE40PLUS)) {
                c = ieee80211_find_channel(ic, scan_order[i], 0, band_flag | IEEE80211_CHAN_HE40PLUS);
           } else if (IEEE80211_SUPPORT_PHY_MODE(ic, IEEE80211_MODE_11AC_VHT40PLUS)) {
                c = ieee80211_find_channel(ic, scan_order[i], 0, band_flag | IEEE80211_CHAN_VHT40PLUS);
           } else if (IEEE80211_SUPPORT_PHY_MODE(ic, IEEE80211_MODE_11NA_HT40PLUS)) {
                c = ieee80211_find_channel(ic, scan_order[i], 0, band_flag | IEEE80211_CHAN_HT40PLUS);
           }
#if ATH_SUPPORT_DFS && ATH_SUPPORT_STA_DFS
            if (c && !IEEE80211_IS_CHAN_RADAR(c))
#else
            ...
            if (IEEE80211_SUPPORT_PHY_MODE(ic, IEEE80211_MODE_11AXA_HE40MINUS)) {
                c = ieee80211_find_channel(ic, scan_order[i], 0, band_flag | IEEE80211_CHAN_HE40MINUS);
            } else if (IEEE80211_SUPPORT_PHY_MODE(ic, IEEE80211_MODE_11AC_VHT40MINUS)) {
                c = ieee80211_find_channel(ic, scan_order[i], 0, band_flag | IEEE80211_CHAN_VHT40MINUS);
            } else if (IEEE80211_SUPPORT_PHY_MODE(ic, IEEE80211_MODE_11NA_HT40MINUS)) {
                c = ieee80211_find_channel(ic, scan_order[i], 0, band_flag | IEEE80211_CHAN_HT40MINUS);
            }
#if ATH_SUPPORT_DFS && ATH_SUPPORT_STA_DFS
            if (c && !IEEE80211_IS_CHAN_RADAR(c))
#else
            ...
            if (IEEE80211_SUPPORT_PHY_MODE(ic, IEEE80211_MODE_11AXA_HE80)) {
                c = ieee80211_find_channel(ic, scan_order[i], 0, band_flag | IEEE80211_CHAN_HE80);
            } else if (IEEE80211_SUPPORT_PHY_MODE(ic, IEEE80211_MODE_11AC_VHT80)) {
                c = ieee80211_find_channel(ic, scan_order[i], 0, band_flag | IEEE80211_CHAN_VHT80);
            }
#if ATH_SUPPORT_DFS && ATH_SUPPORT_STA_DFS
            if (c && !IEEE80211_IS_CHAN_RADAR(c))
#else
            ...
            if (IEEE80211_SUPPORT_PHY_MODE(ic, IEEE80211_MODE_11AXA_HE160)) {
                c = ieee80211_find_channel(ic, scan_order[i], 0, band_flag | IEEE80211_CHAN_HE160);
            } else if (IEEE80211_SUPPORT_PHY_MODE(ic, IEEE80211_MODE_11AC_VHT160)) {
                c = ieee80211_find_channel(ic, scan_order[i], 0, band_flag | IEEE80211_CHAN_VHT160);
            }
#if ATH_SUPPORT_DFS && ATH_SUPPORT_STA_DFS
            if (c && !IEEE80211_IS_CHAN_RADAR(c))
#else
            ...
            if (IEEE80211_SUPPORT_PHY_MODE(ic, IEEE80211_MODE_11AXA_HE80_80)) {
                c = ieee80211_find_channel(ic, scan_order[i], 0, band_flag | IEEE80211_CHAN_HE80_80);
            } else if (IEEE80211_SUPPORT_PHY_MODE(ic, IEEE80211_MODE_11AC_VHT80_80)) {
                c = ieee80211_find_channel(ic, scan_order[i], 0, band_flag | IEEE80211_CHAN_VHT80_80);
            }
    ...
    for(i=0; i < nallchans; ++i) {
        ...
        param->ch_param[i].allow_ht = FALSE;
        param->ch_param[i].allow_he = FALSE;
        /* fill in mode. use 11G for 2G and 11A for 5G */

        if (WLAN_REG_IS_24GHZ_CH_FREQ(c->ic_freq)) {
        ...
        if (ieee80211_find_channel(ic, c->ic_freq, 0, IEEE80211_CHAN_5GHZ | IEEE80211_CHAN_VHT20)) {
        ...
        if (IEEE80211_IS_CHAN_11AXA_HE80_80(c)) {
            ...
            param->ch_param[i].allow_he = TRUE;
            param->ch_param[i].phy_mode = WMI_HOST_MODE_11AX_HE80_80;
            param->ch_param[i].cfreq1 = c->ic_vhtop_freq_seg1;
            param->ch_param[i].cfreq2 = c->ic_vhtop_freq_seg2;
    ...
    frag_list_bytes = (sizeof(struct scan_chan_list_params) +
            (nallchans * sizeof(struct channel_param)));
