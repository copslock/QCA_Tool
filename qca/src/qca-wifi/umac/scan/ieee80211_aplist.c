    ...
    )
{
     ...
     */
    if (active_ap != NULL) {
        hystAdj = ieee80211_aplist_map_value(util_scan_entry_snr(scan_entry),
                                             hysteresis_table,
    ...
    adjusted_utility = ieee80211_aplist_map_value(snr,
                                                  pUtilityTable->data,
    ...
    IEEE80211_DPRINTF(vaphandle, IEEE80211_MSG_SCANENTRY, "BSSID=%02X:%02X:%02X:%02X:%02X:%02X Ut=%5d Rk=%3d (hyst=(%d * %d) RSSI:%2d => %d, mode:%s, load:%d)\n",
        ...
        hystAdj, hystMode,
        util_scan_entry_snr(scan_entry), snr,
    ...
    )
{
    ...
    if (n_entries > 0)
    {
         ...
         */
        {
            ...
            snr    = util_scan_entry_snr(ap_list[0]);
            ...
            IEEE80211_DPRINTF(vaphandle, IEEE80211_MSG_ASSOC,
                ...
                "PhyType=%08X Ch=%3d Rssi=%02d SNR = %02d Rank=%05d Utility=%05d ChFlags=%08X\n",
                bssid[0], bssid[1], bssid[2],
                bssid[3], bssid[4], bssid[5],
                ieee80211_chan2mode(channel),
                channel->ic_ieee,
                ieee80211_channel_frequency(channel),
                util_scan_entry_rssi(ap_list[0]),
                snr,
                rank,
                utility,
            ...
            if (n_entries > 1) {
                ...
                snr    = util_scan_entry_snr(ap_list[1]);
                ...
                IEEE80211_DPRINTF(vaphandle, IEEE80211_MSG_ASSOC,
                    ...
                    channel->ic_ieee,
                    ieee80211_channel_frequency(channel),
                    util_scan_entry_rssi(ap_list[1]),
                    snr,
                    rank,
                    utility,
                    wlan_channel_flags(channel));
    ...
    )
{
    ...
    do {
         ...
         */
        if ((des_ssid->len != 0) && (candidate_ssid_len != 0)) {
            /* 
        ...
        if (! ieee80211_aplist_get_accept_any_bssid(pconfig)) {
            ...
            bIsAcceptable = false;
            bssid = util_scan_entry_bssid(scan_entry);
            ...
            for (i = 0; i < ieee80211_aplist_get_desired_bssid_count(pconfig); i++) {
                ...
                if (IEEE80211_ADDR_EQ(bssid, desired_bssid) == true) {
                    ...
                    if (candidate_ssid_len == 0) {
                         scan_entry->ssid.length = des_ssid->len;
                         qdf_mem_copy(scan_entry->ssid.ssid, des_ssid->ssid, des_ssid->len);
                    }
                    bIsAcceptable = true;
                    break;
                }
    ...
    )
{
    ...
    IEEE80211_DPRINTF(vaphandle, IEEE80211_MSG_SCANENTRY, "%s: %02X:%02X:%02X:%02X:%02X:%02X PhyMode=%08X Ch=%3d SNR=%2d RSSI=%2d",
        __func__,
        mac_address[0], mac_address[1], mac_address[2], 
        mac_address[3], mac_address[4], mac_address[5],
        phy_mode,
        wlan_channel_ieee(channel),
        wlan_channel_frequency(channel),
        util_scan_entry_snr(scan_entry),
    ...
    if (vaphandle->iv_ccx_evtable && vaphandle->iv_ccx_evtable->wlan_ccx_validate_bss &&
       !vaphandle->iv_ccx_evtable->wlan_ccx_validate_bss(vaphandle->iv_ccx_arg,
                                                         scan_entry,
                                                         active_ap ?
                                                         util_scan_entry_snr(active_ap) : 0)){
    ...
    if (calculate_rank) {
        ...
        IEEE80211_DPRINTF(vaphandle, IEEE80211_MSG_SCANENTRY, " - Accepted, RSSI %2d SNR %2d\n",
                util_scan_entry_rssi(scan_entry),
                util_scan_entry_snr(scan_entry));
