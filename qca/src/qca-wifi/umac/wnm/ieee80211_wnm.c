...
int wnm_netlink_init(ieee80211_wnm_t wnm)
{
    if (wnm->wnm_nl_sock == NULL) {
        ...
        if (wnm->wnm_nl_sock == NULL) {
            qdf_nofl_info("%s: %d NETLINK_KERNEL_CREATE failed!\n", __func__, __LINE__);
            return -ENODEV;
        }
    }
    return 0;
                      ...
                      struct ieee80211_bstm_reqinfo *bstmreq)
{
    ...
    ni = ieee80211_vap_find_node(vap, macaddr, WLAN_WNM_ID);
    if (ni == NULL) {
        error = -EINVAL;
        goto exit;
    }
    ...
    ieee80211_free_node(ni, WLAN_WNM_ID);
    ...
    while (tclass_len > 0) {
        ...
        if (IEEE80211_ELEMID_TCLAS == elemid) {
            ...
            if (tclas == NULL) {
                ...
                qdf_nofl_info("%s: %d OS_MALLOC failed!\n", __func__, __LINE__);
                return ENOMEM;
            }
                     ...
                     tclas->type == IEEE80211_WNM_TCLAS_CLASSIFIER_TYPE4) 
            {
                ...
                wlan_wnm_free_tclass(tclass_head);
                qdf_nofl_info("%s: %d unknown tclass classifier type %d\n",
            ...
            wlan_wnm_free_tclass(tclass_head);
            qdf_nofl_info("%s: %d unknown elementid %d\n", __func__, __LINE__, elemid);
            return -EINVAL;
        }
    }
...
int wlan_wnm_set_tfs(wlan_if_t vaphandle, void *data)
{
    ...
    ni = ieee80211_find_node(&vap->iv_ic->ic_sta, vap->iv_bss->ni_bssid, WLAN_WNM_ID);
    if (ni == NULL) {
        wlan_wnm_free_tfs(&tfs_req_head);
        return -EINVAL;
    }
...
int wlan_wnm_set_fms(wlan_if_t vaphandle, void *data)
{
        ...
        while (subelem_count < req_fmsreq->num_subelements) {
            ...
            while (tclas_elem_count < req_subelement->num_tclas_elements) {
                ...
                if (rv != 0) {
                    ...
                    OS_FREE(fms_req);
                    OS_FREE(fmssub);
                    qdf_nofl_info("%s: wlan_wnm_set_tclass failed!\n", __func__);
                    return -ENOMEM;
                }
                tclas_elem_count++;
            }
    ...
    ni = ieee80211_vap_find_node(vap, vap->iv_bss->ni_bssid, WLAN_WNM_ID);
    if (ni == NULL) {
        qdf_nofl_info("%s : %d ieee80211_vap_find_node failed !\n", __func__, __LINE__);
        return -EINVAL;
    }
    event = OS_MALLOC(vap->iv_ic->ic_osdev, sizeof(*event), GFP_KERNEL);
...
int ieee80211_wnm_sleepreq_to_app(wlan_if_t vap, u_int8_t action, u_int16_t intval)
{
    ...
    if (action == IEEE80211_WNMSLEEP_ACTION_ENTER)
        ...
        qdf_nofl_info("Not supported WNM-Sleep action %d\n", action);
        return -1;
    }
...
int ieee80211_set_wnmfilter(wlan_if_t vap, int val)
{
    ieee80211_wnm_t wnm = vap->wnm;

    wnm->wnm_app_filter = val;
...
int ieee80211_wnm_set_appie(wlan_if_t vap, u8 *buf, int len)
{
    ...
    ni = ieee80211_vap_find_node(vap, mac, WLAN_WNM_ID);
    if (!ni)
        return -EINVAL;
                                ...
                                int subtype, struct ieee80211_rx_status *rs)
{
    ...
    /* Send WNM ALD netlink event for all opmodes */
    ald_wnm_frame_recvd_notify(vap, IEEE80211_ACTION_CAT_WNM, ni->ni_macaddr,
                               wbuf_header(wbuf), wbuf_get_pktlen(wbuf));
