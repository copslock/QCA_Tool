...
enum {
    ...
    IEEE80211_SUBELEMID_BR_IE_EXTREQ = 11,
    IEEE80211_SUBELEMID_BR_CHANREP = 51,
    IEEE80211_SUBELEMID_BR_WIDEBAND_CHSWITCH = 163,
    IEEE80211_SUBELEMID_BR_LASTIND = 164,
    IEEE80211_SUBELEMID_BR_VENDOR = 221,
};
...
enum {
    ...
    IEEE80211_SUBELEMID_BREPORT_WB_CHSWITCDH = 163,
};
...
/* custom neighbor report response struct */
struct ieee80211_action_nr_resp_elem {
    u_int8_t  id;
    u_int8_t  len;
    u_int8_t  bssid[6];
    u_int32_t bssi_info;
...
/* GET_FIXED_RRM_MEASRSP_IE_LEN determines ieee80211_measrsp_ie length except id and len*/
#define GET_FIXED_RRM_MEASRSP_IE_LEN(MIE) sizeof(MIE->token) + sizeof(MIE->rspmode) + sizeof(MIE->rsptype)
...
int ieee80211_rrm_beacon_measurement_report(wlan_if_t vap,
        u_int8_t *frm,u_int32_t frm_len,u_int8_t dialog_token);
