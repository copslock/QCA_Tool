            ...
            for (i = 0; i < MAX_RADIO_CNT; i++) {
                ...
                if (tmp_ic) {
                    ...
                    IEEE80211_DPRINTF(vap, IEEE80211_MSG_RRM,
                            "Go through the scan table for IC %d: channel=%d freq=%d\n",
                            i, tmp_ic->ic_curchan->ic_ieee,tmp_ic->ic_curchan->ic_freq);
                                ...
                                struct ieee80211_action_nr_req *nr_req, int frm_len)
{
    ...
    while (remaining_ie_length  >= sizeof(struct ieee80211_ie_header) ) {
        ...
        switch (elem_id) {
            ...
            case IEEE80211_ELEMID_MEASREQ:
                ...
                if (len < 4)
                    break;
                if (nrreq_info.meas_count >= ARRAY_SIZE(nrreq_info.meas_token))
                    break;
    ...
    if (rrm->rrm_app_filter) {
        IEEE80211_DELIVER_EVENT_NEIGHREQ_AP(vap, ni->ni_macaddr, &nrreq_info);
        return 0;
    } else {
        /* send Neigbor info */
...
static int
ieee80211_rrm_handle_report(wlan_if_t vap, u_int8_t *frm, u_int32_t frm_len,
                            u_int8_t dialog_token)
{
    ...
    switch(rsp->rsptype)
    {
        ...
        if (rsp->rspmode & (IEEE80211_RRM_MEASRPT_MODE_BIT_INCAPABLE |
            ...
            ieee80211_rrm_beacon_measurement_report(vap, frm, frm_len, dialog_token);
            break;
        ...
        u_int8_t *frm, u_int32_t frm_len)
{
    ...
    ieee80211_rrm_handle_report(vap,frm,frm_len,dialog_token);
...
int ieee80211_rrm_chload_report(wlan_if_t vap,u_int8_t *frm,u_int32_t frm_len)
{
    ...
    ni = ieee80211_vap_find_node(vap, vap->rrm->rrm_macaddr, WLAN_RRM_ID);
    if( ni == NULL )
        return -EINVAL;

    if(( mie->token > ni->chload_measure_token) || (elen < RRM_MIN_RSP_LEN)) /* minimum length should be three */
    {
        ieee80211_free_node(ni, WLAN_RRM_ID);
        return -EINVAL;
    }
...
int ieee80211_rrm_frm_report(wlan_if_t vap,u_int8_t *frm, u_int32_t frm_len)
{
    ...
    ni = ieee80211_vap_find_node(vap, rrm->rrm_macaddr, WLAN_RRM_ID);
    if( ni == NULL )
       return -EINVAL;
            ...
            case IEEE80211_SUBELEMID_FR_REPORT_COUNT:
                {
                    ...
                    for (i = 0; i < cnt; i++) {
                        ...
                        OS_MEMCPY(&frm_data->ta[0], frm_rpt->ta, QDF_MAC_ADDR_SIZE);
                        OS_MEMCPY(&frm_data->bssid[0], frm_rpt->bssid, QDF_MAC_ADDR_SIZE);
...
int ieee80211_rrm_tsm_report(wlan_if_t vap,u_int8_t *frm, u_int32_t frm_len)
{
    ...
    OS_MEMCPY(tsm->mac,rsp->mac,QDF_MAC_ADDR_SIZE);
    OS_MEMCPY(tsm->bin,rsp->bin,QDF_MAC_ADDR_SIZE);
...
void add_beacon_entry(wlan_if_t vap ,struct ieee80211_beacon_report *bcnrpt)
{
    ...
    RRM_BTABLE_LOCK(btable);
    {
        ...
        report->ant_id = bcnrpt->ant_id;
        report->mduration = bcnrpt->mduration;
        OS_MEMCPY(report->parent_tsf, bcnrpt->parent_tsf, sizeof(report->parent_tsf));
        OS_MEMCPY(report->ms_time, bcnrpt->ms_time, sizeof(report->ms_time));
...
static void ieee80211_rrm_beacon_subelement(uint8_t subelm_len, uint8_t *subelm_frm,
                                            ieee80211_bcnrpt_t *report)
{
    u_int8_t remaining_subelm_len;
    u_int8_t subelm_id, len;
    struct ieee80211_beaconrpt_wb_chan *bcn_wb_chan = NULL;

    remaining_subelm_len = subelm_len;

    while (remaining_subelm_len  >= sizeof(struct ieee80211_subelm_header) ) {
           subelm_id = *subelm_frm++;
           len = *subelm_frm++;

           if (len + sizeof(struct ieee80211_subelm_header) > remaining_subelm_len) {
               break;
           }
           switch (subelm_id) {
                   case IEEE80211_SUBELEMID_BREPORT_WB_CHSWITCDH:
                        report->wb_channel_present = 1;
                        bcn_wb_chan = (struct ieee80211_beaconrpt_wb_chan *)subelm_frm;
                        report->wb_chan.chan_width = bcn_wb_chan->chan_width;
                        report->wb_chan.centerfreq0 = bcn_wb_chan->centerfreq0;
                        report->wb_chan.centerfreq1 = bcn_wb_chan->centerfreq1;
                        break;
                   default:
                        break;
           }
...
static void ieee80211_rrm_handle_measurement_response_ie(
    ...
    u_int8_t token) {
#endif
    u_int8_t zero_bssid[QDF_MAC_ADDR_SIZE] = {0, 0, 0, 0, 0, 0};
#if QCA_SUPPORT_SON
    u_int8_t *subelm_frm = NULL;
    u_int8_t subelm_len;
#endif
    u_int8_t min_ie_len = (sizeof(struct ieee80211_measrsp_ie) -
                           sizeof(struct ieee80211_ie_header) - 1) +
        ...
        if (mie->len >= min_ie_len) {
            ...
            if (report) {
                ...
                */
                ...
                if (mie->len > (GET_FIXED_RRM_MEASRSP_IE_LEN(mie) + FIXED_BCNRPT_LEN)) {
                    subelm_frm = (u_int8_t *)(&bcnrpt->ies[0]);
                    subelm_len = mie->len -
                           (GET_FIXED_RRM_MEASRSP_IE_LEN(mie) + FIXED_BCNRPT_LEN);
                    ieee80211_rrm_beacon_subelement(subelm_len, subelm_frm, report);
                }
            }
        } else {
                /*
...
static int
get_bcnrpt_count_from_measrep(u_int8_t *sfrm, u_int8_t *efrm) {
    uint32_t num_bcn_rpt =0;
    struct ieee80211_measrsp_ie *mie ;
...
int ieee80211_rrm_beacon_measurement_report(wlan_if_t vap,
        u_int8_t *sfrm, u_int32_t frm_len, u_int8_t dialog_token)
{
    struct ieee80211_measrsp_ie *mie ;
#if QCA_SUPPORT_SON
    ieee80211_bcnrpt_t *reports = NULL;
    ieee80211_bcnrpt_t *app_bcnreports = NULL;
    u_int8_t num_bcnrpt_requested = 0;
    u_int8_t bcnrpt_count = 0;
    u_int32_t num_bcnrpt = 0;
    u_int8_t *frame = sfrm;
    u_int8_t total_bcnrpt_count = 0;
    u_int32_t bcnrpt_ev_data_len = 0;
    struct ev_rrm_report_data *bcnrpt_event_data = NULL;
    ieee80211_rrm_t rrm = vap->rrm;
#endif
    u_int8_t *efrm = sfrm + frm_len;
    struct ieee80211_node *ni = NULL;
...
#if QCA_SUPPORT_SON
    son_update_beacon_report_frame(ni->peer_obj, frame, frm_len);
    num_bcnrpt_requested = son_alloc_rrm_bcnrpt(vap->vdev_obj, &reports);

    if (rrm->rrm_app_filter) {

        num_bcnrpt = get_bcnrpt_count_from_measrep(sfrm, efrm);

        if (num_bcnrpt) {
            bcnrpt_ev_data_len = (num_bcnrpt * sizeof(ieee80211_bcnrpt_t)) + sizeof(struct ev_rrm_report_data);
            bcnrpt_event_data = (struct ev_rrm_report_data *)qdf_mem_malloc(bcnrpt_ev_data_len);

            if (bcnrpt_event_data) {
                qdf_mem_zero(bcnrpt_event_data, bcnrpt_ev_data_len);
                app_bcnreports = (ieee80211_bcnrpt_t *)(&bcnrpt_event_data->bcnrpt[0]);
            }
        }
    }
#endif
            ...
            case IEEE80211_ELEMID_MEASREP:
                {
                    ...
                    if (rrm->rrm_app_filter && app_bcnreports) {
                        ieee80211_rrm_handle_measurement_response_ie(
                               vap, mie, sfrm, ni->br_measure_token,
                               app_bcnreports);
                        app_bcnreports++;
    ...
    if (rrm->rrm_app_filter && app_bcnreports) {
        bcnrpt_event_data->dialog_token = dialog_token;
        bcnrpt_event_data->bcnrpt_count = num_bcnrpt;
        IEEE80211_DELIVER_EVENT_BCNRPT_AP(vap, bcnrpt_event_data, bcnrpt_ev_data_len);
        qdf_mem_free(bcnrpt_event_data);
    }
...
static void ieee80211_add_self_nrinfo(struct ieee80211vap *vap, struct ieee80211_rrm_cbinfo * cb_info)
{
    ...
    ninfo.regclass = regdmn_get_opclass(country_iso, chan);
     ...
     * meanings of ic_vhtop_ch_num_seg1 and ic_vhtop_ch_num_seg2 are changed
    ...
    if (IEEE80211_IS_CHAN_11AC_VHT160(chan)) {
        ninfo.cf_s1 = chan->ic_vhtop_ch_num_seg2;
        ninfo.cf_s2 = 0;
    } else {
        ninfo.cf_s1 = chan->ic_vhtop_ch_num_seg1;
        ninfo.cf_s2 = chan->ic_vhtop_ch_num_seg2;
    }
