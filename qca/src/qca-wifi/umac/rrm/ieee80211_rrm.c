...
/*
 * Check pmf is enabled
 */
static int is_pmf_enabled(wlan_if_t vap, wlan_node_t ni)
{
    ...
    switch(nr_info->meas_type[meas_count]) {
        case IEEE80211_MEASRSP_LCI_REPORT:
            ...
            if(nr_info->colocated_bss[1] > 1) {
                ...
                colocated_bss_len = (nr_info->colocated_bss[1]*QDF_MAC_ADDR_SIZE)+1;
...
QDF_STATUS ieee80211_fill_nrinfo(void *arg, wlan_scan_entry_t se)
{
    ...
    if (country_ie) {
        ninfo.regclass = regdmn_get_opclass(country_ie->cc, chan);
    }
...
static void ieee80211_fill_bcn_measrsp(struct wlan_objmgr_pdev *pdev_obj,
                                       ...
                                       u_int8_t *scan_bssid, u_int8_t *start)
{
    ...
    bcnrep->ch_num = wlan_reg_freq_to_chan(pdev_obj, util_scan_entry_channel_frequency(se));
    ...
    bcnrep->rcpi = snr + RRM_DEFAULT_NOISE_FLOOR;
...
{
    struct bcnreq_se_wrapper *bcnreq_wrap = (struct bcnreq_se_wrapper *)arg;
    struct ieee80211_rrm_cbinfo *cb_info = (struct ieee80211_rrm_cbinfo *)bcnreq_wrap->arg;
    struct wlan_objmgr_pdev *pdev_obj = (struct wlan_objmgr_pdev *)bcnreq_wrap->pdev_obj;
    ...
    if (cb_info->ssid_len > 0) {
        if (wssid && wssid->ssid && (cb_info->ssid_len == wssid->length) &&
            ...
            ieee80211_fill_bcn_measrsp(pdev_obj, se, cb_info, scan_bssid, start);
        }
        ...
        if ((wlan_reg_freq_to_chan(pdev_obj, util_scan_entry_channel_frequency(se)) == cb_info->chnum) || (!cb_info->chnum)) {
            ieee80211_fill_bcn_measrsp(pdev_obj, se, cb_info, scan_bssid, start);
        }
    }
 ...
 * @param nrinfo         : User provided Neighbor Report information
 *
 * @return on success return 0.
 *         on failure returns -ve value.
 */

int wlan_send_neighbor_resp(wlan_if_t vaphandle, u_int8_t *macaddr,
                            ieee80211_user_nrresp_info_t *nrrpt_user)
{
    ...
    int error = 0;
    ieee80211_cust_nrresp_info_t *nrinfo = NULL;

    RRM_FUNCTION_ENTER;
    nrinfo = nrrpt_user->custom_nrresp_info;
    if(!(ieee80211_vap_rrm_is_set(vap) && ieee80211_node_is_rrm(vap,macaddr)))
        return -EINVAL;

    ni = ieee80211_vap_find_node(vap, macaddr, WLAN_RRM_ID);
    ...
    frm = ieee80211_add_nrresp_action_field(frm, IEEE80211_ACTION_NR_RESP,
                                            nrrpt_user->dialog_token, ni);
    frm = ieee80211_add_nrresp_ie((u_int8_t *)(frm), ni, nrinfo,
                                  nrrpt_user->num_report);
    wbuf_set_pktlen(wbuf, (frm - (u_int8_t*)wbuf_header(wbuf)));

	if (is_pmf_enabled(vap, ni)) {
                             ...
                             ieee80211_rrm_beaconreq_info_t *binfo)
{
    ...
    if(IEEE80211_ADDR_EQ(vap->iv_myaddr,macaddr)) {
        ...
        pktlen = (frm - (u_int8_t*)wbuf_header(wbuf));
        BUG_ON(pktlen > wbuf_get_len(wbuf));
...
int wlan_send_stastats_req(wlan_if_t vap, u_int8_t *macaddr, ieee80211_rrm_stastats_info_t *stastats)
{
    ...
    ni = ieee80211_find_node(&ic->ic_sta, macaddr, WLAN_RRM_ID);
...
void wlan_rrm_sta_info(void *arg, wlan_node_t node)
{
    ...
    if (sta_info->count < sta_info->max_sta_cnt) {
        if(node->ni_flags & IEEE80211_NODE_RRM ) {
            IEEE80211_ADDR_COPY((sta_info->dest_addr + (sta_info->count * QDF_MAC_ADDR_SIZE)),node->ni_macaddr);
            sta_info->count++;
        }
    }
}
                         ...
                         uint32_t cnt, uint8_t *buff)
{
    ...
    reply_skb = wlan_cfg80211_vendor_cmd_alloc_reply_skb(vap->iv_ic->ic_wiphy,
...
static void ieee80211_rrm_scan_evhandler(struct wlan_objmgr_vdev *vdev, struct scan_event *event, void *arg)
{
    ...
    if (event->type == SCAN_EVENT_TYPE_RADIO_MEASUREMENT_START) {
        ...
        int ieee_chan_num = wlan_reg_freq_to_chan(ic->ic_pdev_obj, event->chan_freq);
...
int ieee80211_rrm_scan_start(wlan_if_t vap, bool active_scan)
{
    ...
    if ((opmode == IEEE80211_M_STA) || (opmode == IEEE80211_M_IBSS)) {
        ...
        if(rrm_req_params->chnum) {
            chan = ieee80211_find_dot11_channel(ic,
                             wlan_reg_chan_band_to_freq(ic->ic_pdev_obj,
                                  rrm_req_params->chnum,
 ...
 * @return
 */
int ieee80211_set_rrmfilter(wlan_if_t vap, int val)
{
    ieee80211_rrm_t rrm = vap->rrm;

    rrm->rrm_app_filter = val;
