 ...
 * @qcn_reqinfo:pointer to qcn request info

 * Return:New buffer pointer after the added IE
 */

u_int8_t *
ieee80211_add_qcn_info_ie(u_int8_t *ie, struct ieee80211vap *vap,
                    u_int16_t* ie_len, qcn_ie_type type, void *qcn_reqinfo)
{
    ...
    start = frm;
    len  += ieee80211_setup_qcn_ver_attr_ie(&frm);
    len  += ieee80211_setup_qcn_attrs(vap, &frm, type, qcn_reqinfo);
...
/*
 * @ieee80211_setup_qcn_ver_attr_ie() - adds QCN version attribute  IE
 * @frm:tracks location where to attach next sub-attribute
 *
 * Return:length of QCN version attribute
 */

u_int8_t ieee80211_setup_qcn_ver_attr_ie(u_int8_t ** frm)
{
                                                ...
                                                u_int8_t *frm)
{
    ...
    *frm++ = (ic->ic_he.hecap_info_internal &
    ...
    /* Add HE MCS12/13 attribute */
    frm = ieee80211_add_qcn_attr_header_info(frm, QCN_ATTRIB_HE_MCS12_13_SUPP,
                                             HE_MCS12_13_SUPP_ATTRIB_LEN, &len);

    *frm++ = vap->iv_he_mcs12_13_supp ? ic->ic_he.hecap_macinfo[IEEE80211_HE_MCS_12_13_SUPP_L80] : 0;
    *frm++ = vap->iv_he_mcs12_13_supp ? ic->ic_he.hecap_macinfo[IEEE80211_HE_MCS_12_13_SUPP_G80] : 0;
                               ...
                               u_int8_t *macaddr)
{
    ...
    u_int8_t  *frm = NULL;
    u_int16_t ie_len;
    frm =  ieee80211_add_qcn_info_ie(ie, vap, &ie_len,
...
/*
 * @ieee80211_peer_he_mcs_12_13_supp() - parses HE MCS12/13 cap from peer caps
 * @ni:Node structure for the connected peer
 * @ic:pointer to the ic structure
 * @he_mcs_11_12_supp: Support for HE MCS12/13
 *
 * Return:void
 */
static void
ieee80211_peer_he_mcs_12_13_supp(struct ieee80211_node *ni, struct ieee80211com *ic, uint16_t he_mcs_11_12_supp)
{
        uint16_t self_higher_mcs_supp = 0;

        self_higher_mcs_supp |= ic->ic_he.hecap_macinfo[IEEE80211_HE_MCS_12_13_SUPP_L80];
        self_higher_mcs_supp |= ic->ic_he.hecap_macinfo[IEEE80211_HE_MCS_12_13_SUPP_G80] << IEEE80211_HE_HIGHER_MCS_G80;

        if (ni->ni_vap->iv_he_mcs12_13_supp) {
                ni->ni_higher_hemcs_supp = self_higher_mcs_supp & he_mcs_11_12_supp;
        } else {
                ni->ni_higher_hemcs_supp = 0;
        }
}
            ...
            switch(attribute_id){
                ...
                case QCN_ATTRIB_VHT_MCS10_11_SUPP:
                    ...
                    updated_len += (attribute_len +
                                sizeof(struct ieee80211_qcn_header));
                    break;
                case QCN_ATTRIB_HE_MCS12_13_SUPP:
                    attribute_len = *frm++;
                    he_mcs_11_12_supp |= *frm++;
                    he_mcs_11_12_supp |= (*frm++ << IEEE80211_HE_HIGHER_MCS_G80);
                    ieee80211_peer_he_mcs_12_13_supp(ni, ic, he_mcs_11_12_supp);
                    frm = frm + 1;
                    updated_len += (attribute_len +
                                sizeof(struct ieee80211_qcn_header));
                    break;
