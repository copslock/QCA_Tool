    ...
    case IEEE80211_ASSOC_EVENT_REASSOC_REQUEST:
        ...
        if (sm->vap_handle->iv_des_mode != IEEE80211_MODE_AUTO) {
            ...
            if (phymode != IEEE80211_MODE_AUTO) {
                chosen_chan = ieee80211_find_dot11_channel(sm->vap_handle->iv_ic, chosen_chan->ic_freq,
                        chosen_chan->ic_vhtop_freq_seg2, phymode | sm->vap_handle->iv_ic->ic_chanbwflag);
...
static void ieee80211_assoc_state_join_init_entry(void *ctx)
{
    ...
    if ((peer != NULL) && (WLAN_PEER_STA == wlan_peer_get_peer_type(peer))) {
        ...
        ieee80211_sm_dispatch(sm->hsm_handle,
                IEEE80211_ASSOC_EVENT_JOIN_FAIL, 0, NULL);
        return;
    }

    /* release the refrence take for peer !WLAN_PEER_STA */
    if (peer != NULL) {
        wlan_objmgr_peer_release_ref(peer, WLAN_MGMT_HANDLER_ID);
    }
