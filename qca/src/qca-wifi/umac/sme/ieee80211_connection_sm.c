...
extern struct ieee80211_ath_channel *ieee80211_find_channel(struct ieee80211com *ic, int freq, u_int16_t des_cfreq2, u_int64_t flags);
...
struct _wlan_connection_sm {
    ...
    u_int8_t                    bgscan_snr_thresh;
    u_int8_t                    bgscan_snr_change;
    u_int8_t                    bgscan_snr_thresh_forced_scan;  /* snr below which it scan is a forces scan */
    ...
    u_int8_t                    last_scan_snr;       /* last bss snr at which scan is done  */
                            ...
                            wlan_assoc_sm_event *smevent)   
{
    ...
    case WLAN_ASSOC_SM_EVENT_FAILED:
        sm->is_connected=0;
        /* set default disconnect reason otherwise we may end up using
         * stale reason code from previous disconnect */
        sm->disconnect_reason = WLAN_CONNECTION_SM_CONNECT_TIMEDOUT;
...
static void ieee80211_connection_state_init_entry(void *ctx) 
{
    ...
    sm->candidate_aplist_index = 0;
    sm->is_aplist_built = 0;
    if ((!vap->iv_roam_inprogress) && (sm->cur_scan_entry)) {
...
static void ieee80211_connection_state_connecting_entry(void *ctx)
{
    ...
    if (ieee80211_ic_enh_ind_rpt_is_set(sm->vap_handle->iv_ic)) {
        ...
        if (!sm->vap_handle->iv_ic->ic_mpsta_vap || (sm->vap_handle->iv_ic->ic_mpsta_vap && sm->vap_handle->iv_mpsta)) {
            ...
            chan.ic_vhtop_ch_num_seg2 = scan_chan->ic_vhtop_ch_num_seg2;
            chan.ic_vhtop_freq_seg2 = scan_chan->ic_vhtop_freq_seg2;

            phychan = ieee80211_find_channel(sm->vap_handle->iv_ic, chan.ic_freq, chan.ic_vhtop_freq_seg2, chan.ic_flags);

            if (phychan == NULL)
                qdf_nofl_info("%s: not able to locate channel with freq+flags.\n", __func__);

#if ATH_SUPPORT_WRAP
        }
        else {
            struct ieee80211vap *mpstavap = sm->vap_handle->iv_ic->ic_mpsta_vap;
            if (ieee80211_channel_frequency(wlan_util_scan_entry_channel(scan_entry)) != ieee80211_channel_frequency(mpstavap->iv_bsschan)) {
                IEEE80211_DPRINTF(sm->vap_handle, IEEE80211_MSG_STATE,"%s: scan_entry chan %u freq %d, ivbss_chan %u freq %d\n",
                                    __func__,wlan_util_scan_entry_channel(scan_entry)->ic_ieee,ieee80211_channel_frequency(wlan_util_scan_entry_channel(scan_entry)),
                                    mpstavap->iv_bsschan->ic_ieee, ieee80211_channel_frequency(mpstavap->iv_bsschan));
                return;
            }
        }
#endif
...
static void ieee80211_scan_check_2040_coexistinfo(wlan_if_t vaphandle)
{
    ...
    if (!(vaphandle->iv_ic->ic_flags & IEEE80211_F_COEXT_DISABLE) &&
        IEEE80211_IS_CHAN_2GHZ(vaphandle->iv_ic->ic_curchan) &&
...
static void ieee80211_connection_state_bgscan_entry(void *ctx) 
{
    ...
    OS_MEMZERO(scan_params,sizeof(*scan_params));
    n_ssid = ieee80211_get_desired_ssidlist(sm->vap_handle, ssid_list, IEEE80211_SCAN_MAX_SSID);
    if (util_scan_entry_snr(sm->cur_scan_entry) <= sm->bgscan_snr_thresh_forced_scan) {
       forced_scan=true;
    }
    status = wlan_update_scan_params(sm->vap_handle,scan_params,IEEE80211_M_STA,
...
static bool ieee80211_connection_state_bgscan_event(void *ctx, u_int16_t event, u_int16_t event_data_len, void *event_data) 
{
    ...
    case IEEE80211_CONNECTION_EVENT_SCAN_END:
        sm->last_scan_snr = util_scan_entry_snr(sm->cur_scan_entry);
        sm->last_scan_time = OS_GET_TIMESTAMP(); 
        /* Opportunistic Roam - add wlan_connection_sm_is_opportunistic_roam() */
        if ((util_scan_entry_snr(sm->cur_scan_entry) > sm->roam_threshold) &&
                !(wlan_connection_sm_is_opportunistic_roam(sm)))
        {
            /* if snr is above the roam threshold , go back to connected state */
            ieee80211_sm_transition_to(sm->hsm_handle,IEEE80211_CONNECTION_STATE_CONNECTED); 
            wlan_objmgr_vdev_release_ref(vdev, WLAN_OSIF_SCAN_ID);
            return true;
        }
        /*
         * snr < roam_threshold. try roaming.
...
static OS_TIMER_FUNC(connection_sm_roam_check_timer_handler)
{
    ...
    cur_snr = util_scan_entry_snr(sm->cur_scan_entry);
    /*
     * if the snr is above the bg scan threshold by certain fudge then
     * clear the last_scan_snr so that when the snr falls below threshold
     * we will trigger bg scan.
     */ 
    if (cur_snr  > (sm->bgscan_snr_thresh + WLAN_CLEAR_LAST_SCAN_SNR_DELTA)) {
        sm->last_scan_snr=0;
    }
    /* Opportunistic Roam - add ignore_thresholds */
    if ((cur_snr > sm->bgscan_snr_thresh) && (ignore_thresholds == 0)) {
        return;
    }
    IEEE80211_DPRINTF(sm->vap_handle,IEEE80211_MSG_STATE,"%s: roam check cur snr %d snr thresh %d policy %d last_scan_snr %d snr_change_thresh %d \n",
                      __func__, cur_snr, sm->bgscan_snr_thresh, sm->bgscan_policy, sm->last_scan_snr, sm->bgscan_snr_change);
    ...
    if (sm->bgscan_policy ==  WLAN_CONNECTION_BGSCAN_POLICY_RSSI_CHANGE ||
        sm->bgscan_policy ==  WLAN_CONNECTION_BGSCAN_POLICY_BOTH) {
        if (!sm->last_scan_snr ||  ((SM_DIFF(cur_snr ,  sm->last_scan_snr) > sm->bgscan_snr_change)
    ...
    /* Opportunistic Roam - add ignore_thresholds */
    if ((util_scan_entry_snr(sm->cur_scan_entry) < sm->roam_threshold) || ignore_thresholds)
    {
       /* if snr is below the roam threshold , generate a roam check event */
...
wlan_connection_sm_t wlan_connection_sm_create(osdev_t oshandle, wlan_if_t vaphandle)
{
    ...
    sm->bgscan_snr_thresh =   DEFAULT_SNR_THRESH;
    sm->bgscan_snr_thresh_forced_scan = DEFAULT_SNR_THRESH;
    sm->bgscan_snr_change  =   DEFAULT_SNR_CHANGE;
...
int wlan_connection_sm_set_param(wlan_connection_sm_t smhandle, wlan_connection_sm_param param,u_int32_t val)
{
    ...
    switch(param) {
    case WLAN_CONNECTION_PARAM_BGSCAN_SNR_THRESH:
        smhandle->bgscan_snr_thresh = (u_int8_t) val;
        break;
    case WLAN_CONNECTION_PARAM_BGSCAN_SNR_FORCED_SCAN:
        /* snr below which the scan is a forces scan */
        smhandle->bgscan_snr_thresh_forced_scan = (u_int8_t) val;
        break;
    ...
    case WLAN_CONNECTION_PARAM_BGSCAN_SNR_CHANGE_THRESH:
        smhandle->bgscan_snr_change = (u_int8_t) val;
        break;
...
u_int32_t wlan_connection_sm_get_param(wlan_connection_sm_t smhandle, wlan_connection_sm_param param)
{
    ...
    switch(param) {
    case WLAN_CONNECTION_PARAM_BGSCAN_SNR_THRESH:
        return smhandle->bgscan_snr_thresh;
        break;

    case WLAN_CONNECTION_PARAM_BGSCAN_SNR_FORCED_SCAN:
        /* snr below which the bg scan is a forces scan */
        return smhandle->bgscan_snr_thresh_forced_scan;
        break;
    ...
    case WLAN_CONNECTION_PARAM_BGSCAN_SNR_CHANGE_THRESH:
        return smhandle->bgscan_snr_change;
        break;
