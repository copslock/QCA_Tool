...
#define ADD_NI_RATES_NUM 8
    ...
    if(ic->ic_global_list->same_ssid_support) {
        for (i=0; i < MAX_RADIO_CNT; i++) {
            if (OS_MEMCMP(wh->i_addr2, &ic->ic_global_list->denied_list_apvap[i][0], QDF_MAC_ADDR_SIZE) == 0) {
    ...
    if (ni == vap->iv_bss) {
        ...
        logically_deleted_ni =
            _ieee80211_find_logically_deleted_node(&vap->iv_ic->ic_sta,
                    wh->i_addr2, vap->iv_myaddr, WLAN_MLME_SB_ID);
        ...
        if (logically_deleted_ni) {
             ...
             */
            ieee80211_free_node(logically_deleted_ni, WLAN_MLME_SB_ID);
    ...
    if(rrmcap) {
        ...
        OS_MEMCPY(ni->ni_rrm_caps, &rrmcap[2], min((u_int8_t) sizeof(ni->ni_rrm_caps), rrmcap[1]));
    ...
    /* A 6Ghz ap may reject a (re)assoc with status-code
     * DENIED_POOR_CHANNEL_CONDITIONS if it receives a
     * (re)Association Request frame from a non- AP STA
     * below a minimum RSSI threshold value. In case of
     * OCE, delay the rejection so that OCE IE can be
     * included in response frame. Currently , min_rssi
     * threshold is initialized as -90dBm. This value
     * needs to be increased to a sufficiently high value
     * to activate this code
     */
    if ((!ieee80211_vap_oce_check(vap)) &&
            IEEE80211_IS_CHAN_6GHZ(ic->ic_curchan) &&
            (ni->ni_abs_rssi < vap->iv_assoc_min_rssi_thrshld)) {
        qdf_info("ni_abs_rssi: %d iv_assoc_min_rssi_thrshld: %d",
                     ni->ni_abs_rssi, vap->iv_assoc_min_rssi_thrshld);
        qdf_info("Assoc rejected due to poor channel condition");
#if WLAN_SUPPORT_SPLITMAC
        /*
        ...
        if (splitmac_is_enabled(vdev)) {
            ...
            acfg_assoc_failure_indication_event(vap, (wh->i_addr2),
                                           IEEE80211_STATUS_POOR_CHAN_CONDITION);
#endif
            son_send_assoc_fail_event(vap->vdev_obj, wh->i_addr2, IEEE80211_STATUS_POOR_CHAN_CONDITION);

            return -EINVAL;
        } else
#endif
        {
            ieee80211_send_assocresp(ni, reassoc, IEEE80211_STATUS_POOR_CHAN_CONDITION, NULL);
...
#endif
                son_send_assoc_fail_event(vap->vdev_obj, wh->i_addr2, IEEE80211_STATUS_INVALID_RSNE_CAP);
#ifdef QCA_SUPPORT_CP_STATS
...
#endif
                son_send_assoc_fail_event(vap->vdev_obj, wh->i_addr2, status);
                IEEE80211_NODE_LEAVE(ni);
#ifdef QCA_SUPPORT_CP_STATS
                vdev_cp_stats_rx_assoc_wpaie_err_inc(vap->vdev_obj, 1);
#endif
...
#endif
            son_send_assoc_fail_event(vap->vdev_obj, wh->i_addr2, IEEE80211_STATUS_CAPINFO);
#ifdef QCA_SUPPORT_CP_STATS
...
#endif
            son_send_assoc_fail_event(vap->vdev_obj, wh->i_addr2, IEEE80211_STATUS_BASIC_RATE);
#ifdef QCA_SUPPORT_CP_STATS
    ...
    if (supp_op_cl != NULL) {
        ...
        ni->ni_operating_bands = regdmn_get_band_cap_from_op_class(
             ...
             */
             if (splitmac_is_enabled(vdev)) {
                ...
                acfg_assoc_failure_indication_event(vap, (wh->i_addr2),
                                                ACFG_REASON_ASSOC_CAP_MISMATCH);
#endif
                son_send_assoc_fail_event(vap->vdev_obj, wh->i_addr2, IEEE80211_STATUS_INVALID_ELEMENT);
                return -EINVAL;
            } else
#endif
            {
...
#endif
            son_send_assoc_fail_event(vap->vdev_obj, wh->i_addr2, IEEE80211_STATUS_NO_HT);
            return -EINVAL;
        } else
#endif
        {
...
#endif
            son_send_assoc_fail_event(vap->vdev_obj, wh->i_addr2, IEEE80211_STATUS_NO_VHT);
            return -EINVAL;
        } else
#endif
        {
...
#endif
                son_send_assoc_fail_event(vap->vdev_obj, wh->i_addr2, IEEE80211_STATUS_UNSPECIFIED);
                return -EINVAL;
            } else
#endif
            {
                          ...
                          struct ieee80211_app_ie_t* optie)
{
    ...
    *(u_int16_t *)frm = htole16(reason);
    ...
    /* 6. EDCA Paramieter Set */
    ieee80211_add_or_retrieve_ie_from_app_opt_ies(vap,
            IEEE80211_FRAME_TYPE_ASSOCRESP, IEEE80211_ELEMID_EDCA, -1, &frm,
            TYPE_ALL_BUF, optie, true);

    /* 7. RCPI */
    ieee80211_add_or_retrieve_ie_from_app_opt_ies(vap,
            IEEE80211_FRAME_TYPE_ASSOCRESP, IEEE80211_ELEMID_RCPI, -1, &frm,
            TYPE_ALL_BUF, optie, true);

    /* 8. RSNI */
    ieee80211_add_or_retrieve_ie_from_app_opt_ies(vap,
            IEEE80211_FRAME_TYPE_ASSOCRESP, IEEE80211_ELEMID_RSNI, -1, &frm,
            TYPE_ALL_BUF, optie, true);


    /* 9. RM Enabled Capbabilities */
    frm = ieee80211_add_rrm_cap_ie(frm, ni);

    /* RSN */
    ieee80211_add_or_retrieve_ie_from_app_opt_ies(vap,
            IEEE80211_FRAME_TYPE_ASSOCRESP, IEEE80211_ELEMID_RSN, -1, &frm,
            TYPE_ALL_BUF, optie, true);

    /* 10. Mobility Domain */
    ieee80211_add_or_retrieve_ie_from_app_opt_ies(vap,
            IEEE80211_FRAME_TYPE_ASSOCRESP, IEEE80211_ELEMID_MOBILITY_DOMAIN, -1, &frm,
            TYPE_ALL_BUF, optie, true);

    /* 11. Fast BSS Transition */
    ieee80211_add_or_retrieve_ie_from_app_opt_ies(vap,
            IEEE80211_FRAME_TYPE_ASSOCRESP, IEEE80211_ELEMID_FT, -1, &frm,
            TYPE_ALL_BUF, optie, true);

    /* 12. DSE registered location */
    ieee80211_add_or_retrieve_ie_from_app_opt_ies(vap,
            IEEE80211_FRAME_TYPE_ASSOCRESP, IEEE80211_ELEMID_DSE_REG_LOCATION, -1, &frm,
            TYPE_ALL_BUF, optie, true);


    /* 13. Timeout Interval */
    if((reason == IEEE80211_STATUS_REJECT_TEMP) &&
            ieee80211_is_pmf_enabled(vap, ni) &&
            (ni->ni_flags & IEEE80211_NODE_AUTH)) {
        if (vap->iv_skip_pmf_reassoc_to_hostap > 0) {
            frm = ieee80211_add_timeout_ie(frm, ni,
                    sizeof(struct ieee80211_ie_timeout), vap->iv_assoc_comeback_time);
        } else {
            ieee80211_add_or_retrieve_ie_from_app_opt_ies(vap,
                    IEEE80211_FRAME_TYPE_ASSOCRESP, IEEE80211_ELEMID_TIMEOUT_INTERVAL, -1,
                    &frm, TYPE_ALL_BUF, optie, true);
        }
    }
    ...
    if (ieee80211_vap_wme_is_set(vap) &&
        ...
        ieee80211_add_or_retrieve_ie_from_app_opt_ies(vap,
            IEEE80211_FRAME_TYPE_ASSOCRESP, IEEE80211_ELEMID_2040_COEXT, -1, &frm,
            TYPE_ALL_BUF, optie, true);


        /* 17. Overlapping BSS Scan Parameters */
        if (!(ic->ic_flags & IEEE80211_F_COEXT_DISABLE));
            frm = ieee80211_add_obss_scan(frm, ni);
    }

    /* 18. Extended Capbabilities, if applicable */
    frm = ieee80211_add_extcap(frm, ni, IEEE80211_FC0_SUBTYPE_ASSOC_RESP);

#if UMAC_SUPPORT_WNM
    /* 19. BSS Max Idle Period */
    if (!ieee80211_add_or_retrieve_ie_from_app_opt_ies(vap, IEEE80211_FRAME_TYPE_ASSOCRESP,
            IEEE80211_ELEMID_BSSMAX_IDLE_PERIOD, -1, &frm, TYPE_OPT_IE_BUF, optie, true)
            && (ieee80211_vap_wnm_is_set(vap) && (vap->wnm) && (ieee80211_wnm_bss_is_set(vap->wnm)))) {
    ...
    /* 22. QMF Policy */
    ieee80211_add_or_retrieve_ie_from_app_opt_ies(vap,
            IEEE80211_FRAME_TYPE_ASSOCRESP, IEEE80211_ELEMID_QMF_POLICY, -1, &frm,
            TYPE_ALL_BUF, optie, true);

    /* 23. Multi-band */
    ieee80211_add_or_retrieve_ie_from_app_opt_ies(vap,
            IEEE80211_FRAME_TYPE_ASSOCRESP, IEEE80211_ELEMID_MULTIBAND, -1, &frm,
            TYPE_ALL_BUF, optie, true);

    /* 24. DMG Capabilities */
    ieee80211_add_or_retrieve_ie_from_app_opt_ies(vap,
            IEEE80211_FRAME_TYPE_ASSOCRESP, IEEE80211_ELEMID_DMG_CAP, -1, &frm,
            TYPE_ALL_BUF, optie, true);

    /* 25. DMG Operation */
    ieee80211_add_or_retrieve_ie_from_app_opt_ies(vap,
            IEEE80211_FRAME_TYPE_ASSOCRESP, IEEE80211_ELEMID_DMG_OPERATION, -1, &frm,
            TYPE_ALL_BUF, optie, true);

    /* 26. Multiple MAC Sublayers */
    ieee80211_add_or_retrieve_ie_from_app_opt_ies(vap,
            IEEE80211_FRAME_TYPE_ASSOCRESP, IEEE80211_ELEMID_MULTIPLE_MAC_SUB, -1, &frm,
            TYPE_ALL_BUF, optie, true);

    /* 27. Neighbor Report */
    ieee80211_add_or_retrieve_ie_from_app_opt_ies(vap,
            IEEE80211_FRAME_TYPE_ASSOCRESP, IEEE80211_ELEMID_NEIGHBOR_RPT, -1, &frm,
            TYPE_ALL_BUF, optie, true);
    ...
    if (ieee80211_vap_wme_is_set(vap) &&
        ...
        /* 27. VHT Capabilities IE
         * number duplicated in spec
         */
    ...
    /* 29. Operating Mode Notification */
    ieee80211_add_or_retrieve_ie_from_app_opt_ies(vap,
            IEEE80211_FRAME_TYPE_ASSOCRESP, IEEE80211_ELEMID_OP_MODE_NOTIFY, -1, &frm,
            TYPE_ALL_BUF, optie, true);

    /* 30. Future Channel Guidance */
    ieee80211_add_or_retrieve_ie_from_app_opt_ies(vap,
            IEEE80211_FRAME_TYPE_ASSOCRESP, IEEE80211_ELEMID_EXTN,
            IEEE80211_ELEMID_EXT_FUTURE_CHANNEL_GUIDE, &frm,
            TYPE_ALL_BUF, optie, true);

    /* 31. FILS Session */
    ieee80211_add_or_retrieve_ie_from_app_opt_ies(vap,
            IEEE80211_FRAME_TYPE_ASSOCRESP, IEEE80211_ELEMID_EXTN,
            IEEE80211_ELEMID_EXT_FILS_SESSION, &frm,
            TYPE_ALL_BUF, optie, true);

    /* 32. FILS Public Key */
    ieee80211_add_or_retrieve_ie_from_app_opt_ies(vap,
            IEEE80211_FRAME_TYPE_ASSOCRESP, IEEE80211_ELEMID_EXTN,
            IEEE80211_ELEMID_EXT_FILS_PUB_KEY, &frm,
            TYPE_ALL_BUF, optie, true);

    /* 33. FILS Key Confirmation */
    ieee80211_add_or_retrieve_ie_from_app_opt_ies(vap,
            IEEE80211_FRAME_TYPE_ASSOCRESP, IEEE80211_ELEMID_EXTN,
            IEEE80211_ELEMID_EXT_FILS_KEY_CONFIRM, &frm,
            TYPE_ALL_BUF, optie, true);

    /* 34. FILS HLP Container */
    ieee80211_add_or_retrieve_ie_from_app_opt_ies(vap,
            IEEE80211_FRAME_TYPE_ASSOCRESP, IEEE80211_ELEMID_EXTN,
            IEEE80211_ELEMID_EXT_FILS_HLP_CONTAINER, &frm,
            TYPE_ALL_BUF, optie, true);

    /* 35. FILS IP Address Range Assignment */
    ieee80211_add_or_retrieve_ie_from_app_opt_ies(vap,
            IEEE80211_FRAME_TYPE_ASSOCRESP, IEEE80211_ELEMID_EXTN,
            IEEE80211_ELEMID_EXT_FILS_IP_ADDRRANGE_ASSIGN, &frm,
            TYPE_ALL_BUF, optie, true);

    /* 36. Key Delivery */
    ieee80211_add_or_retrieve_ie_from_app_opt_ies(vap,
            IEEE80211_FRAME_TYPE_ASSOCRESP, IEEE80211_ELEMID_EXTN,
            IEEE80211_ELEMID_EXT_KEY_DELIVERY, &frm,
            TYPE_ALL_BUF, optie, true);

    /* 37. S1G Sector Operation */
    ieee80211_add_or_retrieve_ie_from_app_opt_ies(vap,
            IEEE80211_FRAME_TYPE_ASSOCRESP, IEEE80211_ELEMID_S1G_SECT_OP, -1, &frm,
            TYPE_ALL_BUF, optie, true);

    /* 38. TWT */
    ieee80211_add_or_retrieve_ie_from_app_opt_ies(vap,
            IEEE80211_FRAME_TYPE_ASSOCRESP, IEEE80211_ELEMID_TWT, -1, &frm,
            TYPE_ALL_BUF, optie, true);

    /* 39. TSF Timer Accuracy */
    ieee80211_add_or_retrieve_ie_from_app_opt_ies(vap,
            IEEE80211_FRAME_TYPE_ASSOCRESP, IEEE80211_ELEMID_TSF_TIMER_ACC, -1, &frm,
            TYPE_ALL_BUF, optie, true);

    /* 41. S1G Capabilities */
    ieee80211_add_or_retrieve_ie_from_app_opt_ies(vap,
            IEEE80211_FRAME_TYPE_ASSOCRESP, IEEE80211_ELEMID_S1G_CAP, -1, &frm,
            TYPE_ALL_BUF, optie, true);

    /* 42. S1G Operation */
    ieee80211_add_or_retrieve_ie_from_app_opt_ies(vap,
            IEEE80211_FRAME_TYPE_ASSOCRESP, IEEE80211_ELEMID_S1G_OP, -1, &frm,
            TYPE_ALL_BUF, optie, true);

    /* 43. AID Response */
    ieee80211_add_or_retrieve_ie_from_app_opt_ies(vap,
            IEEE80211_FRAME_TYPE_ASSOCRESP, IEEE80211_ELEMID_AID_RESPONSE, -1, &frm,
            TYPE_ALL_BUF, optie, true);

    /* 44. Sectorized Group ID List */
    ieee80211_add_or_retrieve_ie_from_app_opt_ies(vap,
            IEEE80211_FRAME_TYPE_ASSOCRESP, IEEE80211_ELEMID_SEC_GRP_ID_LIST, -1, &frm,
            TYPE_ALL_BUF, optie, true);

    /* 45. S1G Relay */
    ieee80211_add_or_retrieve_ie_from_app_opt_ies(vap,
            IEEE80211_FRAME_TYPE_ASSOCRESP, IEEE80211_ELEMID_S1G_RELAY, -1, &frm,
            TYPE_ALL_BUF, optie, true);

    /* 46. Header Compression */
    ieee80211_add_or_retrieve_ie_from_app_opt_ies(vap,
            IEEE80211_FRAME_TYPE_ASSOCRESP, IEEE80211_ELEMID_HEADER_COMPRESSION, -1, &frm,
            TYPE_ALL_BUF, optie, true);

    /* 47. SST Operation element */
    ieee80211_add_or_retrieve_ie_from_app_opt_ies(vap,
            IEEE80211_FRAME_TYPE_ASSOCRESP, IEEE80211_ELEMID_SST_OP, -1, &frm,
            TYPE_ALL_BUF, optie, true);

    /* 48. MAD */
    ieee80211_add_or_retrieve_ie_from_app_opt_ies(vap,
            IEEE80211_FRAME_TYPE_ASSOCRESP, IEEE80211_ELEMID_MAD, -1, &frm,
            TYPE_ALL_BUF, optie, true);

    /* 49. S1G Relay Activiation IE
     * The element ID 224, assigned to this IE, is used for S1G Relay IE
     * causing collision
     */

    /* 50. CDMG Capaiblities */
    ieee80211_add_or_retrieve_ie_from_app_opt_ies(vap,
            IEEE80211_FRAME_TYPE_ASSOCRESP, IEEE80211_ELEMID_EXTN,
            IEEE80211_ELEMID_EXT_CDMG_CAP, &frm,
            TYPE_ALL_BUF, optie, true);

    /* 51. CMMG Capabilities */
    ieee80211_add_or_retrieve_ie_from_app_opt_ies(vap,
            IEEE80211_FRAME_TYPE_ASSOCRESP, IEEE80211_ELEMID_EXTN,
    ...
    /* 53. GLK-GCR Parameter Set */
    ieee80211_add_or_retrieve_ie_from_app_opt_ies(vap,
            IEEE80211_FRAME_TYPE_ASSOCRESP, IEEE80211_ELEMID_EXTN,
            IEEE80211_ELEMID_EXT_GLK_GCR_PARAM_SET, &frm,
            TYPE_ALL_BUF, optie, true);
    ...
    /* 56. BSS Color Change Announcement */
    ieee80211_add_or_retrieve_ie_from_app_opt_ies(vap,
            IEEE80211_FRAME_TYPE_ASSOCRESP, IEEE80211_ELEMID_EXTN,
    ...
    /* 61. NDP Feedback Report Parameter Set */
    ieee80211_add_or_retrieve_ie_from_app_opt_ies(vap,
            IEEE80211_FRAME_TYPE_ASSOCRESP, IEEE80211_ELEMID_EXTN,
            IEEE80211_ELEMID_EXT_NDP_FEEDBACK_REPORT_PARAM, &frm,
            TYPE_ALL_BUF, optie, true);

    /* 64. HE 6GHz Band Capabilities */
    if(IEEE80211_IS_CHAN_6GHZ(ic->ic_curchan)) {
        frm = ieee80211_add_6g_bandcap(frm, ni, ic,
                IEEE80211_FC0_SUBTYPE_ASSOC_RESP);
    }

    /* Adding at the end since spec does not say any order for OWE DH PARAM */
    ieee80211_add_or_retrieve_ie_from_app_opt_ies(vap,
            IEEE80211_FRAME_TYPE_ASSOCRESP, IEEE80211_ELEMID_EXTN,
            IEEE80211_ELEMID_EXT_OWE_DH_PARAM, &frm,
            TYPE_ALL_BUF, optie, true);

    /* Adding RSNX element at the last since no order is mentioned
     * in the specification
     */
    if (ni->omit_rsnxe == false) {
        qdf_debug("Trying to add RSNX in (re)assoc response");
        ieee80211_add_or_retrieve_ie_from_app_opt_ies(vap,
            IEEE80211_FRAME_TYPE_ASSOCRESP, IEEE80211_ELEMID_RSNX,
            -1, &frm, TYPE_ALL_BUF, optie, true);
    }
    ...
    if (ic_list->same_ssid_support)
     ...
     /* HT Cap, and HT Info/Operation vendor elements */
     if (ieee80211_vap_wme_is_set(vap) &&
       (!IEEE80211_IS_CHAN_6GHZ(ic->ic_curchan)) &&
       (IEEE80211_IS_CHAN_11AX(ic->ic_curchan) ||
        IEEE80211_IS_CHAN_11AC(ic->ic_curchan) ||
    ...
    /* Prop NSS IE if EXT NSS is not supported */
    ...
    /* VHT interop capabilities IE with Vht cap and Vht Operation */
    if (ieee80211_vap_wme_is_set(vap) &&
    ...
    /* -------------  LAST. App IE buffer or List, Opt IE Buffer ------------- */
    IEEE80211_VAP_LOCK(vap);
    ieee80211_add_or_retrieve_ie_from_app_opt_ies(vap, IEEE80211_FRAME_TYPE_ASSOCRESP, IEEE80211_ELEMID_VENDOR,
            0, &frm, TYPE_ALL_BUF, optie, false);
...
wlan_send_mgmt(wlan_if_t vaphandle, u_int8_t *macaddr, u_int8_t *mgmt_frm, u_int32_t len)
{
    ...
    ni = ieee80211_find_txnode(vap, macaddr, WLAN_MGMT_TX_ID);
    if (ni == NULL) {
        IEEE80211_DPRINTF(vap, IEEE80211_MSG_OUTPUT,
                          "%s: could not send mgmt found for %s\n",
                          __func__, ether_sprintf(macaddr));
        ni = ieee80211_try_ref_bss_node(vap, WLAN_MGMT_TX_ID);
	bss_peer = 1;
        if(ni == NULL) {
            return -EINVAL;
        }
    ...
    if (ic)
        wbuf = wbuf_alloc(ic->ic_osdev, WBUF_TX_MGMT, MAX_TX_RX_PACKET_SIZE);
    if (wbuf == NULL) {
        ieee80211_free_node(ni, WLAN_MGMT_TX_ID);    /* reclaim node */
        return -EINVAL;
    }
...
int ieee80211_beacon_intersect(struct ieee80211_node *ni, u_int8_t *bcn_frm_body, u_int16_t bcn_body_len, struct ieee80211_frame *wh)
{
    ...
    u_int8_t *vhtcap = NULL, *mbo = NULL, *hecap = NULL, *hecap_6g = NULL;
    uint8_t *hecap_phy_ie;
    bool ldpc = false;
