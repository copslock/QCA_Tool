...
void wlan_mlme_remove_ie_list(wlan_mlme_app_ie_t app_ie_handle)
{
    ...
    if (app_ie_handle == NULL) {
        qdf_nofl_info("%s: handle is NULL. Do nothing.\n", __func__);
        return;
    }
...
int wlan_mlme_app_ie_delete(wlan_mlme_app_ie_t app_ie_handle, ieee80211_frame_type ftype, u_int8_t *app_ie)
{
    ...
    if(app_ie == NULL)
    {
        qdf_nofl_info("%s: appie is NULL. Do nothing.\n", __func__);
        return -EINVAL;
    }
    ...
    u_int8_t                identifier)
{
    ...
    {
        qdf_nofl_info("NULL IE or zero length received");
        error = -EINVAL;
        return error;
    }
    if (ftype >= IEEE80211_FRAME_TYPE_MAX)
    {
        qdf_nofl_info("Invalid frame type");
        error = -EINVAL;
        return error;
    }
    IEEE80211_VAP_LOCK(vap);
    ...
    /* WPS should not be added in "beacon" if Hidden SSID is enabled as per spec */
    ...
    if(ie_entry == NULL)
    {
        qdf_nofl_info("Not able to allocate memory");
        error = -ENOMEM;
        goto exit;
    }
    iebuf = (u_int8_t *)qdf_mem_malloc(buflen*sizeof(u_int8_t));
    if(iebuf == NULL)
    {
        qdf_nofl_info("Not able to allocate memory");
...
int wlan_mlme_app_ie_set_check(struct ieee80211vap *vap, ieee80211_frame_type ftype,const u_int8_t *app_ie, u_int32_t app_ie_length, u_int8_t identifier)
{
    ...
    if (vap->vie_handle == NULL)
    {
        ...
        if (vie_handle == NULL)
        {
            qdf_nofl_info("not able to get vie_handle");
            error = -ENOMEM;
            return error;
        }
    }
