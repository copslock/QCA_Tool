                   ...
                   const u_int8_t *buf, int len, int rate, int rssi)
{
    ...
    case IEEE80211_FC1_DIR_NODS:
        qdf_nofl_info("NODS %s", ether_sprintf(wh->i_addr2));
        qdf_nofl_info("->%s", ether_sprintf(wh->i_addr1));
        qdf_nofl_info("(%s)", ether_sprintf(wh->i_addr3));
        break;
    case IEEE80211_FC1_DIR_TODS:
        qdf_nofl_info("TODS %s", ether_sprintf(wh->i_addr2));
        qdf_nofl_info("->%s", ether_sprintf(wh->i_addr3));
        qdf_nofl_info("(%s)", ether_sprintf(wh->i_addr1));
        break;
    case IEEE80211_FC1_DIR_FROMDS:
        qdf_nofl_info("FRDS %s", ether_sprintf(wh->i_addr3));
        qdf_nofl_info("->%s", ether_sprintf(wh->i_addr1));
        qdf_nofl_info("(%s)", ether_sprintf(wh->i_addr2));
        break;
    case IEEE80211_FC1_DIR_DSTODS:
        qdf_nofl_info("DSDS %s", ether_sprintf((const u_int8_t *)&wh[1]));
        qdf_nofl_info("->%s", ether_sprintf(wh->i_addr3));
        qdf_nofl_info("(%s", ether_sprintf(wh->i_addr2));
        qdf_nofl_info("->%s)", ether_sprintf(wh->i_addr1));
        break;
    ...
    case IEEE80211_FC0_TYPE_MGT:
        qdf_nofl_info(" %s", ieee80211_mgt_subtype_name[subtype]);
        break;
    default:
        qdf_nofl_info(" type#%d", type);
        break;
    }
    if (IEEE80211_QOS_HAS_SEQ(wh)) {
        const struct ieee80211_qosframe *qwh =
            (const struct ieee80211_qosframe *)buf;
        qdf_nofl_info(" QoS [TID %u%s]", qwh->i_qos[0] & IEEE80211_QOS_TID,
    ...
    if (wh->i_fc[1] & IEEE80211_FC1_WEP) {
        ...
        qdf_nofl_info(" WEP [IV %.02x %.02x %.02x",
               buf[off+0], buf[off+1], buf[off+2]);
        if (buf[off+IEEE80211_WEP_IVLEN] & IEEE80211_WEP_EXTIV)
            qdf_nofl_info(" %.02x %.02x %.02x",
                   buf[off+4], buf[off+5], buf[off+6]);
        qdf_nofl_info(" KID %u]", buf[off+IEEE80211_WEP_IVLEN] >> 6);
    }
    if (rate >= 0)
        qdf_nofl_info(" %dM", rate / 1000);
    if (rssi >= 0)
        qdf_nofl_info(" +%d", rssi);
    qdf_nofl_info("\n");
    if (len > 0) {
        for (i = 0; i < len; i++) {
            if ((i & 1) == 0)
                qdf_nofl_info(" ");
            qdf_nofl_info("%02x", buf[i]);
        }
        qdf_nofl_info("\n");
    }
}
...
ath_vap_iter_cac(void *arg, wlan_if_t vap)
{
    ...
    if (IEEE80211_IS_CHAN_RADAR(ic->ic_curchan)) {
        ...
        /* Notify user space of CAC completion via ALD Netlink */
        ald_cac_complete_notify(vap, 0);
        wlan_vdev_mlme_sm_deliver_evt(vap->vdev_obj,
