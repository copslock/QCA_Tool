...
static void ieee80211_bsscolor_post_color_selection(struct ieee80211com *ic) {
    ...
    bsscolor_hdl->selected_bsscolor = bsscolor_hdl->saved_selected_bsscolor;
...
qdf_export_symbol(ieee80211_override_bsscolor_collsion_ap_period);
...
qdf_export_symbol(ieee80211_override_bsscolor_change_count);
...
#endif
    uint16_t cur_ieeefreq;
    uint16_t se_ieeefreq = ieee80211_chan2freq(ic, se_chan);

    /* No need to process beacons here if we are
     * non-HE target or in inactive or user-disabled
     * state
     */
    if ((!ic->ic_he_target) ||
        (bsscolor_hdl->state == IEEE80211_BSS_COLOR_CTX_STATE_INACTIVE) ||
        (bsscolor_hdl->state == IEEE80211_BSS_COLOR_CTX_STATE_COLOR_USER_DISABLED)) {
        return EOK;
    }
...
qdf_export_symbol(ieee80211_process_beacon_for_bsscolor_cache_update);
...
ieee80211_bsscolor_init(struct ieee80211com *ic)
{
    ...
    ic->ic_bsscolor_hdl.prev_bsscolor = IEEE80211_BSS_COLOR_INVALID;
    ic->ic_bsscolor_hdl.selected_bsscolor = IEEE80211_BSS_COLOR_INVALID;
    ic->ic_bsscolor_hdl.saved_selected_bsscolor = IEEE80211_BSS_COLOR_INVALID;
    ...
    cdp_txrx_set_pdev_param(soc_txrx_handle, pdev_id, CDP_CONFIG_BSS_COLOR, value);
}
...
#define IS_RUNNING(_dev)                                            \
    ...
    if (sel_color != IEEE80211_BSS_COLOR_INVALID) {
        bsscolor_hdl->saved_selected_bsscolor = sel_color;
        if (prev_state ==
            ...
            if(!IS_RUNNING(dev) || ieee80211_get_num_vaps_up(ic) == 0) {
                bsscolor_hdl->bcca_not_reqd = true;
            } else {
...
ieee80211_select_new_bsscolor(struct ieee80211com *ic) {
    ...
    if (bsscolor_hdl->bcca_not_reqd &&
           (bsscolor_hdl->saved_selected_bsscolor != IEEE80211_BSS_COLOR_INVALID)) {
        ieee80211_bsscolor_post_color_selection(ic);
    } else {
        /* cancel colorListUpdateOppurtunityPeriod if already running */
        ieee80211_bsscolor_list_update_opp_timer_cancel(bsscolor_hdl);
...
qdf_export_symbol(ieee80211_select_new_bsscolor);
...
qdf_export_symbol(ieee80211_set_ic_heop_bsscolor_disabled_bit);

bool ieee80211_is_bcca_ongoing_on_any_radio(struct ieee80211com *ic,
                                            uint8_t num_radios) {
    uint8_t pdev_id;
    bool bcca_ongoing = false;
...
qdf_export_symbol(ieee80211_is_bcca_ongoing_on_any_radio);
...
qdf_export_symbol(ieee80211_is_bcca_ongoing_for_any_vap);
