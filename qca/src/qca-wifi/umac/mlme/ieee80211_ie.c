 ...
 */
u_int8_t *
ieee80211_add_tpc_ie(u_int8_t *frm, struct ieee80211vap *vap)
{
    *frm++ = IEEE80211_ELEMID_TPCREP;
    *frm++ = IEEE80211_TPCREP_LEN;
    /* Current TX power of the chan */
    *frm++ = (int8_t)(vap->iv_bss->ni_txpower/2);
   ...
   if (RSN_CIPHER_IS_CCMP256(rsn)) {
    ...
    if (vap->iv_opmode == IEEE80211_M_STA && vap->iv_pmkid_count > 0) {
        ...
        for (i = 0; i < vap->iv_pmkid_count; i++) {
            if (!OS_MEMCMP( vap->iv_pmkid_list[i].bssid, ni->ni_bssid, QDF_MAC_ADDR_SIZE)) {
...
#define IEEE80211_ADD_MBSS_IE_TAG(ic, ie)     \
    do { \
        ie->header.element_id = IEEE80211_ELEMID_MBSSID; \
        ie->header.length = 1; \
        ie->max_bssid_indicator = ic->ic_mbss.max_bssid; \
       } while(0);

/*
 * Add a profile to 11ax MBSS IE
 */
uint8_t *
ieee80211_mbss_add_profile(u_int8_t *frm, struct ieee80211_node *ni,
                               uint8_t *profiles_cnt, uint8_t subtype)
{
    struct ieee80211vap *vap = ni->ni_vap;
    struct ieee80211com *ic = vap->iv_ic;
    struct vdev_mlme_obj *vdev_mlme = vap->vdev_mlme;
    uint8_t profile_len, max_len;
    struct ieee80211_ie_header *sub_elem;
    struct ieee80211_mbss_ie_capability *cap_elem;
    struct ieee80211_mbss_ie_ssid *ssid_elem;
    struct ieee80211_mbss_ie_bssid_index *index_elem;
    struct ieee80211_mbss_ie *mb = (struct ieee80211_mbss_ie *) frm;
    ...
    if (*frm != IEEE80211_ELEMID_MBSSID) {
        /* add MBSS IE first time */
        IEEE80211_ADD_MBSS_IE_TAG(ic, mb);
    } else {
        /* skip past any MBSS IEs */
        while (*(frm + sizeof(struct ieee80211_ie_header) + mb->header.length)
                                            ==  IEEE80211_ELEMID_MBSSID) {
            frm += sizeof(struct ieee80211_ie_header) + mb->header.length;
            mb = (struct ieee80211_mbss_ie *) frm;
        }
    }

    /* frm has the IE start address here */
    frm += sizeof(struct ieee80211_ie_header) + mb->header.length;

    /*
     * add profile contents - capability, ssid, bssid index, and security IE.
     * Currently, all the above elements are part of single IE, they cannot
     * be split and stored across another MBSS IE.
     */

    /* max length of IE = 255 bytes */
    max_len = (1 << BITS_PER_BYTE) - 1;

    /* calculate minimum length to accomodate a VAP profile */
    profile_len =
        sizeof(struct ieee80211_ie_header) + sizeof(struct ieee80211_mbss_ie_capability);

    if (!IEEE80211_VAP_IS_HIDESSID_ENABLED(vap)) {
        profile_len += ni->ni_esslen +  sizeof(struct ie_header);
    } else {
        profile_len += sizeof(struct ie_header);
    }

    switch (subtype) {
    case IEEE80211_FRAME_TYPE_BEACON:
        profile_len += sizeof(struct ieee80211_mbss_ie_bssid_index) +
                       vap->iv_app_ie_list[IEEE80211_FRAME_TYPE_BEACON].total_ie_len;
        break;
    case IEEE80211_FRAME_TYPE_PROBERESP:
        profile_len += (sizeof(struct ieee80211_mbss_ie_bssid_index)- 2) +
                       vap->iv_app_ie_list[IEEE80211_FRAME_TYPE_PROBERESP].total_ie_len;
        break;
    default:
        break;
    }
    ...
    /*
     * add new IE header if current IE can't accomodate minimum
     * set of profile members
     */
    if (profile_len > (max_len - mb->header.length)) {
        mb = (struct ieee80211_mbss_ie *) frm;
        IEEE80211_ADD_MBSS_IE_TAG(ic, mb);
        frm += sizeof(struct ieee80211_ie_header) + mb->header.length;
    }

    /* populate vap profile sub-element now */
    sub_elem = (struct ieee80211_ie_header *) frm;
    sub_elem->element_id = IEEE80211_MBSSID_SUB_ELEMID;
    frm += sizeof(struct ieee80211_ie_header);

    /* capability element */
    cap_elem = (struct ieee80211_mbss_ie_capability *) frm;
    cap_elem->hdr.element_id = IEEE80211_ELEMID_MBSSID_NON_TRANS_CAP;
    cap_elem->hdr.length = 2;
    ieee80211_add_capability(frm + 2, ni);
    frm += sizeof(struct ieee80211_mbss_ie_capability);

    /* SSID element */
    ssid_elem = (struct ieee80211_mbss_ie_ssid *) frm;
    ssid_elem->hdr.element_id = IEEE80211_ELEMID_SSID;
    if (IEEE80211_VAP_IS_HIDESSID_ENABLED(vap)) {
        ssid_elem->hdr.length = 0;
    } else {
        ssid_elem->hdr.length = ni->ni_esslen;
        OS_MEMCPY(ssid_elem->ssid, ni->ni_essid, ni->ni_esslen);
    }
    frm += sizeof(struct ieee80211_ie_header) + ssid_elem->hdr.length;

    /* BSSID index element */
    index_elem = (struct ieee80211_mbss_ie_bssid_index *) frm;
    index_elem->hdr.element_id = IEEE80211_ELEMID_MBSSID_INDEX;
    index_elem->hdr.length = 1;
    index_elem->bssid_idx = vdev_mlme->mgmt.mbss_11ax.profile_idx;

    switch (subtype) {
    case IEEE80211_FRAME_TYPE_BEACON:
        index_elem->dtim_period = vdev_mlme->proto.generic.dtim_period;
        index_elem->dtim_count = 0;
        index_elem->hdr.length = 3; /* adjust the length */
        frm += sizeof(struct ieee80211_mbss_ie_bssid_index);
        break;
    case IEEE80211_FRAME_TYPE_PROBERESP:
        frm += (sizeof(struct ieee80211_mbss_ie_bssid_index) - 2);
        break;
    default:
        break;
    }

    /* add the application IEs including security */
    ...
    /* update vap's profile length field */
    sub_elem->length = frm - (uint8_t *) cap_elem;
    mb->header.length += (frm - (uint8_t *) sub_elem);
    (*profiles_cnt)++;
 ...
 *      ic->ic_he_srctrl_non_srg_obsspd_disallowed = 1
 *      ic->ic_he_srctrl_srg_info_present = 1
 ...
 */
static void
ieee80211_update_srg_params(struct ieee80211com *ic, uint8_t *frm)
{
    ...
    *pos = ic->ic_he_srctrl_srg_obsspd_min_offset;
    pos += sizeof(ic->ic_he_srctrl_srg_obsspd_min_offset);

    *pos = ic->ic_he_srctrl_srg_obsspd_max_offset;
    pos += sizeof(ic->ic_he_srctrl_srg_obsspd_max_offset);


    OS_MEMCPY((uint32_t *)pos,
            &ic->ic_he_srp_ie_srg_bss_color_bitmap[0], sizeof(uint32_t));
    OS_MEMCPY((uint32_t *)(pos + sizeof(uint32_t)),
            &ic->ic_he_srp_ie_srg_bss_color_bitmap[1], sizeof(uint32_t));

    pos += sizeof(ic->ic_he_srp_ie_srg_bss_color_bitmap);

    OS_MEMCPY((uint32_t *)pos,
            &ic->ic_he_srp_ie_srg_partial_bssid_bitmap[0], sizeof(uint32_t));
    OS_MEMCPY((uint32_t *)(pos + sizeof(uint32_t)),
            &ic->ic_he_srp_ie_srg_partial_bssid_bitmap[1], sizeof(uint32_t));
...
ieee80211_add_srp_ie(struct ieee80211com *ic, uint8_t *frm)
{
    ...
    ie->sr_ctrl.srg_information_present =
            ...
            !!ic->ic_he_srctrl_srg_info_present : 0;
     ...
     * initialized and therefore should not alter the length
     * if the IE.
     */
    if (ic->ic_is_spatial_reuse_updated &&
            ic->ic_he_srctrl_non_srg_obsspd_disallowed &&
            ic->ic_he_srctrl_srg_info_present) {
        ieee80211_update_srg_params(ic, frm);
    } else {
...
ieee80211_parse_dothparams(struct ieee80211vap *vap, u_int8_t *frm)
{
    ...
    chan = frm[3];
    chan_freq = wlan_reg_chan_band_to_freq(vap->iv_ic->ic_pdev_obj, chan,
                         BIT(wlan_reg_freq_to_band(vap->iv_ic->ic_curchan->ic_freq)));
    ...
    if (tbtt <= 1) {
        ...
        if ((c = ieee80211_doth_findchan(vap, chan_freq)) == NULL) {
            /* XXX something wrong */
            IEEE80211_DISCARD_IE(vap,
                                 IEEE80211_MSG_ELEMID | IEEE80211_MSG_DOTH,
                                 "channel switch",
                     ...
                     bool chan_failure)
{
    ...
    if (vap->iv_opmode == IEEE80211_M_IBSS) {
        ...
        if (pcsaie) {
            ptarget_channel = ieee80211_doth_findchan(vap, wlan_reg_chan_band_to_freq(ic->ic_pdev_obj, pcsaie->newchannel,
                                                              BIT(wlan_reg_freq_to_band(ic->ic_curchan->ic_freq))));
        } else if(ic->ic_flags & IEEE80211_F_CHANSWITCH) {
            ptarget_channel = ieee80211_doth_findchan(vap, ic->ic_chanchange_chan_freq);
        } else {
            if (ic->ic_no_weather_radar_chan) {
                flag |= DFS_RANDOM_CH_FLAG_NO_WEATHER_CH;
            }
            target_chan_freq = mlme_dfs_random_channel(pdev, &ch_params, flag);

            if (target_chan_freq) {
                chan_mode = ieee80211_get_target_channel_mode(ic, &ch_params);
                ptarget_channel = ieee80211_find_dot11_channel(ic,target_chan_freq,ch_params.mhz_freq_seg1,
                                                               chan_mode);
            } else {
    ...
    if (ptarget_channel)
    {
            ...
            !(is_vap_start_success(vap)))
        {
            ...
            if (vap->iv_opmode == IEEE80211_M_IBSS) {
                ...
                vap->iv_channelswitch_ie_data.newchannel = wlan_reg_freq_to_chan(ic->ic_pdev_obj, ic->ic_chanchange_chan_freq);
                ...
                if(IEEE80211_ADDR_EQ(vap->iv_ibssdfs_ie_data.owner, vap->iv_myaddr))
                {
                    ...
                    actionargs->arg2     = wlan_reg_freq_to_chan(ic->ic_pdev_obj, ic->ic_chanchange_chan_freq);
...
ieee80211_add_mmie(struct ieee80211vap *vap, u_int8_t *bfrm, u_int32_t len)
{
    ...
    /* A1 || A2 || A3 */
    memcpy(aad + 2, wh->i_addr1, QDF_MAC_ADDR_SIZE);
    memcpy(aad + 8, wh->i_addr2, QDF_MAC_ADDR_SIZE);
    memcpy(aad + 14, wh->i_addr3, QDF_MAC_ADDR_SIZE);
   /* Init 16 byte mic buffer with zero */
    ...
    if((RSN_CIPHER_IS_CMAC(&vap->iv_rsn)) || RSN_CIPHER_IS_CMAC256(&vap->iv_rsn)){
        ...
        memcpy(nounce, wh->i_addr2, QDF_MAC_ADDR_SIZE);
                                                ...
                                                struct ieee80211com *ic)
{
    ...
    heop_6g->minimum_rate           = IEEE80211_HE_6GOP_DEFAULT_MIN_RATE;
...
#endif
#if SUPPORT_11AX_D3
    heop_param.op_6g_info_present  = (ic->ic_he.heop_param &
                                     IEEE80211_HEOP_6GHZ_INFO_PRESENT_MASK) >>
                                        IEEE80211_HEOP_6GHZ_INFO_PRESENT_S;
#else
...
#if SUPPORT_11AX_D3
    if(heop_param.op_6g_info_present) {
...
static void
ieee80211_parse_he_6g_opinfo(struct ieee80211_node *ni, u_int8_t *he_6g_opinfo)
{
...
#define CO_HOSTED_BSSID_INDICATOR_OFFSET    1
    ...
    if (ni_he->heop_param & IEEE80211_HEOP_6GHZ_INFO_PRESENT_MASK) {
        /* Negotiated HE 6GHz Operation Information */
        ieee80211_parse_he_6g_opinfo(ni, he_6g_opinfo);
    }
#endif /* SUPPORT_11AX_D3*/
}
...
bool ieee80211_check_ie_of_type(struct ieee80211vap *vap, uint8_t *iebuf, uint8_t element_id, uint8_t sub_id)
{
    bool ret = false;
#ifndef WLAN_CONV_CRYPTO_SUPPORTED
    struct ieee80211_rsnparams tmp_rsn;
    qdf_mem_zero(&tmp_rsn, sizeof(tmp_rsn));
#else
...
#ifndef WLAN_CONV_CRYPTO_SUPPORTED
            if (sub_id == 1 && (iswpaoui(iebuf)
                        && ieee80211_parse_wpa(vap, iebuf, &tmp_rsn) == 0)) {
...
#else
            if (sub_id == 1 && (iswpaoui(iebuf) && wlan_crypto_wpaie_check(
                            (struct wlan_crypto_params *)&tmp_crypto_params,
                            iebuf) == 0)) {
                struct wlan_crypto_params *vdev_crypto_params;
                ret = true;
