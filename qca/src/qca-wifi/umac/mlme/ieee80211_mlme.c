                   ...
                   struct ieee80211_app_ie_t* optie)
{
    ...
    ni = ieee80211_vap_find_node(vap, macaddr, WLAN_MLME_HANDLER_ID);
...
int wlan_mlme_deauth_request(wlan_if_t vaphandle, u_int8_t *macaddr, IEEE80211_REASON_CODE reason)
{
    ...
    if (vap->iv_opmode == IEEE80211_M_HOSTAP && ni != vap->iv_bss) {
        ...
        if (!delayed_cleanup ||
            ...
            IEEE80211_NODE_LEAVE(ni);
            IEEE80211_DELIVER_EVENT_MLME_DEAUTH_COMPLETE(vap,macaddr,IEEE80211_STATUS_SUCCESS,ni->ni_associd,reason);
        }
    }
            ...
            if(!(IEEE80211_ADDR_EQ(dst_addr, ieee80211broadcastaddr)) && delayed_cleanup) {
                ...
                if (!leave_ongoing && (ni != ni->ni_bss_node)) {
                    ...
                    if (subtype == IEEE80211_FC0_SUBTYPE_DISASSOC) {
                        ...
                        IEEE80211_DELIVER_EVENT_MLME_DEAUTH_COMPLETE(vap,
                                dst_addr, ts->ts_flags , ni->ni_associd, ni->ni_reason_code);
                    IEEE80211_NODE_LEAVE(ni);
                }
            }
...
#if WLAN_SUPPORT_GREEN_AP
                if ((vap->iv_opmode == IEEE80211_M_HOSTAP) && (!ni->is_sta_node) && (!ts->ts_flags)) {
    ...
    )
{
    ...
    ald_assoc_notify(vap, macaddr, ALD_ACTION_DISASSOC, reason);
...
QDF_STATUS wlan_mlme_restart_bss(struct wlan_objmgr_vdev *vdev)
{
    return wlan_vdev_mlme_sm_deliver_evt(vdev,
                                         WLAN_VDEV_SM_EV_SUSPEND_RESTART,
                                         0, NULL);
}
...
void peer_delete_req_status_set(struct ieee80211_node *ni, void *arg)
{
    u_int8_t *peer_del_req = (u_int8_t *)arg;
    if (peer_del_req) {
        if ((*peer_del_req) == MLME_INDIVIDUAL_PEER_CLEANUP)
            qdf_atomic_set(&(ni->ni_peer_del_req_enable), 1);
        else if ((*peer_del_req) == MLME_MULTIPLE_PEER_CLEANUP)
            qdf_atomic_set(&(ni->ni_peer_del_req_enable), 0);
    }
}
...
cleanup_sta_peer(void *arg, struct ieee80211_node *ni)
{
    ...
    peer_delete_req_status_set(ni, arg);
...
sta_disassoc(void *arg, struct ieee80211_node *ni)
{
    ...
    if (only_rptr_clients &&
        ((*only_rptr_clients) == MLME_REPEATER_PEER_CLEANUP)) {
    ...
    if (ftype == IEEE80211_FRAME_TYPE_BEACON) {
        while ((u_int8_t *)ie < buf + buflen) {
            ...
            if (ie->element_id == IEEE80211_ELEMID_XCAPS) {
                ...
                if ((elem_extcap->elem_len >= 11) &&
                    (le16toh(elem_extcap->ext_capflags5) & IEEE80211_EXTCAPIE_BEACON_PROTECTION))
		{
                    vap->iv_beacon_prot = 1;
		}
                buflen -= ie->length + 2;
...
#define MSECS_IN_SEC  (1000)
                    ...
                    if (ie_msecs > msecs) {
                        ...
                        qdf_nofl_info("FIXME: Time Advt IE crossing day boundary\n");
                    }
#undef MSECS_IN_HOUR
#undef MSECS_IN_MIN
#undef MSECS_IN_SEC
...
*/
    ...
    else if ((ftype == IEEE80211_FRAME_TYPE_PROBEREQ) ||
             (ftype == IEEE80211_FRAME_TYPE_PROBERESP) ||
             (ftype == IEEE80211_FRAME_TYPE_ASSOCREQ) ||
             (ftype == IEEE80211_FRAME_TYPE_ASSOCRESP))  {
        while ((u_int8_t *)ie < buf + buflen) {
            if (ie->element_id == IEEE80211_ELEMID_XCAPS) {
...
/*
 * ieee80211_delete_duplicate_ies: Delete the IE passed from list
 *
 * @vap        : VAP handle
 * @ftype      : Frame type to which duplicate IEs are to be removed
 * @key_ie     : IE whose duplicate to be removed
 * @key_ie_len : Length of the key_ie
 */
static inline void ieee80211_delete_duplicate_ies(struct ieee80211vap *vap,
        ieee80211_frame_type ftype, uint8_t *key_ie, uint32_t key_ie_len)
{
    ...
    STAILQ_FOREACH_SAFE(ie_entry, &vap->iv_app_ie_list[ftype], link_entry, next_entry) {
        ...
        if (ie_entry->app_ie.ie[0] == key_ie[0]) {

            /* Check for vendor and extension IEs */
            if(key_ie[0] != IEEE80211_ELEMID_VENDOR
                    || !(OS_MEMCMP(&ie_entry->app_ie.ie[2],&key_ie[2],4))) {
                ...
                qdf_mem_free(ie_entry->app_ie.ie);
                qdf_mem_free(ie_entry);
                return;
            }
        }
    }
...
int wlan_mlme_set_appie(wlan_if_t vaphandle, ieee80211_frame_type ftype, const u_int8_t *buf, u_int16_t buflen)
{
    ...
    iebuf = qdf_mem_malloc(buflen*sizeof(u_int8_t));
    if (iebuf == NULL) {
        return -ENOMEM;
    }
    qdf_mem_copy(iebuf, buf, buflen);

    IEEE80211_VAP_LOCK(vap);
    IEEE80211_VAP_APPIE_UPDATE_DISABLE(vap);
    while ((i+1 < buflen) && (iebuf[i+1]+1 < buflen)) {
        if (IEEE80211_VAP_IS_HIDESSID_ENABLED(vap)
                && (ftype == IEEE80211_FRAME_TYPE_BEACON) && iswpsoui(&iebuf[i])) {
            i += iebuf[i+1] + 2;
            continue;
        } else {
            ...
            /* Remove duplicates */
            ieee80211_delete_duplicate_ies(vap, ftype, &iebuf[i], iebuf[i+1] + 2);

            ie_entry = (struct app_ie_entry *)qdf_mem_malloc(sizeof(struct app_ie_entry));
            if (!ie_entry) {
            ...
            ie_entry->app_ie.ie = &iebuf[i];
            ie_entry->app_ie.length = iebuf[i+1] + 2;
            ie_entry->identifier = DEFAULT_IDENTIFIER;
...
int wlan_mlme_get_appie(wlan_if_t vaphandle, ieee80211_frame_type ftype, u_int8_t *buf, u_int32_t *ielen, u_int32_t buflen, u_int8_t identifier)
{
    ...
    *ielen = 0;
    ...
    {
        qdf_nofl_info("vie handle is NULL");
        error = -EINVAL;
        goto exit;
    }
    ...
    STAILQ_FOREACH_SAFE(ie_entry, &vap->iv_app_ie_list[ftype], link_entry,next_entry)
    {
        if(ie_entry->identifier == identifier)
        {
            if(ie_entry->app_ie.length != 0)
            {
                ...
                {
                    qdf_nofl_info("buffer length exceeded");
                    error = -EINVAL;
                    IEEE80211_VAP_UNLOCK(vap);
                    goto exit;
                }
...
int wlan_mlme_set_optie(wlan_if_t vaphandle, u_int8_t *buf, u_int16_t buflen)
{
    ...
    if (buflen) {
        ...
        OS_MEMCPY(vap->iv_opt_ie.ie, buf, buflen);
    }
    IEEE80211_VAP_UNLOCK(vap);
...
void wlan_mlme_connection_up(wlan_if_t vaphandle)
{
    ...
    && !vap_is_psta(vap)
       ...
       if (ic->ic_global_list->same_ssid_support) {
           ...
           if (update_beacon) {
               for (i = 0; i < MAX_RADIO_CNT; i++) {
                   ...
                   if (tmp_ic) {
                       wlan_pdev_beacon_update(tmp_ic);
                   }
               }
           }
       }
       ...
       cdp_txrx_set_vdev_param(wlan_psoc_get_dp_handle(psoc), wlan_vdev_get_id(vap->vdev_obj),
...
#ifdef QCA_NSS_WIFI_OFFLOAD_SUPPORT
           if (ic->nss_vops) {
               ic->nss_vops->ic_osif_nss_store_other_pdev_stavap(ic);
           }
#endif
       ...
       && !vap_is_psta(vap)
        ...
        if (ic->ic_global_list->num_stavaps_up == 1) {
            ...
            if (ic->ic_global_list->same_ssid_support) {
                ...
                for (i=0; i < MAX_RADIO_CNT; i++) {
                    OS_MEMZERO(&ic->ic_global_list->preferred_list_stavap[i][0], QDF_MAC_ADDR_SIZE);
                    OS_MEMZERO(&ic->ic_global_list->denied_list_apvap[i][0], QDF_MAC_ADDR_SIZE);
                }
                for (i = 0; i < MAX_RADIO_CNT; i++) {
                    tmp_ic = ic->ic_global_list->global_ic[i];
                    if (tmp_ic) {
...
void ieee80211_mlme_recv_deauth(struct ieee80211_node *ni, u_int16_t reason_code)
{
...
#endif
#if QCA_SUPPORT_SON
                son_send_disconnect_event(vap->vdev_obj, ni->ni_macaddr,
                                          BSTEERING_SOURCE_REMOTE,
                                          BSTEERING_DISASSOC,
                                          reason_code, &disassoc_stats);
#endif
    ...
    default:
        /* Call MLME indication handler */
        IEEE80211_DELIVER_EVENT_MLME_DEAUTH_INDICATION(vap, ni->ni_bssid, 0, reason_code);
    }
}
...
void ieee80211_mlme_recv_disassoc(struct ieee80211_node *ni, u_int32_t reason_code)
{
	...
	default:
		/* Call MLME indication handler */
		{
			...
			IEEE80211_DELIVER_EVENT_MLME_DISASSOC_INDICATION(vap, macaddr, 0, reason_code);
		}
    }
}
    ...
    case MLME_REQ_JOIN_INFRA:
        ...
        if(MLME_STOP_WAITING_FOR_JOIN(mlme_priv) == TRUE){
            if(mlme_priv->im_join_is_timeout == 0){
                ...
                u_int8_t    bcast_addr[QDF_MAC_ADDR_SIZE] = {0xFF,0xFF,0xFF,0xFF,0xFF,0xFF};
...
bool wlan_mlme_connection_sm_is_running(wlan_if_t vaphandle)
{
    osif_dev  *osifp = (osif_dev *)wlan_vap_get_registered_handle(vaphandle);

    return wlan_connection_sm_is_running(osifp->sm_handle);
}
    ...
    ap_info->channel = prim_chan;
     ...
     if (!is_vap_add) {
        while (count <= rnr->rnr_cnt)
            {
               tbtt_info->bss_params.probe_resp_20tu_active = vap->iv_he_6g_bcast_prob_rsp;
               count++;
               tbtt_info++;
            }
        return 0;
...
static struct wlan_objmgr_vdev *wlan_find_vdev_from_psocs_by_macaddr(uint8_t* bssid)
{
 ...
 */
void wlan_fetch_inter_soc_rnr_cache(struct wlan_6ghz_rnr_global_cache **rnr)
{
    uint8_t index = 0;
    struct psoc_mlme_obj *mlme_psoc_priv_obj;
                                  ...
                                  int subtype)
{
    ...
    struct wlan_6ghz_rnr_global_cache *rnr;
    u_int32_t self_short_ssid = htole32(ieee80211_construct_shortssid(vap->iv_bss->ni_essid,
                                vap->iv_bss->ni_esslen));
    u_int32_t short_ssid = htole32(ieee80211_construct_shortssid(ssid, ssid_len));
    uint8_t *org_frm;
    uint32_t tbtt_offset = 255;
    struct ol_ath_softc_net80211 *reporting_scn = NULL;
    ol_ath_soc_softc_t *reported_soc;
    struct ol_ath_softc_net80211 *reported_scn;
    uint64_t tbtt_6ghz = 0;
    ...
    if (!rnr || rnr->rnr_cnt == 0) {
        QDF_TRACE(QDF_MODULE_ID_6GHZ, QDF_TRACE_LEVEL_DEBUG,
                  "%s:%d rnr is null or count is zero!!", __func__, __LINE__);
        return org_frm;
    }
    /* +2 is for element ID and length field (1 byte each) */
    OS_MEMCPY(frm, rnr->rnr_buf, rnr->rnr_size + 2);
    frm+=2;
    ap_info = (ieee80211_rnr_nbr_ap_info_t *)frm;
    tbtt_info = ap_info->tbtt_info;
    ap_info->hdr_filtered_nbr_ap = 0;
    ...
    {
        struct ieee80211vap *rnr_vap;

        /* Get vdev corresponding to first bssid.
         * Call a function to iterate through
         * all psocs to find 6ghz pdev and find vdev with
         * given mac addr.
         */

        vdev = wlan_find_vdev_from_psocs_by_macaddr(tbtt_info->bssid);

         if (!vdev) {
             qdf_err("Vdev corresponding to 6GHz Bssid in RNR cache is NULL");
             OS_MEMZERO(org_frm, rnr->rnr_size+2);
             return org_frm;
         }
         rnr_vap = wlan_vdev_get_mlme_ext_obj(vdev);

         if (!rnr_vap){
             qdf_err("Vap corresponding to rnr cache first vdev is NULL");
             OS_MEMZERO(org_frm, rnr->rnr_size+2);
             wlan_objmgr_vdev_release_ref(vdev, WLAN_MLME_NB_ID);
             return org_frm;
         }
         reporting_scn = OL_ATH_SOFTC_NET80211(vap->iv_ic);
         reported_scn = OL_ATH_SOFTC_NET80211(rnr_vap->iv_ic);
         if (!reporting_scn || !reported_scn) {
             OS_MEMZERO(org_frm, rnr->rnr_size+2);
             qdf_err("Reporting/reported AP Scn is NULL");
             wlan_objmgr_vdev_release_ref(vdev, WLAN_MLME_NB_ID);
             return org_frm;
         }
         reported_soc = reported_scn->soc;
         if (ic->ic_is_target_lithium(psoc)) {
         /* Calculate tbtt offset only if bcn intervals are same.
          * Deferred AI: Handle different bcn interval case.
          * Calculation: Find difference between the tbtt of 6Ghz Vap
          * and lower band Vap and convert to TU (/1024 or >>10). Modulo operation
          * with bcn interval gives nearest prior tbtt.
          * Tbtt Timeline              : ------|---|--|---|--|---|--|---|------
          * Vap Bcn                    :      5g 6g 5g   6g 5g  6g  5g 6g
          * Tbtt offset                :        ----   ----   ----
          * X = Tbtt diff % bcn_intval :            ---    ---    ---
          * bcn_intval - X = tbtt_offset :             ----   ----
          */
             if (rnr_vap->iv_bss->ni_intval == vap->iv_bss->ni_intval) {

                 tbtt_6ghz = reported_soc->tbtt_soc_delta + reporting_scn->soc->qtime_val;
                 if (tbtt_6ghz > vap->iv_vap_tbtt_qtime) {
                     tbtt_offset = qdf_do_div_rem((tbtt_6ghz - vap->iv_vap_tbtt_qtime),
                                    (vap->iv_bss->ni_intval << 10)) >> 10;
                 } else {
                     tbtt_offset = ((rnr_vap->iv_bss->ni_intval << 10) -
                                   qdf_do_div_rem((vap->iv_vap_tbtt_qtime - tbtt_6ghz),
                                   (rnr_vap->iv_bss->ni_intval << 10))) >> 10;
                 }
             }
        }
        wlan_objmgr_vdev_release_ref(vdev, WLAN_MLME_NB_ID);
