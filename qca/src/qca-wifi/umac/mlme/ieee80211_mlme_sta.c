...
ieee80211_vap_iter_get_chan(void *arg, wlan_if_t vap)
{
    ...
    if ((IEEE80211_IS_CHAN_5GHZ(chan) && IEEE80211_IS_CHAN_5GHZ(vap_des_chan)) ||
        (IEEE80211_IS_CHAN_6GHZ(chan) && IEEE80211_IS_CHAN_6GHZ(vap_des_chan)) ||
	...
	if (des_chan_mode > chan_mode) {
		if (IEEE80211_IS_CHAN_ANYG(chan))
			chan->ic_flags = IEEE80211_CHAN_2GHZ | IEEE80211_CHAN_HT20;
		else
                                        ...
                                        u_int32_t             ie_len)
{
    ...
    if (assoc_resp_ie != NULL) {
        son_process_assoc_resp_ie(ni->peer_obj, assoc_resp_ie, ie_len);
    }
