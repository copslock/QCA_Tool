                ...
                struct ieee80211_extendedchannelswitch_ie *echanie, u_int8_t *update_beacon)
{
        ...
        if((vap->iv_opmode == IEEE80211_M_STA)
                 ...
                 */
                if(chanie != NULL) {
                        ic->ic_chanchange_chan_freq = wlan_reg_chan_band_to_freq(ni->ni_ic->ic_pdev_obj, chanie->newchannel,
                                                           BIT(wlan_reg_freq_to_band(ic->ic_curchan->ic_freq)));
                        ic->ic_chanchange_tbtt = chanie->tbttcount ;
                }
                else {
                        ic->ic_chanchange_chan_freq = wlan_reg_chan_band_to_freq(ni->ni_ic->ic_pdev_obj, echanie->newchannel,
                                                           BIT(wlan_reg_freq_to_band(ic->ic_curchan->ic_freq)));
...
int ieee80211_get_snr(ieee80211_scan_entry_t  scan_entry, struct ieee80211vap *vap)
{
    uint8_t                                      snr;
    qdf_list_t                                   *list;
