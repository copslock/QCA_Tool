...
int wlan_disassoc_ind_stats_prepare(struct cdp_peer_stats *peer_stats,
                                    struct bs_node_disassoc_ind *disassoc_stats)
{
    disassoc_stats->tx_bytes = peer_stats->tx.tx_success.bytes;
    disassoc_stats->tx_packets = peer_stats->tx.tx_success.num;
    disassoc_stats->tx_error_packets = peer_stats->tx.tx_failed;
    disassoc_stats->tx_retrans = peer_stats->tx.retry_count;
    disassoc_stats->rx_bytes = peer_stats->rx.to_stack.bytes;
    disassoc_stats->rx_packets = peer_stats->rx.to_stack.num;
    disassoc_stats->rx_error_packets = peer_stats->rx.rx_discard;
...
int wlan_node_get_disassoc_stats(struct ieee80211vap *vap,
                                 ...
                                 struct bs_node_disassoc_ind *disassoc)
{
    ...
    if (QDF_IS_STATUS_ERROR(status)) {
        ...
        qdf_err("peer_stats is null");
        return -EINVAL;
    }

    wlan_vap_get_bssid(vap, (uint8_t *)&disassoc->bssid);
...
int
ieee80211_send_unprotected_deauth(struct ieee80211vap *vap, u_int8_t *dstmac, u_int16_t reason)
{
                                    ...
                                    uint8_t subtype, uint8_t *dstmac)
{
    ...
    switch(sub_type) {
        case IEEE80211_FC0_SUBTYPE_DEAUTH:
            if(protected) {
                return wlan_mlme_deauth_request(vap, dstmac, IEEE80211_REASON_DISASSOC_LOW_ACK);
            } else {
                return ieee80211_send_unprotected_deauth(vap, dstmac, IEEE80211_REASON_DISASSOC_LOW_ACK);
            }
        break;
...
ieee80211_send_deauth(struct ieee80211_node *ni, u_int16_t reason)
{
    ...
    if (vap->iv_opmode == IEEE80211_M_HOSTAP) {
        ...
        son_send_disconnect_event(vap->vdev_obj, da,
                                  BSTEERING_SOURCE_LOCAL,
                                  BSTEERING_DEAUTH,
                                  reason, &disassoc_stats);
    }
#endif
...
struct ieee80211_ath_channel* parse_next_channel_vendor_ie(struct ieee80211vap * vap, struct ieee80211_ie_header *info_element)
{
    ...
    while (sub_remaining_ie_length >= sizeof(struct ieee80211_ie_header)) {
        ...
        if (sub_remaining_ie_length < sub_info_element->length) {
            ...
            qdf_nofl_info("Incomplete corrupted IE:%x\n",IEEE80211_ELEMID_VENDOR);
            return NULL;
        }
...
ieee80211_recv_beacon(struct ieee80211_node *ni, wbuf_t wbuf, int subtype, struct ieee80211_rx_status *rs)
{
    ...
    if (vap->iv_mesh_vap_mode) {
         ...
         ni_mesh = ieee80211_find_node(&ic->ic_sta, (u_int8_t *)&scan_entry->mac_addr, WLAN_MGMT_HANDLER_ID);
         if (ni_mesh) {
...
ieee80211_recv_action(struct ieee80211_node *ni, wbuf_t wbuf, int subtype, struct ieee80211_rx_status *rs, bool* action_taken)
{
    ...
    if (ia->ia_category == IEEE80211_ACTION_CAT_WNM &&
        ...
        if (((ia->ia_category == IEEE80211_ACTION_CAT_VHT) && (!ic->ic_vhtcap)) ||
        ...
        vdev_cp_stats_rx_mgmt_discard_inc(vap->vdev_obj, 1);
            ...
            IEEE80211_NOTE(vap, IEEE80211_MSG_ACTION, ni,
                   "%s: action frames are dropped (cat %u, act %u vhtcap :%u)\n",__func__, ia->ia_category, ia->ia_action, ic->ic_vhtcap);
            return -EINVAL;
        }
    }
    else{
    ...
    case IEEE80211_ACTION_CAT_PROT_DUAL: {
        ...
        switch (ia->ia_action) {
            case IEEE80211_ACTION_GAS_INITIAL_REQUEST:
                ald_anqp_frame_recvd_notify(vap, wh->i_addr2, wbuf_header(wbuf),
...
ieee80211_is_mmie_valid(struct ieee80211vap *vap, struct ieee80211_node *ni, u_int8_t* frm, u_int8_t* efrm)
{
    ...
    */
    ...
    /* A1 || A2 || A3 */
    OS_MEMCPY(aad + 2, wh->i_addr_all, 3 * QDF_MAC_ADDR_SIZE);
...
#ifdef WLAN_CONV_CRYPTO_SUPPORTED
                if (wlan_crypto_is_pmf_enabled(vap->vdev_obj, ni->peer_obj) &&
                    wlan_crypto_is_key_valid(NULL, ni->peer_obj, WLAN_CRYPTO_KEYIX_NONE)) {
#else
                if (ieee80211_is_pmf_enabled(vap, ni) && ni->ni_ucastkey.wk_valid) {
#endif
                    /* MFP is enabled, so we need to set Privacy bit */
