...
void ieee80211_kick_node(struct ieee80211_node *ni)
{
   ...
   if (ni->ni_associd != 0) {
#if QCA_SUPPORT_SON
       if (ic->ic_is_mode_offload(ic) && !son_is_steer_in_prog(ni->peer_obj)) {
    ...
    if(ic->ic_is_mode_offload(ic)) {
#if QCA_SUPPORT_SON
        if(ni->ni_associd == 0 || son_is_steer_in_prog(ni->peer_obj)) {
#else
        if(ni->ni_associd == 0) {
#endif
...
ieee80211_complete_wbuf(wbuf_t wbuf, struct ieee80211_tx_status *ts )
{
    ...
    if(ni == NULL)
    {
       qdf_nofl_info("%s: Error wbuf node ni is NULL, so exiting. wbuf: 0x%pK \n",__func__, wbuf);
       return;
    }
    ...
    if (type == IEEE80211_FC0_TYPE_MGT && subtype == IEEE80211_FC0_SUBTYPE_ACTION) {
        frm = (u_int8_t *)&wh[1];
        /* In case of WEP we have 8 bytes encaped at TX
	 * consider these 8 bytes to reach to action header
	 */
        if (secured)
            frm += 8;
