...
#define MBO_MAX_ASSOC_DISALLOW 0x06
            if (!ieee80211_vap_mbo_is_set(vap)) {
                ...
                /* Notify user space of assoc allowance status */
                ald_assoc_allowance_status_notify(vap, vap->iv_myaddr, !(mbo->usr_assoc_disallow));
#endif
            } else {
                               ...
                               u_int8_t *macaddr)
{
     ...
     */
     ni = ieee80211_find_node(&vap->iv_ic->ic_sta, macaddr, WLAN_MBO_ID);
    ...
    ieee80211_free_node(ni, WLAN_MBO_ID);
...
/**
 * ieee80211_clear_opclass_vars - Set opclass variables to zero
 * @ni - Pointer to ni
 *
 * Return - void
 */
static void ieee80211_clear_opclass_vars(struct ieee80211_node *ni)
{
    ni->ni_supp_op_cl.num_of_supp_class = 0;
    ni->ni_supp_op_cl.curr_op_class = 0;
}
                            ...
                            struct ieee80211_node *ni, u_int16_t countrycode)
{
    ...
    if (opclass_num > MAX_NUM_OPCLASS_SUPPORTED) {
        opclass_num = MAX_NUM_OPCLASS_SUPPORTED;
        qdf_info("Limiting the num opclasses to maximum");
    }

    if (!opclass_num) {
        ieee80211_clear_opclass_vars(ni);
        qdf_err("Number of opclass cannot be zero");
        return -EINVAL;
    }

    ni->ni_supp_op_cl.num_of_supp_class = opclass_num;
    ni->ni_supp_op_cl.curr_op_class = frm[2];
    for (i = 0; i < opclass_num; i++) {
         ni->ni_supp_op_cl.supp_class[i] = frm[i+3];
    }
    ...
    while (len > 0)
    {
        ...
        switch (attr_id) {
            case OCE_ATTRIB_PROBE_SUPPRESSION_BSSID:
                 ...
                 */
                if (attr_len < 0 || attr_len % QDF_MAC_ADDR_SIZE != 0) {
                    IEEE80211_DPRINTF(vap, IEEE80211_MSG_MLME,
                                  "Error parsing probe suppression BSSID attribute length - %u\n", attr_len);
                    break;
                }

                num_mac = attr_len / QDF_MAC_ADDR_SIZE;
                if (num_mac == 0) {
                    /* Wildcard */
...
ieee80211_update_non_oce_ap_presence (struct ieee80211vap *vap)
{
    ...
    TAILQ_FOREACH_SAFE(nbr_vap, &ic->ic_vaps, iv_next, tvap) {
        if (!OS_MEMCMP(nbr_vap->iv_myaddr, vap->iv_myaddr, QDF_MAC_ADDR_SIZE))
            continue;
...
ieee80211_fill_rnr_entry(void *iarg, wlan_scan_entry_t se)
{
    ...
    for (i = 0; i < MAX_RADIO_CNT; i++) {
        ...
        TAILQ_FOREACH_SAFE(nbr_vap, &nbr_ic->ic_vaps, iv_next, tvap) {
            if (!OS_MEMCMP(nbr_vap->iv_myaddr, util_scan_entry_bssid(se), QDF_MAC_ADDR_SIZE))
                return QDF_STATUS_SUCCESS;
        }
    }
