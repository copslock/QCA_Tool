...
{
    if (cbs->cbs_event != 0) {
        if(event == IEEE80211_CBS_DCS_INTERFERENCE) {
            qdf_info("%s: event %d will be processed after the current event %d",
                __FUNCTION__, event, cbs->cbs_event);
            cbs->dcs_intr = 1;
        } else {
            qdf_info("%s: pending event %d, new event %d can't be processed",
...
void cbs_work(void *arg)
{
    ...
    if(cbs->dcs_intr == 1) {
        ieee80211_cbs_event(cbs, IEEE80211_CBS_DCS_INTERFERENCE);
        cbs->dcs_intr = 0;
    }
}
...
static void ieee80211_cbs_scan_evhandler(struct wlan_objmgr_vdev *vdev, struct scan_event *event, void *arg)
{
    ...
    case SCAN_EVENT_TYPE_COMPLETED:
        if (event->reason != SCAN_REASON_COMPLETED) {
            ieee80211_cbs_event(cbs, IEEE80211_CBS_SCAN_RESTART);
            break;
        }
        if (cbs->dwell_split_cnt < 0) {
...
static void ieee80211_cbs_cancel(ieee80211_cbs_t cbs)
{
    ...
    // Do nothing if CBS VAP has already been cleared
    if (!vap) {
        cbs_trace("Returning due to null CBS VAP\n");
        return;
    }
...
int ieee80211_cbs_event(ieee80211_cbs_t cbs, enum ieee80211_cbs_event event)
{
    ...
    case IEEE80211_CBS_SCAN_START:
        ...
        case IEEE80211_CBS_WAIT: /* notice fallthrough */
            ...
            scan_params->scan_req.dwell_time_active = cbs->dwell_split_time;
            scan_params->scan_req.dwell_time_passive = cbs->dwell_split_time + 5;
            ...
            scan_params->scan_req.scan_offset_time = (scan_offset + SCAN_START_OFFSET_MIN);
	    ...
	    if ((status = cbs_scan_start(vap, cbs) != EOK)) {
                ald_cbs_notify(vap, ALD_CBS_CANCELLED);
                ieee80211_cbs_cancel(cbs);
                rc = -EINVAL;
                goto release_ref;
            }
    ...
    case IEEE80211_CBS_SCAN_CANCEL_SYNC:
        ...
        case IEEE80211_CBS_INIT:
            ...
            break;
        default:
            qdf_info("CBS cancelled in state %u\n", cur_state);
            ald_cbs_notify(vap, ALD_CBS_CANCELLED);
    ...
    case IEEE80211_CBS_RANK_COMPLETE:
        ...
        case IEEE80211_CBS_RANK:
            ...
            // Generate CBS complete event
            ald_cbs_notify(vap, ALD_CBS_COMPLETE);
    ...
    case IEEE80211_CBS_DCS_INTERFERENCE:
        ieee80211_ucfg_set_chanswitch(vap, cbs->ch_freq, ch_switch_tbtt, 0);
        break;
    ...
    case IEEE80211_CBS_STATS_COMPLETE:
        ...
        break;
    case IEEE80211_CBS_SCAN_RESTART:
        switch (cur_state) {
        case IEEE80211_CBS_INIT:
            /* do nothing */
            break;
        default:
            ...
            cbs->cbs_state = IEEE80211_CBS_WAIT;
            OS_SET_TIMER(&cbs->cbs_timer, CBS_RESTART_TIME);
            break;
        }
        break;
    default:
...
int ieee80211_cbs_api_change_home_channel(ieee80211_cbs_t cbs)
{
    ...
    do{
        chan_freq = ieee80211_acs_api_get_ranked_chan(cbs->cbs_ic->ic_acs, i++);
        channel = ieee80211_doth_findchan(vap, chan_freq);
        ...
        if (!chan_freq){
            chan_freq = ieee80211_acs_api_get_ranked_chan(cbs->cbs_ic->ic_acs, 1);
            channel = ieee80211_doth_findchan(vap, chan_freq);
            ...
            if (!chan_freq) {
                ...
                return -EINVAL;
            } else if ((ic->ic_curchan->ic_freq == chan_freq) || (channel == NULL) ) {
            /* If first chan is same as current chan or channel obtained is not valid for current mode, get next best chan */
                chan_freq = ieee80211_acs_api_get_ranked_chan(cbs->cbs_ic->ic_acs, 2);
                channel = ieee80211_doth_findchan(vap, chan_freq);
    ...
    } while (((channel == NULL) ? 1 : IEEE80211_IS_CHAN_DFS(channel)) || (ic->ic_curchan->ic_freq == chan_freq));

    cbs->ch_freq = chan_freq;
    cbs_trace("New home channel freq %d\n", chan_freq);
    /* switch ASAP, set the CSA TBTT count to 1 */
    ieee80211_cbs_post_event(cbs, IEEE80211_CBS_DCS_INTERFERENCE);
...
int wlan_bk_scan(struct ieee80211com *ic)
{
    ...
    if (found) {
        ...
        qdf_nofl_info(" failed to start CBS. no vap found\n");
        return -EINVAL;
    }
    return EOK;
}
