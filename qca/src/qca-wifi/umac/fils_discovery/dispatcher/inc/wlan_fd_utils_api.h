...
void wlan_fd_set_valid_fd_period(struct wlan_objmgr_vdev *vdev,
			uint32_t fd_period);
/**
 * wlan_fd_offload() - API to send FD template to FW
 * @vdev: Pointer to vdev
 * @vdev_id: VDEV ID
 *
 * This API is invoked to offload the FD frame.
 *
 * Return: QDF_STATUS_SUCCES on success or QDF_STATUS_E_INVAL on error
 */
