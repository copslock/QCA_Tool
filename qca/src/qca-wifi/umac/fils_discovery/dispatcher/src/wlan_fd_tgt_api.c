...
QDF_STATUS tgt_fd_offload(struct wlan_objmgr_vdev *vdev, uint32_t vdev_id)
{
	struct fd_vdev *fv;
	QDF_STATUS retval = 0;
	...
	retval = wlan_fd_offload(vdev, vdev_id);

	return retval;
}

QDF_STATUS tgt_fd_tmpl_update(struct wlan_objmgr_vdev *vdev)
{
	struct fd_vdev *fv;
	QDF_STATUS retval = 0;
	...
	wlan_fd_update_trigger(vdev);
	retval = wlan_fd_update(vdev);
