...
struct ieee80211_uplinkinfo {
	...
	u_int8_t otherband_bssid[QDF_MAC_ADDR_SIZE];
	u_int8_t island_detected;
	int snr;
};

/*
 * EasyMesh R1 SubElements
 */
struct ieee80211_ie_map {
#define MAP_OUI_LENGTH 3
	/* IEEE 802.11 vendor specific information element */
	u_int8_t map_apinfo_id;
	/* Length of the following fields in the IE in octets */
	u_int8_t map_apinfo_len;
	/* Wi-Fi Alliance specific OUI */
	u_int8_t map_apinfo_oui[MAP_OUI_LENGTH];
	/* Wi-Fi Alliance specific OUI Type for MAP */
	u_int8_t map_apinfo_type;
	/* Multi-AP Profile extension subelement identifier */
	u_int8_t map_ext_subelement;
	/* Number of Bytes in the subelement value */
	u_int8_t map_ext_len;
	/* MapBSS Type */
	u_int8_t map_ext_value;
#undef MAP_OUI_LENGTH
} __packed;

/*
 * EasyMesh R2 SubElements
 */
struct ieee80211_ie_map2 {
	/* Multi-AP Profile subelement identifier */
	u_int8_t map_profile_subelement;
	/* Number of Bytes in the subelement value */
	u_int8_t map_profile_len;
	/* Multi-AP Profile field */
	u_int8_t map_profile_value;
	/* Multi-AP Default 802.1Q Setting subelement identifier */
	u_int8_t map_8021q_subelement;
	/* Number of Bytes in the subelement value */
	u_int8_t map_8021q_len;
	/* Primary VLAN ID */
	u_int16_t map_8021q_value;
} __packed;
...
struct son_pdev_priv {
	...
	u_int8_t son_otherband_uplink_bssid[QDF_MAC_ADDR_SIZE];
	/* Otherband bssid serving the same SSID */
	u_int8_t son_otherband_bssid[QDF_MAC_ADDR_SIZE];
...
#define IEEE80211_BS_MAX_STA_WH_24G 10
	...
	u_int8_t iv_map_version; /* Flag to check map version that is enabled */
	u_int8_t iv_mapbh:1, /* flag to check if map backhaul is set */
	         iv_mapfh:1, /* flag to check if map fronthaul is set */
	         iv_mapbsta:1, /* flag to check if map bSta is set */
	         iv_mapteardown:1, /* flag to check if map tearDown is set */
	         iv_map_r1bsta_assoc_disallow:1, /* flag to check if map R1 bsta association is disAllowed */
	         iv_map_r2above_bsta_assoc_disallow:1; /* flag to check if R2 and above bsta association is disallowed */
	u_int8_t iv_mapvapup:1, /* flag to check if vap is up */
	         iv_mapr2_assoc_status_notify:1; /* flag to check change in new assoc allowance in fronthaul */

	u_int16_t iv_mapr2_sta_primary_vlan;
#if QCA_SUPPORT_SSID_STEERING
	u_int8_t ssid_steering_config; /* ssid steering flag */
#endif
};
...
int son_core_get_map_cac_cap(struct wlan_objmgr_vdev *vdev,
			     mapv2_cac_info_t *map_cac_info);
...
#if QCA_SUPPORT_SSID_STEERING
int son_core_set_ssid_steering_config(struct wlan_objmgr_vdev *vdev, int8_t ssid_steering_config);
