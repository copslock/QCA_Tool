...
#define CHANNEL_LOAD_REQUESTED 2
	...
	if (vdev == pd_priv->son_iv  && wlan_son_is_vdev_valid(vdev)) {
		...
		if ((wlan_vdev_is_up(vdev) == QDF_STATUS_SUCCESS) &&
			...
			chanlist[0] = wlan_get_wlan_band_id_chan_to_freq(wlan_vdev_get_pdev(vdev),
					pd_priv->son_active_ieee_chan_num, band);

			if (!chanlist[0]) {
				SON_LOGW("%s: Inavlid frequncy: %d for chan: %d, band: %d\n",
						__func__, chanlist[0],
						pd_priv->son_active_ieee_chan_num, WLAN_BAND_UNSPECIFIED);
			}
			...
			if (wlan_vdev_acs_set_user_chanlist(vdev, false, chanlist) == EOK) {
		...
		qdf_timer_mod(&pd_priv->son_chan_util_timer,
					 ...
					 void *arg)
{
	...
	if (!pe_priv) {
		qdf_print("%s: peer priv is NULL",__func__);
		return;
	}
...
static OS_TIMER_FUNC(son_core_inact_timeout_handler)
{
	...
	qdf_timer_mod(&pd_priv->son_inact_timer,
					       ...
					       void *arg)
{
	...
	vdev_priv->iv_map_version = 0;
	vdev_priv->iv_mapbh = 0;
	vdev_priv->iv_mapfh = 0;
	vdev_priv->iv_mapbsta = 0;
	vdev_priv->iv_mapvapup = 0;
	vdev_priv->iv_mapteardown = 0;
	vdev_priv->iv_map_r1bsta_assoc_disallow = 0;
	vdev_priv->iv_map_r2above_bsta_assoc_disallow = 0;
	vdev_priv->iv_mapr2_assoc_status_notify = 0;

#if QCA_SUPPORT_SSID_STEERING
	vdev_priv->ssid_steering_config = SON_SSID_STEERING_DEFAULT_VDEV;
#endif
...
#if QCA_SUPPORT_SSID_STEERING
	if ( EOK != son_ssid_steering_netlink_attach()) {
		return QDF_STATUS_E_FAILURE;
	}
#endif
...
#if QCA_SUPPORT_SSID_STEERING
	if (EOK != son_ssid_steering_netlink_destroy()) {
		return QDF_STATUS_E_FAILURE;
	}
#endif
	return QDF_STATUS_SUCCESS;
...
static void son_iterate_peer_update_priv(struct wlan_objmgr_pdev *pdev,
			void *obj,
			void *arg)
{
	struct wlan_objmgr_peer *peer = (struct wlan_objmgr_peer *)obj;
			   ...
			   u_int8_t rssi_num_samples)
{
	...
	do{
		...
		qdf_timer_mod(&pd_priv->son_inst_rssi_timer,INST_RSSI_TIMEOUT * 1000);
...
son_core_get_se_snr(void *arg, wlan_scan_entry_t se)
{
	...
	if(se_rootdistance == 0 && se_isrootap)
		rootinfo->snr = util_scan_entry_snr(se);
...
int son_core_get_best_otherband_uplink_bssid(struct wlan_objmgr_pdev *pdev, char *bssid)
{
	...
	pd_priv = wlan_son_get_pdev_priv(pdev);
	qdf_mem_copy(bssid, &pd_priv->son_otherband_uplink_bssid[0],QDF_MAC_ADDR_SIZE);
...
int son_core_set_best_otherband_uplink_bssid(struct wlan_objmgr_vdev *vdev, char *bssid)
{
	...
	pd_priv = wlan_son_get_pdev_priv(pdev);
	qdf_mem_copy(&pd_priv->son_otherband_uplink_bssid[0], bssid, QDF_MAC_ADDR_SIZE);
...
#if QCA_SUPPORT_SSID_STEERING
int son_core_set_ssid_steering_config(struct wlan_objmgr_vdev *vdev,
					 int8_t ssid_steering_config)
{
	struct son_vdev_priv *vdev_priv = NULL;

	if ((ssid_steering_config != SON_SSID_STEERING_PRIVATE_VDEV) &&
	    (ssid_steering_config != SON_SSID_STEERING_PUBLIC_VDEV)) {
		qdf_print("Set %d for private and %d for public VAP",
	...
	if (vdev_priv) {
		vdev_priv->ssid_steering_config = ssid_steering_config;
...
int son_core_get_ssid_steering_config(struct wlan_objmgr_vdev *vdev)
{
	struct son_vdev_priv *vdev_priv = NULL;
	...
	if (vdev_priv) {
		return vdev_priv->ssid_steering_config;
	}
...
int8_t son_core_set_innetwork_2g_mac(struct wlan_objmgr_vdev *vdev, const u_int8_t *macaddr,
	...
	if (channel == 0x00)  // delete all list
	{
		...
		if (TAILQ_EMPTY(&pd_priv->in_network_table_2g)) {
			...
			OS_MEMCPY(tmpnode->macaddr, macaddr, QDF_MAC_ADDR_SIZE);
			...
			TAILQ_FOREACH(tmpnode, &pd_priv->in_network_table_2g, table_list) {
				if (OS_MEMCMP(tmpnode->macaddr, macaddr, QDF_MAC_ADDR_SIZE) == 0) {
					tmpnode->channel = channel;
					break;
				}
			}
...
int son_core_get_map_operable_channels(struct wlan_objmgr_vdev *vdev, struct map_op_chan_t *map_op_chan)
{
	...
	return wlan_pdev_get_multi_ap_opclass(pdev, NULL /* apcap */, map_op_chan,
					      NULL /* opclass info */,
					      0 /* dfs not required */);
}
...
int son_core_get_map_apcap(struct wlan_objmgr_vdev *vdev, mapapcap_t *apcap)
{
	...
	apcap->hwcap.num_supported_op_classes =
		wlan_pdev_get_multi_ap_opclass(pdev, apcap, NULL /* map_op_chan */,
					       NULL /* opclass info */,
					       0 /* dfs not required */);
...
/**
 * @brief get hardware CAC capabilities
 * @param vdev handle
 * @param *pointer to store CAC info for MAPv2
 * @return EOK
 */

int son_core_get_map_cac_cap(struct wlan_objmgr_vdev *vdev, mapv2_cac_info_t *cac_info)
{
	struct wlan_objmgr_pdev *pdev = NULL;
	map_op_chan_t *map_op_chan = NULL;
	mapv2_cac_cap_t *cac_capability = NULL;
	u_int8_t t, c;
	u_int32_t time = MAPV2_CAC_COMPLETION_TIME;
	...
	map_op_chan = qdf_mem_malloc(MAP_MAX_OPERATING_CLASSES * sizeof(*map_op_chan));
	if (!map_op_chan)
		return -ENOMEM;

	cac_capability = &cac_info->cac_cap;
	cac_capability->num_cac_type = MAP_MAX_CAC_MODES;

	for (t = 0; t < cac_capability->num_cac_type; t++) {
		cac_capability->cac_type[t].cac_mode_supported = MAPV2_CONTINUOUS_WITH_DEDICATED_RADIO;
		cac_capability->cac_type[t].secRequiredForCAC[0] = time << BITS_IN_A_BYTE*2;
		cac_capability->cac_type[t].secRequiredForCAC[1] = time << BITS_IN_A_BYTE;
		cac_capability->cac_type[t].secRequiredForCAC[2] = time;

		cac_capability->cac_type[t].num_classes_supported =
			wlan_pdev_get_multi_ap_opclass(pdev, NULL /* apcap */, map_op_chan,
						       NULL /* opclass info */,
						       1 /* dfs required*/);

		for (c = 0; c < cac_capability->cac_type[t].num_classes_supported; c++) {
			cac_capability->cac_type[t].opclass[c].opclass = map_op_chan[c].opclass;
			cac_capability->cac_type[t].opclass[c].num_chan = map_op_chan[c].num_oper_chan;
			memcpy(cac_capability->cac_type[t].opclass[c].supported_chan_num,
			       map_op_chan[c].oper_chan_num, map_op_chan[c].num_oper_chan);
		}
	}

	qdf_mem_free(map_op_chan);
	cac_info->cac_capabilities_valid = SON_CAPABILITY_IS_VALID;
	return EOK;
}

/**
 * @brief get operable class info
 * @param info pointer to store operable class info
 * @return EOK
 */

int son_core_get_map_opclass_info(struct wlan_objmgr_vdev *vdev,
				  struct map_op_class_t *info)
{
	struct wlan_objmgr_pdev *pdev = NULL;
	...
	return wlan_pdev_get_multi_ap_opclass(pdev, NULL /* apcap */,
					      NULL /* map_op_chan */,
					      info, 0 /* dfs not required */);
}
...
static bool son_core_inact_enable(struct wlan_objmgr_pdev *pdev, bool enable)
{
	...
	if(enable) {
		...
		qdf_timer_sync_cancel(&pd_priv->son_inact_timer);
	}
						    ...
						    void *arg)
{
	...
	else if (wlan_son_is_pdev_enabled(pdev)) {
