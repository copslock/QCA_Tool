 ...
 * @return void
 */
void son_send_disconnect_event(struct wlan_objmgr_vdev *vdev,
			       const u_int8_t *mac_addr,
			       ...
			       struct bs_node_disassoc_ind *stats)
{
	...
	qdf_mem_copy(disc.client_addr, mac_addr, QDF_MAC_ADDR_SIZE);
    qdf_mem_copy((void *)&disc.stats, (void *)stats,
            sizeof(struct bs_node_disassoc_ind));
	disc.source = src;
	disc.type   = type;
	disc.reason = reason;
...
void son_send_assoc_fail_event(struct wlan_objmgr_vdev *vdev,
			      ...
			      u_int32_t reason)
{
	struct bs_assoc_reject_ind assoc;
	struct wlan_objmgr_pdev *pdev = NULL;
	...
	qdf_mem_copy(assoc.client_addr, mac_addr, QDF_MAC_ADDR_SIZE);
	assoc.reason = reason;
	son_notify_user(vdev,
			ATH_EVENT_BSTEERING_TX_ASSOC_FAIL,
			sizeof(assoc),
			(const char *) &assoc);
	return;
}
