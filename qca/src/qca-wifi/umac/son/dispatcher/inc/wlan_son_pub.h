...
#define SON_TARGET_IF_ENABLE \
	wlan_psoc_get_lmac_if_txops(psoc)->son_tx_ops.son_enable

#define SON_TARGET_IF_LMAC_CREATE \
	wlan_psoc_get_lmac_if_txops(psoc)->son_tx_ops.lmac_create

#define SON_TARGET_IF_LMAC_DESTROY \
	wlan_psoc_get_lmac_if_txops(psoc)->son_tx_ops.lmac_destroy

#define SON_TARGET_IF_TX_NULL \
	wlan_psoc_get_lmac_if_txops(psoc)->son_tx_ops.son_send_null

#define SON_TARGET_IF_RSSI_UPDATE \
	wlan_psoc_get_lmac_if_txops(psoc)->son_tx_ops.son_rssi_update

#define SON_TARGET_IF_RATE_UPDATE \
	wlan_psoc_get_lmac_if_txops(psoc)->son_tx_ops.son_rate_update

#define SON_TARGET_IF_CHK_UTIL_INTVL \
	wlan_psoc_get_lmac_if_txops(psoc)->son_tx_ops.son_sanity_util_intvl

#define SON_TARGET_IF_GET_PEER_RATE \
	wlan_psoc_get_lmac_if_txops(psoc)->son_tx_ops.get_peer_rate
...
/**
 * @brief update beacon measurement report frame
 * @param [in] peer
 * @param [in] frm - frame after action frame header
 * @param [in] frm_len : Length of the frame
 */
PUBLIC void son_update_beacon_report_frame(struct wlan_objmgr_peer *peer,
 ...
 *
 * @param [in] vdev  the vdev on which the ioctl was made
 ...
 * @return void
 */
PUBLIC void son_send_disconnect_event(struct wlan_objmgr_vdev *vdev,
				      const u_int8_t *mac_addr,
				      BSTEERING_DISCONNECT_SOURCE src,
				      BSTEERING_DISCONNECT_TYPE type,
				      u_int32_t reason,
				      struct bs_node_disassoc_ind *stats);
...
/**
 * @brief Generate an event indicating that an association response
 *        was sent with a failure code.
 *
 * @param [in] vap  the VAP on which the message was sent
 * @param [in] mac_addr  the MAC address of the client to which the
 *                       message was sent
 * @param [in] reason  reason code for association failure
 */
void son_send_assoc_fail_event(struct wlan_objmgr_vdev *vdev,
			      const u_int8_t *mac_addr,
			      u_int32_t reason);
