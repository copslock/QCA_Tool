...
#define IEEE80211_DELIVER_EVENT_MLME_DEAUTH_COMPLETE( _vap,macaddr,_evt,_associd,_reason)  do {                        \
        int i;                                                                                \
        for(i=0;i<IEEE80211_MAX_VAP_MLME_EVENT_HANDLERS; ++i) {                               \
            if ((_vap)->iv_mlme_evtable[i] &&                                                 \
            (_vap)->iv_mlme_evtable[i]->mlme_deauth_complete) {                               \
                (* (_vap)->iv_mlme_evtable[i]->mlme_deauth_complete)                          \
                                           ((_vap)->iv_mlme_arg[i],macaddr, _evt,_associd,_reason);                    \
             }                                                                                \
        }                                                                                     \
    } while(0)
...
#define IEEE80211_DELIVER_EVENT_NEIGHREQ_AP( _vap,_mac_addr,_val)  do {                   \
    int i;                                                                                \
    for(i=0;i<IEEE80211_MAX_MISC_EVENT_HANDLERS; ++i) {                                   \
        if ((_vap)->iv_misc_evtable[i] &&                                                 \
                        (_vap)->iv_misc_evtable[i]->wlan_recv_neighreq) {                 \
            (* (_vap)->iv_misc_evtable[i]->wlan_recv_neighreq)                            \
             ((_vap)->iv_misc_arg[i],_mac_addr,_val);                                     \
        }                                                                                 \
    }                                                                                     \
} while(0)

#define IEEE80211_DELIVER_EVENT_BCNRPT_AP( _vap,_val,_len)  do {                             \
    int i;                                                                                   \
    for(i=0;i<IEEE80211_MAX_MISC_EVENT_HANDLERS; ++i) {                                      \
        if ((_vap)->iv_misc_evtable[i] &&                                                    \
                        (_vap)->iv_misc_evtable[i]->wlan_recv_beaconrpt) {                   \
            (* (_vap)->iv_misc_evtable[i]->wlan_recv_beaconrpt)                              \
             ((_vap)->iv_misc_arg[i],_val,_len);                                             \
        }                                                                                    \
    }                                                                                        \
} while(0)

#define IEEE80211_DELIVER_EVENT_BSTM_AP( _vap,_val,_len)  do {                               \
    int i;                                                                                   \
    for(i=0;i<IEEE80211_MAX_MISC_EVENT_HANDLERS; ++i) {                                      \
        if ((_vap)->iv_misc_evtable[i] &&                                                    \
                        (_vap)->iv_misc_evtable[i]->wlan_recv_bstm) {                        \
            (* (_vap)->iv_misc_evtable[i]->wlan_recv_bstm)                                   \
             ((_vap)->iv_misc_arg[i],_val,_len);                                             \
        }                                                                                    \
    }                                                                                        \
...
/*
 * mlme_vdev_sm_deliver_csa_complete() - Deliver CSA complete event to VDEV SM
...
/* wlan_tmpl_update_lower_band_vdevs: When 6ghz AP is co-located
 * and comes up after a lower band AP is up, post event to all
 * lower band AP to do a beacon update with RNR info of 6ghz AP
 */
