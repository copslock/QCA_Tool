...
static int ieee80211_switch_to_intercac_chan(wlan_if_t vap, uint16_t *freq)
{
    ...
    if (vap->iv_des_mode == IEEE80211_MODE_11AXA_HE80 ||
        vap->iv_des_mode == IEEE80211_MODE_11AXA_HE160) {
        is_11ax_80_160 = true;
...
int ieee80211_ucfg_set_freq_internal(wlan_if_t vap, uint16_t freq)
{
    ...
    else if (vap->iv_opmode == IEEE80211_M_HOSTAP || vap->iv_opmode == IEEE80211_M_MONITOR)
    {
        ...
        if (freq != (uint16_t)IEEE80211_FREQ_ANY) {
            ...
            {
                qdf_err("radar detected on channel freq .%d",channel->ic_freq);
                return -EINVAL;
            }
...
int ieee80211_ucfg_set_chanswitch(wlan_if_t vaphandle, uint16_t chan_freq, u_int8_t tbtt, u_int16_t ch_width)
{
    ...
    if ((ch_width == 0) &&(ieee80211_find_channel(ic, chan_freq, 0, ic->ic_curchan->ic_flags) == NULL)) {
...
int ieee80211_ucfg_set_txpow(wlan_if_t vaphandle, int txpow)
{
    ...
    if (txpow > ic->ic_curchan->ic_maxregpower) {
        qdf_err("Exceeding Max Tx power of %d", ic->ic_curchan->ic_maxregpower);
        return -EINVAL;
    }
    ...
    if (txpow > 0) {
        ...
        if(ic->ic_set_txPowerLimit)
           ic->ic_set_txPowerLimit(ic, ic->ic_curchan->ic_maxregpower,
                                   ic->ic_curchan->ic_maxregpower, is2GHz);
...
static int ieee80211_ucfg_set_bandwidth( wlan_if_t vap, int bandwidth)
{
      ...
      if ((phymode == IEEE80211_MODE_11NG_HT40) ||
          (phymode == IEEE80211_MODE_11NA_HT40) ||
          (phymode == IEEE80211_MODE_11AC_VHT40) ||
          (phymode == IEEE80211_MODE_11AXA_HE40) ||
          (phymode == IEEE80211_MODE_11AXG_HE40)) {
...
u_int32_t ieee80211_ucfg_get_quality(wlan_if_t vap, void *iq)
{
    ...
    set_quality(iq, rssi_info.avg_rssi, nf_val);
    return 0;
}
...
int ieee80211_ucfg_set_wirelessmode(wlan_if_t vap, int mode)
{
    ...
    if (is_g40_mode(mode)) {
        if (!ic->ic_user_coext_disable) {
            ...
            TAILQ_FOREACH(tmpvap, &ic->ic_vaps, iv_next) {
                if((vap != tmpvap) && is_g40_mode(tmpvap->iv_des_mode)) {
...
ieee80211_ucfg_hmmc(wlan_if_t vap, struct ieee80211_wlanconfig *config)
{
	...
	switch (config->cmdtype) {
		case IEEE80211_WLANCONFIG_HMMC_ADD:
			ret = dp_add_hmmc(soc, pdev_id, hmmc->ip, hmmc->mask);
			break;
		case IEEE80211_WLANCONFIG_HMMC_DEL:
			ret = dp_del_hmmc(soc, pdev_id, hmmc->ip, hmmc->mask);
			break;
...
ieee80211_ucfg_setmlme(struct ieee80211com *ic, void *osif, struct ieee80211req_mlme *mlme)
{
    ...
    switch (mlme->im_op) {
        ...
        case IEEE80211_MLME_CLEAR_STATS:
            ...
            if (vap->iv_opmode != IEEE80211_M_HOSTAP)
                return -EINVAL;
            ni = ieee80211_find_node(&ic->ic_sta, mlme->im_macaddr, WLAN_MLME_SB_ID);
            if (ni == NULL)
                return -ENOENT;
