                                    ...
                                    uint32_t chan_freq, uint16_t dwell_time)
{
    ...
    ni = ieee80211_try_ref_node(vap->iv_bss, WLAN_OFFCHAN_TX_ID);
    if (ni) {
...
int ucfg_offchan_tx_test(struct wlan_objmgr_vdev *vdev, void *netdev,
        u_int32_t freq, u_int16_t dwell_time)
{
    struct ieee80211vap *vap;
    struct ieee80211com *ic;
    ...
    {
        qdf_err("Radar detected on channel %d freq %d.So, no Tx is allowed till the nol expiry",channel->ic_ieee,channel->ic_freq);
        return -EINVAL;
    }
...
int wlan_offchan_rx_test_send_req(struct wlan_objmgr_vdev *vdev, uint32_t chan_freq,
                                  uint16_t dwell_time, uint32_t freq,
...
int ucfg_offchan_rx_test(struct wlan_objmgr_vdev *vdev, u_int32_t freq, u_int16_t dwell_time,
                         u_int8_t bw_mode, u_int8_t sec_chan_offset)
{
    ...
    return wlan_offchan_rx_test_send_req(vdev, channel->ic_freq, dwell_time, freq, bw_mode, sec_chan_offset);
}
