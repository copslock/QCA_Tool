...
void ieee80211_dump_node_ref(struct node_trace *ntb)
{
    ...
    for(idx = index + 1; idx != index; idx = ((idx+1)&(NUM_TRACE_BUF-1)))
    {
        if((idx < sizeof(ntb->inc.func) / sizeof(ntb->inc.func[0])) && ntb->inc.func[idx])
            qdf_nofl_info("cnt %d func %s line %d\n",
                    ntb->inc.count[idx],
                    ntb->inc.func[idx],
                    ntb->inc.line[idx]);
    }
                                      ...
                                      wlan_objmgr_ref_dbgid id)
{
    ...
    if (ic->ic_get_tgt_type(ic) != TARGET_TYPE_QCA8074) {
        return _ieee80211_find_logically_deleted_node(&ic->ic_sta,
                                                      macaddr,
                                                      NULL,
                                                      id);
    }
...
struct ieee80211_node *
#if WLAN_OBJMGR_REF_ID_TRACE
_ieee80211_find_logically_deleted_node_debug(struct ieee80211_node_table *nt,
...
#if WLAN_OBJMGR_REF_ID_TRACE
    logical_del_peer_list = wlan_objmgr_populate_logically_deleted_node_list_debug(nt,
...
{
    return wlan_objmgr_find_node_debug(nt,(uint8_t *)macaddr, id, func, line);
}
#else
_ieee80211_find_node(struct ieee80211_node_table *nt, const u_int8_t *macaddr,
        wlan_objmgr_ref_dbgid id)
{
    return wlan_objmgr_find_node(nt,(uint8_t *)macaddr, id);
}
#endif
...
ieee80211_find_node_debug(struct ieee80211_node_table *nt, const u_int8_t *macaddr,
                          wlan_objmgr_ref_dbgid id, const char *func, int line, const char *file)
#else
ieee80211_find_node(struct ieee80211_node_table *nt, const u_int8_t *macaddr, wlan_objmgr_ref_dbgid id)
#endif
{
    struct ieee80211_node *ni;
    rwlock_state_t lock_state;
...
#if WLAN_OBJMGR_REF_ID_TRACE
    ni = _ieee80211_find_node_debug(nt, macaddr, id, func, line, file);
#else
    ni = _ieee80211_find_node(nt, macaddr, id);
#endif
...
#endif
{
    ...
    if(wlan_pdev_nif_feat_cap_get(wlan_vdev_get_pdev(vdev), WLAN_PDEV_F_WRAP_EN) &&
       (wlan_vdev_mlme_get_opmode(vdev) == QDF_STA_MODE)) {
        ...
        ni = _ieee80211_find_node(nt, (uint8_t *)ta, id);
    }
    wlan_objmgr_vdev_release_ref(vdev, id);
...
#endif
{
    ...
    OS_BEACON_READ_LOCK(&nt->nt_nodelock, &lock_state, flags);
#if WLAN_OBJMGR_REF_ID_TRACE
    ni = _wrap_find_rxnode_debug(nt, vap->iv_myaddr, macaddr, id, func, line, file);
#else
...
#define	IS_BAR(wh) \
    ...
    if (IS_CTL(wh) && !IS_PSPOLL(wh) && !IS_BAR(wh))
        return _ieee80211_find_node(nt, wh->i_addr1, id);

#if ATH_SUPPORT_WRAP
#if WLAN_OBJMGR_REF_ID_TRACE
    return _wrap_find_rxnode_debug(nt, wh->i_addr1, wh->i_addr2, id, func, line, file);
#else
    return _wrap_find_rxnode(nt, wh->i_addr1, wh->i_addr2, id);
#endif
#else
    return _ieee80211_find_node(nt, wh->i_addr2, id);
#endif
#undef IS_BAR
#undef IS_PSPOLL
#undef IS_CTL
...
ieee80211_node_latevdetach(struct ieee80211vap *vap)
{
    ...
    if (!ieee80211_vap_deleted_is_set(vap)) {
        qdf_nofl_info("vap is not deleted by user, vap: 0x%pK, vap->iv_bss: 0x%pK\n",
                      vap, vap->iv_bss);
    }
