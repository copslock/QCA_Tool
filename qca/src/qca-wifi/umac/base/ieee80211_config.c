...
wlan_set_param(wlan_if_t vaphandle, ieee80211_param param, u_int32_t val)
{
    ...
    case IEEE80211_IDLE_TIME:
        ...
        if (ic->ic_vap_pause_set_param)
            ic->ic_vap_pause_set_param(vap, param, val);
#endif
        break;
    ...
    case IEEE80211_CHWIDTH:
        ...
        if(val > ic->ic_cwm_get_width(ic))
        {
            qdf_nofl_info("vap chwidth should be less than or equal to radio channel width");
            return -EINVAL;
        }
        vap->iv_chwidth = val;
        update_beacon = true;
         ...
         if (val == 0) {
             ...
             if (ieee80211_ic_off_channel_support_is_set(ic)) {
                 ...
                 ieee80211_vap_off_channel_support_set(vap);
                  ...
                  qdf_nofl_info("%s: Cannot enable off-channel support IC support=0\n",
                                  __func__);
             }
         }
         break;
...
#undef MAX_TRAFFIC_INTERVAL
    ...
    case IEEE80211_CONFIG_6GHZ_BCAST_PROB_RSP:
        ...
        if(val) {
            ...
            if(val && ic->ic_prb_rsp_tmpl_send) {
...
wlan_get_param(wlan_if_t vaphandle, ieee80211_param param)
{
    ...
    case IEEE80211_IDLE_TIME:
        ...
        if (ic->ic_vap_pause_get_param)
             val = ic->ic_vap_pause_get_param(vap, param);
#endif
        break;
    case IEEE80211_FEATURE_COUNTER_MEASURES:
        ...
        case IEEE80211_FRAME_INJECTOR_TYPE_CTS_TO_SELF:
            if (enable > 1) {
                ...
                if (ic && ic->ic_frame_injector_config) {
                    ret_val = ic->ic_frame_injector_config(vap, frametype,
                                                enable, inject_period, dstmac);
                }
            }
        break;
...
int wlan_set_debug_flags(wlan_if_t vaphandle, u_int64_t val)
{
    ...
    {
        qdf_nofl_info("Setting dbgLVL mask. Note: dbgLVL mac filtering is currently enabled. Use dbgLVLmac_set <set/unset> <mac> <enable/disable> to disable it\n");
    }
#endif
...
#define IS_NEW_CHANNEL(c)		\
    ...
    uint8_t reported_6Ghz[IEEE80211_CHAN_BYTES];
    int i, j;
    ...
    *nchan = 0;
    ...
    memset(reported_6Ghz, 0, sizeof(reported_6Ghz));
    ...
    for (i = 0; i < ic->ic_nchans; i++)
    {
            ...
            IEEE80211_IS_CHAN_QUARTER(c))
        {
            if (IEEE80211_IS_CHAN_6GHZ(c))
                setbit(reported_6Ghz, c->ic_ieee);
            ...
            if(flag == 0) {
                ...
                chan[*nchan].ic_vhtop_freq_seg1 = c->ic_vhtop_freq_seg1;
                chan[*nchan].ic_vhtop_freq_seg2 = c->ic_vhtop_freq_seg2;
                chan[*nchan].ic_vhtop_ch_num_seg1 = c->ic_vhtop_ch_num_seg1;
                chan[*nchan].ic_vhtop_ch_num_seg2 = c->ic_vhtop_ch_num_seg2;
            }
            }
                ...
                if (chan[j].ic_freq == c->ic_freq) {
                    ...
                    if(flag == 0) {
                        if(IEEE80211_IS_CHAN_11AC_VHT80(c) ||
                                ...
                                IEEE80211_IS_CHAN_11AXA_HE80(c) ||
                                IEEE80211_IS_CHAN_11AXA_HE80_80(c)) {
                        chan[j].ic_vhtop_freq_seg1 = c->ic_vhtop_freq_seg1;
                        chan[j].ic_vhtop_freq_seg2 = c->ic_vhtop_freq_seg2;
                        chan[j].ic_vhtop_ch_num_seg1 = c->ic_vhtop_ch_num_seg1;
                        chan[j].ic_vhtop_ch_num_seg2 = c->ic_vhtop_ch_num_seg2;
                    }
                    }
                    if(flag == 1) {
                        if(IEEE80211_IS_CHAN_11AC_VHT160(c) ||
           ...
           char *macaddr, int mac_listsize, uint8_t nac_type)
{
    ...
    if (ic->ic_hw_nac_monitor_support && nac_type == IEEE80211_NAC_MACTYPE_CLIENT) {
        ni = ieee80211_find_node(&ic->ic_sta, (uint8_t *)macaddr, WLAN_MLME_SB_ID);
        if (ni) {
