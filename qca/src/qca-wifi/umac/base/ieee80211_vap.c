...
int
ieee80211_dynamic_vap_setup(struct ieee80211com *ic, struct ieee80211vap *vap,
                            struct vdev_mlme_obj *vdev_mlme)
{
    int status = 0;
    uint8_t *bssid;

    wlan_vdev_obj_lock(vdev_mlme->vdev);
    bssid = wlan_vdev_mlme_get_macaddr(vdev_mlme->vdev);
    wlan_vdev_obj_unlock(vdev_mlme->vdev);

    status = ieee80211_vap_setup(ic, vap, vdev_mlme, vap->iv_opmode,
                                vap->iv_scan_priority_base,
                                vap->iv_create_flags,
                                bssid);
...
qdf_export_symbol(ieee80211_dynamic_vap_setup);
...
#define IEEE80211_C_OPMODE                                              \
...
#endif
#if ATH_SUPPORT_WRAP
    struct ieee80211vap *mpsta_vap = ic->ic_mpsta_vap;
#endif
    ...
    /* vap_doth is disabled by default for 2.4GHZ band. */
    if (ic->ic_pdev_is_2ghz_supported(ic))
        ieee80211_vap_doth_clear(vap);

    /* Initialize iv_user_disabled_vap_doth as 0 */
...
#if ATH_SUPPORT_UORA
    vap->iv_ocw_range = IEEE_EOCW_MIN_DEFAULT | (IEEE_EOCW_MAX_DEFAULT<<3);
#endif
    ...
    vap->iv_des_ibss_chan_freq     = wlan_reg_chan_band_to_freq(ic->ic_pdev_obj, ic->ic_reg_parm.defaultIbssChannel, (BIT(REG_BAND_2G) | BIT(REG_BAND_5G)));
#endif
    ...
    vap->iv_assoc_min_rssi_thrshld = IEEE80211_ASSOC_MIN_RSSI_DEFAULT;
    ...
    /* Set LDPC to default for the time being */
    vap->iv_he_ar_ldpc = IEEE80211_HE_AR_LDPC_DEFAULT;
    ...
    /* By default support for HE MCS12/13 should be enabled for pine*/
    if(ic->ic_get_tgt_type(ic) == TARGET_TYPE_QCN9000) {
        vap->iv_he_mcs12_13_supp = ENABLE_HE_MCS_12_13_SUPP;
    } else {
        vap->iv_he_mcs12_13_supp = !ENABLE_HE_MCS_12_13_SUPP;
    }

    if(vap->iv_he_mcs12_13_supp == ENABLE_HE_MCS_12_13_SUPP) {
        vap->iv_he_max_mcs_supp = IEEE80211_HE_MCS_12_13_IDX_MAX;
    } else {
        vap->iv_he_max_mcs_supp = IEEE80211_HE_MCS_IDX_MAX;
    }
     ...
     */
     if (ic->ic_he_target) {
...
#if SUPPORT_11AX_D3
        vap->iv_he_max_frag_msdu  =
                HECAP_MAC_MAXFRAGMSDUEXP_GET_FROM_IC(ic_hecap_mac);
        vap->iv_he_multi_tid_aggr =
                HECAP_MAC_MTIDRXSUP_GET_FROM_IC(ic_hecap_mac);
        vap->iv_he_multi_tid_aggr_tx =
                HECAP_MAC_MTIDTXSUP_GET_FROM_IC(ic_hecap_mac);
        vap->iv_he_max_ampdu_len_exp  =
                HECAP_MAC_MAXAMPDULEN_EXPEXT_GET_FROM_IC(ic_hecap_mac);
#endif
        vap->iv_he_min_frag_size  =
                HECAP_MAC_MINFRAGSZ_GET_FROM_IC(ic_hecap_mac);
        ...
        vap->iv_he_omi            =
                HECAP_MAC_OMI_GET_FROM_IC(ic_hecap_mac);
        ...
        vap->iv_he_su_bfee        = HECAP_PHY_SUBFME_GET_FROM_IC(ic_hecap_phy);
        vap->iv_he_su_bfer        = HECAP_PHY_SUBFMR_GET_FROM_IC(ic_hecap_phy);
        ...
        vap->iv_he_su_ppdu_1x_ltf_800ns_gi =
                HECAP_PHY_SU_1XLTFAND800NSECSGI_GET_FROM_IC(ic_hecap_phy);
        vap->iv_he_su_mu_ppdu_4x_ltf_800ns_gi =
                HECAP_PHY_4XLTFAND800NSECSGI_GET_FROM_IC(ic_hecap_phy);
        vap->iv_he_ndp_4x_ltf_3200ns_gi =
                HECAP_PHY_LTFGIFORNDP_GET_FROM_IC(ic_hecap_phy);
        vap->iv_he_er_su_ppdu_1x_ltf_800ns_gi =
                HECAP_PHY_ERSU1XLTF800NSGI_GET_FROM_IC(ic_hecap_phy);
        vap->iv_he_er_su_ppdu_4x_ltf_800ns_gi =
        ...
        if(vap->iv_opmode == IEEE80211_M_HOSTAP) {
            ...
            if((ic->ic_get_tgt_type(ic) == TARGET_TYPE_QCA8074V2) ||
               (ic->ic_get_tgt_type(ic) == TARGET_TYPE_QCA6018) ||
               (ic->ic_get_tgt_type(ic) == TARGET_TYPE_QCN9000)) {
                vap->iv_he_ul_muofdma     = ENABLE_HE_UL_MUOFDMA;
                vap->iv_he_muedca         = ENABLE_HE_MUEDCA;
                vap->iv_muedcastate.mu_edca_dynamic_state |=
                    MUEDCA_DYNAMIC_ALGO_ENABLE_STATE_MASK;
        ...
        if ((opmode == IEEE80211_M_STA) && (ic->ic_re_ul_resp)) {
            /* Enable HE-DL-OFDMA & HE-UL-OFDMA and HE-MUMIMO for STA vap
               if re_ul_resp flag is set*/
    ...
    if (vap->iv_opmode == IEEE80211_M_IBSS &&
        ...
        qdf_nofl_info("Disabling aponly path for ad-hoc mode\n");
        vap->iv_aponly = false;
        ic->ic_aponly = false;
    }
#else
    ...
    if ((vap->iv_opmode == IEEE80211_M_HOSTAP) && ic->ic_curchan) {
        if (IEEE80211_IS_CHAN_5GHZ_6GHZ(ic->ic_curchan))
            vdev_mlme->mgmt.rate_info.bcn_tx_rate = DEFAULT_LOWEST_RATE_5G;
            ...
            if (vdev_mlme->vdev->vdev_objmgr.c_flags & IEEE80211_LP_IOT_VAP)
                vdev_mlme->mgmt.rate_info.bcn_tx_rate =
                                                     2 * DEFAULT_LOWEST_RATE_2G;
            else
                vdev_mlme->mgmt.rate_info.bcn_tx_rate = DEFAULT_LOWEST_RATE_2G;
        }
    }
...
qdf_export_symbol(ieee80211_is_vap_state_running);
                ...
                struct vdev_mlme_obj  *vdev_mlme)
{
    ...
    qdf_info("devhandle=0x%pK, opmode=%s, flags=0x%x\n",
    ...
    if (!ic ||((opmode == IEEE80211_M_HOSTAP) &&
        ...
        qdf_nofl_info("%s: cant create more than 1 lp_iot mode vap object \n", __func__);
        return NULL;
    }

    /* Legacy MLME init */
    vap = mlme_ext_vap_create(ic, vdev_mlme, opmode, scan_priority_base, flags,
		              bssid, mataddr);
    if (vap == NULL) {
...
void ic_reset_params(struct ieee80211com *ic)
{
    ...
    if (ic->ic_cbs) {
        ...
        ieee80211_cbs_deinit(&(ic->ic_cbs));
        ieee80211_cbs_init(&(ic->ic_cbs), ic, ic->ic_osdev);
    }
    ...
    ic->ic_num_clients = ic->ic_get_num_clients(pdev);
...
void global_ic_list_reset_params(struct ieee80211com *ic) {
    ...
    ic->ic_global_list->same_ssid_disable = 0;
...
qdf_export_symbol(ieee80211_dfs_reset);
...
qdf_export_symbol(ieee80211_dfs_nol_reset);
...
qdf_export_symbol(wlan_is_connected);
...
int wlan_vap_prep_wow_offload_info(wlan_if_t vaphandle)
{
    ...
    wo_info.flags = 0;
    OS_MEMCPY(wo_info.myaddr, vaphandle->iv_myaddr, QDF_MAC_ADDR_SIZE);
    OS_MEMCPY(wo_info.bssid, iv_bss->ni_macaddr, QDF_MAC_ADDR_SIZE);
    if (iv_bss->ni_flags & IEEE80211_NODE_QOS) {
        /* Use TID 0 by default */
        tid_num = 0;
...
void wlan_restore_vap_params(wlan_if_t vap)
{
    ...
    if (IEEE80211_M_HOSTAP == vap->iv_opmode) {
         ...
         * Set host mgmt default rates */
        if (!ieee80211_rate_is_valid_basic(vap, mgmt_rate)) {
            if (IEEE80211_IS_CHAN_5GHZ_6GHZ(chan)){
                mgmt_rate = DEFAULT_LOWEST_RATE_5G;
            } else if(IEEE80211_IS_CHAN_2GHZ(chan)){
                mgmt_rate = DEFAULT_LOWEST_RATE_2G;
            }
        }
        ...
        /* Configure beacon rate to managemt rate if bcn_rate is 0 */
        if (!bcn_rate) {
            vap->vdev_mlme->mgmt.rate_info.bcn_tx_rate = mgmt_rate;
        }
    }
}
       ...
       if (tmp_ic) {
          ...
          if (max_radio_priority > tmp_ic->ic_radio_priority) {
              if (tmp_ic->ic_sta_vap &&
                  (wlan_vdev_is_up((tmp_ic->ic_sta_vap)->vdev_obj) ==
                                       QDF_STATUS_SUCCESS)) {
