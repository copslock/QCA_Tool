/*
 *
 * Copyright (c) 2011,2017-2018,2020 Qualcomm Innovation Center, Inc.
 * All Rights Reserved
...
uint8_t get_nss_frm_mcsnssmap(uint16_t map)
{
    ...
    for (i = 0; i < MAX_VHT_STREAMS; i++) {
        ...
        if (n < 3) {
        /*
        This is to get nss based on map,
        for some config e.g. 11,11,01,11, original code will get only nss = 1,
        which's wrong, nss should be 2 in this case.
        */
        num_streams = i + 1;
...
void
ieee80211_get_nss_from_hecap_mcsnssset_6g(struct ieee80211_node *ni)
{
    struct ieee80211vap *vap = ni->ni_vap;
    uint16_t rxmcsnssmap[HEHANDLE_CAP_TXRX_MCS_NSS_SIZE];
    uint16_t txmcsnssmap[HEHANDLE_CAP_TXRX_MCS_NSS_SIZE];
    uint16_t temp_peer_txmap;
    uint16_t temp_peer_rxmap;
    uint16_t temp_self_txmap;
    uint16_t temp_self_rxmap;
    uint16_t tx_map;
    uint16_t rx_map;
    ...
    switch (ni->ni_phymode) {
        case IEEE80211_MODE_11AXA_HE80_80:
            temp_peer_txmap = ni_he->hecap_txmcsnssmap[HECAP_TXRX_MCS_NSS_IDX_80_80];
            temp_self_txmap = txmcsnssmap[HECAP_TXRX_MCS_NSS_IDX_80_80];
            temp_peer_rxmap = ni_he->hecap_rxmcsnssmap[HECAP_TXRX_MCS_NSS_IDX_80_80];
            temp_self_rxmap = rxmcsnssmap[HECAP_TXRX_MCS_NSS_IDX_80_80];
            break;
        case IEEE80211_MODE_11AXA_HE160:
            temp_peer_txmap = ni_he->hecap_txmcsnssmap[HECAP_TXRX_MCS_NSS_IDX_160];
            temp_self_txmap = txmcsnssmap[HECAP_TXRX_MCS_NSS_IDX_160];
            temp_peer_rxmap = ni_he->hecap_rxmcsnssmap[HECAP_TXRX_MCS_NSS_IDX_160];
            temp_self_rxmap = rxmcsnssmap[HECAP_TXRX_MCS_NSS_IDX_160];
            break;
        default:
            temp_peer_txmap = ni_he->hecap_txmcsnssmap[HECAP_TXRX_MCS_NSS_IDX_80];
            temp_self_txmap = txmcsnssmap[HECAP_TXRX_MCS_NSS_IDX_80];
            temp_peer_rxmap = ni_he->hecap_rxmcsnssmap[HECAP_TXRX_MCS_NSS_IDX_80];
            temp_self_rxmap = rxmcsnssmap[HECAP_TXRX_MCS_NSS_IDX_80];
            break;
    }
    tx_map = INTERSECT_11AX_MCSNSS_MAP(temp_self_txmap, temp_peer_txmap);
    ni->ni_txstreams = get_nss_frm_mcsnssmap(tx_map);
    ni->ni_streams = QDF_MAX(ni->ni_streams, ni->ni_txstreams);

    rx_map = INTERSECT_11AX_MCSNSS_MAP(temp_self_rxmap, temp_peer_rxmap);
    ni->ni_rxstreams = get_nss_frm_mcsnssmap(rx_map);
}
...
#define	RV(v)	((v) & IEEE80211_RATE_VAL)
    ...
    int i,j;
    u_int8_t tx_streams = ieee80211_get_txstreams(ic, vap);
    struct ieee80211_node *ni = ieee80211_try_ref_bss_node(vap, WLAN_MISC_ID);
    if (NULL == ni)
        return 1;
...
#define IEEE80211_NA_BASIC_RATE	6000
    ...
    if (vap->iv_mcast_fixedrate != 0) {
        ...
        if (IEEE80211_IS_CHAN_5GHZ_6GHZ(vap->iv_bsschan))
                           ...
                           u_int32_t *nrates)
{
    ...
    ni = ieee80211_try_ref_bss_node(vap, WLAN_MISC_ID);
    if (NULL == ni)
        return -EINVAL;
    rs = &ni->ni_rates;
    if (rs->rs_nrates > len){
        ieee80211_free_node(ni, WLAN_MISC_ID);
        return -EINVAL;
    }
 ...
 * Chipsets which allow max NSS of 160/80+80 MHz to be only half of the max NSS
 * of 80 MHz,
 * maxNSS_160MHz = maxNSS_le80MHz/2 (with rounding down)
 *
 * Chipsets which allow max NSS for 160/80+80 MHz to be the same as max NSS for
 * 80 MHz,
 * maxNSS_160MHz = maxNSS_le80MHz
 *
...
ieee80211_compute_nss(struct ieee80211com *ic, u_int8_t chainmask, struct ieee80211_bwnss_map *nssmap)
{
    ...
    if (wlan_psoc_nif_fw_ext_cap_get(psoc, WLAN_SOC_NSS_RATIO_TO_HOST_SUPPORT) &&
        ic->ic_tgt_defined_nss_ratio_enabled) {
        switch (ic->ic_tgt_defined_nss_ratio) {
        case WLAN_NSS_RATIO_1BY2_NSS:
            nssmap->bw_nss_160 = nss_80 >> 1;
            break;
        case WLAN_NSS_RATIO_3BY4_NSS:
            qdf_err("WLAN_NSS_RATIO_3BY4_NSS ratio is not currently supported. Asserting.");
            qdf_assert_always(0);
            break;
        case WLAN_NSS_RATIO_1_NSS:
            nssmap->bw_nss_160 = nss_80;
            break;
        case WLAN_NSS_RATIO_2_NSS:
            qdf_err("WLAN_NSS_RATIO_2_NSS ratio is not currently supported. Asserting.");
            qdf_assert_always(0);
            break;
        default:
            qdf_err("Invalid NSS ratio = %u is received from firmware. Asserting.", ic->ic_tgt_defined_nss_ratio);
            qdf_assert_always(0);
        }
    } else {
        nssmap->bw_nss_160 = nss_80 >> 1;
