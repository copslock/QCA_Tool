 ...
 * @param vdev_mlme          : pointer to mlme object
 *
 ...
 */
int wlan_set_channel(wlan_if_t vaphandle, uint16_t freq, uint16_t freq2);
...
/**
 * @returns the Desired phy mode of AP.
 * @param      des_chan: Pointer to ieee80211_ath_channel
 * @return
 ...
 * @param      vaphandle   : vap handle
 * @param      chan_freq   : the channel frequency to set
 * @return on success it returns 0.
 */
int wlan_set_desired_ibsschan(wlan_if_t vaphandle, uint16_t chan_freq);
...
/**
 * get supported chan ie from a node.
 * @param   vaphandle   : handle to a vap
 * @param   macaddr     : mac address of the node.
 * @param   ie          : ie to return.
 * @param   len         : length of the array passed in.
 *                        len is in/out param.
 *
 * @return
 *       0 on success and -ve value on failure.
...
/**
 * get operation class ie from a node.
 * @param   vaphandle   : handle to a vap
 * @param   macaddr     : mac address of the node.
 * @param   ie          : ie to return.
 * @param   len         : length of the array passed in.
 ...
 * @param vaphandle         : handle to a vap
 * @param frametype         : type of injector frame to enable
 * @param enable            : enable/disable injection
 * @param inject_period     : Periodicity of frame injection
 * @param dstmac            : Destination address of frame
 * @return 0  on success and -ve on failure.
 */
...
#define GET_PN_ENABLE 1
#define GET_PN_DISABLE 0

/**
 * get encryption key.
 * @param   vaphandle   : handle to a vap
 * @param   wpa key.
 * Do not set get_pn_flag as 1 in interrupt context
 */
int wlan_get_key(wlan_if_t vaphandle, u_int16_t keyix, u_int8_t *macaddr, ieee80211_keyval *key, u_int16_t keybuf_len, u_int8_t get_pn_flag);
 ...
 */
int wlan_acs_block_channel_list(wlan_if_t vap,u_int16_t *chan,u_int8_t nchan);
...
/**
 * send custom neighbor report response
 * @param vap       : handle to the vap
 * @param macaddr   : macaddress of the station
 * @param neiginfo  : pointer to custom neigh reponse info struct
 */
int wlan_send_neighbor_resp(wlan_if_t vap, u_int8_t *macaddr, ieee80211_user_nrresp_info_t *nrrpt_user);
...
/**
 * @brief update user channel list for use in acs scan report
 *
 * @param vaphandle
 * @param append: append to existing list or replace
 * @param chan_list: list of channels to set
 *
 * @return 0 if setting is succefull otherwise error code
 */
int wlan_acs_set_user_chanlist(wlan_if_t vaphandle, bool append, qdf_freq_t *chan_list);

/**
 * @brief get user channel list for acs scan report
 *
 * @param vaphandle
...
/**
 * flush mac_filter list for monitor vap.
 *
 * @param devhandle       : handle to the vap.
 * @return void.
 */
...
/*
 * MLME Get if the connection SM is running
 *
 * @param  vaphandle    : handle to the vap.
 * @return              : True if the connection SM is running
 */
...
/*
 * wlan_mlme_restart_bss() - Initiate restart for the given vdev
 * @vdev: Object manager vdev param
 *
 * Return: QDF_STATUS_SUCCESS if event dispatched to vdev state machine
 */
QDF_STATUS wlan_mlme_restart_bss(struct wlan_objmgr_vdev *vdev);

/*
 * ACS_SET_CHAN_LIST_APPEND : whether append new chan list to existing chan
 * list or replace existing chan list with new chan list.
 */
