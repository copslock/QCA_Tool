...
typedef struct ieee80211_chanlist_r {
    u_int16_t n_chan; /* Number of IEEE802.11 Channels*/
    struct ieee80211_chan_def chans[CHAN_SCAN_MAX];
...
typedef struct ieee80211_freqlist_r {
    u_int16_t n_freqs; /* Number of IEEE802.11 frequencies*/

    /* Should actually use qdf_freq_t from qdf_types.h instead of
     * u_int16_t/u_int32_t for the array variable 'freqs'. However this
     * header file(ext_ioctl_drv_if.h) is also included from userspace
     * applications (e.g. thermaltool.c); and userpace applications
     * cannot include qdf_types.h. Therefore, have the following
     * #if #else.
     */
#ifdef CONFIG_16_BIT_FREQ_TYPE
    u_int16_t freqs[CHAN_SCAN_MAX];   /* List of channel frequencies*/
#else
    u_int32_t freqs[CHAN_SCAN_MAX];   /* List of channel frequencies*/
#endif
    u_int8_t chan_width[CHAN_SCAN_MAX];/* chanel width used for scan */
    u_int8_t sec_chan_offset[CHAN_SCAN_MAX]; /* Secondary channel offset*/
} ieee80211_freqlist_t;
