...
#define ATH_GET_VAP_ID_ALTER(bssid, hwbssid, id)                           \
    do {                                                                   \
          ...
          if (IEEE80211_ADDR_EQ(bssid, hwbssid))                           \
              id = 0;                                                      \
          else                                                             \
    ...
    do {                                                 \
        if (ic->ic_is_macreq_enabled(ic)) { \
            ...
            if (IEEE80211_ADDR_EQ(bssid, hwbssid))           \
                ...
                id = ( temp_bssid  - temp_hw_bssid ) & ( ATH_VAP_ID_MASK >> ( OCTET - width ) ); \
        }           \
    } while(0)
...
extern uint8_t used_mac_addr_byte[3];
#define MBSSID_MAC_COLLISION(random_byte_for_curr_pdev, random_byte_for_pdev_x) \
    (((random_byte_for_curr_pdev & ~(MBSSID_POOL_SIZE(ic) - 1)) ==      \
      (random_byte_for_pdev_x & ~(MBSSID_POOL_SIZE(ic) - 1))) ? true : false)
#define MBSSID_MAC_CALC_PREPROCESS_BLOCK(ic, bssid, hwbssid, mode)      \
    do {                                                                \
        uint32_t random_byte = 0;                                       \
        uint8_t pdevID = ic->ic_get_pdev_idx(ic);                       \
        uint8_t random_byte_for_curr_pdev = 0;                          \
        if(!used_mac_addr_byte[pdevID]) {                               \
            uint8_t random_byte_for_pdev_x =                            \
                        used_mac_addr_byte[(pdevID + 1) % 3];           \
            uint8_t random_byte_for_pdev_y =                            \
                        used_mac_addr_byte[(pdevID + 2) % 3];           \
            OS_GET_RANDOM_BYTES(&random_byte, 1);                       \
            random_byte_for_curr_pdev = (uint8_t)random_byte;           \
            while((random_byte_for_pdev_x &&                            \
                MBSSID_MAC_COLLISION(random_byte_for_curr_pdev,         \
                random_byte_for_pdev_x)) || (random_byte_for_pdev_y     \
                && MBSSID_MAC_COLLISION(random_byte_for_curr_pdev,      \
                random_byte_for_pdev_y))) {                             \
                    OS_GET_RANDOM_BYTES(&random_byte, 1);               \
                    random_byte_for_curr_pdev = (uint8_t)random_byte;   \
            }                                                           \
        } else {                                                        \
            random_byte_for_curr_pdev = used_mac_addr_byte[pdevID];     \
        }                                                               \
        if (mode != QDF_SAP_MODE) {                                     \
             ...
             */                                                         \
            hwbssid[ATH_BSSID_INDEX] =                                  \
                (random_byte_for_curr_pdev & ~(MBSSID_POOL_SIZE(ic) - 1)); \
            hwbssid[ATH_BSSID_INDEX] += MBSSID_POOL_SIZE(ic);           \
        } else {                                                        \
            if (!MBSSID_IS_TX_VAP(ic)) {                                \
                bssid[ATH_BSSID_INDEX] = used_mac_addr_byte[pdevID];    \
                bssid[ATH_BSSID_INDEX] |= MBSSID_GET_BSSIDX(ic);        \
            } else {                                                    \
                /* considering max_bssid will be <= 8 for the time being */ \
                bssid[ATH_BSSID_INDEX] =                                \
                    (random_byte_for_curr_pdev & ~(MBSSID_POOL_SIZE(ic) - 1)); \
                used_mac_addr_byte[pdevID] = bssid[ATH_BSSID_INDEX];    \
            }                                                           \
        }                                                               \
    } while(0)

#define MBSSID_MAC_CALC_POSTPROCESS_BLOCK(ic, bssid, hwbssid, saved_hwbssid_byte, mode) \
    do {                                                                  \
        /* Restore hwbssid */                                             \
...
#define MBSSID_ATH_SET_VAP_BSSID(bssid, hwbssid, id, mode)                    \
    do {                                                                      \
        ...
        MBSSID_MAC_CALC_POSTPROCESS_BLOCK(ic, bssid, hwbssid, saved_hwbssid_byte, mode); \
    } while(0)
