...
#define MAPV2_CAC_COMPLETION_TIME 62
...
typedef enum {
    MAPV2_CONTINUOUS_CAC,
    MAPV2_CONTINUOUS_WITH_DEDICATED_RADIO,
    MAPV2_MIMO_DIMENSION_REDUCED,
    MAPV2_TIME_SLICED_CAC,
} MAPV2_CAC_MODE;
...
/**
 * Metadata about an association response message that was sent
 * with a failure code
 */
struct bs_assoc_reject_ind {
    u_int8_t client_addr[IEEE80211_ADDR_LEN]; /* Client mac */
...
typedef struct ieee80211_bsteering_hecap_info_t {
...
} ieee80211_bsteering_hecap_info_t;
...
struct bs_node_associated_ind {
    ...
    /* Band capability that tell if the client is combination of 2G, 5G and 6G */
    u_int8_t band_cap : 3;
    ...
    /* BSSID of the client */
    u_int8_t client_bssid[IEEE80211_ADDR_LEN];
    /* HT capabilities */
    u_int16_t client_htcap;
    /* VHT capabilities */
    u_int32_t client_vhtcap;
    /* HE capabilities */
    ieee80211_bsteering_hecap_info_t client_hecap;
    u_int8_t rrm_caps[RRM_CAPS_LEN];
};

/**
 * Metadata about a STA which is disassociating
 */
struct bs_node_disassoc_ind {
    /* BSSID of the BSS that the STA was associated to */
    u_int8_t bssid[IEEE80211_ADDR_LEN];
    /* Number of packets transmitted to STA */
    u_int32_t tx_packets;
    /* Number of bytes transmitted to STA */
    u_int64_t tx_bytes;
    ...
    /* Number of packets received from STA */
    u_int32_t rx_packets;
    /* Number of bytes received from STA */
    u_int64_t rx_bytes;
    /* Number of packets received from STA that were dropped due to errors */
...
struct ieee80211_beaconrpt_wb_chan {
    u_int8_t chan_width;
...
typedef struct ieee80211_bcnrpt_s {
    ...
    u_int8_t wb_channel_present;
    struct ieee80211_beaconrpt_wb_chan wb_chan;
} ieee80211_bcnrpt_t;
...
typedef enum {
    ...
    /* Indication that an association response was sent with a failure
        status code.*/
    ATH_EVENT_BSTEERING_TX_ASSOC_FAIL = 36,
} ATH_BSTEERING_EVENT;
