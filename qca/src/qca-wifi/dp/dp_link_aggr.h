...
#define DP_SOC_MAX_VDEV_CNT 51

#define dp_link_nbuf_debug_del_record(_skb) \
    {\
 ...
 * @ap_vdev: Connected AP vdev handles
...
typedef struct dp_pdev_link_aggr {
    ...
    uint8_t priority;
    bool is_primary;
    void *ap_vdev[DP_SOC_MAX_VDEV_CNT];
    void *sta_vdev;
    bool is_fast_lane;
} dp_pdev_link_aggr_t;

static inline void dp_link_vap_xmit (qdf_nbuf_t nbuf, struct net_device *dev)
{
    dp_link_nbuf_debug_del_record(nbuf);
