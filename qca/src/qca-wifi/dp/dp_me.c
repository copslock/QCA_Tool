...
static int dp_me_igmp_mld_inspect(struct cdp_soc_t *soc, uint8_t pdev_id,
				  ...
				  void *group, u_int8_t *hmmc_found);
...
void dp_pdev_me_setup(ol_txrx_soc_handle soc, uint8_t pdev_id)
{
	...
	pdev_me_hdl->pdev_hmmc_cnt = 3;
	pdev_me_hdl->pdev_hmmcs[0].ip = be32toh(0xeffffffa); /* 239.255.255.250 */
	pdev_me_hdl->pdev_hmmcs[0].mask = 0xffffffff;
	pdev_me_hdl->pdev_hmmcs[1].ip = be32toh(0xe00000fb); /* 224.0.0.251 */
	pdev_me_hdl->pdev_hmmcs[1].mask = 0xffffffff;
	pdev_me_hdl->pdev_hmmcs[2].ip = be32toh(0xe00000fc); /* 224.0.0.252 */
	pdev_me_hdl->pdev_hmmcs[2].mask = 0xffffffff;
}
		       ...
		       uint8_t *peer_addr, uint8_t *macaddr)
{
	...
	{
		qdf_info("DMS encap: Don't have enough headroom");
		return 1;
	}
	...
	if (qdf_nbuf_push_head(msdu, sizeof(*wh) + sizeof(*llcHdr)) == NULL) {
		qdf_info("Encap: Failed to push nwifi and LLC header for DMS");
		return 1;
	}
				   ...
				   uint8_t *macaddr)
{
	...
	stats.mcast_en.mcast_pkt.num++;
	stats.mcast_en.mcast_pkt.bytes += qdf_nbuf_len(wbuf);
				    ...
				    qdf_nbuf_t wbuf)
{
	...
	new_mac_cnt = cdp_vdev_get_peer_mac_list(soc,vdev_id,newmac, MAX_SNOOP_ENTRIES);
			...
			qdf_nbuf_t wbuf)
{
	...
	if (dp_me_igmp_mld_inspect(soc, pdev_id, wbuf, NULL, &group, &hmmc_found))
		return -1;
