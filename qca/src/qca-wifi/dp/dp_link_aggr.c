...
void dp_lag_pdev_set_fast_lane(struct wlan_objmgr_pdev *pdev, bool value)
{
    ...
    pdev_lag->is_fast_lane = value;
}
qdf_export_symbol(dp_lag_pdev_set_fast_lane);
...
void dp_lag_pdev_set_ap_vdev(struct wlan_objmgr_pdev *pdev, uint8_t vdev_id, void *vdev)
{
    dp_pdev_link_aggr_t *pdev_lag;
    pdev_lag = dp_pdev_get_lag_handle(pdev);
    if (!pdev_lag)
    ...
    if (vdev_id >= DP_SOC_MAX_VDEV_CNT)
        return;

    pdev_lag->ap_vdev[vdev_id] = vdev;

    QDF_TRACE_ERROR(QDF_MODULE_ID_DP, "%s pdev(%p) vdev_id %d ap_vdev %p ", __func__, pdev_lag, vdev_id, vdev);
}
        ...
        for (i = 0; i < soc_lag->num_radios; i++) {
            ...
            if (!pdev_lag->is_primary && pdev_lag->sta_vdev) {
                ...
                if (qdf_unlikely(dp_is_extap_enabled(pdev_lag->sta_vdev) || wlan_is_psta(vap)))
                    return;
#endif
                vdev_id = wlan_vdev_get_id(pdev_lag->sta_vdev);
                val.cdp_vdev_param_wds = 1;
                cdp_txrx_set_vdev_param(soc_lag->soc, vdev_id, CDP_ENABLE_WDS, val);
                val.cdp_vdev_param_mec = 1;
                cdp_txrx_set_vdev_param(soc_lag->soc, vdev_id, CDP_ENABLE_MEC, val);
                val.cdp_vdev_param_da_war = 1;
                cdp_txrx_set_vdev_param(soc_lag->soc, vdev_id, CDP_ENABLE_DA_WAR, val);
            }
        }
    }
}
...
static void dp_vap_send_sec_sta(struct dp_pdev_link_aggr *pdev_lag, qdf_nbuf_t nbuf)
{
    ...
    if (soc_lag->ast_override_support) {
        ((ol_txrx_tx_fp)osdev->iv_vap_send)(soc_lag->soc, wlan_vdev_get_id(pdev_lag->sta_vdev), nbuf);
    } else {
        ...
        ((ol_txrx_tx_exc_fp)osdev->iv_vap_send_exc)(soc_lag->soc, wlan_vdev_get_id(pdev_lag->sta_vdev),
                nbuf, &tx_exc_param);
    }
}
...
bool dp_lag_rx_ap(struct dp_pdev_link_aggr *pdev_lag, qdf_nbuf_t nbuf)
{
    ...
    if (!soc_lag->is_multilink || pdev_lag->is_primary
            || (soc_lag->num_stavaps_up < 2) || pdev_lag->is_fast_lane) {
        return false;
    }
    ...
    if (wlan_is_mpsta(vap) || dp_is_extap_enabled(pdev_lag->sta_vdev)) {
        if (ast_entry_found) {
            ...
            if ((ast_type == CDP_TXRX_AST_TYPE_SELF) || (ast_type == CDP_TXRX_AST_TYPE_STATIC)) {
                return false;
            } else if (dev && ast_type == CDP_TXRX_AST_TYPE_STA_BSS) {
                /*
                 * For packets destined to the rootAP where the bridge MAC
                 * is that of the 2G radio's MAC, from the STA connected
                 * to the 2G link of extAP.
                 */
                dp_link_vap_xmit(nbuf, dev);
                return true;
            }
            /*
...
bool dp_lag_rx_sta(struct dp_pdev_link_aggr *pdev_lag, qdf_nbuf_t nbuf, bool vap_send)
{
    ...
    struct cdp_ast_entry_info ast_entry_info = {0};
    int ast_entry_found = 0;
    struct cdp_ast_entry_info other_ast_entry_info = {0};
    int other_ast_entry_found = 0;
    ...
    if ((soc_lag->num_stavaps_up < 2) && !soc_lag->ast_override_support) {
	    is_mcast = IEEE80211_IS_MULTICAST(eh->ether_dhost);
    ...
    if (is_mcast) {
        ...
        if ((pdev_id != pdev_lag->pdev_id) && (ast_type != CDP_TXRX_AST_TYPE_STA_BSS)) {
            QDF_TRACE(QDF_MODULE_ID_TXRX, QDF_TRACE_LEVEL_DEBUG,FL("shost %pM dhost %pM \n"), eh->ether_shost, eh->ether_dhost);
            if (!soc_lag->is_multilink && (ast_type == CDP_TXRX_AST_TYPE_SELF)) {
                ...
                for (i = 0; i < soc_lag->num_radios; i++) {
                    pdev_lag = soc_lag->pdev[i];
                    if (!pdev_lag->is_primary && pdev_lag->sta_vdev) {
                        uint8_t vdev_id = wlan_vdev_get_id(pdev_lag->sta_vdev);
                        cdp_config_param_type val = {0};
    ...
    /*
     * Handle the unicast packets received on the secondary sta vap.
     */
    if (!pdev_lag->is_primary && soc_lag->is_multilink) {
            struct ieee80211vap *vap;
            osif_dev *osdev;
            uint8_t vdev_id;

            ast_entry_found = cdp_peer_get_ast_info_by_pdev(soc_lag->soc, eh->ether_dhost, pdev_lag->pdev_id, &ast_entry_info);
            if (!ast_entry_found) {
                QDF_TRACE(QDF_MODULE_ID_TXRX, QDF_TRACE_LEVEL_DEBUG,FL("shost %pM dhost %pM \n"), eh->ether_shost, eh->ether_dhost);
                return false;
            }
            ...
            /*
             * If AST entries are present on both the radios, give the packet to stack.
             * This might be a schenario where a STATIC entry is present on one radio
             * and a WDS entry on the other.
             */
            for (i = 0; i < soc_lag->num_radios; i++) {
                if (soc_lag->pdev[i] && (i != pdev_lag->pdev_id)) {
                    other_ast_entry_found = cdp_peer_get_ast_info_by_pdev(soc_lag->soc, eh->ether_dhost, i, &other_ast_entry_info);
                    if (other_ast_entry_found) {
                        QDF_TRACE(QDF_MODULE_ID_TXRX, QDF_TRACE_LEVEL_DEBUG,FL("shost %pM dhost %pM \n"), eh->ether_shost, eh->ether_dhost);
                        return false;
                    }
                }
            ...
            vdev_id = ast_entry_info.vdev_id;
            if (!pdev_lag->ap_vdev[vdev_id])
                return false;

            vap = wlan_vdev_get_mlme_ext_obj(
                                 (struct wlan_objmgr_vdev *)pdev_lag->ap_vdev[vdev_id]);
            ...
            QDF_TRACE(QDF_MODULE_ID_TXRX, QDF_TRACE_LEVEL_DEBUG,FL("shost %pM dhost %pM \n"), eh->ether_shost, eh->ether_dhost);
            if (vap_send) {
                ((ol_txrx_tx_fp)osdev->iv_vap_send)(soc_lag->soc, wlan_vdev_get_id(pdev_lag->ap_vdev[vdev_id]), nbuf);
                return true;
            }
    }
...
static bool dp_lag_tx(struct dp_pdev_link_aggr *pdev_lag, qdf_nbuf_t nbuf, bool vap_send)
{
    ...
    if (soc_lag->is_multilink || soc_lag->always_primary ||
        ...
        /*
	 * Do the DBDC Fast Lane processing at the beginning and then fall back to normal
         * DBDC STA TX if either fast-lane is disabled or the TX is for non-fast lane radio.
         */
        if (pdev_lag->is_fast_lane) {
	    ast_entry_found = cdp_peer_get_ast_info_by_pdev(soc_lag->soc, eh->ether_shost, pdev_lag->pdev_id, &ast_entry_info);
	    if (!ast_entry_found || ast_entry_info.type == CDP_TXRX_AST_TYPE_MEC) {
                for (i = 0; i < soc_lag->num_radios; i++) {
                    if (soc_lag->pdev[i] && (i != pdev_lag->pdev_id) && soc_lag->pdev[i]->is_fast_lane) {
                        other_ast_entry_found = cdp_peer_get_ast_info_by_pdev(soc_lag->soc, eh->ether_shost, i, &other_ast_entry_info);
                        if ((other_ast_entry_found) && (other_ast_entry_info.type == CDP_TXRX_AST_TYPE_STATIC ||
				other_ast_entry_info.type == CDP_TXRX_AST_TYPE_WDS)) {
                            QDF_TRACE(QDF_MODULE_ID_TXRX, QDF_TRACE_LEVEL_DEBUG,FL("Fast Lane Sta TX with shost %pM dhost %pM \n"),
                                           eh->ether_shost, eh->ether_dhost);
                            return false;
                        }
                    }
