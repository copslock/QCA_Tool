 ...
 * Return: void
 */
static void pld_ahb_update_status(struct pci_dev *pcidev,
				const struct pci_device_id *id,
				 int status)
{
	struct pld_context *pld_context;
...
skip_cfg:
	...
	case PLD_EPPING:
		...
		break;
        case PLD_FTM_COLDBOOT_CALIBRATION:
		cnss_mode = CNSS_FTM_CALIBRATION;
		qdf_info("cnss_mode: FTM Cold boot calibration");
		break;
	default:
