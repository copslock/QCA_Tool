...
ath_pci_probe(struct pci_dev *pdev, const struct pci_device_id *id)
{
    ...
    if (pci_set_dma_mask(pdev, 0xffffffff)) {
        qdf_nofl_info(KERN_ERR "ath_pci: 32-bit DMA not available\n");
        goto bad;
    }
    ...
    if (!request_mem_region(pci_resource_start(pdev, 0), pci_resource_len(pdev, 0), "ath")) {
        qdf_nofl_info(KERN_ERR "ath_pci: cannot reserve PCI memory region\n");
        goto bad;
    }

    mem = (unsigned long) ioremap(pci_resource_start(pdev, 0), pci_resource_len(pdev, 0));
    if (!mem) {
        qdf_nofl_info(KERN_ERR "ath_pci: cannot remap PCI memory region\n") ;
        goto bad1;
    }
    ...
    if (dev == NULL) {
        qdf_nofl_info(KERN_ERR "ath_pci: no memory for device state\n");
        goto bad2;
    }
...

    {
        ...
        qdf_nofl_info(KERN_INFO "%s: %s: mem=0x%llx, irq=%d hw_base=0x%pK\n",
...
init_ath_pci(void)
{
    ...
    ahbret = init_ath_ahb();
    if(ahbret < 0 ) {
        qdf_nofl_info("ath_ahb: Error while registering ath wlan ahb driver\n");
    }
#endif

    qdf_nofl_info(KERN_INFO "%s: %s\n", dev_info, version);

    /* Init wlan reg params */
    init_wlan();

    if (pci_register_driver(&ath_pci_driver) < 0) {
        qdf_nofl_info("ath_pci: No devices found, driver not installed.\n");
...
qdf_export_symbol(get_pci_reg_addr);
qdf_export_symbol(get_pci_virt_addr);
