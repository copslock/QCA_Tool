...
init_ath_wmac(u_int16_t devid, u_int16_t wlanNum)
{
    ...
    if (dev == NULL) {
        qdf_nofl_info(KERN_ERR "%s: no memory for device state\n", __func__);
        goto bad1;
    }
    ...
    if(dev_node) {
        ...
        qdf_nofl_info(KERN_ERR "%s: No wifi entry in DTS\n", __func__);
        goto bad2;
	}
#else
    ...
    qdf_nofl_info(KERN_INFO "%s: %s: mem=0x%lx, irq=%d\n",
...
int init_ahb(void)
{
	...
	if(strcmp(sysType, "Atheros AR9330 (Hornet)") == 0) {
        ...
        qdf_nofl_info("Unknow sysType: %s\n", sysType);
		return -ENODEV;
	}
...
init_ath_ahb(void)
{
	...
	if (init_ahb() != 0) {
		qdf_nofl_info("ath_ahb: No devices found, driver not installed.\n");
		return (-ENODEV);
	}
