    ...
    if (resmgr_vap) {
       if (resmgr_vap->chan) {
          ...
          if (resmgr_vap->chan != channel ) {
            IEEE80211_DPRINTF(vap, IEEE80211_MSG_ANY, "%s resetting bss channel (%d freq %d)  pointer after country code change \n",__func__,channel->ic_ieee,channel->ic_freq);
          }
...
static void _ieee80211_resmgr_create_complete(ieee80211_resmgr_t resmgr)
{
    ...
    if (QDF_IS_STATUS_ERROR(status)) {
        qdf_nofl_info("%s : ieee80211_scan_register_event_handler() failed status=%08X\n",
            __func__, status);
    }
}
...
ieee80211_resmgr_t _ieee80211_resmgr_create(struct ieee80211com *ic, ieee80211_resmgr_mode mode)
{
    ...
    if (ic->ic_tsf_timer == NULL) {
        qdf_nofl_info("%s: Multi-Channel Scheduler unavailable since no TSF Timer.\n", __func__);
    }
    else {
    ...
    if (ic->ic_resmgr) {
        qdf_nofl_info("%s : ResMgr already exists \n", __func__);
        return NULL;
    }
    ...
    resmgr = (ieee80211_resmgr_t) OS_MALLOC(ic->ic_osdev,
                                            ...
                                            0);
    if (resmgr == NULL) {
        qdf_nofl_info("%s : ResMgr memory alloction failed\n", __func__);
        return NULL;
    }
    ...
    if (ieee80211_resmgr_sm_create(resmgr) != EOK) {
        OS_FREE(resmgr);
        qdf_nofl_info("%s : ieee80211_resmgr_sm_create failed \n", __func__);
        return NULL;
    }
    ...
    if (ic->ic_caps_ext & IEEE80211_CEXT_MULTICHAN) {
        if (ieee80211_resmgr_oc_scheduler_create(resmgr, OFF_CHAN_SCHED_POLICY_ROUND_ROBIN) != EOK) {
            OS_FREE(resmgr);
            qdf_nofl_info("%s : ieee80211_resmgr_oc_scheduler_create failed \n", __func__);
            return NULL;
        }
        ...
        /* Do not support P2P */
        qdf_nofl_info("Error: %s: no Multi-chan hw caps. Disabling Off Channel scheduler.\n", __func__);
    }
