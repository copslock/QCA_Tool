...
ath_node_set_fixedrate_proc(struct ath_softc_net80211 *scn,
                            const u_int8_t macaddr[QDF_MAC_ADDR_SIZE],
                            u_int8_t fixed_ratecode)
{
    ...
    if (!(ic->ic_ieee80211_find_node_debug && ic->ic_ieee80211_unref_node)) {
        qdf_nofl_info("%s: ic->ic_ieee80211_find_node_debug %pK ic->ic_ieee80211_unref_node %pK\n",
            __func__, ic->ic_ieee80211_find_node_debug, ic->ic_ieee80211_unref_node);
        return;
    }
    ni = ic->ic_ieee80211_find_node_debug(&ic->ic_sta, macaddr, WLAN_MLME_SB_ID, __func__, __LINE__, __FILE__);
#else
    if (!(ic->ic_ieee80211_find_node && ic->ic_ieee80211_unref_node)) {
        qdf_nofl_info("%s: ic->ic_ieee80211_find_node %pK ic->ic_ieee80211_unref_node %pK\n",
            __func__, ic->ic_ieee80211_find_node, ic->ic_ieee80211_unref_node);
        return;
    }
    ni = IC_IEEE80211_FIND_NODE(ic, &ic->ic_sta, macaddr, WLAN_MLME_SB_ID);
#endif
    if (!ni) {
        qdf_nofl_info("%s: No node found!\n", __func__);
        return;
    }
