...
ath_key_map(struct ieee80211vap *vap, const struct wlan_crypto_key *k,
	    const u_int8_t bssid[QDF_MAC_ADDR_SIZE], struct wlan_objmgr_peer *peer);
#else
static int
ath_key_map(struct ieee80211vap *vap, const struct ieee80211_key *k,
	    const u_int8_t bssid[QDF_MAC_ADDR_SIZE], struct ieee80211_node *ni);
#endif
static int __ath_key_set(struct ieee80211vap *vap, const struct ieee80211_key *k,
                         const u_int8_t peermac[QDF_MAC_ADDR_SIZE], int is_proxy_addr);
...
ath_net80211_find_channel(struct ath_softc *sc, int ieee, u_int16_t des_cfreq2, enum ieee80211_phymode mode);
#endif
    ...
    TAILQ_FOREACH(vap, &ic->ic_vaps, iv_next) {
        if (vap->iv_opmode == IEEE80211_M_HOSTAP) {
            if (wlan_vdev_mlme_is_active(vap->vdev_obj) == QDF_STATUS_SUCCESS) {
                ...
                if(!wlan_set_channel(vap, IEEE80211_FREQ_ANY, 0)) {
...
ath_net80211_flowmac_notify_state (ieee80211_handle_t ieee, int en)
{
    ...
    TAILQ_FOREACH(vap, &ic->ic_vaps, iv_next) {
        if (vap) {
            ...
            qdf_nofl_info("Notifying VAP %pK  enabled state %d \n", vap, en);
        }
    }
    spin_unlock(&ic->ic_lock);
}
...
ath_get_numchains(int chainmask)
{
    ...
    default:
        chains = 0;
        qdf_nofl_info("%s: invalid chainmask\n", __func__);
        break;
    case 1:
    case 2:
    case 4:
...
ath_clear_keycache(struct ath_softc_net80211 *scn)
{
    ...
    for (i = 0; i < sc->sc_keymax; i++) {
        ath_hal_keyreset(ah, (u_int16_t)i, 1);
    }
}
               ...
               void               *vdev)
{
   ...
   id = wlan_vdev_get_id(vdev);
    ...
    if (flags & IEEE80211_CLONE_MACADDR) {
        if(id >= ATH_VAPSIZE) {
            qdf_nofl_info("exceeding limit of maximum vap allowed Investigate \n");
            return NULL;
        }
    ...
    if (avn == NULL) {
        qdf_nofl_info("Can't allocate memory for ath_vap.\n");
        return NULL;
    }
     if(IEEE80211_CHK_VAP_TARGET(ic)){
        qdf_nofl_info(" Target vap exceeded  freeing \n");
        OS_FREE_VAP(avn);
        return NULL;
     }
...
#endif
    {
        qdf_nofl_info("Unable to add an interface for ath_dev.\n");
        OS_FREE_VAP(avn);
        return NULL;
    }
...
#define ATH_NSCAN_PSTA_VAPS 0
    ...
    if (flags & IEEE80211_CLONE_MATADDR) {
        ...
        OS_MEMCPY(vap->iv_mat_addr, mataddr, QDF_MAC_ADDR_SIZE);
    }
...
ath_vap_alloc_macaddr(struct ieee80211com *ic, u_int8_t *bssid)
{
    ...
    if (IEEE80211_ADDR_IS_VALID(bssid) ) {
        ...
        if (!IEEE80211_ADDR_EQ(tmp_mac1,tmp_mac2) ) {
            qdf_nofl_info("%s: Invalid mac address requested %s  \n", __func__, ether_sprintf(bssid));
            return -1;
        }
        ATH_GET_VAP_ID(bssid, ic->ic_my_hwaddr, id);
        if ((id_mask & (1 << id)) != 0) {
            qdf_nofl_info("%s: mac address already allocated %s \n", __func__,ether_sprintf(bssid));
            return -1;
        }
     }
     else {
       ...
       if (id == ATH_BCBUF) {
           /* no more ids left */
           qdf_nofl_info("%s: No more free slots left \n", __func__);
           return -1;
       }
    }
...
ath_net80211_node_alloc(struct ieee80211vap *vap, const u_int8_t *mac, bool tmpnode, void *peer)
{
     ...
     */
    if (qdf_atomic_read(&vap->iv_node_count) > (ic->ic_num_clients << 2)) {
        return NULL;
    }
    ...
    if (anode->an_node.trace == NULL) {
        qdf_nofl_info("Can't create an node trace\n");
        return NULL;
    }
...
ath_net80211_node_cleanup(struct ieee80211_node *ni)
{
    ...
    if (scn->sc_ops->cleanup_node(scn->sc_dev, anode->an_sta)) {
        ...
        if (ieee80211_node_refcnt(ni) <= 0) {
            qdf_nofl_info("lmac cleanup skipped when ni refcnt is 0. Possible race??\n");
        }
    }
...
ath_net80211_set_channel(struct ieee80211com *ic)
{
    ...
    if (dfs_rx_ops && dfs_rx_ops->dfs_set_current_channel_for_freq) {
        ...
        dfs_rx_ops->dfs_set_current_channel_for_freq(pdev,
                ...
                chan->ic_flagext,
                chan->ic_ieee,
                chan->ic_vhtop_ch_num_seg1,
                chan->ic_vhtop_ch_num_seg2,
...
ath_keyprint(const char *tag, u_int ix,
             const HAL_KEYVAL *hk, const u_int8_t mac[QDF_MAC_ADDR_SIZE])
{
    ...
    if(type < sizeof(ciphers)/sizeof(ciphers[0]))
        qdf_nofl_info("%s: [%02u] %-7s ", tag, ix, ciphers[type]);
    for (i = 0, n = hk->kv_len; i < n; i++)
        qdf_nofl_info("%02x", hk->kv_val[i]);
    if (mac) {
        qdf_nofl_info(" mac %02x-%02x-%02x-%02x-%02x-%02x", mac[0], mac[1], mac[2], mac[3], mac[4], mac[5]);
    } else {
        qdf_nofl_info(" mac 00-00-00-00-00-00");
    }
    if (type == HAL_CIPHER_TKIP) {
        qdf_nofl_info(" mic ");
        for (i = 0; i < sizeof(hk->kv_mic); i++)
            qdf_nofl_info("%02x", hk->kv_mic[i]);
        qdf_nofl_info(" txmic ");
		for (i = 0; i < sizeof(hk->kv_txmic); i++)
			qdf_nofl_info("%02x", hk->kv_txmic[i]);
    }
#if ATH_SUPPORT_WAPI
	else if (type == HAL_CIPHER_WAPI) {
        qdf_nofl_info(" Mic Key ");
...
ath_keyset_tkip(struct ath_softc_net80211 *scn, const struct ieee80211_key *k,
                HAL_KEYVAL *hk, const u_int8_t mac[QDF_MAC_ADDR_SIZE])
{
		...
		switch (opmode) {
			case IEEE80211_M_STA:
                                ...
                                if (dadp_vdev_is_psta(vap->vdev_obj) && !(dadp_vdev_is_mpsta(vap->vdev_obj))){
                                    qdf_nofl_info("%s:Ignore set group key for psta\n",__func__);
                                    return 1;
                                }
#endif
...
ath_setup_stationwepkey(struct ieee80211_node *ni)
{
    ...
    u_int8_t null_macaddr[QDF_MAC_ADDR_SIZE] = {0,0,0,0,0,0};
...
ath_net80211_input(ieee80211_node_t node, wbuf_t wbuf, ieee80211_rx_status_t *rx_status)
{
    ...
    if (frame_type == IEEE80211_FC0_TYPE_MGT) {
        ...
        rx_event.channel = wlan_freq_to_chan(rs.rs_freq);
    ...
    switch (status->bf_flags) {
        ...
        case ATH_BFRX_PKT_MORE_DELAY:
            {   /*Find & save Node*/
                ...
                ni_T = IC_IEEE80211_FIND_NODE(ic,&ic->ic_sta, wh1->i_addr2, WLAN_MLME_SB_ID);
...
static void ath_net80211_rate_newstate(ieee80211_handle_t ieee, ieee80211_if_t if_data)
{
    ...
    if (ieee80211vap_get_opmode(vap) != IEEE80211_M_STA) {
        ...
        if((vap->iv_bsschan == NULL) ||
            ...
            qdf_nofl_info("VAP channel is not set for vap %pK investigate!\n", vap);
            return;
        }
        /*
...
static void ath_net80211_get_chan_info(struct ieee80211com *ic, u_int8_t flags)
{
    ...
    ieee_chan_freq = ieee80211_chan2freq(ic, ic->ic_curchan);
    ...
    ieee80211_acs_stats_update(ic->ic_acs, flags, ieee_chan_freq,
...
QDF_STATUS ath_net80211_scan_sm_start(struct wlan_objmgr_pdev *pdev, struct scan_start_request *req)
{
    ...
    scn = ATH_SOFTC_NET80211(ic);
    qdf_nofl_info("%s:Calling scan_sm_start\n", __func__);
...
{
	struct ieee80211vap *vap;
	uint8_t vap_macaddr[QDF_MAC_ADDR_SIZE] = {0xff, 0xff, 0xff, 0xff, 0xff, 0xff};
	u_int16_t keyix, clearkeyix;
	struct ieee80211com *ic = NULL;
	struct ath_softc_net80211 *scn = NULL;

	vap = wlan_mlme_get_vdev_ext_hdl(vdev);
	if (!vap) {
	...
	if (key->flags & WLAN_CRYPTO_KEY_GROUP) {
		...
		qdf_mem_copy(vap_macaddr, wlan_vdev_mlme_get_macaddr(vdev), QDF_MAC_ADDR_SIZE);
		psoc = wlan_vdev_get_psoc(vdev);
		if (!psoc) {
...
wlan_crypto_key_set(struct ieee80211vap *vap,
            ...
            u_int8_t peermac[QDF_MAC_ADDR_SIZE],
            int is_proxy_addr)
{
#define TXMIC(k) (k->keyval + 16)
	struct ath_softc_net80211 *scn = ATH_SOFTC_NET80211(vap->iv_ic);
...
#if ATH_SUPPORT_WAPI
		if (key_type == HAL_CIPHER_WAPI)
			qdf_mem_copy(hk.kv_mic, TXMIC(k), WLAN_CRYPTO_MICBUF_SIZE);
#endif
		hk.kv_len  = k->keylen;
		qdf_mem_copy(hk.kv_val, k->keyval, k->keylen);
	} else
		key_type = HAL_CIPHER_CLR;
...
exit:
	...
	qdf_mem_zero(&hk, sizeof(HAL_KEYVAL));
	return status;
}
...
#define IS_INVALID 0
	...
	else
	{
		...
		pdev_id = wlan_objmgr_pdev_get_pdev_id(
						       wlan_vdev_get_pdev(vdev));
		wlan_vdev_obj_lock(vdev);
		qdf_mem_copy(bssid_mac, wlan_vdev_mlme_get_macaddr(vdev), QDF_MAC_ADDR_SIZE);
		psoc = wlan_vdev_get_psoc(vdev);
		if (!psoc) {
			wlan_vdev_obj_unlock(vdev);
			qdf_print("%s[%d] psoc NULL", __func__, __LINE__);
           ...
           struct hal_reg_parm *hal_conf_parm, IEEE80211_REG_PARAMETERS *ieee80211_conf_parm, struct ath_hal *ah)
{
    ...
    if(enable_pktlog_support) {
        ...
        if (!scn->pl_dev) {
            qdf_nofl_info( "%s[%d]: Allocation of memory handle failed!\n", __func__, __LINE__ );
            return -ENOMEM;
        }
    ...
    if (error != 0) {
        qdf_nofl_info( "%s[%d]: Device attach failed!, error[%d]\n", __func__, __LINE__, error );
    ...
    if (error) {
        qdf_nofl_info( "%s[%d]: CWM attach failed!, error[%d]\n", __func__, __LINE__, error );
        ath_dev_free(dev);
        return error;
    }
    ...
    if (ops->have_capability(dev, ATH_CAP_HT)) {
        ...
        if (ops->have_capability(dev, ATH_CAP_TXBF)) {
            ...
            //qdf_nofl_info("==>%s:ATH have TxBF cap, set ie= %x \n",__func__,ic->ic_txbf.value);
        }
#endif
    ...
    KASSERT((ic->ic_my_hwaddr[QDF_MAC_ADDR_SIZE - 1] & 0x07) == 0,
    ...
    if (error) {
        qdf_nofl_info( "%s: failed!, error[%d]\n", __func__, error );
        ath_dev_free(dev);
        return error;
    }
   ...
   ic->ic_scan_enable_txq = ath_net80211_scan_enable_txq;
    ...
    qdf_nofl_info("%s: Set global_ic[%d]..gloabl_ic ptr:%pK\n", __func__, ic_list.num_global_ic,&ic_list);
...
ath_detach(struct ath_softc_net80211 *scn)
{
    ...
    for (i=0; i < MAX_RADIO_CNT; i++) {
        ...
        if (tmp_ic && (ic == tmp_ic)) {
            ...
            qdf_nofl_info("%s: remove global_ic[%d]..gloabl_ic ptr:%pK\n", __func__, ic_list.num_global_ic,&ic_list);
            GLOBAL_IC_UNLOCK_BH(&ic_list);
        }
    }
#if DBDC_REPEATER_SUPPORT
...
#ifdef ATH_TX99_DIAG
static struct ieee80211_ath_channel *
ath_net80211_find_channel(struct ath_softc *sc, int freq, u_int16_t des_cfreq2, enum ieee80211_phymode mode)
{
    struct ieee80211com *ic = (struct ieee80211com *)sc->sc_ieee;
    return ieee80211_find_dot11_channel(ic, freq, des_cfreq2, mode);
}
#endif
...
#else
    qdf_nofl_info("%s[%d] DFS not defined yet got radar pulse\n", __func__, __LINE__);
#endif
    return 0;
}
...
ath_net80211_set_min_rssi(ieee80211_handle_t ieee, int rssi)
{
    ...
    if( !(ic->ic_min_rssi_enable) ) {
        qdf_nofl_info("Cannot set, feature not enabled\n");
        return -1;
    }
    ic->ic_min_rssi = rssi;
