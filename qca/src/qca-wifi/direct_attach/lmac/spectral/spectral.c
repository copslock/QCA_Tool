...
int spectral_check_hw_capability(struct ath_spectral *spectral)
{
    ...
    if (p_sops->get_capability(spectral, HAL_CAP_PHYDIAG) == AH_FALSE) {
        ...
        qdf_nofl_info("SPECTRAL : No PHYDIAG support\n");
    ...
    if (p_sops->get_capability(spectral, HAL_CAP_RADAR) == AH_FALSE) {
        is_spectral_supported = AH_FALSE;
        qdf_nofl_info("SPECTRAL : No RADAR support\n");
        return is_spectral_supported;
    } else {
        pcap->radar_cap = 1;
    }
    ...
    if (p_sops->get_capability(spectral, HAL_CAP_SPECTRAL_SCAN) == AH_FALSE) {
        is_spectral_supported = AH_FALSE;
        qdf_nofl_info("SPECTRAL : No SPECTRAL SUPPORT\n");
        return is_spectral_supported;
    } else {
        pcap->spectral_cap = 1;
    }
    ...
    if (p_sops->get_capability(spectral, HAL_CAP_ADVNCD_SPECTRAL_SCAN)
            == AH_FALSE) {
        qdf_nofl_info("SPECTRAL : No ADVANCED SPECTRAL SUPPORT\n");
    } else {
        pcap->advncd_spectral_cap = 1;
    }

    return is_spectral_supported;
}qdf_export_symbol(spectral_check_hw_capability);
...
void* pdev_spectral_init_da(struct wlan_objmgr_pdev *pdev)
{
    ...
    if (spectral == NULL) {
        qdf_nofl_info("SPECTRAL : Memory allocation failed\n");
        return spectral;
    }
...
qdf_export_symbol(pdev_spectral_postinit);
...
void pdev_spectral_deinit_da(struct wlan_objmgr_pdev *pdev)
{
    ...
    spectral = get_spectral_handle_from_pdev(pdev);
    if (spectral == NULL) {
         qdf_nofl_info("SPECTRAL : Module dosen't exist\n");
         return;
    }
...
int spectral_scan_enable_params(struct ath_spectral *spectral, struct spectral_config* spectral_params)
{
    ...
    if (spectral == NULL) {
        qdf_nofl_info("SPECTRAL : Spectral is NULL\n");
        return 1;
    }
    ...
    if (p_sops == NULL) {
        qdf_nofl_info("SPECTRAL : p_sops is NULL\n");
        return 1;
    }
    ...
    if (spectral->capability.advncd_spectral_cap) {
        ...
        if (spectral->ch_width == IEEE80211_CWM_WIDTH20) {
            //qdf_nofl_info("SPECTRAL : (11AC) 20MHz Channel Width (Channel = %d)\n", current_channel);
            ...
            //qdf_nofl_info("SPECTRAL : (11AC) 40MHz Channel Width (Channel = %d)\n", current_channel);
            ...
            //qdf_nofl_info("SPECTRAL : (11AC) 80MHz Channel Width (Channel = %d)\n", current_channel);
            /* Set the FFT Size */
            ...
            //qdf_nofl_info("SPECTRAL : (11AC) 160MHz Channel Width (Channel = %d)\n", current_channel);
            /* Set the FFT Size */
        ...
        //qdf_nofl_info("SPECTRAL : Legacy (Non-11AC)\n");
        /*
        ...
        if (extension_channel == 0) {
            //qdf_nofl_info("SPECTRAL : (Legacy) 20MHz Channel Width (Channel = %d)\n", current_channel);
            ...
            //qdf_nofl_info("SPECTRAL : (Legacy) 40MHz Channel Width (Channel = %d)\n", current_channel);
    ...
    spectral->num_spectral_data                     = 0;
     ...
     if (!p_sops->is_spectral_active(spectral)) {
        ...
        //qdf_nofl_info("Enabled spectral scan on channel %d\n", p_sops->get_current_channel(spectral));
    } else {
        //qdf_nofl_info("Spectral scan is already ACTIVE on channel %d\n", p_sops->get_current_channel(spectral));
    }
                            ...
                            const u_int32_t value)
{
    ...
    p_sops = GET_SPECTRAL_OPS(spectral);
    if (spectral == NULL) {
        qdf_nofl_info(  "%s: sc_spectral is NULL\n", __func__);
        return 0;
    }
                             ...
                             CHAIN_NOISE_PWR_INFO* ext_c2)  /* reports for chain 2 ext */
{
    ...
    if(p_sops->is_spectral_active(spectral)) {
        qdf_nofl_info("%s[%d]: spectral already active - cancel\n", __func__, __LINE__);
    ...
    if (!error && spectral->noise_pwr_reports_recv < spectral->noise_pwr_reports_reqd) {
        error = EAGAIN;
        qdf_nofl_info("%s[%d]: noise pwr done but only %d/%d reports_recv\n", __func__, __LINE__, spectral->noise_pwr_reports_recv,
    ...
    */
    ...
    if (!error) {
        qdf_nofl_info("%s", "rssi ave: ");
        for(i = 0; i < ATH_HOST_MAX_ANTENNA; i++) {
            qdf_nofl_info(  "%3d.%02d ",
        ...
        qdf_nofl_info(  "%s", "\n");
        qdf_nofl_info(  "Temp data: %d [0x%x]\n",
    ...
    if (error == EBUSY) {
        ...
        qdf_nofl_info(  "%s[%d]: TIME_OUT: force reset on exit\n", __func__, __LINE__);
...
}qdf_export_symbol(spectral_register_funcs);
...
qdf_export_symbol(da_ath_sptrl_register_tx_ops);
...
qdf_export_symbol(spectral_scan_enable);
qdf_export_symbol(ath_process_spectraldata);
qdf_export_symbol(is_spectral_phyerr);
qdf_export_symbol(spectral_send_intf_found_msg);
qdf_export_symbol(spectral_scan_enable_params);
qdf_export_symbol(spectral_record_chan_info);
