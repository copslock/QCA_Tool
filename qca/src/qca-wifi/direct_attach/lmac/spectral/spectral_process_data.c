...
#define DEFAULT_CHAN_NOISE_FLOOR    -110
    ...
    if (((rxs->rs_phyerr != HAL_PHYERR_RADAR)) &&
        ...
        qdf_nofl_info("%s : Unknown PHY error (0x%x)\n", __func__, rxs->rs_phyerr);
        return;
    }

    if (spectral == NULL) {
        qdf_nofl_info("Spectral Module not attached\n");
        return;
    }
    ...
    switch((datalen & 0x3)) {
        ...
        case 3:
            ...
            break;
        default:
             qdf_nofl_info(  "datalen mod4=%d spectral_data_len=%d\n", (datalen%4), spectral->spectral_data_len);
    }
    ...
    if (pulse_bw_info & SPECTRAL_SCAN_DATA) {
        ...
        if (datalen > spectral->spectral_data_len + 2) {
            //qdf_nofl_info("Invalid spectral scan datalen = %d\n", datalen);
            return;
        }
        ...
        if (spectral->num_spectral_data == 0) {
            ...
            //qdf_nofl_info( "First FFT data tstamp = %u rssi=%d\n", tstamp, fix_rssi_inv_only(combined_rssi));
        }
      ...
      if ((datalen == (spectral->spectral_data_len - 1)) ||
          ...
          qdf_nofl_info(  "%s %d missing 1 byte datalen=%d expected=%d\n", __func__, __LINE__, datalen, spectral->spectral_data_len);
      ...
      if ((datalen == (spectral->spectral_data_len + 1)) ||
          ...
          //qdf_nofl_info(  "%s %d extra bytes datalen=%d expected=%d\n", __func__, __LINE__, datalen, spectral->spectral_data_len);
