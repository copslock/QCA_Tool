...
tx99_channel_setup(struct ath_softc *sc)
{
    ...
    if (tx99->txfreq != 0) {
        ...
        c = sc->sc_ieee_ops->ath_net80211_find_channel(sc, tx99->txfreq, 0, tx99->txmode);
...
qdf_export_symbol(tx99_attach);
qdf_export_symbol(tx99_detach);
qdf_export_symbol(tx99_ioctl);
#endif
