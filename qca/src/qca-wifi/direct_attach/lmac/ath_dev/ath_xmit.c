...
ath_tx_init(ath_dev_t dev, int nbufs)
{
    ...
    do {
        ...
        if (error != 0) {
            qdf_nofl_info("failed to allocate tx descriptors: %d\n", error);
            break;
        }
        ...
        if (error != 0) {
            qdf_nofl_info("failed to allocate beacon descripotrs: %d\n", error);
            break;
        }
#if UMAC_SUPPORT_WNM
        ...
        if (error != 0) {
            qdf_nofl_info("failed to allocate tim descripotrs: %d\n", error);
            break;
        }
#endif
        ...
        if (error != 0) {
            qdf_nofl_info("failed to allocate UAPSD descripotrs: %d\n", error);
            break;
        }
#endif
        ...
        if (error != 0) {
            qdf_nofl_info("failed to allocate tx status descriptors: %d\n", error);
            break;
        }
        ...
        if (ath_hal_hasmci(sc->sc_ah)) {
            ...
            if (error != 0) {
                qdf_nofl_info("failed to allocate MCI buffers: %d\n", error);
                break;
            }
        }
#endif
...
#define    N(a)    (sizeof(a)/sizeof(a[0]))
    ...
    if (qnum >= N(sc->sc_txq)) {
        qdf_nofl_info("hal qnum %u out of range, max %u!\n",
...
ath_set_tx(struct ath_softc *sc)
{
    if (!ath_tx_setup(sc, HAL_WME_AC_BK)) {
        qdf_nofl_info("unable to setup xmit queue for BE traffic!\n");
...
#define    N(a)    (sizeof(a)/sizeof(a[0]))
    ...
    if (haltype >= N(sc->sc_haltype2q)) {
        qdf_nofl_info("HAL AC %u out of range, max %zu!\n",
...
ath_txq_update(ath_dev_t dev, int qnum, HAL_TXQ_INFO *qi0)
{
    ...
    if (!ath_hal_settxqueueprops(ah, qnum, &qi)) {
        qdf_nofl_info("%s: unable to update hardware queue %u!\n",
               __func__, qnum);
        error = -EIO;
    } else {
...
_ath_tx_txqaddbuf(struct ath_softc *sc, struct ath_txq *txq, ath_bufhead *head)
{
    ...
    if (ath_hal_gettxbuf(ah, txq->axq_qnum) == 0) {
        ...
        qdf_nofl_info("%s: FATAL ERROR: NULL TXDP while enabling TX.\n", __func__);
        ASSERT(FALSE);
    } else
#endif
...
#define DESC2PA(_sc, _va)	\
    ...
    if (sc->sc_reg_parm.c3WarTimerPeriod &&
        ...
        if (!sc->c3_war_timer_active) {
            ...
            qdf_nofl_info("%s: sc_c3_war_timer is started!!\n", __func__);
        }
...
/*
        ...
        qdf_nofl_info(KERN_DEBUG " %s: Queueing to HwQ SeqNo %d @ %x ",
                __func__,
...
qdf_export_symbol(__wbuf_map_sg);
...
qdf_export_symbol(__wbuf_unmap_sg);
...
//       qdf_nofl_info(KERN_DEBUG "%s: Sending %d buffer: %d ", __func__, cnt_wifipos, txctl->seqno);
    }
*/
    ...
    if (firstbf) {
        ...
        if (!no_wait_for_vap_pause) {
            ...
            if(sc->sc_vap_pause_timeout){
                ath_vap_pause_txq_use_dec(sc);
                qdf_nofl_info("%s:txq_pause in progress flag is set\n",__func__);
                goto bad;
            }
#endif
        }
...
qdf_export_symbol(ath_ratecode_to_ratekbps);
...
#endif
{
    ...
    if (sc->sc_reg_parm.c3WarTimerPeriod && bf->bf_isdata) {
        ...
        if (!atomic_read(&sc->sc_pending_tx_data_frames)) {
            ...
            if (sc->c3_war_timer_active) {
                ...
                qdf_nofl_info("%s: sc_c3_war_timer is stopped!!\n", __func__);
            }
            spin_unlock(&sc->sc_c3_war_lock);
        }
    }
    ...
    if (bf->bf_state.bfs_ispaprd) {
        ...
        //qdf_nofl_info("%s[%d]: ath_tx_paprd_complete called txok %d\n", __func__, __LINE__, txok);
        return;
    }
        ...
        if(cnt_tx_wifipos == 10) {
           ...
           tsf_test = tx_end_time[cnt_wifipos] - tx_start_time[cnt_wifipos];
            if(tsf_test < 32000){
                ...
                qdf_nofl_info(KERN_DEBUG"%s[%d]: tx_completion_time: %d",__func__,cnt_wifipos,tsf_test);
                cnt_wifipos++;
            }
            cnt_tx_wifipos = 0;
            if( cnt_wifipos == 1000){
                qdf_nofl_info(KERN_INFO"%s: Avg_tx_completion_time: %d",__func__,tx_completion_time/1000);
           ...
           qdf_nofl_info(KERN_DEBUG "%s: Completing %d buffer: %d, status: %x @ %x ", __func__, cnt_wifipos,
                                            (*(u_int16_t *)&((struct ieee80211_frame *)wbuf_header(bf->bf_mpdu))->i_seq[0]) >> 4,
        ...
        if (sc->sc_enhanceddmasupport) {
            ...
            if (bf == NULL) {
                if (txq->axq_headindex != txq->axq_tailindex)
                    qdf_nofl_info("ath_tx_draintxq: ERR head %d tail %d\n",
...
void ath_print_tbf_usage(struct ath_softc *sc)
{
    ...
    qdf_nofl_info("Txq Id\taxq_depth\taxq_fifo\ttid_bufq\tpaused_tid\tbar_paused\n");
    for (i = 0; i < HAL_NUM_TX_QUEUES; i++) {
