...
ath_vap_info_attach(struct ath_softc *sc, struct ath_vap *avp)
{
    ...
    h_module = (struct ath_vap_info *) OS_MALLOC(sc->sc_osdev,
                                               ...
                                               GFP_KERNEL);
    if (h_module == NULL) {
        qdf_nofl_info("%s: can't alloc module object. Size=%d\n", __func__, (int)sizeof(struct ath_vap_info));
        return;
    }
...
ath_vap_info_detach(struct ath_softc *sc, struct ath_vap *avp)
{
    ...
    if (avp->av_vap_info == NULL) {
        qdf_nofl_info("%s: sc->sc_vap_info is already NULL\n", __func__);
        return;
    }
    ...
    u_int32_t                   *ret_param2)
{
    ...
    if ((vap_if_id == ATH_IF_ID_ANY) || (sc->sc_vaps[vap_if_id] == NULL)) {
        qdf_nofl_info("%s: invalid if_id=%d or vap is NULL.\n", __func__,
               vap_if_id);
    ...
    if (avp->av_vap_info == NULL) {
        qdf_nofl_info("%s: avp->av_vap_info is NULL. vap_if_id=%d\n", __func__, vap_if_id);
    ...
    case ATH_VAP_INFOTYPE_SLOT_OFFSET:
        ...
        *ret_param1 = le64_to_cpu(avp->av_tsfadjust);
        break;
    default:
        qdf_nofl_info("%s: unknown VAP info type=%d\n", __func__, vap_info_type);
        return -EINVAL;
    }
    ...
    void                        *arg)
{
    ...
    avp = sc->sc_vaps[vap_if_id];
    if (avp == NULL) {
        qdf_nofl_info("%s: invalid VAP. if_id=%d\n", __func__, vap_if_id);
        return -EINVAL;      /* VAP is not valid */
    }

    if (avp->av_vap_info == NULL) {
        qdf_nofl_info("%s: avp->av_vap_info is NULL\n", __func__);
        return -EINVAL;
    }
    h_module = avp->av_vap_info;
    ...
    if ((h_module->info_req_mask != 0) || (h_module->change_callback)) {
        qdf_nofl_info("%s: Error: Already registered.\n", __func__);
        return -EINVAL;
    }
    ...
    int         vap_if_id)
{
    ...
    if (atomic_read(&h_module->callback_inprog) != 0) {
        ...
        qdf_nofl_info("%s: Note: callback still in progress (%d)\n", 
