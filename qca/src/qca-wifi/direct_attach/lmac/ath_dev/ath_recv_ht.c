...
ath_ampdu_input(struct ath_softc *sc, struct ath_node *an, wbuf_t wbuf, ieee80211_rx_status_t *rx_status)
{
    ...
    if (!sc->sc_enableproxysta) {
        ...
        if (OS_MEMCMP(&(wh->i_addr1[1]), &(sc->sc_myaddr[1]), QDF_MAC_ADDR_SIZE - 2)) {
            if (((wh->i_addr1[0] & sc->sc_bssidmask[0]) == (sc->sc_myaddr[0] & sc->sc_bssidmask[0])) &&
                ((wh->i_addr1[QDF_MAC_ADDR_SIZE - 1] & sc->sc_bssidmask[QDF_MAC_ADDR_SIZE - 1]) ==
                (sc->sc_myaddr[QDF_MAC_ADDR_SIZE - 1] & sc->sc_bssidmask[QDF_MAC_ADDR_SIZE - 1]))) {
#else
        if (!ismcast &&
            (OS_MEMCMP(&(wh->i_addr1[1]), &(sc->sc_myaddr[1]), QDF_MAC_ADDR_SIZE - 1) ||
...
#endif
            qdf_nofl_info("%s[%d] rxtid timer active = %d, baw_head = %d, baw_tail = %d"
