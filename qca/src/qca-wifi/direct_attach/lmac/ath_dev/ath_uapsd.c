...
#define ATH_FRAG_PER_QOSNULL_MSDU                   1
    ...
    if (error != 0) {
        qdf_nofl_info("failed to allocate UAPSD QoS NULL tx descriptors: %d\n", error);
        return error;
    }
    ...
    TAILQ_FOREACH(bf, &sc->sc_uapsdqnulbf, bf_list)
    {
        ...
        if (wbuf == NULL) {
            error = -ENOMEM;
            qdf_nofl_info("failed to allocate UAPSD QoS NULL wbuf\n");
            break;
        }
        ...
        if (sc->sc_enhanceddmasupport) {
            ...
            if (bf == NULL) {
                if (txq->axq_headindex != txq->axq_tailindex)
                    qdf_nofl_info("ath_tx_uapsd_draintxq: ERR head %d tail %d\n",
