...
const u_int8_t ath_bcast_mac[QDF_MAC_ADDR_SIZE] = {0xff, 0xff, 0xff, 0xff, 0xff, 0xff};
...
#define WAIT_SC_INTERVAL 100
    while (atomic_read(&sc->sc_inuse_cnt)){
        ...
        if (elapsed_time > timeout) {
           qdf_nofl_info("%s: sc_inuse count(%d) stuck after waiting for %d msec. Investigate!!!\n",
...
ath_halchan2mode(HAL_CHANNEL *chan)
{
    ...
    qdf_nofl_info("%s: cannot map channel to mode; freq %u flags 0x%x\n",
...
__LINE__, sc->sc_dpc_stop);
        ...
        qdf_nofl_info("%s %d %d\n", __func__,
__LINE__, sc->sc_dpc_stop);
        qdf_nofl_info("Begin chan_set sequence......\n");
#endif
        ...
        if (!ath_hal_reset(ah, sc->sc_opmode, hchan,
                           ...
                           sc->sc_scanning)) {
            qdf_nofl_info("%s: unable to reset channel %u (%uMhz) "
            ...
            if (ATH_STARTRECV(sc) != 0) {
                qdf_nofl_info("%s: unable to restart recv logic\n", __func__);
            }
            return -EIO;
        }
        ATH_HTC_ABORTTXQ(sc);
        ...
        if (sc->sc_hasbtcoex && (!fastcc)) {
            ...
            if (!sc->sc_enhanceddmasupport) {
                ...
                if (atomic_read(&sc->sc_hold_reset))
                    qdf_nofl_info("Wow, still multiple reset in process,--investigate---\n");
...
ath_sc_chk_intr(struct ath_softc *sc, HAL_ANISTATS *ani_stats)
{
    ...
    if( !dcs_params->intr_cnt ){
        ...
        qdf_nofl_info("Interference Detected, Changing Channel\n");
#if WLAN_SPECTRAL_ENABLE
        if (sc->sc_icm_active) {
        ...
        if (longcal || shortcal || multical) {
            ...
            if (sc->sc_curchan.channel_flags & CHANNEL_CW_INT) {
                ...
                if( sc->sc_dcs_enabled & ATH_CAP_DCS_CWIM) {
                    ...
                    qdf_nofl_info("Too much interference, Trying to change channels\n");
...
#ifdef PRINT_CHAN_CHANGE
    qdf_nofl_info("====================================================\n");
    qdf_nofl_info(" Chan %u(%08x) - %u(%08x) \n", sc->sc_curchan.channel,
                                            sc->sc_curchan.channel_flags,
                                            hchan->channel,
                                            hchan->channel_flags);
    ts1 = read_c0_count();
...
static void
ath_get_mac_address(ath_dev_t dev, u_int8_t macaddr[QDF_MAC_ADDR_SIZE])
{
    ATH_ADDR_COPY(macaddr, ATH_DEV_TO_SC(dev)->sc_myaddr);
}

static void
ath_get_hw_mac_address(ath_dev_t dev, u_int8_t macaddr[QDF_MAC_ADDR_SIZE])
{
...
static void
ath_set_mac_address(ath_dev_t dev, const u_int8_t macaddr[QDF_MAC_ADDR_SIZE])
{
    struct ath_softc *sc = ATH_DEV_TO_SC(dev);
    OS_MEMCPY(sc->sc_myaddr, macaddr, QDF_MAC_ADDR_SIZE);
...
static void ath_set_bssid_mask(ath_dev_t dev, const u_int8_t bssid_mask[QDF_MAC_ADDR_SIZE])
{
    struct ath_softc *sc = ATH_DEV_TO_SC(dev);
    OS_MEMCPY(sc->sc_bssidmask, bssid_mask, QDF_MAC_ADDR_SIZE);
    ...
    if (avp->av_opmode == HAL_M_HOSTAP || avp->av_opmode == HAL_M_IBSS) {
        ...
        if (!sc->sc_removed)
        {
            ...
            } else
            {
                ...
                qdf_nofl_info("%s:%d  Unable to stop CABQ. Now Stopping all Queues \n", __func__, __LINE__);
...
static int
ath_vap_join(ath_dev_t dev, int if_id, const u_int8_t bssid[QDF_MAC_ADDR_SIZE], u_int flags)
{
    struct ath_softc *sc = ATH_DEV_TO_SC(dev);
    struct ath_hal *ah = sc->sc_ah;
    struct ath_vap *avp;
...
static int
ath_vap_up(ath_dev_t dev, int if_id, const u_int8_t bssid[QDF_MAC_ADDR_SIZE],
    ...
    if (!sc->sc_invalid) {
        if (ah != NULL) {
            if (ah->ah_set_interrupts != NULL)
                ...
                qdf_nofl_info("%s[%d] : ah_set_interrupts was NULL\n", __func__, __LINE__);

        }
        else {
            qdf_nofl_info("%s[%d] : ah was NULL\n", __func__, __LINE__);
        }
    }
...
ath_open(ath_dev_t dev, HAL_CHANNEL *initial_chan)
{
    ...
    if (ATH_STARTRECV(sc) != 0) {
        qdf_nofl_info("%s: unable to start recv logic\n", __func__);
        error = -EIO;
        goto done;
    }
...
__LINE__, sc->sc_dpc_stop);
    ...
    qdf_nofl_info("%s %d %d\n", __func__,
__LINE__, sc->sc_dpc_stop);
    qdf_nofl_info("Begin reset sequence......\n");
#endif
...
ath_reset(ath_dev_t dev)
{
    ...
    {
        qdf_nofl_info("%s: unable to reset hardware; hal status %u,\n"
    ...
    case ATH_CAP_NUMTXCHAINS:
        {
            ...
            {
                qdf_nofl_info("%s[%d] tx chainmask mismatch actual %d sc_chainmak %d\n", __func__, __LINE__,
    ...
    case ATH_CAP_NUMRXCHAINS:
        {
            ...
            {
                qdf_nofl_info("%s[%d] rx chainmask mismatch actual %d sc_chainmak %d\n", __func__, __LINE__,
...
ath_getiqueconfig(ath_dev_t dev)
{
    ...
    qdf_nofl_info("AC\tPER\tProbe Interval\n\n");
    qdf_nofl_info("BE&BK\t%d\t\t%d \n",
           sc->sc_rc_params[0].per_threshold,
           sc->sc_rc_params[0].probe_interval);
    qdf_nofl_info("VI&VO\t%d\t\t%d \n",
           sc->sc_rc_params[1].per_threshold,
           sc->sc_rc_params[1].probe_interval);

    qdf_nofl_info("\n========= Access Category Config===========\n");
    qdf_nofl_info("AC\tRTS \tAggr Scaling\tMin Rate(Kbps)\tHBR \tPER LO \tRXTIMEOUT\n\n");
    qdf_nofl_info("BE\t%s\t\t%d\t%6d\t\t%s\t%d\t%d\n",
...
#define    N(a)    (sizeof(a)/sizeof(a[0]))
    ...
    u_int32_t enableapm;
    u_int8_t empty_macaddr[QDF_MAC_ADDR_SIZE] = {0, 0, 0, 0, 0, 0};
    ...
    if (sc == NULL) {
        qdf_nofl_info("%s: Unable to allocate device object.\n", __func__);
        error = -ENOMEM;
        goto bad;
    }
    ...
    *ops = &sc->sc_ath_ops;
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,0))
    if (macaddr_override) {
        u_int8_t addr[QDF_MAC_ADDR_SIZE];
    ...
    if (sc->sc_keymax > ATH_KEYMAX) {
        qdf_nofl_info("%s: Warning, using only %u entries in %u key cache\n",
    ...
    if (sc->sc_bhalq == (u_int) -1) {
        qdf_nofl_info("%s: unable to setup a beacon xmit queue!\n", __func__);
        error = -EIO;
        goto bad2;
    }
    ...
    if (sc->sc_cabq == NULL) {
        qdf_nofl_info("%s: unable to setup CAB xmit queue!\n", __func__);
        error = -EIO;
        goto bad2;
    }
   ...
   if (error) goto bad2;
    ...
    if (!ATH_SET_TX(sc)) {
        qdf_nofl_info( "%s[%d]: Failed to set Tx h/w queue\n", __func__, __LINE__ );
        error = -EIO;
        goto bad2;
    }
    ...
    if (sc->sc_hasWow)
    {
        ...
        if (sc->sc_wowInfo != NULL) {
            ...
            qdf_nofl_info( "%s[%d]: Allocation of WoW information failed!\n", __func__, __LINE__ );
            error = -ENOMEM;
            goto bad2;
        }
    }
    ...
    if (sc->sc_rc == NULL) {
        qdf_nofl_info( "%s[%d]: Rate attach failed!\n", __func__, __LINE__ );
        error = EIO;
        goto bad2;
    }
    ...
    if (tx99_attach(sc)) {
        error = EIO;
        qdf_nofl_info("%s TX99 init failed\n", __func__);
    }
#endif
    ...
    if (sc->sc_hasgentimer) {
        sc->sc_rx_looplimit_timer = ath_gen_timer_alloc(sc,
                                                  ...
                                                  NULL,
                                                  sc);
        qdf_nofl_info("Register the looplimit timer\n");
    } else {
...
#endif
{
    ...
    {
        qdf_nofl_info("%s: Invalid interface id = %u\n", __func__, if_id);
        return -EINVAL;
    }
    ...
    if (sc->sc_vaps[if_id] != NULL) {
        ...
        qdf_nofl_info("%s: WAR_DELETE_VAP sc_nvaps=%d\n", __func__, sc->sc_nvaps);
    }
#else
...
ath_get_device_info(ath_dev_t dev, u_int32_t type)
{
    ...
    case ATH_WIRELESSMODE:       /* Wireless mode */
        ...
        break;
    default:
        //qdf_nofl_info("AthDeviceInfo: Unknown type %d\n", type);
...
ath_node_set_fixedrate(ath_node_t node, u_int8_t fixed_ratecode)
{
    ...
    qdf_nofl_info("%s: an_fixedrate_enable = %d, an_fixedrix = %d, an_fixedratecode = 0x%x\n",
...
ath_node_cleanup(ath_dev_t dev, ath_node_t node)
{
    ...
    if (atomic_read(&an->an_active_tx_cnt)) {
        qdf_nofl_info("skipping lmac node %pK cleanup due to tx activity\n", an);
        return 1;
    }
...
#ifdef ATH_KEEP_KEYCACHE_FOR_KR00K_VARIANT
       ath_hal_keyreset(sc->sc_ah, keyix, 0);
#else
       ath_hal_keyreset(sc->sc_ah, keyix, 1);
#endif
    if (freeslot) {
...
qdf_export_symbol(ath_keycache_print);
#endif
...
ath_set_regdomain(ath_dev_t dev, int regdomain, HAL_BOOL wrtEeprom)
{
    ...
    qdf_nofl_info(" %s: ret %d, regdomain %d \n", __func__, ret, regdomain);
...
void ath_set_rx_chainmask(ath_dev_t dev, u_int8_t mask)
{
    ...
    qdf_nofl_info("%s[%d] sc->sc_config.rxchainmask %d mask %d\n", __func__, __LINE__,
...
void ath_set_tx_chainmask(ath_dev_t dev, u_int8_t mask)
{
    ...
    qdf_nofl_info("%s[%d] sc->sc_config.txchainmask %d mask %d\n", __func__, __LINE__,
...
void ath_set_tx_chainmaskopt(ath_dev_t dev, u_int8_t mask)
{
    ...
    qdf_nofl_info("Primary txchainmask 0x%02x, optional txchainmask 0x%02x\n",
...
ath_key_save_halkey(ath_node_t node, HAL_KEYVAL *hk, u_int16_t keyix, const u_int8_t *macaddr)
{
    ...
    OS_MEMCPY(an->an_macaddr, macaddr, QDF_MAC_ADDR_SIZE);
}
...
qdf_export_symbol(ath_checkandplumb_key);
#endif
...
void ath_dev_stop_spectral_scan(ath_dev_t dev)
{
    ...
    if (state != ATH_PWRSAVE_AWAKE) {
        qdf_nofl_info("%s: Warning - Not in Awake state. Explicitly waking up\n",
                __func__);
        ATH_PS_WAKEUP(sc);
    }
...
qdf_export_symbol(ath_green_ap_get_enable_print);
...
qdf_export_symbol(ath_green_ap_get_enable);
...
qdf_export_symbol(ath_green_ap_set_enable);
...
qdf_export_symbol(ath_gen_randomness);
#endif
...
#define   HAL_CAP_STRONG_DIV 2
    ...
    if (ath_hal_getcapability(sc->sc_ah, HAL_CAP_DIVERSITY, HAL_CAP_STRONG_DIV,
           ...
           qdf_nofl_info( "%s[%d] SHOULD ABORT \n", __func__, __LINE__);
    }
    } else {
...
qdf_export_symbol(ath_process_phyerr);
#endif /* ATH_SUPPORT_DFS */
...
static int ath_smart_ant_enable(ath_dev_t dev, int enable, int mode)
{
    ...
    if (retval == -1)
    {
        qdf_nofl_info("Smart antenna is not supported\n");
        return 0;
    }
...
static int ath_rng_kthread(void *data)
{
    ...
    while (!kthread_should_stop()) {
        ...
        if(last_mode != mode)
        {
            qdf_nofl_info("mode changed to %d\n",ic->random_gen_mode);
            last_mode = mode;
        }
        if (mode ==0) {
            rssi_const_count  = 0;
