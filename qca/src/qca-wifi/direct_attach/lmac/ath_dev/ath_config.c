    ...
    switch(ID)
    {
        ...
        case ATH_PARAM_AGGR_BURST_DURATION:
            if (sc->sc_aggr_burst) {
                ...
                qdf_nofl_info("Error: enable bursting before setting duration\n");
            }
        break;
        ...
        case ATH_PARAM_ATH_3WIRE_BT_COEX:
        {
            ...
            if (val == 1) {
                ...
                if (!ath_hal_enable_basic_3wire_btcoex(sc->sc_ah)) {
                    qdf_nofl_info("BT_COEX 3WIRE MODE Enabled \n");
                    qdf_nofl_info("WL Weight: 0x%lx BT Weight:0x%lx\n",
                            ah->ah_bt_coex_wl_weight,ah->ah_bt_coex_bt_weight);
                } else {
                    qdf_nofl_info(" BT_COEX not supported for this Radio \n");
                }
            } else if (val == 0) {
                sc->sc_3wire_bt_coex_enable = val;
                ah->ah_3wire_bt_coex_enable = val;
                if (!ath_hal_disable_basic_3wire_btcoex(sc->sc_ah)) {
                    qdf_nofl_info(" BT_COEX 3WIRE MODE Disasbled \n");
                } else {
                    qdf_nofl_info(" BT_COEX not supported for this Radio \n");
                }
            } else {
                qdf_nofl_info("Enter: 1 to Enable BT_COEX , 0 to Disable the BT_COEX \n");
            }
        }
        break;
        ...
        case ATH_PARAM_VOWEXT:
            qdf_nofl_info("Setting new VOWEXT parameters buffers with %d\n",ATH_TXDESC);
            sc->sc_vowext_flags = (*(u_int32_t*)buff) & ATH_CAP_VOWEXT_MASK;
            qdf_nofl_info("VOW enabled features: \n");
            qdf_nofl_info("\tfair_queuing: %d\n\t"\
                   "retry_delay_red %d,\n\tbuffer reordering %d\n\t"\
        ...
        case ATH_PARAM_VSP_ENABLE:
            ...
            if(*(int *)buff)
            {
                qdf_nofl_info ("ATH_TX_BUF_FLOW_CNTL is not compiled in so can't enable VSP\n");
            }
            #endif
        break;
        ...
        case ATH_PARAM_DCS_ENABLE:
            ...
            if ( (sc->sc_dcs_enabled & ATH_CAP_DCS_WLANIM) && (!((sc->sc_curmode == WIRELESS_MODE_11NA_HT20)
                ...
                qdf_nofl_info("Disabling DCS for 11G mode");
                sc->sc_dcs_enabled &= (~ATH_CAP_DCS_WLANIM);
            }
             qdf_nofl_info("\tDCS for CW interference mitigation:   %d\n\t"\
        ...
        case ATH_PARAM_CNTRX_NUM:
            ...
            if((sc->sc_process_rx_num % 2)
                ...
                qdf_nofl_info("Adjusting to Nearest Even Value %d \n",sc->sc_process_rx_num);
            }
            break;
#endif
        ...
        case ATH_PARAM_RTS_CTS_RATE:
            ...
            {
                sc->sc_cus_cts_set=0;
                qdf_nofl_info ("ATH_PARAM_RTS_CTS_RATE not support\n");
            }
            break;
        case ATH_PARAM_NODEBUG:
            sc->sc_nodebug = *(int *)buff;
            qdf_nofl_info("sc nodebug %d \n",sc->sc_nodebug);
        ...
        case ATH_PARAM_ALDSTATS:
            ...
            qdf_nofl_info("sc aldstatsenabled %d \n",sc->sc_aldstatsenabled);
        ...
        case ATH_PARAM_EN_SELECTIVE_RTS:
            ...
            {
                qdf_nofl_info("Enable Rate limit first \r\n");
                retval = (-1);
            }
            break;
        ...
        case ATH_MIN_RSSI:
            {
                ...
                if (val <= 0) {
                    qdf_nofl_info("snr should be a positive value\n");
                    return (-1);
                } else
        ...
        case ATH_PARAM_CLIENT_MCAST:
            ...
            if (*(int *)buff) {
                ...
                qdf_nofl_info("Enabling MCAST client traffic to go on corresponding STA VAP\n");
            } else {
                ic->ic_global_list->force_client_mcast_traffic = 0;
                qdf_nofl_info("Disabling MCAST client traffic to go on corresponding STA VAP\n");
            }
            GLOBAL_IC_UNLOCK_BH(ic->ic_global_list);
            break;
#endif
...
ath_get_config(ath_dev_t dev, ath_param_ID_t ID, void *buff)
{
    ...
    switch(ID)
    {
        ...
        case ATH_PARAM_ATH_RXQ_INFO:
        {
            ...
            qdf_nofl_info("sc_rxbuf num of elements = %d, bf=%pK, bf_mpdu=%pK\n", i, bf, bf?bf->bf_mpdu:0 );
        }
        break;
