...
#define AGING_TIMEOUT_MS    300000
    ...
    TAILQ_FOREACH_SAFE(ap, &sascan->sa_aplist, ap_next, next){
        if (!OS_MEMCMP(ap->addr, wh->i_addr2, QDF_MAC_ADDR_SIZE)) {
    ...
    if (!ap) {
        ...
        OS_MEMCPY(ap->addr, wh->i_addr2, QDF_MAC_ADDR_SIZE);
        sascan->sa_ap_count++;
    }
...
ath_smartant_get_apconf(struct ath_sascan *sascan, const u_int8_t *bssid)
{
    ...
    TAILQ_FOREACH_SAFE(ap, &sascan->sa_aplist, ap_next, next) {
        if (!OS_MEMCMP(ap->addr, bssid, QDF_MAC_ADDR_SIZE)) {
...
ath_smartant_start(struct ath_sascan *sascan, const u_int8_t *bssid)
{
    ...
    OS_MEMCPY(sascan->bssid, bssid, QDF_MAC_ADDR_SIZE);
...
u_int32_t ath_smartant_query(ath_dev_t dev, u_int32_t *buffer)
{
    ...
    OS_MEMCPY(smartAntInfo->bssid, sascan->bssid, QDF_MAC_ADDR_SIZE);

    itemlen = 0;
    TAILQ_FOREACH_SAFE(ap, &sascan->sa_aplist, ap_next, next) {
        OS_MEMCPY(smartAntInfo->sa_ap_list + itemlen, ap->addr, QDF_MAC_ADDR_SIZE);
        itemlen += QDF_MAC_ADDR_SIZE;
