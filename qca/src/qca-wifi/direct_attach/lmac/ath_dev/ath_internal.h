...
#define ATH_NODE_LEAVE           0x20    /* indicates the node leave */
    ...
    u_int8_t     an_macaddr[QDF_MAC_ADDR_SIZE];    /* macaddr of the node */
...
struct ath_softc {
    ...
    u_int8_t                sc_curbssid[QDF_MAC_ADDR_SIZE];
    u_int8_t                sc_myaddr[QDF_MAC_ADDR_SIZE]; /* current mac address */
    u_int8_t                sc_my_hwaddr[QDF_MAC_ADDR_SIZE]; /* mac address from EEPROM */
    ...
    u_int8_t                sc_bssidmask[QDF_MAC_ADDR_SIZE];
...
#define ath_hal_keyreset(_ah, _ix, _clear_key) \
    ((*(_ah)->ah_reset_key_cache_entry)((_ah), (_ix), (_clear_key)))
