    ...
    else if ( param & SPECIAL_PARAM_SHIFT )
    {
        if ( param == (SPECIAL_PARAM_SHIFT | SPECIAL_PARAM_COUNTRY_ID) ) {
            ...
            if ( !TAILQ_EMPTY(&ic->ic_vaps) ) {
                qdf_nofl_info("%s: We do not support this if there are VAPs present\n",__func__);
                retval = -EBUSY;
            } else {
...
qdf_export_symbol(ath_ucfg_setparam);
...
qdf_export_symbol(ath_ucfg_getparam);
...
qdf_export_symbol(ath_ucfg_set_countrycode);
...
qdf_export_symbol(ath_ucfg_get_country);
...
qdf_export_symbol(ath_ucfg_set_dscp_tid_map);
...
qdf_export_symbol(ath_ucfg_get_dscp_tid_map);
#endif
...
qdf_export_symbol(ath_ucfg_set_mac_address);
...
qdf_export_symbol(ath_ucfg_set_smartantenna_param);
...
qdf_export_symbol(ath_ucfg_get_smartantenna_param);
#endif
...
int ath_ucfg_diag(struct ath_softc_net80211 *scn, struct ath_diag *ad)
{
    ...
    if (ath_hal_getdiagstate(ah, id, indata, insize, &outdata, &outsize)) {
        qdf_nofl_info("alloc size = %d, new = %d\n", ad->ad_out_size, outsize);
...
qdf_export_symbol(ath_ucfg_diag);
...
qdf_export_symbol(ath_ucfg_phyerr);
#endif
