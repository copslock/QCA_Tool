...
ath_tx_sched_rifs(struct ath_softc *sc, struct ath_txq *txq, ath_atx_tid_t *tid)
{
    ...
    /* If TID queue is empty, break */
        ...
        if (bf->bf_nrifsubframes == 1) {
             ...
             */
            if (ath_tx_txqaddbuf(sc, txq, &bf_q) != 0) {
                qdf_nofl_info("--- %s[%d] --- HW Q is full ---\n", __func__, __LINE__);
            }
        }
...
#define    OFDM_SIFS_TIME    16
    ...
    if (ath_tx_txqaddbuf(sc, txq, bf_qfirst) != 0) {
        qdf_nofl_info("--- %s[%d] --- HW Q Full, Could not add Aggr to HW -- Not being Freed !! LEAK !! \n", __func__, __LINE__);
    }
...
ath_tx_sched_normal(struct ath_softc *sc, struct ath_txq *txq, ath_atx_tid_t *tid)
{
    ...
    if (sc->sc_atf_enable) {
        ...
        */
        ...
        if (ath_tx_txqaddbuf(sc, txq, &bf_q) != 0) {
            qdf_nofl_info("ath_tx_sched_normal: txqadd failed for single\n");
#ifdef ATH_SUPPORT_TxBF
                ath_tx_complete_buf(sc, bf, &bf_q, 0, 0, 0);
#else
                ath_tx_complete_buf(sc, bf, &bf_q, 0);
    ...
    do {
        ...
        if (!TAILQ_EMPTY(&tid->buf_q)) {
            ...
            if ((status == ATH_AGGR_BAW_CLOSED) && (TAILQ_EMPTY(&bf_q)) &&
                ...
                qdf_nofl_info("%s: Recovering from error condition: Flushing TID queue until head \
                       is within the window\n", __func__);
                qdf_nofl_info("%s: TID[%d] seq_start:%d seq_next:%d Head:%d Tail:%d BAW size:%d seqno:%d\n",
        ...
        if (sc->sc_atf_enable) {
            ...
            if (ath_tx_txqaddbuf(sc, txq, &bf_q) != 0) {
                ...
                ath_tx_complete_buf(sc, bf, &bf_q, 0);
#endif
                qdf_nofl_info("ath_tx_sched_aggr: txqadd failed for single\n");
            }
        } else {
            /*
             * setup first desc with rate and aggr info
            ...
            if (status == ATH_AGGR_8K_LIMITED) {
                ...
                if (ath_tx_txqaddbuf(sc, txq, &bf_q) != 0) {
                    qdf_nofl_info("ath_tx_sched_aggr: txqadd failed for aggregate\n");
                }
            }
        }
