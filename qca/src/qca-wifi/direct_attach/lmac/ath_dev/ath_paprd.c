...
int ath_paprd_cal(struct ath_softc *sc)
{
    ...
    {
        /* Intentional print */
        qdf_nofl_info("%s sc %pK chan_mask %d PAPRD excessive failure disabling PAPRD now\n", __func__, sc, sc->sc_tx_chainmask);
...
int ath_tx_paprd_init(struct ath_softc *sc)
{
...
#if 0
    {
        ...
        qdf_nofl_info("%s sc %pK ---Enable PAPRD DEBUG ---- fail count %d\n", __func__, sc, sc->sc_paprd_failed_count);
    }
#endif
    ...
    TAILQ_FOREACH(bf, &sc->sc_paprdbuf, bf_list){
        ...
        //qdf_nofl_info("%s[%d] buf %pK, seg %d, len %d\n", __func__, __LINE__,
        //                                    bf->bf_buf_addr[0],
         //                                   ctx->dm_nseg,
         //                                   ctx->dm_size);
...
void ath_tx_paprd_cleanup(struct ath_softc *sc)
{
    ...
    if (bf) {
	    ...
	    TAILQ_FOREACH(bf, &sc->sc_paprdbuf, bf_list) {
		/* Unmap this frame */
		//qdf_nofl_info( "%s[%d]: txq dot11nrate = %x bf_buf_len %d\n", __func__, __LINE__, bf->bf_buf_len[0]);
	#if 0
