        ...
        struct ieee80211_rateset *negotiated_htrates)
{
    ...
    if (node_index >= ATH_HTC_MAX_NODE_NUM) {
        qdf_nofl_info("%s: node_index exceeds the HTC capability, node_index = %d !\n", __func__, node_index);
        return;
    }

    /* qdf_nofl_info("%s: node_index = %d, isnew = %d, capflag = 0x%x, rate num = %d(legacy) - %d(11n)\n", 
...
ath_wmi_aggr_enable(ath_dev_t dev, struct ath_node *an, u_int8_t tidno, u_int8_t aggr_enable)
{
    ...
    if (tidno >= ATH_HTC_WME_NUM_TID) {
        //qdf_nofl_info("%s: tidno exceeds the HTC capability, tidno = %d !\n", __func__, tidno);
        return;
    }
    ...
    //qdf_nofl_info("%s: node_index = %d, tidno = %d, aggr_enable = %d\n", __func__, node_index, tidno, aggr_enable);
...
ath_htc_rx(ath_dev_t dev, wbuf_t wbuf)
{
    ...
    if (length <= sizeof(struct rx_frame_header)) {
        //qdf_nofl_info("%s: drop rx garbage\n", __func__);
        wbuf_free(wbuf);
        ATH_RXBUF_UNLOCK(sc);
        return;
    }
...
ath_htc_rx_tasklet(ath_dev_t dev, int flush)
{
    ...
    do {
        ...
        if (flush == RX_DROP) {
             ...
             */
            qdf_nofl_info("%s: drop rx frame when flush rx queue\n", __func__);
...
ath_htc_uapsd_sendqosnull(struct ath_softc *sc, struct ath_node *an, u_int8_t ac)
{
    ...
    if (wbuf == NULL) {
        ...
        qdf_nofl_info("%s: qosnul buffers unavailable \n",__func__);
        return;
    }
...
ath_htc_process_uapsd_trigger(ath_dev_t dev, ath_node_t node, u_int8_t maxsp, u_int8_t ac, u_int8_t flush, bool *sent_eosp, u_int8_t maxqdepth)
{
    ...
    for (i = 0; i < count; i++) {
        ...
        if(ath_htc_send_pkt(dev, wbuf, sc->sc_uapsd_ep, an) != 0) {
            ...
            qdf_nofl_info(" %s unable to send UAPSD pkts,SW queue depth %d\n",__FUNCTION__,an->an_uapsd_qdepth);
...
#define ATH_80211_QOSPAD_LENGTH         4  /* 2 + 2 */
    ...
    if (txctl->isdata)
    {
        ...
        if (txctl->istxfrag == 0) {
            ...
            //qdf_nofl_info("%s: ipVersion = 0x%x, ipv4FlagAndFragOffset = 0x%x, ethertypeOffset = %d\n", __func__, ipVersion, ipv4FlagAndFragOffset, ethertypeOffset);
        }
...
//                    wbuf_free(wbuf1);
//#endif                
                qdf_nofl_info("Rejecting unicast encrypted frame as key is not set\n");
                return 0;
            }
            qdf_nofl_info("clearing privacy bit for mcast packet\n");
...
queue:
    ...
    else
    {
        ...
        if (qdf_unlikely((htcstatus != A_OK) )){
            qdf_nofl_info("Freeing Management pkt due to credit unavailibility \n");
            wbuf_free(wbuf);
            wbuf = NULL;
        }
#endif
...
#define ATH_GMAC_FRAG_BUF_SIZE          512
    ...
    if (tid == NULL) {
        qdf_nofl_info("wrong tid, tidno %d\n",tfh->tidno);
        return -1;
    }
...
ath_htc_rx_init(ath_dev_t dev, int nbufs)
{
    ...
    do {
        ...
        if (error != 0) {
            qdf_nofl_info("failed to allocate rx descriptors: %d\n", error);
            break;
        }
...
ath_wmi_get_target_stats(ath_dev_t dev, HTC_HOST_TGT_MIB_STATS *stats )
{
    ...
    if(cmd_status) {
        qdf_nofl_info("%s, get_status fail = %x\n", __FUNCTION__, cmd_status);
    }
    else {
...
ath_htc_beacon_generate(struct ath_softc *sc, int if_id, int *needmark)
{
     ...
     */
    {
        ...
        if (wbuf_org != NULL) {
            ...
            if (wbuf == NULL) {
                qdf_nofl_info("%s : allocate wbuf fail for beacon\n", __func__);
                return NULL;
            }
            else {
...
ath_wmi_beacon_stuck_internal(struct ath_softc *sc)
{
    ...
    sc->sc_bmisscount++;
#ifdef MAGPIE_HIF_GMAC   
    //qdf_nofl_info("bmiss %d \n", sc->sc_bmisscount);  
#else
    qdf_nofl_info("%s: bmisscount = %d, show_cycles = %d, rx_clear = %d, rx_frame = %d, tx_frame = %d\n", __func__, sc->sc_bmisscount, show_cycles, rx_clear, rx_frame, tx_frame);
#endif    
#ifdef ATH_SUPPORT_DFS    
    ...
    if((sc->sc_3stream_sigflag1) && (sc->sc_3stream_sigflag2)) {
            ...
            qdf_nofl_info("%s engaing fix 0x9944 %x 0x9970 %x\n",__FUNCTION__,OS_REG_READ(sc->sc_ah,0x9944),
    ...
    if (sc->sc_bmisscount < (BSTUCK_THRESH_PERVAP * sc->sc_nvaps)) {
        ...
        if (sc->sc_noreset) {
            qdf_nofl_info("%s: missed %u consecutive beacons\n",
            ...
            if (show_cycles) {
                 ...
                 */
                qdf_nofl_info("%s: busy times: rx_clear=%d, rx_frame=%d, tx_frame=%d\n", __func__, rx_clear, rx_frame, tx_frame);
            } else {
                qdf_nofl_info("%s: unable to obtain busy times\n", __func__);
            }
        } else {
        ...
        if (sc->sc_noreset) {
            if (sc->sc_bmisscount == (BSTUCK_THRESH_PERVAP * sc->sc_nvaps)) {
                qdf_nofl_info("%s: beacon is officially stuck\n",
                       __func__);
                ath_hal_dmaRegDump(ah);
            }
        } else {
            ...
            if (show_cycles && rx_clear >= 99) {
                ...
                if (rx_frame >= 99) {
                    qdf_nofl_info("Busy environment detected\n");
                } else {
                    qdf_nofl_info("Interference detected\n");
                }
    
                qdf_nofl_info("rx_clear=%d, rx_frame=%d, tx_frame=%d\n",
                       rx_clear, rx_frame, tx_frame);
...
//        ath_hal_set_immunity(ah, AH_FALSE);
    ...
    {
        if (sc->sc_noreset) {
            qdf_nofl_info("%s: resume beacon xmit after %u misses\n",
                   __func__, sc->sc_bmisscount);
        } else {
            DPRINTF(sc, ATH_DEBUG_BEACON_PROC,
...
//        tsf = ath_hal_gettsf64(ah);
    ...
    if (bfaddr != 0)
    {
        ...
        if (wbuf_hdrspace(wbuf) < need_headroom)
        {
            ...
            if (wbuf == NULL) {
                qdf_nofl_info("%s: Insufficient header room space and allocate new buffer fail I\n", __func__);
                return;
            }
        }
#endif
        ...
        if (qdf_unlikely((status != A_OK) )){
            qdf_nofl_info("Freeing Beacon pkt due to credit unavailibility \n");
			wbuf_free(wbuf);
            wbuf = NULL;
...
#ifdef  ATH_HTC_TX_SCHED
    if (qdf_unlikely((status != A_OK) )){
        qdf_nofl_info("Freeing adhoc beacon pkt due to credit unavailibility \n");
        wbuf_free(wbuf);
        wbuf = NULL;
...
void ath_htc_host_txep_init(struct ath_softc *sc)
{
    ...
    qdf_nofl_info("%s: Epnum to Hardware Queue Mapping\n", __func__);	
...
ath_wmi_set_btcoex(struct ath_softc *sc, u_int8_t dutyCycle, u_int16_t period,  u_int8_t stompType)
{
    ...
    if(cmd_status) {
        qdf_nofl_info("Host : WMI COMMAND READ FAILURE stat = %x\n", cmd_status);
