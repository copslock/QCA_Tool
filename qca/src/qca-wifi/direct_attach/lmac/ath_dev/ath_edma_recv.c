...
//    ATH_RXBUF_RESET(bf);
    ...
    */
    ...
    if (rxedma->rxfifodepth > rxedma->rxfifohwsize)
        qdf_nofl_info("PANIC depth more than size %d sz %d\n",
...
ath_rx_addbuffer(struct ath_softc *sc, HAL_RX_QUEUE qtype, int size)
{
    ...
    TAILQ_FOREACH_SAFE(bf, &sc->sc_rxbuf, bf_list, tbf) {
        ...
        if (rxedma->rxfifodepth >= rxedma->rxfifohwsize) {
            qdf_nofl_info("%s: size = %d rxfifodepth=%d\n", __func__, size, rxedma->rxfifodepth);
            break;
        }
        ...
        if (rxs->rs_status == 0) {
            ...
            if (rxs->rs_more) {
                ...
                */
                ...
                //qdf_nofl_info("rx_tasklet: received frames with MORE set in rx_status\n");
                goto rx_next;
