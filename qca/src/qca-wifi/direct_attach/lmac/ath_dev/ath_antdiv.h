 ...
 */
    ...
    u_int8_t                antdiv_bssid[QDF_MAC_ADDR_SIZE];
...
typedef struct ath_sa_aplist {
    ...
    u_int8_t            addr[QDF_MAC_ADDR_SIZE]; /* ap list to record bssid and the Beacon rssi of
                                                     given antenna. */
...
struct ath_sascan {
    ...
    u_int8_t            bssid[QDF_MAC_ADDR_SIZE];  /* associated/joining to bssid */
