						   ...
						   u_int32_t retries)
{
	...
	case 3:
		...
		break;
	default:
		qdf_nofl_info("Invalid rateindex \n\r");
	}
	return totalretries;
}
			    ...
			    int dir)
{
	...
	if (dir == IEEE80211_FC1_DIR_DSTODS) {
		if (IEEE80211_ADDR_EQ
			...
			qdf_nofl_info("mac %s should not in here\n",
...
static void ath_rx_removebuffer_aponly(struct ath_softc *sc, HAL_RX_QUEUE qtype)
{
	...
	for (i = 0; i < size; i++) {
		...
		if (wbuf) {
			...
			if (!bf) {
				qdf_nofl_info("%s[%d] PANIC wbuf %pK Index %d\n",
	...
	if (rxedma->rxfifodepth)
		qdf_nofl_info("PANIC depth non-zero %d\n", rxedma->rxfifodepth);
}
...
void ath_rx_intr_aponly(ath_dev_t dev, HAL_RX_QUEUE qtype)
{
	...
	do {
		...
		if (unlikely(rxedma->rxfifodepth == 0))
			qdf_nofl_info("ath_rx_intr: depth 0 PANIC\n");
...
rx_next:
	...
	if (unlikely(chainreset)) {
		qdf_nofl_info("Reset rx chain mask. Do internal reset. (%s)\n",
		       __func__);
		ath_internal_reset(sc);
	}
...
qdf_export_symbol(ath_tx_start_dma_aponly);
		      ...
		      int nextfraglen, ieee80211_tx_control_t * txctl)
{
	...
	if (type == IEEE80211_FC0_TYPE_MGT) {
		...
		qdf_nofl_info("bogus frame type 0x%x (%s)\n",
		...
		if (likely(subtype & IEEE80211_FC0_SUBTYPE_QOS)) {
			...
			if (wh->i_fc[1] & IEEE80211_FC1_ORDER) {
				...
				if ((htc[2] == IEEE80211_HTC2_CSI_COMP_BF) ||
					...
					//qdf_nofl_info("==>%s,txctl flag before attach sounding%x,\n",__func__,txctl->flags);
					...
					//qdf_nofl_info("==>%s,txctl flag %x,tx staggered sounding %x, rx staggered sounding %x\n"
...
ieee80211_encap_8023_aponly(struct wlan_objmgr_peer *peer, wbuf_t wbuf)
{
	...
	if (wlan_vdev_mlme_feat_cap_get(vdev, WLAN_VDEV_F_WDS)) {
		...
		if (isMulticast &&
			...
			use4addr = 1;
#endif
		hdrsize = hdrsize + (use4addr ? QDF_MAC_ADDR_SIZE : 0);
	}
#endif
