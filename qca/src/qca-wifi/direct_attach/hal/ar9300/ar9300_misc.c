...
ar9300_get_mac_address(struct ath_hal *ah, u_int8_t *mac)
{
    ...
    OS_MEMCPY(mac, ahp->ah_macaddr, QDF_MAC_ADDR_SIZE);
}
...
ar9300_set_mac_address(struct ath_hal *ah, const u_int8_t *mac)
{
    ...
    OS_MEMCPY(ahp->ah_macaddr, mac, QDF_MAC_ADDR_SIZE);
    return true;
}
...
ar9300_get_bss_id_mask(struct ath_hal *ah, u_int8_t *mask)
{
    ...
    OS_MEMCPY(mask, ahp->ah_bssid_mask, QDF_MAC_ADDR_SIZE);
}
...
ar9300_set_bss_id_mask(struct ath_hal *ah, const u_int8_t *mask)
{
    ...
    OS_MEMCPY(ahp->ah_bssid_mask, mask, QDF_MAC_ADDR_SIZE);
    ...
    u_int16_t assoc_id)
{
    ...
    OS_MEMCPY(ahp->ah_bssid, bssid, QDF_MAC_ADDR_SIZE);
...
ar9300_set_txchainmaskopt(struct ath_hal *ah, u_int8_t mask)
{
    ...
    if ((mask & ahp->ah_tx_chainmask) != mask) {
        ...
        qdf_nofl_info("Error: ah_tx_chainmask=%d, mask=%d\n", ahp->ah_tx_chainmask, mask);
        return;
    }
...
void ar9300_reset_nav(struct ath_hal *ah)
{
	...
	if ((regval != 0xdeadbeef) && (regval > 0x7fff)) {
		qdf_nofl_info("%s: abnormal nav value found 0x%x\n", __func__, regval);
