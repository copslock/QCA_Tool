...
get_wmac_irq(u_int16_t wmac_num)
{
	...
	if (res == NULL) {
		qdf_nofl_info("no IRQ resource found\n");
		ret = -ENXIO;
		goto out;
	}
...
get_wmac_base(u_int16_t wmac_num)
{
	...
	res = (struct resource *)rsrc;
	if (res == NULL) {
		qdf_nofl_info("no memory resource found\n");
		ret = -ENXIO;
		goto err_out;
	}
...
static const struct platform_device_id ath9k_platform_id_table[] = {
	...
	{
		.name = "ipq50xx_wmac", /* Update the name with proper device name (ipqxxxx) */
		.driver_data = QCA5018_DEVICE_ID,
	},
    {},
};
...
/* Add only lithium AHB chipsets in this list */
const struct of_device_id cnss_ahb_wifi_of_match[] = {
    ...
    {.compatible = "qcom,cnss-qca5018", .data = (void *) &ath9k_platform_id_table[3] },
    { /*sentinel*/},
};
#endif
...
static int ath_ahb_probe(struct platform_device *pdev)
{
    ...
    if (!of_id)
    {
        ...
        if (!pdev->dev.platform_data) {
            qdf_nofl_info("no platform data specified\n");
            ret = -EINVAL;
            goto out;
        }
#ifdef CONFIG_OF
...
const struct platform_device_id *ath_ahb_get_pdev_id(struct platform_device *pdev)
{
    ...
    /* Match dev in lithium ahb table */
    of_id = of_match_device(cnss_ahb_wifi_of_match, &pdev->dev);
    if(!of_id) {
            /* Match dev in legacy ahb table */
            of_id = of_match_device(ath_wifi_of_match, &pdev->dev);
            if(!of_id)
                return NULL;
    }
...
ath_ahb_recovery_probe(struct platform_device *pdev, const struct platform_device_id *pdid)
{
    ...
    if (of_id) {
        ...
        if (id->driver_data == IPQ4019_DEVICE_ID) {
            ol_ath_wifi_recovery_probe(pdev, id);
            return 0;
        }
    }
#endif
...
int ath_ahb_recover(struct ol_ath_soc_softc *soc)
{
    ...
    if ((lmac_get_tgt_type(soc->psoc_obj) !=  TARGET_TYPE_QCA8074) &&
        ...
        ol_ath_wifi_recovery_probe((struct platform_device *)(aps_osdev->bdev), NULL);
    ...
    if (soc->recovery_enable) {
        ...
        if (recovery_enable == RECOVERY_ENABLE_AUTO) {
            if ((target_type !=  TARGET_TYPE_QCA8074) &&
                ...
                ol_ath_wifi_recovery_probe(pdev, NULL);
...
qdf_export_symbol(ol_ath_ahb_driver);
