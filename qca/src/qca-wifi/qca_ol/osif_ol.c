...
osif_ol_vap_send_wifi3_fast(struct sk_buff *skb, struct net_device *dev)
{
    ...
    if (qdf_unlikely(wlan_vdev_is_up(vap->vdev_obj) != QDF_STATUS_SUCCESS) ||
        (qdf_unlikely(IEEE80211_IS_CHAN_RADAR(vap->iv_bsschan)))) {
        goto bad;
    }
...
void osif_vap_setup_ol (struct ieee80211vap *vap, osif_dev *osifp) {
	...
	ops.tx.tx_free_ext = (ol_txrx_tx_free_ext_fp)os_if_tx_free_ext;
        ...
        osifp->nss_nwifi = cdp_get_nwifi_mode(soc_txrx_handle, vdev_id);
...
osif_nss_ol_vap_hardstart(struct sk_buff *skb, struct net_device *dev)
{
    ...
    if (stats_ops->txrx_update_vdev_stats) {
        stats_ops->txrx_update_vdev_stats(soc_txrx_handle, wlan_vdev_get_id(vap->vdev_obj), skb,
                                          DP_VDEV_STATS_PKT_CNT_ONLY);
    }
...
void osif_rawsim_getastentry (struct ieee80211vap *vap, qdf_nbuf_t *pnbuf, void *raw_ast)
{
    ...
    cdp_rawsim_get_astentry(soc_txrx_handle, wlan_vdev_get_id(vap->vdev_obj), pnbuf, (struct cdp_raw_ast *)raw_ast);
}
...
int osif_get_sec_type(wlan_if_t vap, uint8_t *peer_mac, uint8_t sec_idx)
{
    ...
    return cdp_get_sec_type(soc_txrx_handle, wlan_vdev_get_id(vap->vdev_obj),
                                   peer_mac, sec_idx);
}
