 ...
 *
 =============================================================================================*/
int LOWIRanging::send_nl_msg(int fd, char *data, unsigned int hdrLen, unsigned int msgLen, unsigned int metaLen)
{
  ...
  unsigned int len = hdrLen + msgLen + metaLen;
...
int LOWIRanging::RomeSendRegReq()
{
  ...
  LOWI_LOG_VERB("%s: Sending APP reg Message over NL\n", __FUNCTION__);
  /* ANI Message over Netlink Socket */
  if (send_nl_msg(nl_sock_fd, aniMessage, sizeof(tAniMsgHdr), APP_REG_SIGNATURE_LENGTH, 0) < 0)
  {
...
int LOWIRanging::RomeSendChannelInfoReq(IwOemDataCap iwOemDataCap)
{
  ...
  if(send_nl_msg(nl_sock_fd, aniMessage, sizeof(tAniMsgHdr), aniMsgLen, 0) < 0)
  {
 ...
 *    SUCCESS/FAILURE
 =============================================================================================*/
int LOWIRanging::RomeSendRangingCapReq()
{
...
int LOWIRanging::RomeSendLCIRequest(tANI_U16 /*reqId*/, LOWISendLCIRequest *request)
{
  ...
  *count = 1;
  ...
  /* ANI Message over Netlink Socket */
  if (send_nl_msg(nl_sock_fd, aniMessage, sizeof(tAniMsgHdr), aniMsgLen, aniMetaLen) < 0)
  {
    LOWI_LOG_ERROR("%s: NL Send Failed", __FUNCTION__);
    free(aniMessage);
    return -1;
