 ...
 **********************************************************************************/
int LOWIHeliumRanging::RomeSendRangingCapReq()
{
  LRH_ENTER()
  int retVal = -1;
  ...
  do
  {
    ...
    char *aniMessage = allocAniMsgHdr(aniMsgLen, aniHdrLen, aniMetaLen);
    if (aniMessage == NULL)
    {
    ...
    *count = 1;
    ...
    if (send_nl_msg(nl_sock_fd, aniMessage, aniHdrLen, aniMsgLen, aniMetaLen) < 0)
    {
...
{
  char *aniMsg = NULL;
  aniMsg = (char *)calloc(aniHdrLen + aniMsgLen + aniMetaLen, 1);
  if (aniMsg != NULL)
  {
