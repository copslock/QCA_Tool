...
//LOWI Test Version
#define LOWI_TEST_VERSION "LTST_1.4.18"
...
struct LOWIPostProcessNode
{
  ...
  bool skip_first;
  int total_cnt;
  vector <result_data>  rtt_cache;
  enum eRangingBandwidth req_bw;
  int band;
  int frame_per_burst;
...
struct t_lowi_test_cmd
{
  ...
  uint32                  kpi_mask;                     // KPI Debug MASK
  uint32                  trueDist;                     //True distance for all ap in this list
  int32                   req_bw;                       // Requested BW for rtt test mode
