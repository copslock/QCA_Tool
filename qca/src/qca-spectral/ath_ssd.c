...
void restore_spectral_configuration(void)
{
    ...
    if ((SPECTRAL_SCAN_MODE_AGILE == pinfo->spectral_mode) &&
            (pinfo->prev_spectral_params.ss_frequency.cfreq1 != 0)) {
        param.id = SPECTRAL_PARAM_FREQUENCY;
        param.value1 = pinfo->prev_spectral_params.ss_frequency.cfreq1;
        param.value2 = pinfo->prev_spectral_params.ss_frequency.cfreq2;
        ath_ssd_set_spectral_param(pinfo, &param);
    }
}
...
int replay_from_file(ath_ssd_info_t *pinfo)
{
    ...
    if (file_version == SPECTRAL_LOG_VERSION_ID3) {
        int num_params;
...
int main(int argc, char* argv[])
{
    ...
    while ((optc = getopt(argc, argv, "ab:dc:f:g:Hhi:j:p:Tts:nx:")) != -1) {
        switch (optc) {
            case 'a':
#ifdef SPECTRAL_SUPPORT_CFG80211
                if (IS_CFG80211_ENABLED(pinfo)) {
